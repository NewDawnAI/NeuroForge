...
- Run all commands from the repository root (`<repo_root>`) so relative paths resolve.
...
- Run location: execute commands from the repo root (`<repo_root>`) so relative paths resolve.
