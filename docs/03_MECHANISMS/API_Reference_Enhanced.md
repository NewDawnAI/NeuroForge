...
- CLI flag parsing: `src/main.cpp` (search for the flag name in the argument parser).
- Shaped computation: `src/main.cpp` and `src/core/LearningSystem.cpp` (search for `computeShapedReward` and `applyExternalReward`).
...
```powershell
& ".\build\neuroforge.exe" `
  --phase7=on --phase9=on --substrate-mode=native `
