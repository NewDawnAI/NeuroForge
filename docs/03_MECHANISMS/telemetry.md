...
- Implementation reference: `src/main.cpp` (search for `vision`, `retina`, and `foveation` in the JSON logging path).
...
- The JSON payload contains:
  ...
  - Optional Stage C cap fields: `autonomy_cap_multiplier` and `effective_autonomy`
...
Notes:
- One row per applied revision, capturing what changed (`revision_json`) and why (`driver_explanation`).
- When Stage C is enabled, the self-revision pipeline may apply an autonomy cap derived from recent outcomes; inspect `autonomy_envelope_log.driver_json` and `metacognition.self_explanation_json` for `stage_c_v1`, `stage_c_v2`, `stage_c_v3`, or `stage_c_v4`.

### self_revision_outcomes (Stage 7.5, evaluation-only)
- `revision_id` INTEGER
- `run_id` INTEGER
- `eval_ts_ms` INTEGER
- `outcome_class` TEXT
...
Notes:
- One row per revision evaluation (not approval), enabling audit of whether revisions were beneficial, neutral, or harmful over a rolling window.
- Phase 10 may embed the latest evaluated outcome in `metacognition.self_explanation_json` under `stage7_5`.

### autonomy_credit_log (Stage C v2/v3/v4)
- `id` INTEGER PRIMARY KEY
- `run_id` INTEGER
- `ts_ms` INTEGER
- `credit_value` REAL
...
Notes:
- One row per Autonomy Credit update (or periodic decay checkpoint).
- Used by Stage C v2/v3/v4 to compute `autonomy_credit_cap_multiplier` and the final `autonomy_cap_multiplier`.
- `driver_json` includes a structured explanation of the update reason and inputs (for example `reason`, `prev_credit`, `decayed_credit`, `updated_credit`, `dt_ms`, and recent outcome window counts).

### preference_memory (Stage C v3/v4)
- `id` INTEGER PRIMARY KEY
- `run_id` INTEGER
- `key` TEXT
...
- `strength01` REAL
- `evidence_n` INTEGER
- `beneficial_n` INTEGER
- `harmful_n` INTEGER
- `updated_ts_ms` INTEGER

Notes:
- One row per run+parameter key (upserted), representing a smoothed preferred value derived from `parameter_history` joined with `self_revision_outcomes` by `revision_id`.
- Used by Stage C v3/v4 to scale Phase 11 parameter deltas away from preferred values, and to emit preference telemetry in `metacognition.self_explanation_json` (`stage_c_v3`/`stage_c_v4`).

### goal_nodes (Stage 8 + Stage C v4 bounded goal formation)
- `goal_id` INTEGER PRIMARY KEY
- `run_id` INTEGER
- `description` TEXT
- `priority` REAL
- `stability` REAL
- `origin_reflection_id` INTEGER (nullable)
- `created_ts_ms` INTEGER
- `expires_ts_ms` INTEGER
- `reaffirm_n` INTEGER
- `vetoed` INTEGER (0/1)
- `veto_reason` TEXT (nullable)

Notes:
- Phase 8 can create and update goals in the goal graph.
- Stage C v4 can upsert a bounded set of goals derived from stable preferences when governance allows it; goals are short-lived (TTL via `expires_ts_ms`) and are reaffirmed by incrementing `reaffirm_n`.

### language_grounding_map (Stage C v5 / Language grounding)
- `id` INTEGER PRIMARY KEY
- `run_id` INTEGER
- `ts_ms` INTEGER
- `step` INTEGER
...
- `correlation` REAL
- `description_length_delta` REAL (nullable)
- `source` TEXT

Notes:
- Records token grounding observations for audit and offline analysis.

### language_audit_log (Stage C v5 scope gating + language audit)
- `id` INTEGER PRIMARY KEY
- `run_id` INTEGER
- `ts_ms` INTEGER
- `step` INTEGER
...
- `token` TEXT
- `details_json` TEXT
- `wrote_preference_memory` INTEGER (0/1)
- `wrote_goal_nodes` INTEGER (0/1)
- `wrote_autonomy_credit` INTEGER (0/1)
- `wrote_identity_vector` INTEGER (0/1)
- `allowed` INTEGER (0/1)

Notes:
- Stage C v5 writes scope gate events such as `scope_open`, `scope_allow`, and `scope_block`.
- The `token` field is used as the scope key (for example `substrate`, `language_dev`, `phase_a`) or the relevant language token, depending on the event.
- The `wrote_*` flags support hard auditing of forbidden writes during scoped-learning runs.
