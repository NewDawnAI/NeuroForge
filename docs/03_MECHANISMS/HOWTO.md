_Updated: 2026-01-23 • Version: Phase6-11 v2.5 • Phase15 v1.3_
...
# Conversational Mode (requires microphone)
.\build\neuroforge.exe --genesis --enable-parallel --chat
```
...
- `--chat` — Enables native Windows Speech-to-Text via microphone and Text-to-Speech responses
- `--camera` — Enables real webcam vision via OpenCV (requires `NF_HAVE_OPENCV`)
...
## N‑Series Harness Quick Start (N4–N7)

These harnesses are designed for auditable closed-loop testing. Each run writes a CSV to the repo root.
...
- `unified_bounded_log.csv`
- `unified_bounded_summary.csv`

See `docs/04_EXPERIMENTS/N_SERIES_HARNESSES.md` for expected phenotypes and interpretation.
...
.\neuroforge.exe --phase7=on --phase8=on --phase9=on --phase10=on --phase11=on ^
  --phase11-revision-interval=300000 ^
  --phase11-min-gap-ms=60000 --phase11-outcome-window-ms=60000 ^
...
```powershell
# Run with a shorter revision interval; inspect DB afterwards
.\neuroforge.exe --phase10=on --phase11=on --phase11-revision-interval=180000 ^
  --phase11-min-gap-ms=60000 --phase11-outcome-window-ms=60000 ^
  --memory-db=phasec_mem.db --steps=500 --log-json=on
```

Inspect the latest revision and its embedded explanation:
```powershell
sqlite3 phasec_mem.db "SELECT id, ts_ms, revision_json, driver_explanation FROM self_revision_log ORDER BY ts_ms DESC LIMIT 1;"
```

Inspect the latest evaluated outcome (Stage 7.5 evaluation-only governance):
```powershell
sqlite3 phasec_mem.db "SELECT revision_id, eval_ts_ms, outcome_class, trust_pre, trust_post, prediction_error_pre, prediction_error_post, coherence_pre, coherence_post FROM self_revision_outcomes ORDER BY eval_ts_ms DESC LIMIT 1;"
```

Inspect the latest metacognition explanation and check for a `stage7_5` object:
```powershell
sqlite3 phasec_mem.db "SELECT id, ts_ms, self_trust, substr(self_explanation_json, 1, 200) FROM metacognition ORDER BY ts_ms DESC LIMIT 1;"
```

Inspect Stage C v1 autonomy gating (governance-only cap) when enabled:
```powershell
# Check for a `stage_c_v1` object in the latest self-explanation
sqlite3 phasec_mem.db "SELECT id, ts_ms, substr(self_explanation_json, 1, 400) FROM metacognition ORDER BY ts_ms DESC LIMIT 1;"

# Check for autonomy cap fields in the latest autonomy envelope payload
sqlite3 phasec_mem.db "SELECT id, ts_ms, substr(driver_json, 1, 400) FROM autonomy_envelope_log ORDER BY ts_ms DESC LIMIT 1;"
```

See `docs/03_MECHANISMS/StageC_v1_Autonomy_Gating.md` for the mapping and audit surfaces.

### Stage C v2: Enable earned Autonomy Credit + harm-risk cap
```powershell
# Enable Stage C gating and select v2
.\neuroforge.exe --phase7=on --phase8=on --phase9=on --phase10=on --phase11=on ^
  --stagec=v2 ^
  --memory-db=phasec_mem.db --steps=500 --log-json=on
```

Inspect Stage C v2 cap narrative and Autonomy Credit telemetry:
```powershell
# Check for a `stage_c_v2` object in the latest self-explanation
sqlite3 phasec_mem.db "SELECT id, ts_ms, substr(self_explanation_json, 1, 600) FROM metacognition ORDER BY ts_ms DESC LIMIT 1;"

# Check for Autonomy Credit updates (Stage C v2)
sqlite3 phasec_mem.db "SELECT id, ts_ms, credit_value, decay_rate, substr(driver_json, 1, 400) FROM autonomy_credit_log ORDER BY ts_ms DESC LIMIT 5;"

# Check the latest autonomy envelope payload for cap fields
sqlite3 phasec_mem.db "SELECT id, ts_ms, substr(driver_json, 1, 500) FROM autonomy_envelope_log ORDER BY ts_ms DESC LIMIT 1;"
```

See `docs/03_MECHANISMS/StageC_v2_Autonomy_Gating.md` for the v2 computation, cap combination rule, and audit surfaces.

### Stage C v3: Enable preference stabilization (delta scaling)
```powershell
# Enable Stage C gating and select v3
.\neuroforge.exe --phase7=on --phase8=on --phase9=on --phase10=on --phase11=on ^
  --stagec=v3 ^
  --memory-db=phasec_mem.db --steps=800 --log-json=on
```

Inspect Stage C v3 narrative fields and stored preferences:
```powershell
# Check for a `stage_c_v3` object in the latest self-explanation
sqlite3 phasec_mem.db "SELECT id, ts_ms, substr(self_explanation_json, 1, 800) FROM metacognition ORDER BY ts_ms DESC LIMIT 1;"

# Check persisted preference memory (highest strength first)
sqlite3 phasec_mem.db "SELECT key, preferred_value, strength01, evidence_n, beneficial_n, harmful_n, updated_ts_ms FROM preference_memory ORDER BY strength01 DESC, updated_ts_ms DESC LIMIT 20;"
```

See `docs/03_MECHANISMS/StageC_v2_Autonomy_Gating.md` for the v3 preference stabilization algorithm and telemetry fields.

### Stage C v4: Enable bounded goal formation (governance-only)
Stage C v4 preserves the Stage C v3 cap + preference stabilization behavior and adds a bounded goal formation pass that writes to `goal_nodes` when governance allows it.

```powershell
# Enable Stage C gating and select v4
.\neuroforge.exe --phase7=on --phase8=on --phase9=on --phase10=on --phase11=on --phase15=on ^
  --stagec=v4 ^
  --memory-db=phasec_mem.db --steps=800 --log-json=on
```

Inspect Stage C v4 narrative fields, preferences, and any created/reaffirmed goals:
```powershell
# Check for a `stage_c_v4` object in the latest self-explanation
sqlite3 phasec_mem.db "SELECT id, ts_ms, substr(self_explanation_json, 1, 1000) FROM metacognition ORDER BY ts_ms DESC LIMIT 1;"

# Inspect persisted preference memory (v4 uses preferences as goal candidates)
sqlite3 phasec_mem.db "SELECT key, preferred_value, strength01, evidence_n, beneficial_n, harmful_n, updated_ts_ms FROM preference_memory ORDER BY strength01 DESC, updated_ts_ms DESC LIMIT 20;"

# Inspect bounded goals (newest first)
sqlite3 phasec_mem.db "SELECT goal_id, description, priority, stability, created_ts_ms, expires_ts_ms, reaffirm_n, vetoed, veto_reason FROM goal_nodes ORDER BY created_ts_ms DESC LIMIT 20;"

# Inspect active (unexpired, not vetoed) goals
sqlite3 phasec_mem.db "SELECT goal_id, description, priority, stability, created_ts_ms, expires_ts_ms, reaffirm_n FROM goal_nodes WHERE vetoed=0 AND expires_ts_ms > strftime('%s','now')*1000 ORDER BY priority DESC, stability DESC LIMIT 20;"
```

See `docs/03_MECHANISMS/StageC_v2_Autonomy_Gating.md` for the v4 bounded goal formation rules and governance veto criteria.

### Stage C v5: Scope-gated learning (audit-required)
Stage C v5 keeps the v4 governance computation and adds runtime scope-gating for experimental learning subsystems. When `--stagec=v5` is active:

- A MemoryDB run must be active (`--memory-db=...`) for any gated learning to be allowed.
- Learning is allowed only inside explicitly opened scopes (`--open-scope=NAME`).
- Scope decisions are logged to `language_audit_log` as `scope_open`, `scope_allow`, and `scope_block`.
- **Coherence Penalty**: Implements a 6.0x standard deviation penalty to filter noise and enforce high-fidelity assembly formation.

Example: validate substrate learning governance with exactly one open scope and authority unchanged:
```powershell
.\neuroforge.exe --unified-substrate --enable-learning ^
  --stagec=v5 --open-scope=substrate ^
  --phase8=off --phase9=off --phase10=off --phase11=off --phase13=off ^
  --memory-db=phasec_mem.db --memdb-interval=100 ^
  --steps=200 --step-ms=10
```

Inspect audit events:
```powershell
sqlite3 phasec_mem.db "SELECT ts_ms, step, event, token, allowed, substr(details_json, 1, 200) FROM language_audit_log ORDER BY ts_ms DESC LIMIT 50;"
```

### Embodiment & Procedural Maze (New)
Launch the agent in an 8x8 procedural maze with first-person vision and unified brain control:
```powershell
.\neuroforge.exe --unified-substrate --embodiment --steps=1000
```
- **Vision**: 3-sector depth perception (Left/Center/Right).
- **Action**: Heuristic mapping bootstraps exploration (Forward/Turn).
- **Metrics**: Real-time tracking of CIP and Self-Model Confidence.
...
### Notes
- Flags: `--phase10` persists `metacognition.self_explanation_json`; `--phase11` writes revisions and embeds `phase10_revision_explanation` into `self_revision_log.driver_explanation`
- Controls: `--phase11-revision-interval=<ms>` (default 300000)
