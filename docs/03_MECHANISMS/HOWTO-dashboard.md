_Updated: 2025-12-18 • Version: Phase6-11 v2.5 • Phase15 v1.3_
...
- Phase 10 persists `metacognition.self_explanation_json` to explain the latest self-trust update; Phase 11 consumes recent explanations for pattern detection.
- Phase 11 applies parameter revisions; `self_revision_log.driver_explanation` is persisted as a human-readable string for auditability. Revision metadata is also emitted into `personality_history.trait_json` (JSON) with `proposal=1, approved=0`.
- Stage 7.5: Phase 11 evaluates revision outcomes and persists them in `self_revision_outcomes`. Phase 10 may inject the latest evaluated outcome into `metacognition.self_explanation_json` under `stage7_5`.
- Stage C v1–v5: Phase 11 may apply a governance-only autonomy cap derived from recent `self_revision_outcomes` (with v2/v3/v4 extending the cap computation). Phase 10 may inject the selected governance block into `metacognition.self_explanation_json` under `stage_c_v1`, `stage_c_v2`, `stage_c_v3`, `stage_c_v4`, or `stage_c_v5`. Stage C v5 scope gating emits `scope_open`/`scope_allow`/`scope_block` events into `language_audit_log`.
...
- For Phase 11, you may optionally augment the export with a `revisions` array for UI summaries:
 - Phase 11 controls: `--phase11-revision-interval=<ms>` (default `300000`), `--phase11-min-gap-ms=<ms>` (default `60000`), `--phase11-outcome-window-ms=<ms>` (default `60000`).
```json
{
  "metacognition": [...],
...
- Provide toggles for revision-interval overlays and revision event tooltips from `driver_explanation`.
