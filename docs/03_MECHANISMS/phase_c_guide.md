...
```powershell
.\neuroforge.exe --unified-substrate --gpu --enable-learning ^
  --stagec=v5 --open-scope=substrate ^
  --memory-db=phasec_mem.db --memdb-interval=50 ^
  --steps=1000
```
- **Embodiment Mode (Procedural Maze)**:
```powershell
.\neuroforge.exe --unified-substrate --embodiment --steps=500
```
...
```
Notes:
- When `--stagec=v5` is enabled, Phase C substrate learning is scope-gated: `--enable-learning` is not sufficient without `--open-scope=substrate` and an active MemoryDB run.
- **Coherence Penalty**: Stage C v5 uses a 6.0x standard deviation penalty (`coherence = mean * (1.0 - 6.0 * std_dev)`) to enforce strict assembly synchronization.

## Embodiment Integration (New)
The system now supports a first-person embodiment mode using `FirstPersonMazeRenderer` and `SubstratePhaseCAdapter`.
- **Vision**: 8x8 procedural maze rendered to a 3-sector visual field (Left, Center, Right).
- **Action**: Heuristic mapping converts vision tokens (e.g., `S_OPEN_WALL_WALL`) to motor commands (`FORWARD`, `LEFT`, `RIGHT`).
- **Metrics**: Tracks CIP (Coherence Index), Self-Model Confidence, and Adaptation Rate.
- **Collision**: Reset mechanism prevents stalling upon wall impact.
...
```powershell
python .\scripts\db_inspect.py --db "<repo_root>\phasec_mem.db" --limit-json 1
```
...
- Logs: `<repo_root>\analysis\phase_c\run_learn_new\`
- Plots/Summary: saved alongside the CSV logs
- Telemetry DB: `<repo_root>\phasec_mem.db`
- Validation JSON: `<repo_root>\analysis\phase_c\phase_c_validation_run6_latest.json`
