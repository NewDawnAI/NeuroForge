...
#### **Stage D: Meta-Cognition & Hive (New)**
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `--embodiment[=on\|off]` | Boolean | off | Enable First-Person Embodiment Loop |
| `--unified-substrate[=on\|off]` | Boolean | off | Enable Unified Phase C Substrate |
| `--hive[=on\|off]` | Boolean | off | Enable Hive Telepathy (P2P State Sharing) |
| `--hive-id=ID` | String | node_1 | Unique ID for this Hive Node |
| `--phase15[=on\|off]` | Boolean | off | Enable Ethics Regulation |
| `--phase15-risk-threshold=VAL` | Float | 0.5 | Threshold for Ethics Block |
...
- References: `src/main.cpp` (search for `triplet_ingestion`, `dataset-mode=triplets`, and `snapshot:phase_a`).
