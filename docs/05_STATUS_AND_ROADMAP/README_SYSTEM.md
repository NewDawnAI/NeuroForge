...
- `--phase6[=on|off]` Reasoner (shadow logging; no behavior changes by default)
- `--phase7[=on|off]` Affective State + Reflection (requires MemoryDB)
- `--phase8[=on|off]` Goal System
- `--phase9[=on|off]` Metacognition Core
- `--phase10[=on|off]` Self-Explanation (persists `metacognition.self_explanation_json`)
- `--phase11[=on|off]` Self-Revision (persists `self_revision_log`, `parameter_history`, `self_revision_outcomes`)
- `--phase12[=on|off]` Consistency
- `--phase13[=on|off]` Autonomy Envelope
- `--phase14[=on|off]` Meta-Reasoner
- `--phase15[=on|off]` Ethics & Constraint Regulator

Phase 11 controls:
- `--phase11-revision-interval=<ms>` (default `300000`)
- `--phase11-min-gap-ms=<ms>` (default `60000`)
- `--phase11-outcome-window-ms=<ms>` (default `60000`)

Example combined run:

`build\neuroforge.exe --phase6=on --phase7=on --phase8=on --phase9=on --phase10=on --phase11=on --phase12=on --phase13=on --phase14=on --phase15=on`
