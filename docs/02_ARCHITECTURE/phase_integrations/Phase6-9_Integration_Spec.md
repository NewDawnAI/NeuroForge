...
Stage C v2 is a governance-only cap mechanism that computes two independent caps:

- A harm-risk cap from a conservative upper bound over recent `self_revision_outcomes`
- An earned-cap from an `Autonomy Credit` scalar persisted to `autonomy_credit_log`

The effective Stage C v2 cap is:

- `autonomy_cap_multiplier = min(harm_risk_cap_multiplier, autonomy_credit_cap_multiplier)`

Key properties:
- Reads `self_revision_outcomes`, `autonomy_credit_log` (latest credit), and supporting governance signals (`self_consistency_log`, `ethics_regulator_log`)
- Writes only via `AutonomyEnvelope::applyAutonomyCap` and by persisting credit telemetry to `autonomy_credit_log`
- Never increases autonomy above the base envelope (cap multiplier is always `<= 1.0`)
- Emits audit surfaces in:
  - `metacognition.self_explanation_json` (`stage_c_v2`)
  - `autonomy_envelope_log.driver_json` (`autonomy_cap_multiplier`, `effective_autonomy`)

References:
- Gate + credit logic: `src/core/StageC_AutonomyGate.cpp`
- Phase 11 wiring (update credit then apply cap): `src/core/Phase11SelfRevision.cpp`
...
- Emits audit surfaces in:
  - `metacognition.self_explanation_json` (`stage_c_v3`)
  - `autonomy_envelope_log.driver_json` (`autonomy_cap_multiplier`, `effective_autonomy`)
...
- Emits telemetry in `metacognition.self_explanation_json` under `stage_c_v4`.
...
- Phase 10 Self‑Explanation
  - Purpose: Persist a structured explanation for the latest Phase 9 metacognition update
  - Produces: `metacognition.self_explanation_json` (JSON)
  - Location: `src/core/Phase10SelfExplanation.cpp`, header `include/core/Phase10SelfExplanation.h`
  - Interfaces:
    - `bool runForLatest(const std::string& context = "")`
    - `bool runForMetacogId(std::int64_t metacog_id, const std::string& context = "")`
    - `static std::string synthesizeRevisionNarrative(...)` (used by Phase 11 to explain revisions)
  - Wiring:
    - Invoked after Phase 9 persists a metacognition row (or on a periodic metacog heartbeat)
    - Phase 11 later consumes recent explanations via `MemoryDB::getRecentExplanations`

- Phase 11 Self‑Revision
  - Purpose: Analyze recent Phase 10 explanations and Phase 9 metacognition trends, then propose safe parameter deltas
  - Produces: persisted revisions in `self_revision_log` and `parameter_history`; optional proposal rows in `personality_history`
  ...
  - Interfaces:
    ...
    - `bool runForLatest(const std::string& context = "")`
    - Configuration: `setRevisionInterval`, `setTrustDriftThreshold`, `setAnalysisWindow`, `setMinRevisionGap`
  - Explainability:
    - Each `self_revision_log.driver_explanation` is JSON containing a Phase 11 summary plus an embedded Phase 10 revision narrative under `phase10_revision_explanation`
...
- `--phase10[=on|off]` (default on)
- `--phase11[=on|off]` (default on)
...
-- Phase 11: Parameter history (per revision)
...
1) Phase 9 persists metacognition; Phase 10 explains it
- Inputs: Phase 9 metacognition deltas and prediction/resolution outcomes
- Outputs: `metacognition.self_explanation_json` via `Phase10SelfExplanation::runForLatest(...)`

2) Phase 11 evaluates revision necessity
- Consumes recent Phase 10 explanations via `MemoryDB::getRecentExplanations` and recent metacognition rows via `MemoryDB::getRecentMetacognition`
- Generates parameter deltas, clamps and validates them, and persists to `self_revision_log` + `parameter_history`
- Emits optional `personality_history` proposals linked by `revision_id`
- Attaches a structured Phase 10 revision narrative into `self_revision_log.driver_explanation` (JSON)

3) Phase 15 ethical decisioning and personality approval
...
- Phase 10 Self‑Explanation
  - `class Phase10SelfExplanation { public: bool runForLatest(const std::string& context=""); bool runForMetacogId(std::int64_t metacog_id, const std::string& context=""); }`

- Phase 11 Self‑Revision
  - `class Phase11SelfRevision { public: bool maybeRevise(const std::string& context=""); bool runForLatest(const std::string& context=""); }`
