...
public:
  ...
  using SpeechOutputCallback = std::function<void(const std::string &)>;

  /**
   * @brief Set callback for speech output (TTS integration)
   */
  void setSpeechOutputCallback(SpeechOutputCallback callback) {
    speech_output_callback_ = callback;
  }
  ...
  struct SymbolicToken {
    std::string
        symbol; ///< DEBUG TAG — human label for logging/TTS, not for cognition
    TokenType type; ///< Token category
    std::vector<float>
        embedding; ///< Neural embedding vector (PRIMARY identifier)
    ...
    std::vector<NeuroForge::NeuronID>
        associated_neurons; ///< Linked neural patterns
    std::unordered_map<std::string, float>
        sensory_associations; ///< DEBUG — modality labels, use embedding for
                              ///< cognition
  };
  ...
  struct PhonemeCluster {
    std::string phonetic_symbol;       ///< DEBUG TAG — IPA label for logging,
                                       ///< matching uses formant_pattern
    AcousticFeatures acoustic_profile; ///< Associated acoustic features
    std::vector<float>
        formant_pattern; ///< Formant frequency pattern (PRIMARY identifier)
    float vowel_consonant_ratio = 0.5f; ///< V/C classification score
    std::vector<std::string> variants;  ///< DEBUG — acoustic variation labels
    ...
    std::vector<std::string>
        proto_word_candidates; ///< DEBUG — emerging pattern labels
    std::chrono::steady_clock::time_point
        last_reinforced; ///< Last strengthening event
  };

  /**
   * @brief Get currently active symbolic tokens
   * @param activation_threshold Minimum activation to include
   */
  std::vector<SymbolicToken>
  getActiveTokens(float activation_threshold = 0.5f) const;

  /**
   * @brief Inject a concept from the reasoning system
   * @param concept_key The concept identifier (word/token)
   * @param confidence The confidence/strength of the reasoned concept
   */
  void injectReasonedConcept(const std::string &concept_key, double confidence);

  /**
   * @brief Proto-word structure for tracking emerging word patterns
   */
  struct ProtoWord {
    std::string pattern; ///< DEBUG TAG — phoneme sequence label, stability
                         ///< measured by vectors
    std::vector<std::string>
        phoneme_sequence;               ///< DEBUG — phoneme labels for logging
    float stability_score = 0.0f;       ///< Pattern stability measure
    std::uint64_t occurrence_count = 0; ///< Number of times pattern occurred
    float caregiver_response_strength =
        0.0f; ///< Caregiver attention/response level

    // Cross-modal associations (PRIMARY — vector-based)
    ...
    std::chrono::steady_clock::time_point
        first_occurrence; ///< When pattern first emerged
    std::chrono::steady_clock::time_point
  ...
  struct MultimodalAttentionState {
    ...
    bool is_joint_attention_active =
        false; ///< Whether joint attention is occurring

    // Temporal dynamics
    std::chrono::steady_clock::time_point last_attention_peak;
    float attention_persistence = 0.0f; ///< How long attention is maintained
    std::vector<std::pair<float, std::chrono::steady_clock::time_point>>
        attention_events;
  };
  ...
  struct GroundingAssociation {
    std::size_t token_id;        ///< Associated language token (PRIMARY key)
    std::string object_category; ///< DEBUG TAG — grounding uses feature
                                 ///< vectors, not this label
    std::vector<float> visual_features;   ///< Visual object features (PRIMARY)
    std::vector<float> tactile_features;  ///< Tactile/haptic features (PRIMARY)
    std::vector<float> auditory_features; ///< Object-related sounds (PRIMARY)
    ...
    float tactile_grounding_confidence =
        0.0f; ///< Tactile association confidence
    float auditory_grounding_confidence =
        0.0f; ///< Auditory association confidence

    // Contextual information (vector-based)
    ...
    std::string interaction_type;        ///< DEBUG TAG — interaction label

    // Learning dynamics
    std::uint64_t exposure_count = 0; ///< Number of exposures to this object
    std::chrono::steady_clock::time_point
        first_encounter; ///< First time object was seen
    std::chrono::steady_clock::time_point
        last_encounter;               ///< Most recent encounter
  ...
  struct ProsodicPattern {
    std::string pattern_name; ///< DEBUG TAG — human label for logging
    std::vector<float> pitch_trajectory; ///< Pitch contour over time (PRIMARY)
    std::vector<float>
        energy_trajectory;             ///< Energy envelope over time (PRIMARY)
    std::vector<float> rhythm_pattern; ///< Temporal rhythm structure (PRIMARY)
    ...
    std::vector<std::string>
        associated_tokens; ///< DEBUG — token labels that co-occur

    // Attention and learning effects
    float attention_weight = 0.0f; ///< How much attention this pattern draws
    float learning_boost_factor =
        0.0f; ///< Learning enhancement from this pattern
    bool is_motherese_pattern =
        false; ///< Whether this is infant-directed speech
    ...
    std::vector<std::chrono::steady_clock::time_point>
        detection_history; ///< Detection timeline
  };
  ...
  struct CaregiverContext {
    std::vector<float> face_embedding; ///< Caregiver face features (PRIMARY)
  ...
  struct IntonationGuidedAttention {
    float current_intonation_salience =
        0.0f; ///< Current intonation attention level
    std::vector<float> intonation_history; ///< Recent intonation patterns
    float rising_intonation_preference =
        0.8f; ///< Preference for rising intonation
    float falling_intonation_preference =
        0.3f; ///< Preference for falling intonation

    // Pattern-based attention modulation
    std::unordered_map<std::string, float>
        pattern_attention_weights; ///< Pattern → attention weight
    float prosodic_attention_threshold =
        0.4f; ///< Threshold for prosodic attention activation
    bool is_prosodic_attention_active =
        false; ///< Whether prosodic attention is currently active

    // Learning guidance
    float intonation_learning_boost =
        0.0f; ///< Current learning boost from intonation
    std::vector<std::pair<float, float>>
        intonation_learning_history; ///< (intonation, learning_boost) pairs
  ...
  struct FaceSpeechCoupling {
    std::unordered_map<std::string, float>
        proto_word_face_associations; ///< Proto-word → face association
                                      ///< strength
    ...
    bool is_caregiver_interaction =
        false; ///< Whether current interaction is with caregiver
    float caregiver_recognition_confidence =
        0.0f;                       ///< Confidence in caregiver recognition
  ...
  struct SensoryExperience {
    std::string experience_type;     ///< DEBUG TAG — experience label
    std::vector<float> sensory_data; ///< Raw sensory data (PRIMARY)
    float salience_score = 0.0f;     ///< Importance/attention score
    std::chrono::steady_clock::time_point
        timestamp;                  ///< When experience occurred
    std::string associated_context; ///< DEBUG TAG — context label
    float emotional_valence = 0.0f; ///< Emotional response
    std::vector<std::string>
        co_occurring_tokens;            ///< DEBUG — token labels for logging
    std::vector<float> sensory_pattern; ///< Processed sensory pattern (PRIMARY)
  ...
  struct CrossModalAssociation {
    std::size_t token_id; ///< Associated language token (PRIMARY key)
    std::string modality; ///< DEBUG TAG — modality label (should use enum)
    std::vector<float> pattern;        ///< Sensory pattern vector (PRIMARY)
    float association_strength = 0.0f; ///< Binding strength
    float temporal_alignment = 0.0f;   ///< Synchronization score
    std::chrono::steady_clock::time_point
        last_reinforced; ///< Temporal tracking
  ...
  struct SpeechProductionFeatures {
    std::string original_text; ///< EXTERNAL I/O ONLY — text for TTS output
    std::vector<PhonemeCluster>
        phoneme_sequence;               ///< Sequence of phonemes to produce
    ...
    std::vector<std::vector<float>>
        lip_motion_sequence;                  ///< Lip shapes for each phoneme
  ...
  struct Config {
    ...
    float narration_threshold =
        0.3f; ///< Activation threshold for internal speech

    // Acoustic processing parameters
    float prosody_attention_weight =
        0.4f; ///< Boost for prosodic salience (increased from 0.3f)
    float intonation_threshold =
        0.1f; ///< Hz change for attention trigger (lowered from 0.3f)
    float motherese_boost = 0.4f; ///< Amplification for infant-directed speech
    float formant_clustering_threshold =
        50.0f; ///< Hz threshold for phoneme clustering
    ...
    float cross_modal_decay =
        0.002f; ///< Association decay rate (reduced from 0.005f)

    // Token similarity and cohesion parameters
    float token_similarity_threshold =
        0.3f; ///< Similarity threshold for token clustering (reduced from 0.5f)
    float cohesion_boost_factor =
        2.0f; ///< Boost factor for co-occurring tokens (increased from 1.5f)
    ...
    bool enable_acoustic_preprocessing =
        true; ///< Enable acoustic-first babbling approach
    bool enable_prosodic_embeddings =
        true; ///< Enable prosodic pattern embeddings
    bool enable_sound_attention_bias =
        true; ///< Enable sound-based attention biasing
    float proto_word_crystallization_rate =
        0.05f; ///< Rate of proto-word formation
    float phoneme_stability_threshold =
        0.6f; ///< Threshold for stable phoneme patterns
    float caregiver_response_boost =
        0.8f; ///< Boost for caregiver-reinforced patterns
    std::uint64_t min_occurrences_for_crystallization =
        3; ///< Minimum pattern repetitions
    float pattern_similarity_threshold =
        0.8f; ///< Similarity threshold for pattern matching

    // Cross-modal integration enhancements
    float multimodal_attention_weight =
        0.7f; ///< Weight for multimodal attention
    float joint_attention_threshold =
        0.6f; ///< Threshold for joint attention detection
    float joint_attention_learning_boost =
        0.8f; ///< Learning boost from joint attention events
    std::uint32_t attention_history_length =
        10; ///< Length of attention history buffer
    float face_speech_coupling_rate =
        0.08f; ///< Rate of face-speech coupling learning
    float caregiver_recognition_boost =
        0.9f; ///< Boost for recognized caregiver faces
    bool enable_enhanced_multimodal_attention =
        true; ///< Enable enhanced attention system
    float proto_word_face_association_strength =
        0.5f; ///< Strength of proto-word face associations

    // Grounding association parameters
    float grounding_association_strength =
        0.6f; ///< Base strength for new grounding associations
    ...
    float grounding_stability_threshold =
        0.7f; ///< Threshold for stable grounding
    std::uint64_t min_exposures_for_stable_grounding =
        5; ///< Minimum exposures for stability

    // Prosodic pattern learning parameters
    float prosodic_pattern_learning_rate =
        0.06f; ///< Rate of prosodic pattern learning
    float intonation_attention_boost =
        0.7f; ///< Attention boost for salient intonation
    float motherese_pattern_boost =
        0.9f; ///< Extra boost for motherese patterns
    float motherese_face_coupling_boost =
        0.5f; ///< Boost for face-speech coupling during motherese
    float prosodic_pattern_stability_threshold =
        0.6f; ///< Threshold for stable prosodic patterns
    std::uint64_t min_pattern_occurrences =
        3; ///< Minimum occurrences for pattern recognition

    // Intonation-guided learning parameters
    float rising_intonation_learning_boost =
        0.8f; ///< Learning boost for rising intonation
    float falling_intonation_learning_boost =
        0.4f; ///< Learning boost for falling intonation
    float prosodic_attention_adaptation_rate =
        0.05f; ///< Rate of attention threshold adaptation
    bool enable_prosodic_pattern_learning =
        true; ///< Enable prosodic pattern learning system
    std::uint64_t prosodic_pattern_history_length =
        30; ///< Length of prosodic pattern history

    // Sensory experience processing parameters
    std::uint64_t sensory_experience_history_length =
        100; ///< Length of sensory experience history
    float salience_threshold =
        0.5f; ///< Threshold for salient sensory experiences
    float sensory_experience_learning_rate =
        0.02f; ///< Learning rate for sensory experience integration

    // Target proto-words for biased generation
    std::vector<std::string> target_proto_words; ///< Target proto-word patterns
                                                 ///< for biased generation
  };
  ...
  struct NarrationEntry {
    ...
    std::string context;           ///< DEBUG TAG — context label for logging
...
private:
  ...
  MemoryDB *audit_db_{nullptr};
  std::int64_t audit_run_id_{0};
  std::uint64_t audit_step_{0};
  int audit_stage_{2};

  // Core vocabulary (embedding-indexed, token_id is the position)
  std::vector<SymbolicToken> vocabulary_;
  std::unordered_map<std::string, std::size_t>
      token_lookup_; // DEBUG INDEX — convenience only
  ...
  std::unordered_map<std::string, std::size_t>
      proto_word_lookup_; // DEBUG INDEX
  ...
  std::unordered_map<std::string, AcousticFeatures>
      acoustic_memory_; // DEBUG INDEX
  ...
  std::unordered_map<std::string, std::vector<SensoryExperience>>
      experience_type_lookup_;
  ...
  // Grounding associations
  std::unordered_map<NeuroForge::NeuronID, std::vector<std::size_t>>
      neuron_to_tokens_;
  ...
  std::unordered_map<std::size_t, std::vector<VisualLanguageFeatures>>
      token_visual_features_;
...
public:
  ...
  void setAuditSink(MemoryDB *db, std::int64_t run_id);
  void setAuditStep(std::uint64_t step);
  void setAuditLanguageGateStage(int stage);
  bool observeGroundingToken(const std::string &token,
  ...
  findSimilarTokens(const std::vector<float> &embedding,
                    float threshold = 0.8f) const;

  // Mimicry learning
  void setTeacherEmbedding(const std::string &label,
                           const std::vector<float> &embedding);
  void processTeacherSignal(const std::string &label,
                            float reward_signal = 1.0f);
  std::vector<float>
  generateMimicryResponse(const std::vector<float> &teacher_embedding);

  // Enhanced multimodal grounding with visual-linguistic integration
  void associateTokenWithNeuron(std::size_t token_id,
                                NeuroForge::NeuronID neuron_id,
                                float association_strength = 1.0f);
  void associateTokenWithModality(std::size_t token_id,
                                  const std::string &modality,
                                  const std::vector<float> &pattern,
                                  float strength = 1.0f);
  void associateTokenWithVisualFeatures(
      std::size_t token_id, const VisualLanguageFeatures &visual_features,
  ...
  void updateCrossModalAssociations(
  ...
  generateSpeechOutput(const std::vector<std::string> &token_sequence) const;
  std::vector<PhonemeCluster>
  generatePhonemeSequence(const std::string &text) const;
  std::vector<std::vector<float>>
  generateLipMotionSequence(const std::vector<PhonemeCluster> &phonemes) const;
  std::vector<float>
  generateProsodyContour(const std::vector<PhonemeCluster> &phonemes,
  ...
  SpeechOutputState getCurrentSpeechState() const {
    return speech_output_state_;
  }

  // Self-monitoring and feedback
  void processSelfAcousticFeedback(const std::vector<float> &heard_audio);
  void
  processCaregiverResponse(const VisualLanguageFeatures &caregiver_reaction,
  ...
  void
  reinforceCaregiverMimicry(const std::string &spoken_token,
  ...
  void enableTrajectoryTracking(
      const std::string &log_directory = "trajectory_logs");
  void captureTrajectorySnapshot();
  void generateDevelopmentalReport();

  // Cross-modal pattern retrieval
  std::vector<std::size_t> getTokensForNeuralPattern(
      const std::vector<NeuroForge::NeuronID> &neurons) const;
  std::vector<std::size_t>
  getTokensForVisualPattern(const std::vector<float> &visual_pattern,
                            float similarity_threshold = 0.7f) const;
  std::vector<CrossModalAssociation>
  getCrossModalAssociations(std::size_t token_id) const;
  ...
  AcousticFeatures
  extractAcousticFeatures(const std::vector<float> &audio_samples,
  ...
  std::vector<float> generateAudioSnippet(const PhonemeCluster &phoneme,
                                          float duration_ms) const;
  std::vector<float> generateProsodicallyEnhancedEmbedding(
      const AcousticFeatures &acoustic_features) const;

  // Proto-word crystallization and babbling stage enhancements
  void processProtoWordCrystallization();
  std::size_t createProtoWord(const std::string &pattern,
                              const std::vector<std::string> &phonemes);
  void reinforceProtoWord(std::size_t proto_word_id,
                          float reinforcement_strength);
  void updatePhonemeStability(const std::string &phoneme,
                              float usage_boost = 1.0f);
  float calculatePatternSimilarity(const std::string &pattern1,
  ...
  std::size_t
  createGroundingAssociation(std::size_t token_id,
  ...
  void reinforceGroundingAssociation(std::size_t grounding_id,
                                     float reinforcement_strength);
  void updateGroundingAssociation(std::size_t grounding_id,
  ...
  std::vector<std::size_t>
  findGroundingAssociationsForToken(std::size_t token_id) const;
  std::vector<std::size_t>
  findGroundingAssociationsForObject(const std::string &object_category) const;

  // Multimodal grounding and semantic anchoring
  void
  processMultimodalGroundingEvent(const std::string &spoken_token,
                                  const std::vector<float> &visual_features,
                                  const std::vector<float> &tactile_features,
                                  const std::vector<float> &auditory_features,
                                  const std::string &object_category);
  void strengthenSemanticAnchoring(std::size_t token_id,
                                   const std::vector<float> &sensory_pattern,
                                   const std::string &modality,
                                   float anchoring_strength);
  float calculateSemanticGroundingStrength(std::size_t token_id) const;
  void promoteToSemanticallGrounded(std::size_t token_id);

  // Prosodic pattern learning and intonation-guided attention
  void
  processProsodicPatternLearning(const AcousticFeatures &acoustic_features,
                                 const std::string &co_occurring_token = "");
  std::size_t detectProsodicPattern(const AcousticFeatures &acoustic_features);
  void reinforceProsodicPattern(std::size_t pattern_id,
                                float reinforcement_strength);
  bool isStableProsodicPattern(const ProsodicPattern &pattern) const;
  std::vector<std::size_t>
  findSimilarProsodicPatterns(const AcousticFeatures &acoustic_features,
                              float similarity_threshold = 0.7f) const;

  // Intonation-guided attention and learning
  void
  updateIntonationGuidedAttention(const AcousticFeatures &acoustic_features);
  float calculateIntonationLearningBoost(
      const AcousticFeatures &acoustic_features) const;
  void adaptProsodicAttentionThreshold(float current_intonation_salience);
  void
  processIntonationGuidedLearning(const std::string &vocalization,
                                  const AcousticFeatures &acoustic_features);

  // Enhanced prosodic analysis and pattern recognition
  ProsodicPattern extractProsodicPattern(
      const std::vector<AcousticFeatures> &acoustic_sequence) const;
  float
  calculateProsodicPatternSimilarity(const ProsodicPattern &pattern1,
                                     const ProsodicPattern &pattern2) const;
  void updateProsodicPreferences(const std::string &pattern_name,
  ...
  void
  enhanceProtoWordWithProsodicPattern(const std::string &proto_word_pattern,
                                      const ProsodicPattern &prosodic_pattern);
  float calculateProsodicBoostForProtoWord(
  ...
  float calculateFaceSpeechCouplingStrength(
      const std::vector<float> &face_embedding,
      const AcousticFeatures &acoustic_features) const;
  bool detectMotherese(const AcousticFeatures &acoustic_features) const;
  void
  processCaregiverFaceRecognition(const std::vector<float> &face_embedding);
  ...
  void processAttentionGuidedLearning(
      const std::string &vocalization,
  ...
  float calculateAttentionBasedLearningRate(
  ...
  void trackPhonemeSequencePatterns(
      const std::vector<std::string> &phoneme_sequence);
  std::vector<std::string>
  generateProtoWordSequence(const std::string &target_pattern);

  // Pattern analysis and crystallization
  void analyzeEmergingPatterns();
  std::vector<std::string>
  extractPatternsFromVocalization(const std::string &vocalization);
  ...
  float
  computeIntonationSalience(const std::vector<float> &pitch_contour) const;
  void
  updateAttentionWeights(const std::vector<AcousticFeatures> &acoustic_stream);
  ...
  void setNeuronBiasCallback(NeuronBiasCallback cb) {
    neuron_bias_callback_ = std::move(cb);
  }
  ...
  std::vector<std::string>
  getActiveVocabulary(float activation_threshold = 0.1f) const;
  ...
  // ========== TransE Relation Scoring (for RelationGateManager) ==========

  /**
   * @brief Score a relation triple using TransE: score = 1 / (1 + ||subj + rel
   * - obj||)
   * @param subject Subject token symbol
   * @param relation Relation token symbol
   * @param object Object token symbol
   * @return Score in [0, 1], higher = more plausible
   */
  float scoreRelationTriple(const std::string &subject,
   ...
   * @return Vector of (relation_symbol, score) pairs, sorted descending
   */
  std::vector<std::pair<std::string, float>>
  proposeRelationsFor(const std::string &subject, const std::string &object,
                      int top_k = 5) const;

  /**
   * @brief Get all tokens of type Relation
   * @return Vector of relation token symbols
   */
  std::vector<std::string> getAllRelationTokens() const;

  /**
   * @brief Compute TransE distance: ||subj + rel - obj||
   */
  float transeDistance(const std::vector<float> &subj_emb,
  ...
  private:
    ...
    float calculateCrossModalStrength(const LanguageSystem &language_system,
                                      std::size_t token_id);
    void analyzeTrajectoryProgression(std::ostringstream &report,
                                      const LanguageSystem &language_system);
    void analyzeCrossModalBinding(std::ostringstream &report,
                                  const LanguageSystem &language_system);
    void generateStagePredictions(std::ostringstream &report,
                                  const LanguageSystem &language_system);
  ...
  public:
    ...
    std::string
    generateDevelopmentalReport(const LanguageSystem &language_system);
    void reset();
  };
...
private:
  ...
  // Token operations
  float calculateTokenSimilarity(const SymbolicToken &token1,
  ...
  std::vector<float>
  normalizeEmbedding(const std::vector<float> &embedding) const;
  ...
  // Phase 15a: Symbolic similarity for cold-start TransE fallback
  float computeSymbolicSimilarity(const std::string &subject,
                                  const std::string &relation,
                                  const std::string &object) const;
  float computeLexicalSimilarity(const std::string &a,
                                 const std::string &b) const;
  float getPredicatePrior(const std::string &relation) const;

  // Caregiver response functions
  float
  calculateCaregiverResponseStrength(const CaregiverContext &context) const;
  void reinforceBasedOnCaregiverFeedback(const std::string &vocalization,
                                         const CaregiverContext &context);

  // Multimodal attention functions
  void updateMultimodalAttention(const VisualLanguageFeatures &visual_features,
                                 const AcousticFeatures &acoustic_features);
  void processJointAttentionEvent(const std::vector<float> &shared_gaze_target,
                                  const std::string &spoken_token,
                                  float attention_strength);
  float calculateJointAttentionScore(
      const VisualLanguageFeatures &visual_features) const;
  void
  reinforceProtoWordFaceAssociation(const std::string &proto_word_pattern,
                                    const std::vector<float> &face_embedding,
                                    float association_strength);

  // Sensory experience integration
  void
  integrateSensoryExperienceWithProtoWords(const SensoryExperience &experience);
  void updateSensoryContext(const std::vector<float> &sensory_data);
  void boostProtoWordFromSensoryExperience(const std::string &proto_word,
                                           float boost_strength);
  float
  calculateSensoryExperienceNovelty(const SensoryExperience &experience) const;
  std::vector<SensoryExperience>
  getSimilarExperiences(const SensoryExperience &experience,
                        float threshold) const;
  void processExperienceDrivenLearning(
      const std::string &vocalization,
      const std::vector<SensoryExperience> &concurrent_experiences);
  float calculateExperienceBasedLearningBoost(
      const SensoryExperience &experience) const;

  // Developmental transitions
  bool shouldAdvanceStage() const;
  void onStageTransition(DevelopmentalStage from_stage,
  ...
  void logDevelopmentalEvent(const std::string &event,
