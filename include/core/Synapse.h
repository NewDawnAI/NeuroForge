...
public:
  ...
  struct VirtualSynapse {
    static float weight(uint64_t pre, uint64_t post,
                        uint64_t seed = 0x517cc1e6) {
    ...
    static float delay(uint64_t pre, uint64_t post,
                       uint64_t seed = 0x517cc1e6) {
    ...
    static bool exists(uint64_t pre, uint64_t post, float p = 0.01f,
                       uint64_t seed = 0x517cc1e6) {
  ...
  enum class PlasticityRule {
    ...
    Universal = 5 ///< Universal learning: Δw = η·δ·∇I(gain)
  };
   ...
   */
  Synapse(NeuroForge::SynapseID id, NeuroForge::NeuronWeakPtr source,
   ...
   */
  void setPlasticityRule(PlasticityRule rule) noexcept {
    plasticity_rule_ = rule;
  }
   ...
   */
  void updateWeight(float pre_activation, float post_activation,
                    float delta_time);
   ...
   * @param delta_time Time step
   */
  void applyHebbianLearning(float pre_activation, float post_activation,
                            float delta_time);
   ...
   */
  void applySTDP(NeuroForge::TimePoint pre_spike_time,
                 NeuroForge::TimePoint post_spike_time);
   ...
   */
  void setWeightBounds(NeuroForge::Weight min_weight,
                       NeuroForge::Weight max_weight) noexcept {
    min_weight_ = min_weight;
    max_weight_ = max_weight;
  }

  /**
   * @brief Public wrapper to apply safety guardrails to a proposed weight
   * change. Useful for accelerated update paths to preserve CPU semantics.
   * @param delta_w Proposed weight change
   * @return Guardrail-adjusted, safe weight change
   */
  float applySafetyGuardrailsPublic(float delta_w) const noexcept {
    return applySafetyGuardrails(delta_w);
  }
  ...
  float getEligibility() const noexcept {
    return eligibility_.load(std::memory_order_relaxed);
  }
  void setEligibility(float e) noexcept {
    eligibility_.store(e, std::memory_order_relaxed);
  }
  void decayEligibility(float decay_rate, float dt) noexcept;
  void accumulateEligibility(float pre_activation, float post_activation,
...
private:
  ...
  mutable std::vector<DelayedSignal>
      signal_buffer_; ///< Buffer for delayed signals
  mutable std::mutex
      signal_buffer_mutex_; ///< Mutex to protect signal_buffer_ access
  ...
  mutable std::atomic<std::uint64_t>
      signal_count_; ///< Signal propagation count
  mutable std::atomic<std::uint64_t> update_count_; ///< Weight update count
  mutable std::atomic<NeuroForge::Weight>
      weight_sum_; ///< Sum for average calculation
  mutable std::atomic<NeuroForge::Weight>
      min_recorded_weight_; ///< Minimum recorded weight
  mutable std::atomic<NeuroForge::Weight>
      max_recorded_weight_; ///< Maximum recorded weight
  ...
  // --- Safety guardrails ---
  static constexpr float MAX_GRADIENT_MAGNITUDE =
      0.5f; ///< Maximum allowed gradient magnitude
  static constexpr float MAX_WEIGHT_CHANGE_PER_STEP =
      0.1f; ///< Maximum weight change per update
  static constexpr float STABILITY_EPSILON =
      1e-8f; ///< Small value to prevent division by zero
  mutable std::atomic<std::uint32_t> consecutive_large_updates_{
      0}; ///< Count of consecutive large updates
  static constexpr std::uint32_t MAX_CONSECUTIVE_LARGE_UPDATES =
      10; ///< Threshold for instability detection
...
public:
  ...
  static NeuroForge::SynapsePtr createSynapse(
      NeuroForge::NeuronWeakPtr source, NeuroForge::NeuronWeakPtr target,
   ...
   */
  static NeuroForge::SynapsePtr createSynapse(
      NeuroForge::SynapseID id, NeuroForge::NeuronWeakPtr source,
      NeuroForge::NeuronWeakPtr target, NeuroForge::Weight weight = 0.1f,
      NeuroForge::SynapseType type = NeuroForge::SynapseType::Excitatory);
