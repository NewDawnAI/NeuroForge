...
/**
 * @brief Memory system type enum (replaces string-keyed maps)
 */
...
/**
 * @brief Weight-space cross-system link (replaces string-keyed maps)
 */
struct IntegrationLink {
  MemorySystemType source_system;
  std::uint64_t source_id = 0;
  MemorySystemType target_system;
  std::uint64_t target_id = 0;
  float strength = 1.0f;
  std::vector<float> link_embedding; // optional embedding for the link
};
...
struct CrossMemoryQuery {
  ...
  std::vector<std::string>
      target_systems; // "episodic", "semantic", "procedural"
...
public:
  ...
  explicit MemoryIntegrator(
      const IntegrationConfig &config = IntegrationConfig{});
  ~MemoryIntegrator() = default;

  // ════════════════════════════════════════════════
  //  EMBEDDING-BASED API (primary)
  // ════════════════════════════════════════════════

  /// Query all systems by embedding vector (no strings needed)
  std::vector<IntegratedMemoryResult>
  queryByEmbedding(const std::vector<float> &query_embedding,
                   std::size_t max_results = 10, float threshold = 0.3f);

  /// Create a typed cross-system link
  void createLink(MemorySystemType source, std::uint64_t source_id,
                  MemorySystemType target, std::uint64_t target_id,
                  float strength = 1.0f);

  /// Check system availability by enum
  bool canAccessSystem(MemorySystemType system) const;

  /// Get all integration links
  const std::vector<IntegrationLink> &getLinks() const {
    return integration_links_;
  }
  ...
  std::vector<IntegratedMemoryResult>
  queryAllSystems(const CrossMemoryQuery &query);
  void createCrossSystemLink(const std::string &source_system,
                             std::uint64_t source_id,
                             const std::string &target_system,
                             std::uint64_t target_id, float strength = 1.0f);
  ...
  std::vector<IntegratedMemoryResult>
  retrieveWithContext(const std::string &query, const std::string &context = "",
                      std::size_t max_results = 10);

  // Retrieval of relevant memories
  std::vector<std::vector<float>>
  retrieveRelevantMemories(const std::vector<float> &query,
...
private:
  // Helper methods
  float calculateCrossSystemSimilarity(const std::string &system1,
                                       std::uint64_t id1,
                                       const std::string &system2,
                                       std::uint64_t id2) const;
  void updateIntegrationStatistics();
  std::vector<float> extractEmbedding(const std::string &system,
...
private:
  ...
  // Cross-system links (weight-space)
  std::vector<IntegrationLink> integration_links_;

  // Cross-system links (legacy string-keyed)
  ...
  mutable std::shared_mutex integration_mutex_;
};
