...
struct ConsolidationConfig {
  ...
  // Weight-space config (new)
  float weight_decay_during_sleep = 0.01f; // global weight decay per cycle
  float replay_reinforcement_lr =
      0.05f; // learning rate for replay reinforcement
};
...
struct Statistics {
  ...
  // Weight-space stats (new)
  std::uint64_t weight_replay_count = 0;
  float total_weight_decay_applied = 0.0f;
};

class SleepConsolidation {
public:
  explicit SleepConsolidation(
      const ConsolidationConfig &config = ConsolidationConfig{});
  ~SleepConsolidation() = default;

  // Backward-compatible nested alias so tests using
  // SleepConsolidation::SleepPhase still compile
  ...
  void setEpisodicMemory(EpisodicMemoryManager *episodic_memory) {
    registerEpisodicMemory(episodic_memory);
  }
  void registerSemanticMemory(SemanticMemory *semantic_memory);
  void setSemanticMemory(SemanticMemory *semantic_memory) {
    registerSemanticMemory(semantic_memory);
  }
  ...
  // Control
  bool triggerConsolidation(bool force_consolidation = false,
                            std::uint64_t duration_ms = 0);
  bool startSleepCycle(std::uint64_t duration_ms = 0) {
    return triggerConsolidation(false, duration_ms);
  }
  ...
  // Replay
  std::size_t replayEpisodes(const std::vector<EnhancedEpisode> &episodes,
                             float replay_speed = -1.0f);
  std::vector<EnhancedEpisode>
  selectEpisodesForReplay(std::size_t max_episodes) const;
  ...
  std::size_t applySynapticScalingToRegion(const std::string &region_name,
  ...
  // ═══════════════════════════════════════════════════════════
  //  WEIGHT-SPACE CONSOLIDATION (operates on weight matrices)
  // ═══════════════════════════════════════════════════════════

  /// Replay episodic patterns through procedural weight matrix
  std::size_t replayWeightPatterns(std::size_t max_patterns = 50);

  /// Apply global synaptic homeostasis: scale all weight matrices
  void scaleAllWeights(float factor = -1.0f);

  /// Decay all weight matrices (forgetting during sleep)
  void decayAllWeights(float rate = -1.0f);
...
private:
  ...
  std::vector<EnhancedEpisode>
  selectRandomEpisodes(const std::vector<EnhancedEpisode> &episodes,
                       std::size_t max_episodes) const;
  std::vector<EnhancedEpisode>
  selectPriorityEpisodes(const std::vector<EnhancedEpisode> &episodes,
                         std::size_t max_episodes) const;
  ...
  void logConsolidationActivity(const std::string &activity_type,
...
private:
  ...
  mutable std::shared_mutex consolidation_mutex_;
