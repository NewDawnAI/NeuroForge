 ...
 *
 * Implements explicit dream generation, narrative construction, and creative
 * synthesis during REM sleep phases. Extends the basic sleep consolidation with
 * sophisticated dream content generation, emotional processing, and creative
 * problem-solving.
 */
...
public:
  ...
  struct DreamNarrative {
    ...
    std::string narrative_text;           // legacy label
    std::vector<float> dream_pattern;     // primary: consolidated dream content
    ...
    std::vector<std::string> memory_sources; ///< Source memories used (debug)
  ...
  struct DreamConfig {
    ...
    float problem_solving_probability{
        0.2f}; ///< Probability of problem-solving dreams

    // Memory integration
    std::size_t max_memory_sources{10}; ///< Max memories to combine per dream
    float memory_distortion_factor{
        0.3f};                        ///< How much to distort memories [0,1]
    ...
    float stress_processing_weight{
        0.7f};                          ///< Weight for stress/trauma processing
...
private:
  ...
  DreamConfig config_;
  mutable std::shared_mutex dream_mutex_;
  ...
  NeuroForge::Core::CompositionMetrics *composition_metrics_{
      nullptr}; // Phase 5
...
public:
   ...
   */
  void
  registerLearningSystem(NeuroForge::Core::LearningSystem *learning_system);

  /**
   * @brief Register composition metrics system (Phase 5)
   * @param composition_metrics CompositionMetrics evaluator
   */
  void registerCompositionMetrics(
   ...
   */
  DreamNarrative
  generateSpecificDream(DreamType dream_type, std::uint64_t duration_ms,
   ...
   */
  std::size_t
  processREMDreams(std::uint64_t rem_duration_ms,
   ...
   */
  std::vector<float>
  generateEpisodicContent(const std::vector<EnhancedEpisode> &source_episodes,
   ...
   */
  std::vector<float> generateCreativeContent(
   ...
   */
  std::vector<float> generateProblemSolvingContent(
   ...
   */
  std::vector<float> generateEmotionalContent(
   ...
   */
  std::vector<float>
  blendCrossModalContent(const std::vector<float> &visual_content,
  ...
  struct DreamAnalysis {
    ...
    std::vector<std::string> insights;                     // legacy
    std::vector<std::string> creative_connections;         // legacy
    std::vector<std::vector<float>> insight_embeddings;    // primary
    std::vector<std::vector<float>> connection_embeddings; // primary
   ...
   */
  std::vector<DreamNarrative>
  getDreamsByType(DreamType dream_type, std::size_t max_dreams = 10) const;
   ...
   */
  std::vector<DreamNarrative>
  getRecentDreams(std::uint64_t hours_back = 24,
                  std::size_t max_dreams = 10) const;
  ...
  /// Generate dream pattern by blending/distorting episodic
  /// and semantic patterns (operates on weight-space data)
  std::vector<float>
  generateDreamPattern(const std::vector<float> &seed_pattern,
                       float distortion = 0.3f, float creativity = 0.5f);
...
private:
   ...
   */
  DreamType
  selectDreamType(const std::vector<float> &emotional_state, float stress_level,
   ...
   */
  std::uint64_t calculateDreamDuration(DreamType dream_type,
   ...
   */
  std::vector<EnhancedEpisode> selectSourceMemories(DreamType dream_type,
   ...
   */
  std::vector<float>
  applyMemoryDistortion(const std::vector<float> &original_content,
   ...
   */
  float
  calculateCreativityScore(const std::vector<float> &dream_content,
