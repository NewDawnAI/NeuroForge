...
struct CriticalPeriod {
  ...
  CriticalPeriod(const std::string &name, std::uint64_t start_ms,
                 std::uint64_t end_ms, float plasticity_mult,
...
public:
  ...
  std::unique_ptr<CriticalPeriod>
  getCriticalPeriod(const std::string &period_name) const;
  std::vector<CriticalPeriod> getActiveCriticalPeriods() const;
  std::vector<CriticalPeriod> getCriticalPeriods() const {
    return critical_periods_;
  }
  ...
  // Learning modulation
  LearningModulation
  getLearningModulation(const std::string &learning_type,
...
private:
  ...
  // Cached multipliers
  mutable std::unordered_map<std::string, float>
      current_plasticity_multipliers_;
  mutable std::unordered_map<std::string, float>
      current_learning_rate_multipliers_;
  mutable std::unordered_map<std::string, float>
      current_consolidation_multipliers_;
