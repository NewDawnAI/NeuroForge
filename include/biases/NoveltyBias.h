...
public:
  ...
  struct Config {
    ...
    float complexity_weight{
        0.5f}; // Weight of complexity in novelty calculation
    ...
    bool enable_exploration_bonus{
        true}; // Whether to provide exploration bonuses
  };
...
private:
  ...
  // WorldModelCortex integration tracking
  std::atomic<std::uint64_t> world_model_updates_{0};
  std::atomic<float> total_world_model_error_{0.0f};
...
public:
  ...
  /**
   * @brief Update novelty bias from WorldModel prediction error
   *
   * Called by WorldModelCortex when prediction errors are computed.
   * This integrates JEPA-style world modeling with curiosity.
   *
   * @param prediction_error L2 distance in latent space
   * @param uncertainty Epistemic uncertainty of the world state
   */
  void updateFromWorldModel(float prediction_error, float uncertainty) {
    std::lock_guard<std::mutex> lock(buffer_mutex_);

    // Convert world model prediction error to novelty metrics
    NoveltyMetrics metrics;
    metrics.prediction_error = prediction_error;
    metrics.surprise_level =
        std::tanh(prediction_error * config_.surprise_sensitivity);
    metrics.exploration_bonus =
        config_.enable_exploration_bonus
            ? prediction_error * uncertainty * config_.exploration_bonus_scale
            : 0.0f;

    // Update statistics
    updateStatisticsUnsafe(metrics);

    // Track as world model experience
    world_model_updates_++;
    total_world_model_error_ += prediction_error;
  }

  /**
   * @brief Get average world model prediction error
   */
  float getAverageWorldModelError() const {
    return world_model_updates_ > 0
               ? total_world_model_error_ / world_model_updates_
               : 0.0f;
  }

  /**
   * @brief Get world model update count
   */
  std::uint64_t getWorldModelUpdates() const { return world_model_updates_; }
