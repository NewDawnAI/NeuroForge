...
LanguageSystem::TokenTrajectoryLogger::TokenTrajectoryLogger(
    const std::string &log_dir, std::size_t snapshot_interval)
    : log_directory_(log_dir), snapshot_interval_(snapshot_interval),
      current_step_(0) {
  std::filesystem::create_directories(log_directory_);
}

void LanguageSystem::TokenTrajectoryLogger::captureSnapshot(
    const LanguageSystem &lang_system, std::size_t token_id) {
  ...
  snapshot.cross_modal_strength =
      calculateCrossModalStrength(lang_system, token_id);

  // Get associated tokens (simplified - could be enhanced)
  // For now, we consider other currently active tokens as associated
  snapshot.associated_tokens = {};
  auto active_tokens = lang_system.getActiveTokens(
      0.2f); // Low threshold to catch weak associations
  for (const auto &other : active_tokens) {
    if (other.symbol != snapshot.symbol) {
      snapshot.associated_tokens.push_back(other.symbol);
    }
  }

  // Calculate cluster stability using simple history-based statistics
  // Tokens that appear frequently with consistent activation are treated as
  // more stable.
...
float LanguageSystem::TokenTrajectoryLogger::calculateCrossModalStrength(
    const LanguageSystem &lang_system, std::size_t token_id) {
  // Implementation for calculating cross-modal binding strength
  auto stats = lang_system.getStatistics();

  // Simple heuristic: combine visual-linguistic and auditory-linguistic
  // associations
...
void LanguageSystem::TokenTrajectoryLogger::writeTrajectoryLog() {
  ...
  // Write CSV header
  log_file
      << "timestamp,token_id,symbol,activation_strength,usage_count,cluster_"
         "stability,cross_modal_strength,stage,associated_tokens\n";
  ...
  for (const auto &snapshot : trajectory_log_) {
    ...
    log_file << std::put_time(std::localtime(&time_t), "%Y-%m-%d %H:%M:%S")
             << "," << snapshot.token_id << ",\"" << snapshot.symbol << "\","
             << snapshot.activation_strength << "," << snapshot.usage_count
...
/// Static helper: cosine similarity between two embedding vectors.
static float embeddingCosineSimilarity(const std::vector<float> &a,
...
void LanguageSystem::TokenTrajectoryLogger::writeClusterEvolutionLog() {
  ...
  // Write CSV header
  cluster_file << "formation_step,cluster_name,member_count,cohesion_score,is_"
                  "proto_word,members\n";

  // Generate cluster evolution data based on trajectory snapshots.
  // Clusters are formed by EMBEDDING cosine similarity (≥ 0.7) —
  // NOT by string prefix. This is the vector-only cognition principle.
  std::map<std::string, std::vector<std::string>> clusters;
  std::map<std::string, std::vector<float>>
      cluster_representative; // first member's embedding
  std::map<std::string, float> cluster_cohesion;
  std::map<std::string, int> cluster_formation_step;
  int next_cluster_id = 0;
  constexpr float kClusterThreshold = 0.7f;
  ...
  for (const auto &snapshot : trajectory_log_) {
    ...
    // Compare this snapshot's embedding against existing cluster
    // representatives. Assign to the most similar cluster if ≥ threshold,
    // otherwise create a new cluster.
    std::string cluster_key;
    float best_sim = -1.0f;

    if (!snapshot.embedding.empty()) {
      for (const auto &[key, rep_emb] : cluster_representative) {
        float sim = embeddingCosineSimilarity(snapshot.embedding, rep_emb);
        if (sim >= kClusterThreshold && sim > best_sim) {
          best_sim = sim;
          cluster_key = key;
        }
      }
    ...
    if (cluster_key.empty()) {
      ...
      cluster_key = "emb_cluster_" + std::to_string(next_cluster_id++);
      cluster_representative[cluster_key] = snapshot.embedding;
    }

    // Add token to cluster if not already present
    auto &cluster_members = clusters[cluster_key];
    if (std::find(cluster_members.begin(), cluster_members.end(),
                  snapshot.symbol) == cluster_members.end()) {
      cluster_members.push_back(snapshot.symbol);
      cluster_formation_step[cluster_key] = step;
    }

    // Update cluster cohesion (running average of member stabilities)
    cluster_cohesion[cluster_key] =
        (cluster_cohesion[cluster_key] + snapshot.cluster_stability) / 2.0f;
  }
  ...
  for (const auto &cluster_pair : clusters) {
    ...
    if (members.size() >= 2) { // Only write clusters with multiple members
      bool is_proto_word =
          (cluster_cohesion[cluster_name] > 0.6f && members.size() >= 2);

      cluster_file << cluster_formation_step[cluster_name] << ","
                   << cluster_name << "," << members.size() << "," << std::fixed
                   << std::setprecision(2) << cluster_cohesion[cluster_name]
...
std::string LanguageSystem::TokenTrajectoryLogger::generateDevelopmentalReport(
    const LanguageSystem &lang_system) {
  ...
  report << "- **Current Stage**: "
         << lang_system.stageToString(lang_system.getCurrentStage()) << "\n";
  ...
  report << "- **Total Vocabulary Size**: " << stats.total_vocabulary_size
         << "\n";
  report << "- **Successful Mimicry Attempts**: "
         << stats.successful_mimicry_attempts << "\n";
  report << "- **Grounding Associations Formed**: "
...
void LanguageSystem::TokenTrajectoryLogger::analyzeTrajectoryProgression(
    std::ostringstream &report, const LanguageSystem &language_system) {
  if (trajectory_log_.empty()) {
  ...
  for (const auto &snapshot : trajectory_log_) {
    ...
    avg_activation_by_stage[snapshot.stage_at_snapshot] +=
  ...
  for (const auto &stage_count_pair : stage_counts) {
    ...
    report << "- **" << language_system.stageToString(stage) << "**: " << count
           << " snapshots, " << std::fixed << std::setprecision(3)
...
void LanguageSystem::TokenTrajectoryLogger::analyzeCrossModalBinding(
    std::ostringstream &report, const LanguageSystem &language_system) {
  if (trajectory_log_.empty()) {
  ...
  std::map<LanguageSystem::DevelopmentalStage, std::vector<float>>
      cross_modal_by_stage;

  for (const auto &snapshot : trajectory_log_) {
    cross_modal_by_stage[snapshot.stage_at_snapshot].push_back(
        snapshot.cross_modal_strength);
  }
  ...
  for (const auto &stage_strengths_pair : cross_modal_by_stage) {
    ...
    float avg_strength =
        std::accumulate(strengths.begin(), strengths.end(), 0.0f) /
        strengths.size();

    report << "- **" << language_system.stageToString(stage)
           << "**: " << std::fixed << std::setprecision(3) << avg_strength
...
void LanguageSystem::TokenTrajectoryLogger::generateStagePredictions(
    std::ostringstream &report, const LanguageSystem &language_system) {
  ...
  if (current_stage == LanguageSystem::DevelopmentalStage::Chaos) {
    ...
    report << "- Stability: " << std::fixed << std::setprecision(3)
           << stats.average_cluster_stability << "/0.5\n\n";
  }
  // Add more stage-specific predictions as needed
}

// LanguageSystem trajectory tracking methods implementation
void LanguageSystem::enableTrajectoryTracking(
  ...
  std::cout << "📊 Full trajectory tracking enabled in: " << log_directory
            << std::endl;
...
void LanguageSystem::captureTrajectorySnapshot() {
  if (!trajectory_logger_) {
    std::cout << "⚠️ Trajectory tracking not enabled. Call "
                 "enableTrajectoryTracking() first."
              << std::endl;
    return;
  }
  ...
  for (std::size_t token_id = 0;
       token_id < stats.total_vocabulary_size && token_id < 100; ++token_id) {
...
void LanguageSystem::generateDevelopmentalReport() {
  ...
  std::cout << "\n🧠 NeuroForge Comprehensive Developmental Report"
            << std::endl;
  std::cout << "=================================================" << std::endl;
  ...
  std::string report_path =
      trajectory_logger_->log_directory_ + "/developmental_report.md";
