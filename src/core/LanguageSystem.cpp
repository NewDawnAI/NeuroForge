...
namespace {
static std::string escape_json_string_ls(const std::string &in) {
  std::string out;
  out.reserve(in.size());
  for (char c : in) {
    switch (c) {
...
void LanguageSystem::setAuditSink(MemoryDB *db, std::int64_t run_id) {
  audit_db_ = db;
  audit_run_id_ = run_id;
}

void LanguageSystem::setAuditStep(std::uint64_t step) { audit_step_ = step; }

void LanguageSystem::setAuditLanguageGateStage(int stage) {
  if (stage < 0)
    stage = 0;
  if (stage > 2)
    stage = 2;
...
bool LanguageSystem::observeGroundingToken(
    const std::string &token, const std::string &internal_state_json,
    double correlation, std::optional<double> description_length_delta,
    const std::string &source) {
  if (!audit_db_ || audit_run_id_ <= 0)
    return false;
  ...
  if (audit_stage_ < 1) {
    ...
    js << "{\"source\":\"" << escape_json_string_ls(source)
       << "\",\"correlation\":" << correlation << "}";
    (void)audit_db_->insertLanguageAuditLog(
        audit_run_id_, now_ms_ls(), audit_step_, audit_stage_, "ground_blocked",
        token, js.str(), false, false, false, false, false, audit_id);
    return false;
  }

  std::int64_t grounding_id = 0;
  const bool ok = audit_db_->insertLanguageGroundingMap(
      audit_run_id_, now_ms_ls(), audit_step_, audit_stage_, token,
      internal_state_json, correlation, description_length_delta, source,
      grounding_id);
  ...
  if (description_length_delta.has_value()) {
    js << ",\"description_length_delta\":" << *description_length_delta;
  } else {
    js << ",\"description_length_delta\":null";
  }
  js << ",\"grounding_id\":" << grounding_id
     << ",\"ok\":" << (ok ? "true" : "false") << "}";

  (void)audit_db_->insertLanguageAuditLog(
      audit_run_id_, now_ms_ls(), audit_step_, audit_stage_,
      ok ? "ground" : "ground_db_error", token, js.str(), false, false, false,
      false, ok, audit_id);
  return ok;
}
...
std::size_t LanguageSystem::createToken(const std::string &symbol,
                                        TokenType type,
  ...
  if (embedding.empty()) {
    ...
    // Ensure embedding matches configured dimension (pad/truncate then
    // normalize)
    std::vector<float> adjusted = embedding;
    const std::size_t target_dim =
        static_cast<std::size_t>(config_.embedding_dimension);
    if (adjusted.size() != target_dim) {
      ...
      std::cerr
          << "[LanguageSystem] Warning: adjusted embedding dimension from "
          << embedding.size() << " to " << target_dim << " for token '"
          << symbol << "'\n";
    }
...
LanguageSystem::SymbolicToken *
LanguageSystem::getToken(const std::string &symbol) {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);
...
LanguageSystem::findSimilarTokens(const std::vector<float> &embedding,
  ...
  std::sort(similar_tokens.begin(), similar_tokens.end(),
            [this, &embedding](std::size_t a, std::size_t b) {
              float sim_a =
                  cosineSimilarity(embedding, vocabulary_[a].embedding);
              float sim_b =
                  cosineSimilarity(embedding, vocabulary_[b].embedding);
              return sim_a > sim_b;
            });
...
void LanguageSystem::processTeacherSignal(const std::string &label,
                                          float reward_signal) {
  ...
  // Handle token creation/update with single vocabulary lock
  {
    ...
    if (token) {
      ...
      for (std::size_t i = 0;
           i < token->embedding.size() && i < teacher_embedding.size(); ++i) {
        token->embedding[i] +=
            learning_rate * (teacher_embedding[i] - token->embedding[i]);
      }
...
std::vector<float> LanguageSystem::generateMimicryResponse(
  ...
  for (std::size_t i = 0; i < vocabulary_.size(); ++i) {
    float similarity =
        cosineSimilarity(teacher_embedding, vocabulary_[i].embedding);
    if (similarity >= threshold) {
      similar_tokens.push_back(i);
    }
  }
  ...
  std::sort(similar_tokens.begin(), similar_tokens.end(),
            [this, &teacher_embedding](std::size_t a, std::size_t b) {
              float sim_a =
                  cosineSimilarity(teacher_embedding, vocabulary_[a].embedding);
              float sim_b =
                  cosineSimilarity(teacher_embedding, vocabulary_[b].embedding);
              return sim_a > sim_b;
            });
  ...
  float best_score = vocabulary_[selected_token].activation_strength *
                     cosineSimilarity(teacher_embedding,
                                      vocabulary_[selected_token].embedding);

  for (std::size_t i = 1; i < std::min(similar_tokens.size(), std::size_t(5));
       ++i) {
    std::size_t token_id = similar_tokens[i];
    float score =
        vocabulary_[token_id].activation_strength *
...
void LanguageSystem::associateTokenWithNeuron(std::size_t token_id,
                                              NeuroForge::NeuronID neuron_id,
  ...
  auto it = std::find(token.associated_neurons.begin(),
                      token.associated_neurons.end(), neuron_id);
...
void LanguageSystem::associateTokenWithModality(
    std::size_t token_id, const std::string &modality,
...
std::vector<std::size_t> LanguageSystem::getTokensForNeuralPattern(
  ...
  std::vector<std::pair<std::size_t, int>> sorted_tokens(token_votes.begin(),
                                                         token_votes.end());
...
void LanguageSystem::generateNarration(
  ...
  std::vector<std::size_t> relevant_tokens =
      findSimilarTokens(context_embedding, config_.narration_threshold);

  // Generate token sequence (simple approach: select top activated tokens)
  std::size_t max_sequence_length =
      std::min(std::size_t(10), relevant_tokens.size());

  for (std::size_t i = 0; i < max_sequence_length; ++i) {
    std::size_t token_id = relevant_tokens[i];
    if (vocabulary_[token_id].activation_strength >
        config_.narration_threshold) {
  ...
  if (!entry.token_sequence.empty()) {
    ...
    if (audit_db_ && audit_run_id_ > 0 && audit_stage_ >= 2) {
      std::int64_t audit_id = 0;
      std::ostringstream js;
      js.setf(std::ios::fixed);
      js << "{\"context\":\"" << escape_json_string_ls(context_description)
         << "\","
         << "\"token_count\":" << entry.token_sequence.size() << ","
         << "\"confidence\":" << entry.confidence << "}";
      (void)audit_db_->insertLanguageAuditLog(
          audit_run_id_, now_ms_ls(), audit_step_, audit_stage_, "narration",
          std::string(), js.str(), false, false, false, false, true, audit_id);
    }
  }
}

std::vector<LanguageSystem::NarrationEntry>
LanguageSystem::getRecentNarration(std::size_t count) const {
  ...
  std::size_t start_idx = internal_narration_.size() > count
                              ? internal_narration_.size() - count
                              : 0;
...
void LanguageSystem::logSelfNarration(
    const std::vector<std::string> &token_sequence, float confidence,
  ...
  if (audit_db_ && audit_run_id_ > 0 && audit_stage_ >= 2) {
    ...
    std::ostringstream js;
    js.setf(std::ios::fixed);
    js << "{\"context\":\"" << escape_json_string_ls(context) << "\","
       << "\"token_count\":" << token_sequence.size() << ","
       << "\"confidence\":" << confidence << "}";
    (void)audit_db_->insertLanguageAuditLog(
        audit_run_id_, now_ms_ls(), audit_step_, audit_stage_, "self_narration",
        std::string(), js.str(), false, false, false, false, true, audit_id);
  }
}
...
void LanguageSystem::exploreTokenCombinations(std::size_t sequence_length) {
  ...
  for (std::size_t i = 0; i < sequence_length; ++i) {
    std::size_t random_idx =
        static_cast<std::size_t>(uniform_dist_(rng_) * vocabulary_.size());
...
void LanguageSystem::processNeuralActivation(
  ...
  std::vector<std::size_t> associated_tokens =
      getTokensForNeuralPattern(active_neurons);
  ...
  for (std::size_t token_id : associated_tokens) {
    if (token_id < vocabulary_.size()) {
      ...
      vocabulary_[token_id].activation_strength =
          std::min(vocabulary_[token_id].activation_strength, 1.0f);
    }
  }
  ...
  if (associated_tokens.size() >= 2 && narration_active_.load()) {
    std::vector<float> context_embedding =
        generateRandomEmbedding(); // Simplified
    generateNarration(context_embedding, "Neural activation pattern");
  }
}

void LanguageSystem::influenceNeuralActivation(
    const std::vector<std::size_t> &token_ids, float influence_strength) {
  if (audit_db_ && audit_run_id_ > 0 && audit_stage_ >= 2) {
    std::int64_t audit_id = 0;
    std::ostringstream js;
    js.setf(std::ios::fixed);
    js << "{\"token_id_count\":" << token_ids.size()
       << ",\"requested_strength\":" << influence_strength << "}";
    (void)audit_db_->insertLanguageAuditLog(
        audit_run_id_, now_ms_ls(), audit_step_, audit_stage_,
        "influence_neural_activation_blocked", std::string(), js.str(), false,
        false, false, false, false, audit_id);
    return;
  }
  ...
  for (std::size_t token_id : token_ids) {
    if (token_id < vocabulary_.size()) {
      ...
      const float per_neuron =
          clamped_strength / static_cast<float>(std::max<std::size_t>(
                                 1, token.associated_neurons.size()));
  ...
  if (!vocabulary_.empty()) {
    ...
    for (const auto &token : vocabulary_) {
      ...
      const float deviation =
          std::fabs(token.activation_strength - mean_activation);
...
LanguageSystem::getActiveVocabulary(float activation_threshold) const {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);
...
void LanguageSystem::processChaosStage(float delta_time) {
  ...
  for (auto &token : vocabulary_) {
    ...
    token.activation_strength =
        std::max(0.0f, std::min(1.0f, token.activation_strength));
  }
}
...
void LanguageSystem::processBabblingStage(float delta_time) {
  ...
  if (development_step_counter_.load() % 2 == 0) {
    // Create dummy features for attention update (in real system, these would
    // come from sensors)
    VisualLanguageFeatures dummy_visual;
    AcousticFeatures dummy_acoustic;
    // updateMultimodalAttention(dummy_visual, dummy_acoustic); // Function not
    // implemented yet
  }
...
void LanguageSystem::processMimicryStage(float delta_time) {
  ...
  if (config_.enable_teacher_mode && !teacher_embeddings_.empty()) {
    std::size_t random_teacher = static_cast<std::size_t>(
        uniform_dist_(rng_) * teacher_embeddings_.size());
...
void LanguageSystem::processGroundingStage(float delta_time) {
  ...
  // This stage would heavily integrate with sensory input and neural
  // activations
...
float LanguageSystem::calculateTokenSimilarity(
    const SymbolicToken &token1, const SymbolicToken &token2) const {
  return cosineSimilarity(token1.embedding, token2.embedding);
}

void LanguageSystem::updateTokenActivation(std::size_t token_id,
                                           float activation_delta) {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);

  if (token_id < vocabulary_.size()) {
    vocabulary_[token_id].activation_strength += activation_delta;
    vocabulary_[token_id].activation_strength = std::max(
        0.0f, std::min(1.0f, vocabulary_[token_id].activation_strength));
...
std::vector<float>
LanguageSystem::normalizeEmbedding(const std::vector<float> &embedding) const {
...
float LanguageSystem::cosineSimilarity(const std::vector<float> &a,
...
std::vector<float>
LanguageSystem::interpolateEmbeddings(const std::vector<float> &a,
...
bool LanguageSystem::shouldAdvanceStage() const {
  ...
  // ACCELERATED DEVELOPMENT for User Interaction
  switch (current_stage_.load()) {
  case DevelopmentalStage::Chaos:
    return current_step > 5; // Advance quickly
  case DevelopmentalStage::Babbling:
    return current_step > 10;
  case DevelopmentalStage::Mimicry:
    return current_step > 15;
  case DevelopmentalStage::Grounding:
    return current_step > 20;
  case DevelopmentalStage::Reflection:
    return current_step > 25;
  case DevelopmentalStage::Communication:
...
void LanguageSystem::onStageTransition(DevelopmentalStage from_stage,
                                       DevelopmentalStage to_stage) {
  ...
  case DevelopmentalStage::Communication:
    ...
    if (!config_.enable_speech_output) {
      Config speech_config = config_;
      speech_config.enable_speech_output = true;
      updateConfig(speech_config);
    }
    logDevelopmentalEvent("Speech production enabled",
                          "Advanced to Communication stage");
    break;
...
LanguageSystem::TokenType
LanguageSystem::inferTokenType(const std::string &symbol) const {
...
const LanguageSystem::SymbolicToken *
LanguageSystem::getToken(const std::string &symbol) const {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);

  auto it = std::find_if(
      vocabulary_.begin(), vocabulary_.end(),
      [&symbol](const SymbolicToken &token) { return token.symbol == symbol; });

  return (it != vocabulary_.end()) ? &(*it) : nullptr;
}

const LanguageSystem::SymbolicToken *
LanguageSystem::getToken(std::size_t token_id) const {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);
...
bool LanguageSystem::getTokenId(const std::string &symbol,
                                std::size_t &out_token_id) const {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);
  auto it = token_lookup_.find(symbol);
  if (it != token_lookup_.end()) {
...
void LanguageSystem::logDevelopmentalEvent(const std::string &event,
...
float LanguageSystem::calculateExperienceBasedLearningBoost(
  ...
  boost *= (1.0f + experience.salience_score *
                       config_.sensory_experience_learning_rate);
...
std::size_t
LanguageSystem::createProtoWord(const std::string &pattern,
...
void LanguageSystem::reinforceProtoWord(std::size_t proto_word_id,
  ...
  if (proto_word_id >= proto_words_.size())
    return;

  ProtoWord &proto_word = proto_words_[proto_word_id];
  proto_word.occurrence_count++;
  proto_word.stability_score +=
      reinforcement_strength * config_.proto_word_crystallization_rate;
...
void LanguageSystem::updatePhonemeStability(const std::string &phoneme,
  ...
  phoneme_stability_scores_[phoneme] +=
      usage_boost * config_.proto_word_crystallization_rate;
  phoneme_stability_scores_[phoneme] =
      std::min(1.0f, phoneme_stability_scores_[phoneme]);
  ...
  auto timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(
                       ...
                       .count();
  phoneme_usage_history_.push_back(
      {phoneme, static_cast<std::uint64_t>(timestamp)});
...
float LanguageSystem::calculatePatternSimilarity(
    const std::string &pattern1, const std::string &pattern2) const {
  ...
  std::vector<std::vector<std::size_t>> dp(len1 + 1,
  ...
  return 1.0f -
         static_cast<float>(dp[len1][len2]) / static_cast<float>(max_len);
}

bool LanguageSystem::shouldCrystallizePattern(
    const ProtoWord &proto_word) const {
  // Check multiple criteria for crystallization
  bool stability_met =
      proto_word.stability_score >= proto_word.crystallization_threshold;
  bool frequency_met = proto_word.occurrence_count >=
                       config_.min_occurrences_for_crystallization;
  bool caregiver_response = proto_word.caregiver_response_strength > 0.3f;

  return stability_met && frequency_met &&
         (caregiver_response || proto_word.occurrence_count >= 5);
}

void LanguageSystem::processCaregiverResponse(
  ...
  context.emotional_valence =
      caregiver_reaction.motherese_face_boost > 0.5f ? 1.0f : 0.0f;
  ...
  context.response_strength = std::max(caregiver_reaction.face_salience,
                                       caregiver_audio.energy_envelope);
  ...
  if (caregiver_interaction_history_.size() > 100) {
    caregiver_interaction_history_.erase(
        caregiver_interaction_history_.begin());
  }

  // Update speech output state with caregiver attention detection
  // Caregiver attention is detected if attention level is high and there's
  // significant response
  bool attention_detected =
      (context.attention_level > 0.5f) && (context.response_strength > 0.3f);
  ...
  if (context.attention_level > 0.5f) {
    ...
    for (ProtoWord &proto_word : proto_words_) {
      auto time_diff = std::chrono::duration_cast<std::chrono::seconds>(
                           now - proto_word.last_occurrence)
                           .count();

      if (time_diff < 5) { // Within last 5 seconds
        float response_strength =
            context.response_strength * context.attention_level;
        proto_word.caregiver_response_strength +=
            response_strength * config_.caregiver_response_boost;
        proto_word.caregiver_response_strength =
            std::min(1.0f, proto_word.caregiver_response_strength);
      }
    }
  }
}

float LanguageSystem::calculateCaregiverResponseStrength(
    const CaregiverContext &context) const {
  float strength = 0.0f;
  ...
  if (context.interaction_type == "praise" ||
      context.interaction_type == "positive") {
    strength += 0.2f;
  }
...
void LanguageSystem::reinforceBasedOnCaregiverFeedback(
    const std::string &vocalization, const CaregiverContext &context) {
  // Extract patterns from vocalization and reinforce based on caregiver
  // response
  std::vector<std::string> patterns =
      extractPatternsFromVocalization(vocalization);
...
void LanguageSystem::performEnhancedBabbling(std::size_t num_phonemes) {
  ...
  for (std::size_t i = 0; i < num_phonemes; ++i) {
    ...
    std::string babble_token = "babble_" + phoneme + "_" +
                               std::to_string(development_step_counter_.load());
...
std::string LanguageSystem::generateBiasedPhoneme(float proto_word_bias) {
  ...
  if (proto_word_bias > 0.3f && !config_.target_proto_words.empty()) {
    if (uniform_dist_(rng_) < proto_word_bias) {
      ...
      std::size_t target_idx = static_cast<std::size_t>(
          uniform_dist_(rng_) * config_.target_proto_words.size());
  ...
  std::vector<std::string> common_phonemes = {"m", "b", "d", "p", "a",
                                              "e", "i", "o", "u"};
  std::size_t phoneme_idx =
      static_cast<std::size_t>(uniform_dist_(rng_) * common_phonemes.size());
  return common_phonemes[phoneme_idx];
}

void LanguageSystem::trackPhonemeSequencePatterns(
    const std::vector<std::string> &phoneme_sequence) {
  if (phoneme_sequence.size() < 2)
    return;
  ...
  for (std::size_t i = 0; i < phoneme_sequence.size() - 1; ++i) {
    ...
    if (i + 1 < phoneme_sequence.size() &&
        phoneme_sequence[i] == phoneme_sequence[i + 1]) {
      // Found repeating pattern, create or reinforce proto-word
      std::vector<std::string> pattern_phonemes = {phoneme_sequence[i],
                                                   phoneme_sequence[i]};
  ...
  for (const std::string &target : config_.target_proto_words) {
    for (std::size_t i = 0; i < phoneme_sequence.size(); ++i) {
      if (phoneme_sequence[i] == std::string(1, target[0])) {
        ...
        std::size_t proto_word_id = createProtoWord(pattern, pattern_phonemes);
        reinforceProtoWord(proto_word_id,
                           config_.caregiver_response_boost * 0.5f);
      }
    }
  }
}
...
std::vector<std::string>
LanguageSystem::generateProtoWordSequence(const std::string &target_pattern) {
...
void LanguageSystem::analyzeEmergingPatterns() {
  ...
  for (ProtoWord &proto_word : proto_words_) {
    ...
    proto_word.stability_score =
        std::max(proto_word.stability_score, readiness * 0.1f);
...
std::vector<std::string> LanguageSystem::extractPatternsFromVocalization(
  ...
  if (vocalization.length() >= 4) {
    for (std::size_t i = 0; i < vocalization.length() - 1; ++i) {
      if (i + 1 < vocalization.length()) {
        std::string potential_pattern = std::string(1, vocalization[i]) + "-" +
                                        std::string(1, vocalization[i]);
...
float LanguageSystem::calculateCrystallizationReadiness(
    const ProtoWord &proto_word) const {
  ...
  float frequency_score =
      std::min(1.0f, static_cast<float>(proto_word.occurrence_count) / 10.0f);
  ...
  auto time_span = std::chrono::duration_cast<std::chrono::minutes>(
                       now - proto_word.first_occurrence)
                       .count();
  float persistence_score = std::min(1.0f, static_cast<float>(time_span) /
                                               60.0f); // 1 hour = full score
...
void LanguageSystem::updateMultimodalAttention(
  ...
  multimodal_attention_state_.face_attention_weight =
      visual_features.face_salience * config_.multimodal_attention_weight;

  // Calculate speech attention weight
  multimodal_attention_state_.speech_attention_weight =
      acoustic_features.attention_score * config_.multimodal_attention_weight;

  // Calculate joint attention score
  multimodal_attention_state_.joint_attention_score =
      calculateJointAttentionScore(visual_features);

  // Update attention history
  float combined_attention =
      (multimodal_attention_state_.face_attention_weight +
       multimodal_attention_state_.speech_attention_weight) /
      2.0f;
  ...
  // Limit history size
  if (multimodal_attention_state_.attention_history.size() >
      config_.attention_history_length) {
  ...
  if (multimodal_attention_state_.joint_attention_score >
      config_.joint_attention_threshold) {
    multimodal_attention_state_.is_joint_attention_active = true;
    multimodal_attention_state_.last_attention_peak =
        std::chrono::steady_clock::now();

    // Record attention event
    auto now = std::chrono::steady_clock::now();
    multimodal_attention_state_.attention_events.push_back(
        {combined_attention, now});
...
void LanguageSystem::processJointAttentionEvent(
    ...
    const std::string &spoken_token, float attention_strength) {

  std::lock_guard<std::recursive_mutex> attention_lock(
      multimodal_attention_mutex_);
  std::lock_guard<std::recursive_mutex> vocab_lock(vocabulary_mutex_);
  ...
  if (token_it != token_lookup_.end()) {
    ...
    float attention_boost =
        attention_strength * config_.joint_attention_learning_boost;
    ...
    if (!shared_gaze_target.empty()) {
      token.sensory_associations["gaze_target"] =
          shared_gaze_target[0]; // Simplified
    }
...
void LanguageSystem::reinforceProtoWordFaceAssociation(
    ...
    const std::vector<float> &face_embedding, float association_strength) {

  std::lock_guard<std::recursive_mutex> coupling_lock(
      multimodal_attention_mutex_);
  ...
  face_speech_coupling_.proto_word_face_associations[proto_word_pattern] =
      std::min(1.0f, face_speech_coupling_
                         .proto_word_face_associations[proto_word_pattern]);

  // Update overall coupling strength
  face_speech_coupling_.coupling_strength += association_strength * 0.1f;
  face_speech_coupling_.coupling_strength =
      std::min(1.0f, face_speech_coupling_.coupling_strength);

  // Record coupling history
  face_speech_coupling_.coupling_history.push_back(
      face_speech_coupling_.coupling_strength);
  if (face_speech_coupling_.coupling_history.size() > 50) {
    face_speech_coupling_.coupling_history.erase(
        face_speech_coupling_.coupling_history.begin());
  }

  // Calculate stability measure
  if (face_speech_coupling_.coupling_history.size() > 10) {
    float mean =
        std::accumulate(face_speech_coupling_.coupling_history.end() - 10,
                        face_speech_coupling_.coupling_history.end(), 0.0f) /
        10.0f;
    float variance = 0.0f;
    ...
    variance /= 10.0f;
    face_speech_coupling_.stability_measure =
        1.0f / (1.0f + variance); // Higher stability = lower variance
  }
}
...
void LanguageSystem::updateFaceSpeechCoupling(
  ...
  float coupling_strength =
      calculateFaceSpeechCouplingStrength(face_embedding, acoustic_features);

  // Update coupling state
  face_speech_coupling_.coupling_strength +=
      coupling_strength * face_speech_coupling_.coupling_learning_rate;
  face_speech_coupling_.coupling_strength =
      std::min(1.0f, face_speech_coupling_.coupling_strength);
  ...
  if (detectMotherese(acoustic_features)) {
    ...
    face_speech_coupling_.coupling_strength +=
        config_.motherese_face_coupling_boost * 0.1f;
    face_speech_coupling_.coupling_strength =
        std::min(1.0f, face_speech_coupling_.coupling_strength);
  }
  ...
  face_speech_coupling_.temporal_synchrony =
      acoustic_features.attention_score * 0.5f +
  ...
  std::vector<std::string> patterns =
      extractPatternsFromVocalization(vocalization);
  for (const std::string &pattern : patterns) {
    reinforceProtoWordFaceAssociation(pattern, face_embedding,
                                      coupling_strength);
  }
}
...
float LanguageSystem::calculateFaceSpeechCouplingStrength(
  ...
  if (!face_embedding.empty()) {
    ...
    coupling_strength +=
        std::sqrt(face_magnitude) / face_embedding.size() * 0.3f;
  }
...
bool LanguageSystem::detectMotherese(
    const AcousticFeatures &acoustic_features) const {
  // Simple motherese detection based on acoustic features
  bool high_pitch =
      acoustic_features.pitch_contour > 200.0f; // Higher than normal speech
...
void LanguageSystem::processCaregiverFaceRecognition(
  ...
  if (!caregiver_id.empty()) {
    ...
    std::lock_guard<std::recursive_mutex> attention_lock(
        multimodal_attention_mutex_);
    multimodal_attention_state_.caregiver_face_priority =
        config_.caregiver_recognition_boost;
  } else {
...
void LanguageSystem::registerCaregiverFace(
    const std::vector<float> &face_embedding, const std::string &caregiver_id) {
...
bool LanguageSystem::isCaregiverFace(const std::vector<float> &face_embedding,
...
std::string LanguageSystem::identifyCaregiver(
  ...
  for (const auto &[caregiver_id, confidence] : caregiver_face_confidences_) {
    // Find corresponding face embedding (simplified - in real system would use
    // proper indexing)
    if (!known_caregiver_faces_.empty()) {
      float similarity =
          cosineSimilarity(face_embedding, known_caregiver_faces_[0]);
...
void LanguageSystem::updateCaregiverInteractionHistory(
    const std::string &caregiver_id, float interaction_quality) {
  ...
  if (caregiver_face_confidences_.find(caregiver_id) !=
      caregiver_face_confidences_.end()) {
...
void LanguageSystem::boostProtoWordBasedOnAttention(
    const std::string &proto_word_pattern, float attention_boost) {
  ...
  if (it != proto_word_lookup_.end()) {
    ...
    proto_word.stability_score +=
        attention_boost * config_.joint_attention_learning_boost;
    proto_word.stability_score = std::min(1.0f, proto_word.stability_score);
...
std::size_t LanguageSystem::createGroundingAssociation(
  ...
  grounding.grounding_strength =
      initial_strength * config_.grounding_association_strength;
  ...
  stats_.grounding_associations_formed++;
  logDevelopmentalEvent("Grounding association created",
                        object_category + " -> token_" +
...
void LanguageSystem::reinforceGroundingAssociation(
  ...
  grounding.grounding_strength +=
      reinforcement_strength * grounding.learning_rate;
...
bool LanguageSystem::isStableGrounding(
    const GroundingAssociation &grounding) const {
  // Check multiple criteria for stable grounding
  bool strength_met =
      grounding.grounding_strength >= config_.grounding_stability_threshold;
  bool exposure_met =
      grounding.exposure_count >= config_.min_exposures_for_stable_grounding;
  bool multimodal =
      (grounding.visual_grounding_confidence > 0.0f ? 1 : 0) +
          (grounding.tactile_grounding_confidence > 0.0f ? 1 : 0) +
          (grounding.auditory_grounding_confidence > 0.0f ? 1 : 0) >=
      2;
...
std::vector<std::size_t>
LanguageSystem::findGroundingAssociationsForToken(std::size_t token_id) const {
...
void LanguageSystem::processSensoryExperience(
  ...
  // Limit history size
  if (sensory_experience_history_.size() >
      config_.sensory_experience_history_length) {
...
float LanguageSystem::calculateSensoryExperienceNovelty(
  ...
  std::vector<SensoryExperience> similar =
      getSimilarExperiences(experience, 0.8f);
...
void LanguageSystem::updateSensoryContext(
    const std::vector<float> &new_context) {
  std::lock_guard<std::recursive_mutex> lock(sensory_experience_mutex_);
  ...
  if (current_sensory_context_.empty()) {
    ...
    if (current_sensory_context_.size() == new_context.size()) {
      for (std::size_t i = 0; i < current_sensory_context_.size(); ++i) {
        current_sensory_context_[i] =
            current_sensory_context_[i] * 0.7f + new_context[i] * 0.3f;
      }
    } else {
...
LanguageSystem::getSimilarExperiences(
    const SensoryExperience &target_experience,
  ...
  auto type_it =
      experience_type_lookup_.find(target_experience.experience_type);
  if (type_it != experience_type_lookup_.end()) {
    for (const SensoryExperience &experience : type_it->second) {
      if (experience.sensory_pattern.size() ==
          target_experience.sensory_pattern.size()) {
...
void LanguageSystem::processMultimodalGroundingEvent(
    const std::string &spoken_token, const std::vector<float> &visual_features,
    const std::vector<float> &tactile_features,
    const std::vector<float> &auditory_features,
  ...
  std::vector<std::size_t> existing_groundings =
      findGroundingAssociationsForToken(token_id);

  bool found_matching_grounding = false;
  for (std::size_t grounding_id : existing_groundings) {
    if (grounding_associations_[grounding_id].object_category ==
        object_category) {
  ...
  if (!found_matching_grounding) {
    ...
    std::size_t grounding_id =
        createGroundingAssociation(token_id, object_category, visual_features,
...
void LanguageSystem::strengthenSemanticAnchoring(
  ...
  token.sensory_associations[association_key] =
      std::min(1.0f, token.sensory_associations[association_key]);
...
float LanguageSystem::calculateSemanticGroundingStrength(
    std::size_t token_id) const {
  std::lock_guard<std::recursive_mutex> lock(grounding_associations_mutex_);

  std::vector<std::size_t> groundings =
      findGroundingAssociationsForToken(token_id);
  ...
  for (std::size_t grounding_id : groundings) {
    if (grounding_id < grounding_associations_.size()) {
      total_strength +=
          grounding_associations_[grounding_id].grounding_strength;
    }
  }
...
void LanguageSystem::boostProtoWordFromSensoryExperience(
    const std::string &proto_word, float boost_strength) {
  ...
  if (it != proto_word_lookup_.end()) {
    ...
    proto_word_obj.stability_score +=
        boost_strength * config_.sensory_experience_learning_rate;
    proto_word_obj.stability_score =
        std::min(1.0f, proto_word_obj.stability_score);
...
void LanguageSystem::processProsodicPatternLearning(
  ...
  if (pattern_id != std::numeric_limits<std::size_t>::max()) {
    ...
    if (recent_acoustic_features_.size() >= 3) {
      std::vector<AcousticFeatures> sequence(
          recent_acoustic_features_.end() - 3, recent_acoustic_features_.end());
      sequence.push_back(acoustic_features);

      ProsodicPattern new_pattern = extractProsodicPattern(sequence);
      new_pattern.pattern_name =
          "pattern_" + std::to_string(prosodic_patterns_.size());
      ...
      logDevelopmentalEvent("New prosodic pattern detected",
                            new_pattern.pattern_name);
    }
  }
  ...
  if (recent_acoustic_features_.size() >
      config_.prosodic_pattern_history_length) {
...
std::size_t LanguageSystem::detectProsodicPattern(
  ...
  std::vector<AcousticFeatures> current_sequence(
      recent_acoustic_features_.end() - 2, recent_acoustic_features_.end());
  ...
  for (std::size_t i = 0; i < prosodic_patterns_.size(); ++i) {
    float similarity = calculateProsodicPatternSimilarity(
        current_pattern, prosodic_patterns_[i]);
...
void LanguageSystem::reinforceProsodicPattern(std::size_t pattern_id,
                                              float reinforcement_strength) {

  std::lock_guard<std::recursive_mutex> lock(prosodic_pattern_mutex_);

  if (pattern_id >= prosodic_patterns_.size())
    return;
  ...
  pattern.recognition_confidence =
      std::min(1.0f, pattern.recognition_confidence);
  ...
  pattern.attention_weight =
      pattern.pattern_stability * config_.intonation_attention_boost;

  // Update learning boost factor
  pattern.learning_boost_factor =
...
bool LanguageSystem::isStableProsodicPattern(
    const ProsodicPattern &pattern) const {
  bool stability_met =
      pattern.pattern_stability >= config_.prosodic_pattern_stability_threshold;
  bool occurrence_met =
      pattern.occurrence_count >= config_.min_pattern_occurrences;
...
void LanguageSystem::updateIntonationGuidedAttention(
  ...
  if (intonation_salience >
    ...
    intonation_attention_state_.intonation_learning_boost *=
        0.9f; // Gradual decay
  }
...
float LanguageSystem::calculateIntonationLearningBoost(
  ...
  if (acoustic_features.motherese_score > 0.5f) {
    boost +=
        config_.motherese_pattern_boost * acoustic_features.motherese_score;
  }

  // High pitch provides attention boost
  if (acoustic_features.pitch_contour > 200.0f) {
    boost +=
        0.2f * (acoustic_features.pitch_contour / 400.0f); // Normalize to 0-0.2
  }
  ...
  if (intonation_attention_state_.is_prosodic_attention_active) {
    ...
    for (const std::string &pattern : patterns) {
      ...
      if (it != proto_word_lookup_.end()) {
        ...
        proto_word.stability_score +=
            learning_boost * config_.prosodic_pattern_learning_rate;
        proto_word.stability_score = std::min(1.0f, proto_word.stability_score);
...
LanguageSystem::ProsodicPattern LanguageSystem::extractProsodicPattern(
    const std::vector<AcousticFeatures> &acoustic_sequence) const {

  ProsodicPattern pattern;

  if (acoustic_sequence.empty())
    return pattern;
...
float LanguageSystem::calculateProsodicPatternSimilarity(
  ...
  if (!pattern1.pitch_trajectory.empty() &&
      !pattern2.pitch_trajectory.empty()) {
    float pitch_similarity =
        cosineSimilarity(pattern1.pitch_trajectory, pattern2.pitch_trajectory);
    similarity += pitch_similarity * 0.4f;
    weight_sum += 0.4f;
  }

  // Compare energy trajectories
  if (!pattern1.energy_trajectory.empty() &&
      !pattern2.energy_trajectory.empty()) {
    float energy_similarity = cosineSimilarity(pattern1.energy_trajectory,
                                               pattern2.energy_trajectory);
  ...
  if (!pattern1.rhythm_pattern.empty() && !pattern2.rhythm_pattern.empty()) {
    float rhythm_similarity =
        cosineSimilarity(pattern1.rhythm_pattern, pattern2.rhythm_pattern);
...
void LanguageSystem::updateProsodicPreferences(const std::string &pattern_name,
                                               float preference_update) {
...
void LanguageSystem::enhanceProtoWordWithProsodicPattern(
  ...
  if (it != proto_word_lookup_.end()) {
    ...
    proto_word.stability_score +=
        prosodic_boost * config_.prosodic_pattern_learning_rate;
    proto_word.stability_score = std::min(1.0f, proto_word.stability_score);
    ...
    if (proto_word.contextual_embeddings.empty()) {
      ...
      for (std::size_t i = 0;
           i < std::min(proto_word.contextual_embeddings.size(),
                        prosodic_pattern.pitch_trajectory.size());
           ++i) {
...
void LanguageSystem::processProsodicallGuidedBabbling(
  ...
  for (std::size_t i = 0; i < num_phonemes; ++i) {
    ...
    if (target_pattern.is_motherese_pattern) {
      ...
      std::size_t vowel_idx =
          static_cast<std::size_t>(uniform_dist_(rng_) * vowel_phonemes.size());
    ...
    if (it == token_lookup_.end()) {
      ...
      token.type = TokenType::Phoneme;
      token.activation_strength =
          uniform_dist_(rng_) + target_pattern.attention_weight;
      token.activation_strength = std::min(1.0f, token.activation_strength);
      token.usage_count = 0;
      token.last_used = std::chrono::steady_clock::now();
      token.embedding = generateRandomEmbedding();
      ...
      token.sensory_associations["prosodic_pattern"] =
          target_pattern.learning_boost_factor;
...
float LanguageSystem::calculateAcousticNovelty(
  ...
  if (recent_acoustic_features_.size() < 5)
    return 1.0f; // Everything is novel initially
  ...
  for (auto it = recent_acoustic_features_.end() - 5;
    ...
    float pitch_sim =
        1.0f - std::abs(features.pitch_contour - it->pitch_contour) / 400.0f;
    float energy_sim =
        1.0f - std::abs(features.energy_envelope - it->energy_envelope);
    float intonation_sim =
        1.0f -
        std::abs(features.intonation_slope - it->intonation_slope) / 2.0f;
...
void LanguageSystem::updateAcousticAttentionWeights(
  ...
  if (attention_weight > 0.6f) {
    ...
    std::string pattern_key =
        "high_attention_" +
        std::to_string(static_cast<int>(features.pitch_contour));
...
void LanguageSystem::performAcousticBabbling(std::size_t num_phonemes) {
  ...
  for (std::size_t i = 0; i < num_phonemes; ++i) {
    ...
    features.pitch_contour =
        uniform_dist_(rng_) * 200.0f + 100.0f;                    // 100-300 Hz
    ...
    if (it == token_lookup_.end()) {
      ...
      vocabulary_[it->second].activation_strength +=
          features.attention_score * 0.1f;
      vocabulary_[it->second].activation_strength =
          (vocabulary_[it->second].activation_strength < 1.0f)
              ? vocabulary_[it->second].activation_strength
              : 1.0f;
    }
  }
...
std::vector<LanguageSystem::SymbolicToken>
LanguageSystem::getActiveTokens(float activation_threshold) const {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);
  std::vector<SymbolicToken> active_tokens;
  active_tokens.reserve(vocabulary_.size());
  for (const auto &token : vocabulary_) {
    if (token.activation_strength > activation_threshold) {
...
void LanguageSystem::injectReasonedConcept(const std::string &concept_key,
                                           double confidence) {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);
  auto it = token_lookup_.find(concept_key);
  if (it != token_lookup_.end()) {
    // Boost existing token
    auto &token = vocabulary_[it->second];
    token.activation_strength += static_cast<float>(confidence);
    if (token.activation_strength > 1.0f)
      token.activation_strength = 1.0f;
    token.last_used = std::chrono::steady_clock::now();
    token.usage_count++;
  } else {
    // Create new token
    SymbolicToken token;
    token.symbol = concept_key;
    token.type = TokenType::Word; // Assuming reasoned concepts are words
    token.activation_strength = static_cast<float>(confidence);
    token.usage_count = 1;
    token.last_used = std::chrono::steady_clock::now();
    token.embedding = generateRandomEmbedding();

    // Add special reasoned association
    token.sensory_associations["reasoning_source"] = 1.0f;

    std::size_t token_id = vocabulary_.size();
    vocabulary_.push_back(std::move(token));
    token_lookup_[concept_key] = token_id;
  }
}

// ========== TransE Relation Scoring ==========

float LanguageSystem::transeDistance(const std::vector<float> &subj_emb,
                                     const std::vector<float> &rel_emb,
                                     const std::vector<float> &obj_emb) const {
  if (subj_emb.size() != rel_emb.size() || rel_emb.size() != obj_emb.size() ||
      subj_emb.empty()) {
  ...
  // TransE: ||s + r - o||
  float sum_sq = 0.0f;
  for (std::size_t i = 0; i < subj_emb.size(); ++i) {
    float diff = subj_emb[i] + rel_emb[i] - obj_emb[i];
    sum_sq += diff * diff;
  }
...
float LanguageSystem::scoreRelationTriple(const std::string &subject,
                                          const std::string &relation,
                                          const std::string &object) const {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);

  auto subj_it = token_lookup_.find(subject);
  auto rel_it = token_lookup_.find(relation);
  auto obj_it = token_lookup_.find(object);

  // Phase 15a: Check if we have valid embeddings
  bool have_embeddings =
      (subj_it != token_lookup_.end() && rel_it != token_lookup_.end() &&
       obj_it != token_lookup_.end());

  if (have_embeddings) {
    const auto &subj_emb = vocabulary_[subj_it->second].embedding;
    const auto &rel_emb = vocabulary_[rel_it->second].embedding;
    const auto &obj_emb = vocabulary_[obj_it->second].embedding;

    // Check if embeddings are non-degenerate (not empty)
    bool embeddings_valid =
        !subj_emb.empty() && !rel_emb.empty() && !obj_emb.empty();
    if (embeddings_valid) {
      float dist = transeDistance(subj_emb, rel_emb, obj_emb);
      float score = 1.0f / (1.0f + dist);

      // If score is meaningful (not near-zero from random noise), use it
      if (score > 0.05f) {
        return score;
      }
    }
  ...
  // Phase 15a: Symbolic fallback for cold-start/uninitialized embeddings
  return computeSymbolicSimilarity(subject, relation, object);
}

std::vector<std::string> LanguageSystem::getAllRelationTokens() const {
  std::lock_guard<std::recursive_mutex> lock(vocabulary_mutex_);

  std::vector<std::string> relations;
  for (const auto &token : vocabulary_) {
    if (token.type == TokenType::Relation) {
      relations.push_back(token.symbol);
    }
  }
  return relations;
}

std::vector<std::pair<std::string, float>> LanguageSystem::proposeRelationsFor(
    const std::string &subject, const std::string &object, int top_k) const {
  ...
  if (subj_it == token_lookup_.end() || obj_it == token_lookup_.end()) {
    return {};
  }
  ...
  // Score all relation tokens
  std::vector<std::pair<std::string, float>> scored;

  for (const auto &token : vocabulary_) {
    if (token.type == TokenType::Relation) {
      float dist = transeDistance(subj_emb, token.embedding, obj_emb);
      float score = 1.0f / (1.0f + dist);
      scored.push_back({token.symbol, score});
    }
  }
  ...
  // Return top-k
  if (top_k > 0 && static_cast<int>(scored.size()) > top_k) {
    scored.resize(top_k);
  }
...
// ========== Phase 15a: Symbolic Similarity (Cold-Start TransE Fallback)
// ==========

float LanguageSystem::computeSymbolicSimilarity(
    const std::string &subject, const std::string &relation,
    const std::string &object) const {
  // Lexical similarity: shared character n-grams between subject and object
  float lexical_sim = computeLexicalSimilarity(subject, object);

  // Predicate prior: known relation types get higher base scores
  float predicate_prior = getPredicatePrior(relation);

  // Combine: weighted average
  float score = 0.4f * lexical_sim + 0.6f * predicate_prior;

  // Hard cap at 0.19 to guarantee provisional status (below CONFIRMED_THRESHOLD
  // of 0.2) Minimum of 0.05 to ensure visibility in perception pipeline
  return std::min(0.19f, std::max(0.05f, score));
}

float LanguageSystem::computeLexicalSimilarity(const std::string &a,
  ...
  auto getBigrams = [](const std::string &s) {
    ...
    for (std::size_t i = 0; i + 1 < s.size(); ++i) {
      bigrams.insert(s.substr(i, 2));
    }
    return bigrams;
  };

  auto bigrams_a = getBigrams(a);
  auto bigrams_b = getBigrams(b);

  if (bigrams_a.empty() || bigrams_b.empty())
    return 0.0f;

  std::size_t intersection = 0;
  for (const auto &bg : bigrams_a) {
    if (bigrams_b.count(bg) > 0)
      intersection++;
  }

  float dice = (2.0f * static_cast<float>(intersection)) /
               static_cast<float>(bigrams_a.size() + bigrams_b.size());
  return dice;
}

float LanguageSystem::getPredicatePrior(const std::string &relation) const {
  // Known relation types with semantic priors
  // These values are capped so that even max (0.18) + lexical (0.4*1.0) =
  // 0.18*0.6 + 0.4 = ~0.51 But we clamp final output to 0.19 anyway
  static const std::unordered_map<std::string, float> priors = {
      {"is_a", 0.18f},       // High prior - very common taxonomic relation
      {"has", 0.15f},        // Common possession/attribute relation
      {"includes", 0.14f},   // Set membership
      {"contains", 0.14f},   // Physical/logical containment
      {"part_of", 0.13f},    // Meronymy
      {"belongs_to", 0.12f}, // Inverse of has
      {"used_for", 0.15f},   // Functional relation
      {"causes", 0.12f},     // Causal relation
      {"produces", 0.11f},   // Output relation
      {"located_in", 0.12f}, // Spatial relation
      {"lives_in", 0.11f},   // Habitat relation
      {"made_of", 0.11f},    // Material composition
  };

  auto it = priors.find(relation);
  if (it != priors.end()) {
    return it->second;
  }

  // Unknown relations get low but non-zero prior
  return 0.08f;
}
