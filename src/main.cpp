...
#ifdef _WIN32
// Windows headers with proper include order
#ifndef NOMINMAX
#define NOMINMAX
#endif
#include <windows.h>

// Multimedia API only for non-MinGW (Strawberry Perl MinGW has broken headers)
...
#include "runtime/SemanticN6Runner.h"
...
// New integrations from unified stub
...
// WorldModel and VocalMotor for full unification
...
// Memory systems for production integration
...
// Global memory systems for production integration
static std::shared_ptr<NeuroForge::Memory::EpisodicMemoryManager>
    g_episodic_memory;
static std::shared_ptr<NeuroForge::Memory::SemanticMemory> g_semantic_memory;
static std::shared_ptr<NeuroForge::Memory::ProceduralMemory>
    g_procedural_memory;
static std::shared_ptr<NeuroForge::Memory::MemoryIntegrator>
    g_memory_integrator;

// Initialize memory systems after MemoryDB is ready
static void InitMemorySystems() {
  // Create episodic memory manager
  NeuroForge::Memory::EpisodicConfig episodic_cfg;
  episodic_cfg.max_episodes = 10000;
  g_episodic_memory =
      std::make_shared<NeuroForge::Memory::EpisodicMemoryManager>(episodic_cfg);

  // Create semantic memory
  NeuroForge::Memory::SemanticConfig semantic_cfg;
  g_semantic_memory =
      std::make_shared<NeuroForge::Memory::SemanticMemory>(semantic_cfg);

  // Create procedural memory
  NeuroForge::Memory::ProceduralConfig procedural_cfg;
  g_procedural_memory =
      std::make_shared<NeuroForge::Memory::ProceduralMemory>(procedural_cfg);

  // Create memory integrator to coordinate all systems
  NeuroForge::Memory::IntegrationConfig integrator_cfg;
  g_memory_integrator =
      std::make_shared<NeuroForge::Memory::MemoryIntegrator>(integrator_cfg);

  std::cout << "[Memory] All memory systems initialized (episodic, semantic, "
               "procedural, integrator)"
            << std::endl;
}
...
// Restored globals
std::deque<
    std::pair<NeuroForge::NeuronID, std::chrono::system_clock::time_point>>
    spike_events;
...
static BOOL WINAPI NfCtrlHandler(DWORD ctrl) {
  ...
  if (ctrl == CTRL_C_EVENT)
    ...
    t = "CTRL_BREAK";
  else if (ctrl == CTRL_CLOSE_EVENT)
    t = "CTRL_CLOSE";
  else if (ctrl == CTRL_LOGOFF_EVENT)
    t = "CTRL_LOGOFF";
  else if (ctrl == CTRL_SHUTDOWN_EVENT)
    t = "CTRL_SHUTDOWN";
  g_abort.store(true);
  if (g_memdb && g_memdb_run_id > 0) {
    ...
    (void)g_memdb->insertRunEvent(g_memdb_run_id, ts_ms, g_last_step.load(),
                                  std::string("signal"), std::string(t), 0,
                                  nf_process_rss_mb(), 0.0, event_id);
  }
  return TRUE;
}
#else
...
void print_usage() {
  std::cout
      ...
      << "Options:\n"
      << "  --help                                 Show this help\n"
      << "  --steps=N                              Number of processing steps "
         "(default: 1)\n"
      << "  --step-ms=MS                           Milliseconds per step "
         "(default: 10)\n"
      << "  --enable-learning                      Enable learning system "
         "(default: off)\n"
      << "  --gpu                                   Prefer GPU acceleration "
         "when available (optional)\n"
      << "  --hebbian-rate=R                       Hebbian learning rate "
         "(default: from code)\n"
      << "  --stdp-rate=R                          STDP learning rate "
         "(default: from code)\n"
      << "  --stdp-rate-multiplier=M               Global STDP rate multiplier "
         "(> 0)\n"
      << "  --attention-boost=F                    Attention boost factor (> "
         "0; default: from code)\n"
      << "  --homeostasis[=on|off]                 Enable or disable "
         "homeostasis (default: from code)\n"
      << "  --consolidation-interval=MS            Consolidation/update "
         "interval in milliseconds\n"
      << "  -a, --alpha=F                          Phase-4 novelty weight "
         "alpha (default: 0.50)\n"
      << "  -g, --gamma=F                          Phase-4 task weight gamma "
         "(default: 1.00)\n"
      << "  -u, --eta=F                            Phase-4 uncertainty weight "
         "eta (default: 0.20)\n"
      << "  -l, --lambda=F                         Phase-4 eligibility decay "
         "lambda in [0,1] (default: 0.90)\n"
      << "  -e, --eta-elig=F                       Phase-4 eligibility update "
         "rate etaElig (default: 0.50)\n"
      << "  -k, --kappa=F                          Phase-4 reward-to-weight "
         "scale kappa (default: 0.15)\n"
      << "  --phase4-unsafe[=on|off]               Bypass validation of "
         "Phase-4 params (default: off)\n"
      << "  --attention-mode=none|external|saliency|topk  Attention modulation "
         "mode (default: external)\n"
      << "  --p-gate=F                             Probability [0,1] to apply "
         "a plasticity update per synapse\n"
      << "  --competence-mode=off|scale-pgate|scale-lr  Competence gating mode "
         "(default: off)\n"
      << "  --competence-rho=F                   EMA rate for competence "
         "update in [0,1] (default: from code)\n"
      << "  --auto-eligibility=on|off              Automatic eligibility "
         "accumulation (default: off)\n"
      << "  --homeostasis-eta=F                    Step size for homeostatic "
         "scaling (>= 0)\n"
      << "  --attention-Amin=F                     Min attention gain (> 0)\n"
      << "  --attention-Amax=F                     Max attention gain (>= "
         "Amin)\n"
      << "  --attention-anneal-ms=MS               Linear anneal duration for "
         "attention (0 = off)\n"
      << "  --chaos-steps=N                        Steps in chaos/explore "
         "phase (default: 0)\n"
      << "  --consolidate-steps=N                  Steps in consolidation "
         "phase (default: 0)\n"
      << "  --novelty-window=N                     Window size for "
         "novelty/saliency (0 = off)\n"
      << "  --prune-threshold=F                    Prune/freeze synapses with "
         "|w| < F during consolidation\n"
      << "  --snapshot-csv=PATH                    Export synapse snapshot "
         "(pre,post,weight) to CSV at end of run\n"
      << "  --snapshot-live=PATH                   Periodically export "
         "snapshot CSV to PATH during run (overwrites)\n"
      << "  --snapshot-interval=MS                 Interval for "
         "--snapshot-live in ms (default: 1000)\n"
      << "  --heatmap-view[=on|off]                Live synapse weight heatmap "
         "via OpenCV window (default: off)\n"
      << "  --heatmap-interval=MS                  Heatmap refresh interval in "
         "ms (default: 1000)\n"
      << "  --heatmap-size=N                       Heatmap resolution N (NxN) "
         "(default: 256)\n"
      << "  --heatmap-threshold=F                  Minimum |weight| to include "
         "(default: 0.0)\n"
      << "  --viewer[=on|off]                      Launch 3D viewer and stream "
         "live synapses (default: off)\n"
      << "  --viewer-exe=PATH                      Path to neuroforge_viewer "
         "executable (optional)\n"
      << "  --viewer-layout=shells|layers          3D layout strategy for "
         "neurons (default: shells)\n"
      << "  --viewer-refresh-ms=MS                 Viewer refresh interval in "
         "ms (default: 1000)\n"
      << "  --viewer-threshold=F                   Minimum |weight| to draw in "
         "viewer (default: 0.0)\n"
      // Sandbox window options (Windows-only)
      << "  --sandbox[=on|off]                     Enable browser sandbox "
         "window (default: off)\n"
      << "  --sandbox-url=URL                      Navigate sandbox to a URL "
         "(default: https://www.youtube.com)\n"
      << "  --sandbox-size=WxH                     Sandbox window size "
         "(default: 1280x720)\n"
      << "  --no-web-actions[=on|off]              Disable web actions in "
         "sandbox (bare flag = on)\n"
      << "  --simulate-blocked-actions=N           Simulate N blocked actions "
         "per step (debug)\n"
      << "  --simulate-rewards=N                   Simulate N reward events "
         "per step (debug)\n"
      << "  --save-brain=PATH                      Save brain checkpoint "
         "(JSON) to PATH at end of run\n"
      << "  --load-brain=PATH                      Load brain checkpoint "
         "(JSON) from PATH at startup (skips demo init)\n"
      << "  --memory-db=PATH                       Enable memory database "
         "logging (experimental; no-op if unsupported)\n"
      << "  --memdb-debug[=on|off]                 Verbose MemoryDB debug "
         "logging (default: off)\n"
      << "  --memdb-color[=auto|on|off]            Colorize MemoryDB debug "
         "output; bare flag = auto (TTY only), default: auto (TTY only)\n"
      << "  --memdb-interval=MS                    Periodic MemoryDB logging "
         "interval in ms (default: 1000)\n"
      << "  --list-episodes=RUN_ID                 List episodes for RUN_ID "
         "from MemoryDB and exit\n"
      << "  --recent-rewards=RUN_ID[,LIMIT]        List recent rewards for "
         "RUN_ID (optional LIMIT, default 10) and exit\n"
      << "  --recent-run-events=RUN_ID[,LIMIT]     List recent run events for "
         "RUN_ID (optional LIMIT, default 10) and exit\n"
      << "  --list-runs                            List all runs in MemoryDB "
         "and exit\n"
      << "\nContext Hooks:\n"
      << "  --context-gain=F                      Context sampling gain "
         "(default: 1.0)\n"
      << "  --context-update-ms=MS                Context sampling interval in "
         "ms (default: 1000)\n"
      << "  --context-window=N                    Context window size for "
         "recent samples (default: 5)\n"
      << "  --context-peer=NAME,GAIN,UPDATE_MS,WINDOW[,LABEL]  Register a "
         "context peer with sampling config; LABEL optional\n"
      << "  --context-couple=PEER_A:PEER_B,WEIGHT              Couple two "
         "peers with influence weight in [0,1]\n"
      << "\nM6 Memory Internalization:\n"
      << "  --hippocampal-snapshots[=on|off]       Enable hippocampal "
         "snapshotting (default: off)\n"
      << "  --memory-independent[=on|off]          Enable memory-independent "
         "learning (default: off)\n"
      << "  --consolidation-interval-m6=MS         Memory consolidation "
         "interval in ms (default: 1000)\n"
      << "\nM7 Autonomous Operation:\n"
      << "  --autonomous-mode[=on|off]             Enable autonomous operation "
         "mode (default: off)\n"
      << "  --substrate-mode=off|mirror|train|native  Neural substrate "
         "operation mode (default: off)\n"
      << "  --curiosity-threshold=F                Curiosity-driven task "
         "threshold in [0,1] (default: 0.3)\n"
      << "  --uncertainty-threshold=F              Uncertainty-based task "
         "threshold in [0,1] (default: 0.4)\n"
      << "  --prediction-error-threshold=F         Prediction error task "
         "threshold in [0,1] (default: 0.5)\n"
      << "  --max-concurrent-tasks=N               Maximum concurrent "
         "autonomous tasks (default: 5)\n"
      << "  --task-generation-interval=MS          Task generation interval in "
         "ms (default: 1000)\n"
      << "  --eliminate-scaffolds[=on|off]         Enable external scaffold "
         "elimination (default: off)\n"
      << "  --autonomy-metrics[=on|off]            Enable autonomy measurement "
         "system (default: off)\n"
      << "  --autonomy-target=F                    Target autonomy level in "
         "[0,1] (default: 0.9)\n"
      << "  --motivation-decay=F                   Motivation signal decay "
         "rate in [0,1] (default: 0.95)\n"
      << "  --exploration-bonus=F                  Exploration behavior bonus "
         "(>= 0, default: 0.2)\n"
      << "  --novelty-memory-size=N                Novelty detection memory "
         "size (>= 1, default: 100)\n"
      << "  --enable-selfnode[=on|off]             Enable SelfNode integration "
         "in autonomous loop (default: off)\n"
      << "  --enable-pfc[=on|off]                  Enable PrefrontalCortex "
         "integration in autonomous loop (default: off)\n"
      << "  --enable-motor-cortex[=on|off]         Enable MotorCortex "
         "integration in autonomous loop (default: off)\n"
      << "\nLearning Stage 3.5 (RWCI):\n"
      << "  --rwci[=on|off]                        Enable curriculum-driven "
         "real-world ingestion (default: off)\n"
      << "  --rwci-drives-revision[=on|off]        Forbidden: use RWCI "
         "outcomes to trigger self-revision (always rejected)\n"
      << "\nN‑Series / Unified Bounded Harness:\n"
      << "  --unified-bounded                      Run the unified bounded "
         "GridWorld harness (shadow mode by default)\n"
      << "  --unified-bounded-steps=N              Max steps for unified "
         "bounded "
         "(default: 5000)\n"
      << "  --unified-bounded-warmup-steps=N       Warmup steps before "
         "obs-model "
         "and WM suggestions (default: 50)\n"
      << "  --unified-bounded-episode-steps=N      Episode timeout in steps "
         "(default: 128)\n"
      << "  --unified-bounded-seed=N               Base seed for GridWorld "
         "reset "
         "(default: 7)\n"
      << "  --unified-bounded-collision-budget=N   Stop after N collisions "
         "(default: 50)\n"
      << "  --unified-bounded-log=PATH             Per-step CSV output path "
         "(default: unified_bounded_log.csv)\n"
      << "  --unified-bounded-summary=PATH         Summary CSV output path "
         "(default: unified_bounded_summary.csv)\n"
      << "  --unified-bounded-live[=on|off]        Enable Stage D3 limited "
         "live control (world model may drive actions under strict gates)\n"
      << "  --unified-bounded-live-mode=match|advantage  Stage D3 gate mode "
         "(default: match)\n"
      << "  --unified-bounded-live-match-window=N  Rolling window for shadow "
         "match rate (default: 500)\n"
      << "  --unified-bounded-live-min-match=F     Minimum shadow match rate "
         "to permit world-model driving (default: 0.80)\n"
      << "  --unified-bounded-live-min-advantage=F Minimum WM score advantage "
         "over obs-model to permit driving (default: 17.0)\n"
      << "  --unified-bounded-live-min-pred-dist-advantage=F  Minimum "
         "predicted "
         "distance advantage vs obs-model to permit WM driving (default: 0.0)\n"
      << "  --unified-bounded-live-max-collision-prob=F  Max predicted "
         "collision probability to permit world-model driving (default: 0.05)\n"
      << "  --unified-bounded-live-min-dist-improve=F    Minimum predicted "
         "distance improvement to permit world-model driving (default: 0.05)\n"
      << "\nN8 Phase (Human Gated):\n"
      << "  --n8-human-gated                       Run Phase N8 Human-Gated "
         "Goal Execution\n"
      << "\nSpike overlays (3D viewer):\n"
      << "  --spikes-live=PATH                    Periodically export recent "
         "spikes CSV to PATH (overwrites)\n"
      << "  --spike-ttl=SEC                       Time-to-live for spikes "
         "window in seconds (default: 2.0)\n"
      << "\nDemo selection:\n"
      << "  --vision-demo[=on|off]                 Enable vision demo "
         "(default: off unless built with NF_ENABLE_VISION_DEMO)\n"
      << "  --audio-demo[=on|off]                  Enable audio demo (default: "
         "off)\n"
      << "  --motor-cortex[=on|off]                Enable motor cortex demo "
         "(default: off)\n"
      << "  --social-perception[=on|off]           Enable advanced social "
         "perception with face masking and gaze vectors (default: off)\n"
      << "  --social-view[=on|off]                 Live social perception "
         "visualization via OpenCV window (default: off)\n"
      << "  --audio-mic[=on|off]                   Use live microphone input "
         "(Windows only; default: off)\n"
      << "  --audio-system[=on|off]                Use system loopback audio "
         "(Windows; default: off)\n"
      << "  --audio-file=PATH                      Use audio from WAV file "
         "(mono 16-bit PCM)\n"
      << "  --camera-index=N                       Select camera device index "
         "(default: 0)\n"
      << "  --camera-backend=any|msmf|dshow        Force OpenCV backend "
         "(Windows: msmf or dshow; default: any)\n"
      << "  --vision-source=camera|screen|maze|synthetic  Select visual input "
         "source (default: camera)\n"
      << "  --retina-screen-rect=X,Y,W,H           Screen rectangle for "
         "--vision-source=screen (default: 0,0,1280,720)\n"
      << "  --foveation[=on|off]                    Enable dynamic retina "
         "focusing (default: off)\n"
      << "  --fovea-size=WxH                        Fovea size in pixels "
         "(default: 640x360)\n"
      << "  --fovea-mode=cursor|center|attention    Fovea follow mode "
         "(default: cursor)\n"
      << "  --fovea-alpha=F                        Fovea center EMA smoothing "
         "in [0,1] (default: 0.3)\n"
      << "  --youtube-mode[=on|off]                Preset: vision=screen, "
         "audio=system (default: off)\n"
      << "\nVision encoder config:\n"
      << "  --vision-grid=N                        Vision grid size G (input "
         "length = G*G; default: 16)\n"
      << "  --vision-edge[=on|off]                 Include edge magnitude in "
         "fusion (default: on)\n"
      << "  --vision-edge-weight=F                 Edge weight (default: 0.6)\n"
      << "  --vision-intensity-weight=F            Intensity weight (default: "
         "0.4)\n"
      << "  --vision-motion[=on|off]               Include simple motion term "
         "(default: off)\n"
      << "  --vision-motion-weight=F               Motion weight (default: "
         "0.3)\n"
      << "  --vision-temporal-decay=F              Reserved EMA decay in [0,1] "
         "(default: 0.9)\n"
      << "\nAudio encoder config:\n"
      << "  --audio-samplerate=N                   Audio sample rate (default: "
         "16000)\n"
      << "  --audio-feature-bins=N                 Output feature bins "
         "(default: 256)\n"
      << "  --audio-spectral-bins=N                Internal spectral bins "
         "(default: 64)\n"
      << "  --audio-mel-bands=N                    Mel bands (default: 64)\n"
      << "  --audio-preemphasis[=on|off]           Enable pre-emphasis "
         "(default: on)\n"
      << "\nMultimodal options:\n"
      << "  --cross-modal[=on|off]                 Enable Visual<->Auditory "
         "cross-modal connectivity (default: off)\n"
      << "\nMaze demo:\n"
      << "  --maze-demo[=on|off]                   Enable simple grid maze "
         "demo (default: off)\n"
      << "  --maze-first-person[=on|off]           Enable first-person visual "
         "navigation mode (default: off)\n"
      << "  --maze-size=N                          Maze grid size N x N "
         "(default: 8)\n"
      << "  --maze-wall-density=F                  Fraction of cells as walls "
         "in [0,0.45] (default: 0.20)\n"
      << "  --epsilon=F                            Epsilon-greedy rate in "
         "[0,1] (debug / Q-learning mode)\n"
      << "  --softmax-temp=F                       Softmax temperature > 0 for "
         "neural-style stochastic policy (default: 0.5)\n"
      << "  --maze-view[=on|off]                   Live maze visualization via "
         "OpenCV window (default: off)\n"
      << "  --maze-view-interval=MS                Maze view refresh interval "
         "in ms (default: 300)\n"
      << "  --maze-max-episode-steps=N             Terminate episode as "
         "failure after N steps without reaching goal (default: 4*N*N)\n"
      << "  --maze-shaping=off|euclid|manhattan    Potential-based shaping "
         "mode (default: off)\n"
      << "  --maze-shaping-k=F                     Shaping scale beta "
         "(default: 0.01)\n"
      << "  --maze-shaping-gamma=F                 Shaping discount gamma in "
         "[0,1] (default: 0.99)\n"
      << "  --episode-csv=PATH                     Append per-episode metrics "
         "to PATH as CSV\n"
      << "  --summary[=on|off]                     Print end-of-run episode "
         "summary (default: off)\n"
      << "  --qlearning[=on|off]                   Use Q-learning baseline "
         "policy (default: off; neural control is default)\n"
      << "  --hybrid-lambda=F                      Blend motor cortex (lambda) "
         "with Q-table (1-lambda); 1=motor only, 0=Q only; omit to use pure "
         "neural or pure Q based on --qlearning\n"
      << "  --teacher-policy=none|greedy|bfs       Optional maze teacher "
         "policy (default: none)\n"
      << "  --teacher-mix=F                        Blend teacher one-hot into "
         "scores in [0,1] (default: 0.0)\n"
      << "\nMimicry shaping (Phase-5, optional):\n"
      << "  --mimicry[=on|off]                     Enable mimicry term in "
         "shaped reward (default: off)\n"
      << "  --mimicry-weight=F                     Weight mu for mimicry term "
         "(default: 0.0)\n"
      << "  --mimicry-internal[=on|off]           Route Phase A "
         "similarity/novelty internally in LearningSystem (default: off)\n"
      << "  --teacher-embed=PATH                   Path to teacher embedding "
         "file (comma/space-separated floats)\n"
      << "  --student-embed=PATH                   Path to initial student "
         "embedding file (optional)\n"
      << "                                         When maze demo is active, "
         "the student embedding is updated each step from the blended policy "
         "scores.\n"
      << "  --mirror-mode=off|vision|audio         Use sensory features as "
         "student embedding source instead of action scores (default: off)\n"
      << "  --student-learning-rate=F              Set Phase A student "
         "learning rate (default: 0.05)\n"
      << "\nUnified substrate (WM + Phase C + SurvivalBias + Language):\n"
      << "  --unified-substrate[=on|off]          Enable unified substrate run "
         "(default: off)\n"
      << "  --wm-neurons=N                         Override "
         "WM/binding/sequence neurons per region (default: 64)\n"
      << "  --phasec-neurons=N                     Override Phase C neurons "
         "per region (default: 64)\n"
      << "  --adaptive=on|off                      Toggle unified adaptive "
         "reflection (default: on)\n"
      << "  --survival-bias=on|off                 Toggle SurvivalBias "
         "effector in unified mode (default: on)\n"
      << "\nDataset ingestion:\n"
      << "  --dataset-triplets=PATH                Root of triplet dataset "
         "(audio/text/images)\n"
      << "  --dataset-mode=triplets               Enable triplet ingestion "
         "mode\n"
      << "  --dataset-limit=N                     Limit number of triplets "
         "loaded\n"
      << "  --dataset-shuffle[=on|off]            Shuffle loaded triplets "
         "(default: off)\n"
      << "  --reward-scale=F                      Scale delivered reward "
         "(default: 1.0)\n"
      << "\nLanguage/Phase A (experimental):\n"
      << "  --phase5-language[=on|off]            Initialize Phase-5 "
         "LanguageSystem (default: off)\n"
      << "  --phase-a[=on|off]                     Initialize Phase A Baby "
         "Mimicry system (requires LanguageSystem) (default: off)\n"
      << "  --phase-a-similarity-threshold=F       Set Phase A similarity "
         "success threshold (default: 0.6)\n"
      << "  --phase-a-novelty-threshold=F          Set Phase A novelty success "
         "threshold (default: 0.1)\n"
      << "  --phase-a-ema[=on|off]                 Toggle Phase A EMA "
         "stabilizer (default: on)\n"
      << "  --phase-a-ema-min=F                    Minimum EMA coefficient "
         "alpha_min (default: 0.02)\n"
      << "  --phase-a-ema-max=F                    Maximum EMA coefficient "
         "alpha_max (default: 0.2)\n"
      << "  --phase-a-replay-interval=N            Replay top attempts every N "
         "steps (>=1)\n"
      << "  --phase-a-replay-top-k=K               Number of past attempts to "
         "replay (>=1)\n"
      << "  --phase-a-replay-boost=F               Scale reward during replay "
         "(>=0; default: 1.0)\n"
      << "  --phase-a-replay-lr-scale=F            Scale learning rate during "
         "replay (>=0; default: 1.0)\n"
      << "  --phase-a-replay-include-hard-negatives=on|off  Enable "
         "hard-negative replay (default: on)\n"
      << "  --phase-a-replay-hard-k=K               Number of hard negatives "
         "to include (>=1; default: 3)\n"
      << "  --phase-a-replay-repulsion-weight=F     Repulsion weight for hard "
         "negatives (>=0; default: 0.5)\n"
      << "  --phase-a-export=DIR                   Export Phase A JSON to DIR "
         "at end of run\n"
      << "  --phase6[=on|off]                      Enable Phase 6 Reasoner "
         "(shadow logging; no behavior changes)\n"
      << "  --phase6-active=on|off|audit           Phase 6 control mode "
         "(default: off)\n"
      << "  --phase6-margin=F                      Override margin in [0,1] "
         "(default: 0.08)\n"
      << "  --phase7[=on|off]                      Initialize Phase 7 "
         "Affective State and Reflection (requires MemoryDB)\n"
      << "  --phase7-affect[=on|off]               Initialize Phase 7 "
         "Affective State only (requires MemoryDB)\n"
      << "  --phase7-reflect[=on|off]              Initialize Phase 7 "
         "Reflection only (requires MemoryDB)\n"
      << "  --phase8[=on|off]                      Initialize Phase 8 Goal "
         "System (default: on)\n"
      << "  --phase9[=on|off]                     Enable/disable Phase 9 "
         "metacognition (default: on)\n"
      << "  --phase9-modulation[=on|off]           Enable Phase 9 metacog "
         "modulation (default: off)\n"
      << "  --phase10[=on|off]                    Enable/disable Phase 10 "
         "self-explanation (default: on)\n"
      << "  --phase11[=on|off]                    Enable/disable Phase 11 "
         "self-revision (default: on)\n"
      << "  --phase11-revision-interval=N         Revision interval in ms "
         "(default: 300000)\n"
      << "  --phase11-min-gap-ms=N                Minimum gap between "
         "revisions in ms (default: 60000)\n"
      << "  --phase11-outcome-window-ms=N         Outcome evaluation pre/post "
         "window in ms (default: 60000)\n"
      << "  --stagec[=on|off]                     Enable/disable Stage C v1 "
         "autonomy gating (default: on)\n"
      << "  --phase13[=on|off]                    Enable/disable Phase 13 "
         "autonomy envelope (default: on)\n"
      << "  --phase13-window=N                    Autonomy analysis window "
         "size (default: 10)\n"
      << "  --phase13-trust-tighten=F             Self-trust tighten threshold "
         "(default: 0.35)\n"
      << "  --phase13-trust-expand=F              Self-trust expand threshold "
         "(default: 0.70)\n"
      << "  --phase13-consistency-tighten=F       Self-consistency tighten "
         "threshold (default: 0.50)\n"
      << "  --phase13-consistency-expand=F        Self-consistency expand "
         "threshold (default: 0.80)\n"
      << "  --phase13-contraction-hysteresis-ms=N Contraction hysteresis in ms "
         "(default: 60000)\n"
      << "  --phase13-expansion-hysteresis-ms=N   Expansion hysteresis in ms "
         "(default: 60000)\n"
      << "  --phase13-min-log-interval-ms=N       Minimum log interval in ms "
         "(default: 30000)\n"
      << "  Note: Phase A embedding dimension is auto-derived in this order:\n"
      << "        1) teacher vector length from --teacher-embed\n"
      << "        2) mirror mode: vision uses G*G from --vision-grid; audio "
         "uses --audio-feature-bins\n"
      << "        3) otherwise the Phase A config default\n"
      << "\nTelemetry:\n"
      << "  --telemetry-extended[=on|off]          Include Phase A "
         "last-attempt metrics in experience snapshots (default: off)\n"
      << "\nMachine-readable logs:\n"
      << "  --log-json[=PATH|on|off]               Emit line-delimited JSON "
         "events (stdout by default; append to PATH if provided)\n"
      << "  --log-json-sample=N                    Log every Nth event "
         "(default 1 = no sampling)\n"
      << "  --log-json-events=list                 Comma-separated allowlist "
         "of events to log; items are 'event' or 'Phase:event'\n"
      << "                                         Examples: "
         "--log-json-events=decision,episode_end or A:decision,B:reward\n"
      << "\nPhase C (Global Workspace prototype):\n"
      << "  --phase-c[=on|off]                     Run Phase C "
         "variable-binding/sequence task (default: off)\n"
      << "  --phase-c-mode=binding|sequence        Select Phase C task mode "
         "(default: binding)\n"
      << "  --phase-c-out=PATH                     Output directory for Phase "
         "C CSV logs (default: PhaseC_Logs)\n"
      << "  --phase-c-wm-capacity=N                WorkingMemory capacity "
         "(default: 6)\n"
      << "  --binding-threshold=F                  Activation threshold for "
         "binding regions in [0,1] (default: 0.7)\n"
      << "  --sequence-threshold=F                 Activation threshold for "
         "sequence regions in [0,1] (default: 0.6)\n"
      << "  --binding-coherence-min=F              Minimum coherence to accept "
         "binding assemblies in [0,1] (default: 0.5)\n"
      << "  --sequence-coherence-min=F             Minimum coherence to accept "
         "sequence assemblies in [0,1] (default: 0.4)\n"
      << "  --prune-coherence-threshold=F          Prune assemblies below "
         "coherence in [0,1] (default: 0.3)\n"
      << "  --phase-c-wm-decay=F                   WorkingMemory decay per "
         "step in (0,1] (default: 0.90)\n"
      << "  --phase-c-seq-window=N                 Optional: keep at most N "
         "recent sequence tokens in WM (0 = unlimited; default: 0)\n"
      << "  --phase-c-seed=N                       RNG seed (default: random "
         "if omitted or 0)\n"
      << "  --phase-c-survival-bias[=on|off]       Enable SurvivalBias "
         "modulation and telemetry (default: off)\n"
      << "  --phase-c-variance-sensitivity=F       Scale variance contribution "
         "to risk in SurvivalBias (default: 1.0)\n"
      << "  --phase-c-survival-scale=F             Scale shaped reward "
         "magnitude (default: 1.0)\n"
      << "  --hazard-density=F                     Fixed hazard rate in [0,1]; "
         ">0 overrides audio; 0 = audio RMS fallback (default: unset)\n"
      << "  --phase-c-hazard-weight=F              Hazard coherence "
         "down-modulation weight in [0,1] (default: 0.2)\n"
      << "  --phase-c-hazard-alpha=F               Sensitivity of modulation "
         "to external hazard in [0,1] (default: 0.0)\n"
      << "  --phase-c-hazard-beta=F                Sensitivity of modulation "
         "to arousal in [0,1] (default: 0.0)\n"
      << "  --phase-c-lag-align=N                  Offset reward log step by N "
         "(default: 0)\n"
      << "  Note: Phase C runs as an independent demo path and does not "
         "currently coexist with Phase A/B\n"
      << "        in the same invocation. To collect A/B and C telemetry, run "
         "separate executions.\n"
      << "\nRegion creation:\n"
      << "  --add-region=KEY[:NAME[:COUNT]]        Create a region by registry "
         "key; NAME defaults to KEY; COUNT (>=0) creates that many neurons.\n"
      << "                                         Examples: "
         "--add-region=visual:VisIn:1024 --add-region=hippocampus\n"
      << "  --list-regions                         List available region "
         "keys/aliases and exit\n"
      << "\nEmergence mode:\n"
      << "  --emergent-only[=on|off]               Force pure emergent "
         "control: disable Q-learning, teacher mix, epsilon/softmax; action = "
         "argmax(neural)\n"
      << "  --true-emergence[=on|off]              Alias for --emergent-only\n"
      << "\nUnified substrate:\n"
      << "  --unified-substrate[=on|off]          Run WM + Phase C + "
         "SurvivalBias + Language integration concurrently\n"
      << std::endl;
...
bool parse_on_off_flag(const std::string &v, bool &out) {
  std::string vl = v;
  std::transform(vl.begin(), vl.end(), vl.begin(), [](unsigned char c) {
    return static_cast<char>(std::tolower(c));
  });
  if (vl == "1" || vl == "true" || vl == "on") {
    out = true;
    return true;
  }
  if (vl == "0" || vl == "false" || vl == "off") {
    out = false;
    return true;
  }
  return false;
...
static bool handle_audio_arg(const std::string &arg,
  ...
  if (starts_with(arg, "--audio-samplerate=")) {
    ...
    try {
      acfg.sample_rate = std::stoi(v);
      if (acfg.sample_rate <= 0) {
        std::cerr << "Error: --audio-samplerate must be positive" << std::endl;
        return true;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --audio-samplerate" << std::endl;
      return true;
    }
    return true;
  }
  if (starts_with(arg, "--audio-feature-bins=")) {
    ...
    try {
      acfg.feature_bins = std::stoi(v);
      if (acfg.feature_bins <= 0) {
        std::cerr << "Error: --audio-feature-bins must be positive"
                  << std::endl;
        return true;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --audio-feature-bins"
                << std::endl;
      return true;
    }
    return true;
  ...
  if (starts_with(arg, "--audio-spectral-bins=")) {
    ...
    try {
      acfg.spectral_bins = std::stoi(v);
      if (acfg.spectral_bins <= 0) {
        std::cerr << "Error: --audio-spectral-bins must be positive"
                  << std::endl;
        return true;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --audio-spectral-bins"
                << std::endl;
      return true;
    }
    return true;
  ...
  if (starts_with(arg, "--audio-mel-bands=")) {
    ...
    try {
      acfg.mel_bands = std::stoi(v);
      if (acfg.mel_bands <= 0) {
        std::cerr << "Error: --audio-mel-bands must be positive" << std::endl;
        return true;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --audio-mel-bands" << std::endl;
      return true;
    }
    return true;
  }
  if (arg == "--audio-preemphasis") {
    acfg.pre_emphasis = true;
    return true;
  }
  if (starts_with(arg, "--audio-preemphasis=")) {
    auto v = arg.substr(std::string("--audio-preemphasis=").size());
    if (!parse_on_off_flag(v, acfg.pre_emphasis)) {
      std::cerr << "Error: --audio-preemphasis must be on|off|true|false|1|0"
                << std::endl;
      return true;
    }
    return true;
...
static bool handle_learning_arg(
    const std::string &arg, NeuroForge::Core::LearningSystem::Config &lconf,
    bool &hebbian_rate_set, bool &stdp_rate_set, bool &stdp_mult_set,
    bool &attention_boost_set, bool &homeostasis_set,
    bool &consolidation_interval_set, bool &consolidation_strength_set,
    bool &attention_mode_set, bool &competence_mode_set, bool &p_gate_set,
    bool &competence_rho_set, bool &auto_elig_set, bool &homeostasis_eta_set,
    bool &chaos_steps_set, bool &consolidate_steps_set,
    bool &novelty_window_set, bool &prune_threshold_set,
    bool &attention_Amin_set, bool &attention_Amax_set,
  ...
  if (starts_with(arg, "--stdp-rate-multiplier=")) {
    ...
    try {
      ...
      std::cerr << "Error: invalid float for --stdp-rate-multiplier"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--homeostasis=")) {
    ...
    if (!parse_on_off_flag(v, lconf.enable_homeostasis)) {
      std::cerr << "Error: --homeostasis must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    homeostasis_set = true;
  ...
  if (starts_with(arg, "--consolidation-interval=")) {
    ...
    try {
      int ms = std::stoi(v);
      if (ms < 0) {
        std::cerr << "Error: --consolidation-interval must be non-negative"
                  << std::endl;
        exit(2);
      }
      ...
      std::cerr << "Error: invalid integer for --consolidation-interval"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--consolidation-strength=")) {
    ...
    try {
      ...
      std::cerr << "Error: invalid float for --consolidation-strength"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--attention-mode=")) {
    ...
    if (vlow == "none" || vlow == "off") {
      lconf.attention_mode =
          NeuroForge::Core::LearningSystem::AttentionMode::Off;
    } else if (vlow == "external" || vlow == "map" || vlow == "externalmap") {
      lconf.attention_mode =
          NeuroForge::Core::LearningSystem::AttentionMode::ExternalMap;
    } else if (vlow == "saliency") {
      lconf.attention_mode =
          NeuroForge::Core::LearningSystem::AttentionMode::Saliency;
    } else if (vlow == "topk" || vlow == "top-k") {
      lconf.attention_mode =
          NeuroForge::Core::LearningSystem::AttentionMode::TopK;
    } else {
      std::cerr << "Error: --attention-mode must be one of: none, external, "
                   "saliency, topk"
                << std::endl;
      exit(2);
    }
    attention_mode_set = true;
    if (lconf.attention_mode !=
        NeuroForge::Core::LearningSystem::AttentionMode::Off) {
  ...
  if (starts_with(arg, "--competence-mode=")) {
    ...
    if (vlow == "off" || vlow == "none") {
      lconf.competence_mode =
          NeuroForge::Core::LearningSystem::CompetenceMode::Off;
    } else if (vlow == "scale-pgate" || vlow == "scale-p_gate" ||
               vlow == "scale-p") {
      lconf.competence_mode =
          NeuroForge::Core::LearningSystem::CompetenceMode::ScalePGate;
    } else if (vlow == "scale-lr" || vlow == "scale-learning-rates" ||
               vlow == "scale-learning") {
      lconf.competence_mode =
          NeuroForge::Core::LearningSystem::CompetenceMode::ScaleLearningRates;
    } else {
      std::cerr << "Error: --competence-mode must be one of: off, scale-pgate, "
                   "scale-lr"
                << std::endl;
      exit(2);
    }
  ...
  if (starts_with(arg, "--auto-eligibility=")) {
    ...
    if (!parse_on_off_flag(v, auto_elig_enabled)) {
      std::cerr << "Error: --auto-eligibility must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    auto_elig_set = true;
  ...
  if (starts_with(arg, "--attention-anneal-ms=")) {
    ...
    try {
      ...
      std::cerr << "Error: invalid integer for --attention-anneal-ms"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--consolidate-steps=")) {
    ...
    try {
      ...
      std::cerr << "Error: invalid integer for --consolidate-steps"
                << std::endl;
      exit(2);
    }
    return true;
...
static bool handle_phase4_arg(const std::string &arg, int argc, char *argv[],
                              int &i, float &alpha_weight, bool &alpha_set,
                              float &gamma_weight, bool &gamma_set,
                              float &eta_weight, bool &eta_set,
                              float &lambda_param, bool &lambda_set,
                              float &etaElig_param, bool &etaElig_set,
                              float &kappa_param, bool &kappa_set,
                              bool &phase4_unsafe) {
  if (arg == "-a" || starts_with(arg, "-a=") || starts_with(arg, "--alpha=")) {
    std::string v;
    if (arg == "-a") {
      if (i + 1 >= argc) {
        std::cerr << "Error: -a requires a float value" << std::endl;
        exit(2);
      }
      v = argv[++i];
    } else {
      v = starts_with(arg, "-a=") ? arg.substr(std::string("-a=").size())
                                  : arg.substr(std::string("--alpha=").size());
    }
    try {
      alpha_weight = std::stof(v);
      alpha_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --alpha" << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "-g" || starts_with(arg, "-g=") || starts_with(arg, "--gamma=")) {
    ...
    if (arg == "-g") {
      if (i + 1 >= argc) {
        std::cerr << "Error: -g requires a float value" << std::endl;
        exit(2);
      }
      v = argv[++i];
    } else {
      v = starts_with(arg, "-g=") ? arg.substr(std::string("-g=").size())
                                  : arg.substr(std::string("--gamma=").size());
    }
    try {
      gamma_weight = std::stof(v);
      gamma_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --gamma" << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "-u" || starts_with(arg, "-u=") || starts_with(arg, "--eta=")) {
    ...
    if (arg == "-u") {
      if (i + 1 >= argc) {
        std::cerr << "Error: -u requires a float value" << std::endl;
        exit(2);
      }
      v = argv[++i];
    } else {
      v = starts_with(arg, "-u=") ? arg.substr(std::string("-u=").size())
                                  : arg.substr(std::string("--eta=").size());
    }
    try {
      eta_weight = std::stof(v);
      eta_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --eta" << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "-l" || starts_with(arg, "-l=") || starts_with(arg, "--lambda=")) {
    ...
    if (arg == "-l") {
      if (i + 1 >= argc) {
        std::cerr << "Error: -l requires a float value" << std::endl;
        exit(2);
      }
      v = argv[++i];
    } else {
      v = starts_with(arg, "-l=") ? arg.substr(std::string("-l=").size())
                                  : arg.substr(std::string("--lambda=").size());
    }
    try {
      lambda_param = std::stof(v);
      lambda_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --lambda" << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "-e" || starts_with(arg, "-e=") ||
      starts_with(arg, "--eta-elig=")) {
    std::string v;
    if (arg == "-e") {
      if (i + 1 >= argc) {
        std::cerr << "Error: -e requires a float value" << std::endl;
        exit(2);
      }
      v = argv[++i];
    } else {
      v = starts_with(arg, "-e=")
              ? arg.substr(std::string("-e=").size())
              : arg.substr(std::string("--eta-elig=").size());
    }
    try {
      etaElig_param = std::stof(v);
      etaElig_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --eta-elig" << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "-k" || starts_with(arg, "-k=") || starts_with(arg, "--kappa=")) {
    ...
    if (arg == "-k") {
      if (i + 1 >= argc) {
        std::cerr << "Error: -k requires a float value" << std::endl;
        exit(2);
      }
      v = argv[++i];
    } else {
      v = starts_with(arg, "-k=") ? arg.substr(std::string("-k=").size())
                                  : arg.substr(std::string("--kappa=").size());
    }
    try {
      kappa_param = std::stof(v);
      kappa_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --kappa" << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "--phase4-unsafe") {
    phase4_unsafe = true;
    return true;
  }
  if (starts_with(arg, "--phase4-unsafe=")) {
    auto v = arg.substr(std::string("--phase4-unsafe=").size());
    if (!parse_on_off_flag(v, phase4_unsafe)) {
      std::cerr << "Error: --phase4-unsafe must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
...
static bool
handle_vision_arg(const std::string &arg,
                  NeuroForge::Encoders::VisionEncoder::Config &vcfg) {
  if (starts_with(arg, "--vision-grid=")) {
    auto v = arg.substr(std::string("--vision-grid=").size());
    try {
      vcfg.grid_size = std::stoi(v);
      if (vcfg.grid_size <= 0) {
        std::cerr << "Error: --vision-grid must be positive" << std::endl;
        return true;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --vision-grid" << std::endl;
      return true;
    }
    return true;
  }
  ...
  if (starts_with(arg, "--vision-edge=")) {
    ...
    if (!parse_on_off_flag(v, vcfg.use_edge)) {
      std::cerr << "Error: --vision-edge must be on|off|true|false|1|0"
                << std::endl;
      return true;
    }
    return true;
  ...
  if (starts_with(arg, "--vision-edge-weight=")) {
    ...
    try {
      vcfg.edge_weight = std::stof(v);
    } catch (...) {
      std::cerr << "Error: invalid float for --vision-edge-weight" << std::endl;
      return true;
    }
    return true;
  }
  if (starts_with(arg, "--vision-intensity-weight=")) {
    ...
    try {
      vcfg.intensity_weight = std::stof(v);
    } catch (...) {
      std::cerr << "Error: invalid float for --vision-intensity-weight"
                << std::endl;
      return true;
    }
    return true;
  }
  if (arg == "--vision-motion") {
    vcfg.use_motion = true;
    return true;
  }
  if (starts_with(arg, "--vision-motion=")) {
    auto v = arg.substr(std::string("--vision-motion=").size());
    if (!parse_on_off_flag(v, vcfg.use_motion)) {
      std::cerr << "Error: --vision-motion must be on|off|true|false|1|0"
                << std::endl;
      return true;
    }
    return true;
  ...
  if (starts_with(arg, "--vision-motion-weight=")) {
    ...
    try {
      vcfg.motion_weight = std::stof(v);
    } catch (...) {
      std::cerr << "Error: invalid float for --vision-motion-weight"
                << std::endl;
      return true;
    }
    return true;
  ...
  if (starts_with(arg, "--vision-temporal-decay=")) {
    ...
    try {
      vcfg.temporal_decay = std::stof(v);
      vcfg.temporal_decay = std::clamp(vcfg.temporal_decay, 0.0f, 1.0f);
    } catch (...) {
      std::cerr << "Error: invalid float for --vision-temporal-decay"
                << std::endl;
      return true;
    }
    return true;
...
static bool handle_telemetry_arg(
    const std::string &arg, bool &log_json, std::string &log_json_path,
    int &log_json_sample_val, std::string &log_json_events_csv,
    std::string &memory_db_path, bool &memdb_debug, int &memdb_interval_ms,
    bool &memdb_interval_cli_set, bool &flag_list_runs,
    bool &flag_list_episodes, std::string &list_episodes_run_id,
    bool &flag_recent_rewards, std::string &recent_rewards_run_id,
    int &recent_rewards_limit, bool &flag_recent_run_events,
    std::string &recent_run_events_run_id, int &recent_run_events_limit) {
  // JSON logging
  if (arg == "--log-json") {
    log_json = true;
    return true;
  ...
  if (starts_with(arg, "--log-json=")) {
    ...
    if (vlow == "on" || vlow == "true" || vlow == "1") {
      log_json = true;
    } else if (vlow == "off" || vlow == "false" || vlow == "0") {
      log_json = false;
    } else {
      log_json = true;
      log_json_path = v;
  ...
  if (starts_with(arg, "--memory-db=")) {
    ...
    if (memory_db_path.empty()) {
      std::cerr << "Error: --memory-db requires a file path" << std::endl;
      exit(2);
    }
    return true;
  }
  ...
  if (starts_with(arg, "--memdb-debug=")) {
    ...
    if (!parse_on_off_flag(v, memdb_debug)) {
      std::cerr << "Error: --memdb-debug must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--memdb-interval=")) {
    ...
    try {
      ...
      if (memdb_interval_ms <= 0) {
        std::cerr << "Error: --memdb-interval must be > 0" << std::endl;
        exit(2);
      }
      memdb_interval_cli_set = true;
    } catch (...) {
      std::cerr << "Error: invalid integer for --memdb-interval" << std::endl;
      exit(2);
    }
    return true;
  }
  ...
  if (starts_with(arg, "--list-episodes=")) {
    ...
    if (list_episodes_run_id.empty()) {
      std::cerr << "Error: --list-episodes requires RUN_ID" << std::endl;
      exit(2);
    }
  ...
  if (starts_with(arg, "--recent-rewards=")) {
    ...
    if (v.empty()) {
      std::cerr << "Error: --recent-rewards requires RUN_ID[,LIMIT]"
                << std::endl;
      exit(2);
    }
    auto comma = v.find(',');
    if (comma == std::string::npos) {
      recent_rewards_run_id = v;
    } else {
      recent_rewards_run_id = v.substr(0, comma);
      auto lims = v.substr(comma + 1);
      try {
        recent_rewards_limit = std::stoi(lims);
        if (recent_rewards_limit <= 0)
          recent_rewards_limit = 10;
      } catch (...) {
        std::cerr
            << "Error: invalid LIMIT for --recent-rewards (expected integer)"
            << std::endl;
        exit(2);
      }
    }
    if (recent_rewards_run_id.empty()) {
      std::cerr << "Error: --recent-rewards requires RUN_ID before comma"
                << std::endl;
      exit(2);
    }
  ...
  if (starts_with(arg, "--recent-run-events=")) {
    ...
    if (v.empty()) {
      std::cerr << "Error: --recent-run-events requires RUN_ID[,LIMIT]"
                << std::endl;
      exit(2);
    }
    auto comma = v.find(',');
    if (comma == std::string::npos) {
      recent_run_events_run_id = v;
    } else {
      recent_run_events_run_id = v.substr(0, comma);
      auto lims = v.substr(comma + 1);
      try {
        recent_run_events_limit = std::stoi(lims);
        if (recent_run_events_limit <= 0)
          recent_run_events_limit = 10;
      } catch (...) {
        std::cerr
            << "Error: invalid LIMIT for --recent-run-events (expected integer)"
            << std::endl;
        exit(2);
      }
    }
    if (recent_run_events_run_id.empty()) {
      std::cerr << "Error: --recent-run-events requires RUN_ID before comma"
                << std::endl;
      exit(2);
    }
...
bool handle_m6_m7_parameters(
    const std::string &arg, bool &hippocampal_snapshots,
    bool &hippocampal_snapshots_set, bool &memory_independent,
    bool &memory_independent_set, int &consolidation_interval_m6,
    bool &consolidation_interval_m6_set, bool &autonomous_mode,
    bool &autonomous_mode_set, std::string &substrate_mode,
    bool &substrate_mode_set, float &curiosity_threshold,
    bool &curiosity_threshold_set, float &uncertainty_threshold,
    bool &uncertainty_threshold_set, float &prediction_error_threshold,
    bool &prediction_error_threshold_set, int &max_concurrent_tasks,
    bool &max_concurrent_tasks_set, int &task_generation_interval,
    bool &task_generation_interval_set, bool &eliminate_scaffolds,
    bool &eliminate_scaffolds_set, bool &autonomy_metrics,
    bool &autonomy_metrics_set, float &autonomy_target,
    bool &autonomy_target_set, float &motivation_decay,
    bool &motivation_decay_set, float &exploration_bonus,
    bool &exploration_bonus_set, int &novelty_memory_size,
    bool &novelty_memory_size_set, bool &enable_selfnode,
    bool &enable_selfnode_set, bool &enable_pfc, bool &enable_pfc_set,
  ...
  if (arg == "--hippocampal-snapshots") {
    ...
    if (!parse_on_off_flag(v, hippocampal_snapshots)) {
      std::cerr
          << "Error: --hippocampal-snapshots must be on|off|true|false|1|0"
          << std::endl;
      exit(2);
    }
    hippocampal_snapshots_set = true;
    ...
    if (!parse_on_off_flag(v, memory_independent)) {
      std::cerr << "Error: --memory-independent must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    memory_independent_set = true;
    ...
    try {
      ...
      if (consolidation_interval_m6 < 0) {
        std::cerr << "Error: --consolidation-interval-m6 must be non-negative"
                  << std::endl;
        exit(2);
      }
      consolidation_interval_m6_set = true;
    } catch (...) {
      std::cerr << "Error: invalid integer for --consolidation-interval-m6"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (arg == "--autonomous-mode") {
    ...
    if (!parse_on_off_flag(v, autonomous_mode)) {
      std::cerr << "Error: --autonomous-mode must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    autonomous_mode_set = true;
    ...
    if (v == "off" || v == "mirror" || v == "train" || v == "native") {
      ...
      std::cerr << "Error: --substrate-mode must be off|mirror|train|native"
                << std::endl;
      exit(2);
    }
    return true;
    ...
    try {
      ...
      if (curiosity_threshold < 0.0f || curiosity_threshold > 1.0f) {
        std::cerr << "Error: --curiosity-threshold must be in [0,1]"
                  << std::endl;
        exit(2);
      }
      curiosity_threshold_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --curiosity-threshold"
                << std::endl;
      exit(2);
    }
    return true;
    ...
    try {
      ...
      if (uncertainty_threshold < 0.0f || uncertainty_threshold > 1.0f) {
        std::cerr << "Error: --uncertainty-threshold must be in [0,1]"
                  << std::endl;
        exit(2);
      }
      uncertainty_threshold_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --uncertainty-threshold"
                << std::endl;
      exit(2);
    }
    return true;
    ...
    try {
      ...
      if (prediction_error_threshold < 0.0f ||
          prediction_error_threshold > 1.0f) {
        std::cerr << "Error: --prediction-error-threshold must be in [0,1]"
                  << std::endl;
        exit(2);
      }
      prediction_error_threshold_set = true;
    } catch (...) {
      std::cerr << "Error: invalid float for --prediction-error-threshold"
                << std::endl;
      exit(2);
    }
    return true;
    ...
    try {
      ...
      std::cerr << "Error: invalid integer for --max-concurrent-tasks"
                << std::endl;
      exit(2);
    }
    return true;
    ...
    try {
      ...
      if (task_generation_interval < 0) {
        std::cerr << "Error: --task-generation-interval must be non-negative"
                  << std::endl;
        exit(2);
      }
      task_generation_interval_set = true;
    } catch (...) {
      std::cerr << "Error: invalid integer for --task-generation-interval"
                << std::endl;
      exit(2);
    }
    return true;
    ...
    if (!parse_on_off_flag(v, eliminate_scaffolds)) {
      std::cerr << "Error: --eliminate-scaffolds must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    eliminate_scaffolds_set = true;
    ...
    if (!parse_on_off_flag(v, autonomy_metrics)) {
      std::cerr << "Error: --autonomy-metrics must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    autonomy_metrics_set = true;
    ...
    try {
      ...
      std::cerr << "Error: invalid integer for --novelty-memory-size"
                << std::endl;
      exit(2);
    }
    return true;
    ...
    if (!parse_on_off_flag(v, enable_selfnode)) {
      std::cerr << "Error: --enable-selfnode must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    enable_selfnode_set = true;
    ...
    if (!parse_on_off_flag(v, enable_pfc)) {
      std::cerr << "Error: --enable-pfc must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    enable_pfc_set = true;
    ...
    if (!parse_on_off_flag(v, enable_motor_cortex)) {
      std::cerr << "Error: --enable-motor-cortex must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    enable_motor_cortex_set = true;
...
static bool handle_rwci_parameters(const std::string &arg, bool &rwci_enable,
                                   bool &rwci_enable_set,
                                   bool &rwci_drives_revision,
                                   bool &rwci_drives_revision_set) {
  if (arg == "--rwci") {
    rwci_enable = true;
    rwci_enable_set = true;
    return true;
  }
  if (starts_with(arg, "--rwci=")) {
    auto v = arg.substr(std::string("--rwci=").size());
    if (!parse_on_off_flag(v, rwci_enable)) {
      std::cerr << "Error: --rwci must be on|off|true|false|1|0" << std::endl;
      exit(2);
    }
    rwci_enable_set = true;
    return true;
  }
  if (arg == "--rwci-drives-revision") {
    rwci_drives_revision = true;
    rwci_drives_revision_set = true;
    return true;
  }
  if (starts_with(arg, "--rwci-drives-revision=")) {
    auto v = arg.substr(std::string("--rwci-drives-revision=").size());
    if (!parse_on_off_flag(v, rwci_drives_revision)) {
      std::cerr << "Error: --rwci-drives-revision must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    rwci_drives_revision_set = true;
...
static bool handle_demo_arg(
    const std::string &arg, bool &heatmap_view, int &heatmap_interval_ms,
    int &heatmap_size, float &heatmap_threshold, bool &vision_demo,
    bool &audio_demo, bool &motor_cortex, bool &social_perception,
    bool &social_view, bool &cross_modal, bool &audio_mic, bool &audio_system,
    std::string &audio_file_path, int &camera_index,
    std::string &camera_backend, std::string &vision_source, int &retina_rect_x,
    int &retina_rect_y, int &retina_rect_w, int &retina_rect_h,
    bool &youtube_mode, bool &foveation_enable, int &fovea_w, int &fovea_h,
    std::string &fovea_mode, double &fovea_alpha) {
  // Heatmap viewer
  if (arg == "--heatmap-view") {
    heatmap_view = true;
    return true;
  ...
  if (starts_with(arg, "--heatmap-view=")) {
    ...
    if (!parse_on_off_flag(v, heatmap_view)) {
      std::cerr << "Error: --heatmap-view must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--heatmap-interval=")) {
    ...
    try {
      ...
      if (heatmap_interval_ms < 0) {
        std::cerr << "Error: --heatmap-interval must be non-negative"
                  << std::endl;
        exit(2);
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --heatmap-interval" << std::endl;
      exit(2);
    }
    return true;
  }
  if (starts_with(arg, "--heatmap-size=")) {
    ...
    try {
      ...
      if (heatmap_size <= 0) {
        std::cerr << "Error: --heatmap-size must be positive" << std::endl;
        exit(2);
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --heatmap-size" << std::endl;
      exit(2);
    }
    return true;
  }
  if (starts_with(arg, "--heatmap-threshold=")) {
    ...
    try {
      heatmap_threshold = std::stof(v);
      if (heatmap_threshold < 0.0f)
        heatmap_threshold = 0.0f;
    } catch (...) {
      std::cerr << "Error: invalid float for --heatmap-threshold" << std::endl;
      exit(2);
    }
    return true;
  }
  ...
  if (starts_with(arg, "--vision-demo=")) {
    ...
    if (!parse_on_off_flag(v, vision_demo)) {
      std::cerr << "Error: --vision-demo must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--audio-demo=")) {
    ...
    if (!parse_on_off_flag(v, audio_demo)) {
      std::cerr << "Error: --audio-demo must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--motor-cortex=")) {
    ...
    if (!parse_on_off_flag(v, motor_cortex)) {
      std::cerr << "Error: --motor-cortex must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "--social-perception") {
    social_perception = true;
    return true;
  }
  if (starts_with(arg, "--social-perception=")) {
    auto v = arg.substr(std::string("--social-perception=").size());
    if (!parse_on_off_flag(v, social_perception)) {
      std::cerr << "Error: --social-perception must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--social-view=")) {
    ...
    if (!parse_on_off_flag(v, social_view)) {
      std::cerr << "Error: --social-view must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--cross-modal=")) {
    ...
    if (!parse_on_off_flag(v, cross_modal)) {
      std::cerr << "Error: --cross-modal must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--audio-mic=")) {
    ...
    if (!parse_on_off_flag(v, audio_mic)) {
      std::cerr << "Error: --audio-mic must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--audio-system=")) {
    ...
    if (!parse_on_off_flag(v, audio_system)) {
      std::cerr << "Error: --audio-system must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--audio-file=")) {
    ...
    if (audio_file_path.empty()) {
      std::cerr << "Error: --audio-file requires a file path" << std::endl;
      exit(2);
    }
    return true;
  }
  ...
  if (starts_with(arg, "--camera-index=")) {
    ...
    try {
      camera_index = std::stoi(v);
      if (camera_index < 0) {
        std::cerr << "Error: --camera-index must be non-negative" << std::endl;
        exit(2);
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --camera-index" << std::endl;
      exit(2);
    }
    return true;
  }
  if (starts_with(arg, "--camera-backend=")) {
    ...
    if (camera_backend != "any" && camera_backend != "msmf" &&
        camera_backend != "dshow") {
      std::cerr << "Error: --camera-backend must be one of: any, msmf, dshow"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--vision-source=")) {
    ...
    if (v == "camera" || v == "screen" || v == "maze" || v == "synthetic") {
      vision_source = v;
    } else {
      std::cerr << "Error: --vision-source must be one of: camera, screen, "
                   "maze, synthetic"
                << std::endl;
      exit(2);
    }
  ...
  if (starts_with(arg, "--retina-screen-rect=")) {
    ...
    if (!(ss >> x >> comma) || comma != ',' || !(ss >> y >> comma) ||
        comma != ',' || !(ss >> w >> comma) || comma != ',' || !(ss >> h)) {
      std::cerr << "Error: --retina-screen-rect must be X,Y,W,H" << std::endl;
      exit(2);
    }
    if (w <= 0 || h <= 0) {
      std::cerr << "Error: --retina-screen-rect width/height must be positive"
                << std::endl;
      exit(2);
    }
    retina_rect_x = x;
  ...
  if (arg == "--foveation") {
    foveation_enable = true;
    return true;
  }
  if (starts_with(arg, "--foveation=")) {
    auto v = arg.substr(std::string("--foveation=").size());
    if (!parse_on_off_flag(v, foveation_enable)) {
      std::cerr << "Error: --foveation must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--fovea-size=")) {
    ...
    if (!(ss2 >> w >> xchar) || xchar != 'x' || !(ss2 >> h) || w <= 0 ||
        h <= 0) {
      std::cerr << "Error: --fovea-size must be WxH with positive integers"
                << std::endl;
      exit(2);
    }
    fovea_w = w;
  ...
  if (starts_with(arg, "--fovea-mode=")) {
    ...
    std::transform(
        fovea_mode.begin(), fovea_mode.end(), fovea_mode.begin(),
        [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    if (!(fovea_mode == "cursor" || fovea_mode == "center" ||
          fovea_mode == "attention")) {
      std::cerr << "Error: --fovea-mode must be cursor|center|attention"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--fovea-alpha=")) {
    ...
    try {
      fovea_alpha = std::stod(v);
      fovea_alpha = std::clamp(fovea_alpha, 0.0, 1.0);
    } catch (...) {
      std::cerr << "Error: invalid float for --fovea-alpha" << std::endl;
      exit(2);
    }
    return true;
  }
  ...
  if (starts_with(arg, "--youtube-mode=")) {
    ...
    bool on = false;
    if (!parse_on_off_flag(v, on)) {
      std::cerr << "Error: --youtube-mode must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    youtube_mode = on;
...
static bool handle_dataset_arg(const std::string &arg,
                               std::string &dataset_triplets_root,
                               std::string &dataset_mode, int &dataset_limit,
                               bool &dataset_shuffle, double &reward_scale) {
  if (starts_with(arg, "--dataset-triplets=")) {
    dataset_triplets_root =
        arg.substr(std::string("--dataset-triplets=").size());
    return true;
  }
  if (starts_with(arg, "--dataset-mode=")) {
    dataset_mode = arg.substr(std::string("--dataset-mode=").size());
    std::transform(
        dataset_mode.begin(), dataset_mode.end(), dataset_mode.begin(),
        [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    if (dataset_mode != "triplets") {
      std::cerr << "Error: --dataset-mode must be 'triplets'" << std::endl;
      exit(2);
    }
    return true;
  }
  if (starts_with(arg, "--dataset-limit=")) {
    ...
    try {
      ...
      if (ll < 0) {
        std::cerr << "Error: --dataset-limit must be non-negative" << std::endl;
        exit(2);
      }
      dataset_limit = static_cast<int>(ll);
    } catch (...) {
      std::cerr << "Error: invalid integer for --dataset-limit" << std::endl;
      exit(2);
    }
    return true;
  }
  if (arg == "--dataset-shuffle") {
    dataset_shuffle = true;
    return true;
  }
  if (starts_with(arg, "--dataset-shuffle=")) {
    auto v = arg.substr(std::string("--dataset-shuffle=").size());
    if (!parse_on_off_flag(v, dataset_shuffle)) {
      std::cerr << "Error: --dataset-shuffle must be on|off|true|false|1|0"
                << std::endl;
      exit(2);
    }
    return true;
  ...
  if (starts_with(arg, "--reward-scale=")) {
    ...
    try {
      reward_scale = std::stod(v);
      if (reward_scale < 0.0) {
        std::cerr << "Error: --reward-scale must be >= 0" << std::endl;
        exit(2);
      }
    } catch (...) {
      std::cerr << "Error: invalid float for --reward-scale" << std::endl;
      exit(2);
    }
    return true;
  }
...
static bool
handle_io_arg(const std::string &arg, std::string &snapshot_csv_path,
              std::string &snapshot_live_path, int &snapshot_interval_ms,
              std::string &spikes_live_path, double &spikes_ttl_sec,
              std::string &save_brain_path, std::string &load_brain_path) {
  ...
  if (starts_with(arg, "--snapshot-interval=")) {
    ...
    try {
      ...
      if (snapshot_interval_ms < 0) {
        std::cerr << "Error: --snapshot-interval must be non-negative"
                  << std::endl;
        exit(2);
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --snapshot-interval"
                << std::endl;
      exit(2);
    }
    return true;
...
static std::string json_escape(const std::string &s) {
  ...
  for (unsigned char c : s) {
    ...
    default:
      if (c < 0x20) {
        o << "\\u" << std::hex << std::uppercase << std::setw(4)
...
// Helper: directory where the executable resides (used to place default output
// folders near the binary)
...
static std::vector<std::pair<std::string, std::string>>
    g_log_json_allow; // pair<phase (or ""), event>
...
static inline bool parse_phase_event(const std::string &line,
                                     std::string &phase, std::string &event) {
  phase.clear();
  event.clear();
  ...
  if (ppos != std::string::npos) {
    ...
    if (pend != std::string::npos)
      phase = line.substr(ppos, pend - ppos);
  }
  ...
  if (epos != std::string::npos) {
    ...
    if (eend != std::string::npos)
      event = line.substr(epos, eend - epos);
  }
  return !event.empty();
}

static void emit_json_line(bool enabled, const std::string &path,
  ...
  if (g_log_json_filter_enabled) {
    ...
    if (parse_phase_event(line, ph, ev)) {
      ...
      for (const auto &pe : g_log_json_allow) {
        const std::string &req_ph = pe.first;
        const std::string &req_ev = pe.second;
        if (!req_ph.empty()) {
          if (req_ph == ph && req_ev == ev) {
            allowed = true;
            break;
          }
        } else {
...
std::vector<float> make_synthetic_audio(std::size_t N, int sample_rate,
                                        int step_idx) {
  std::vector<float> x(N, 0.0f);
  float t0 =
      static_cast<float>(step_idx) * (N / static_cast<float>(sample_rate));
...
static bool nf_load_wav_any_mono(const std::string &path,
  ...
  char wave[4];
  f.read(wave, 4);
  if (f.gcount() != 4 || std::strncmp(wave, "WAVE", 4) != 0)
    return false;
  ...
  while (f && !(have_fmt && have_data)) {
    ...
    if (id == "fmt ") {
      ...
      if (chunk_size > 0)
        f.read(data_bytes.data(), static_cast<std::streamsize>(chunk_size));
    } else {
  ...
  if (audio_format == 1) {
    if (bits_per_sample == 16) {
      std::size_t frame_count = static_cast<std::size_t>(data_bytes.size()) /
                                (num_channels * sizeof(std::int16_t));
      out_samples.resize(frame_count);
      const std::int16_t *p =
          reinterpret_cast<const std::int16_t *>(data_bytes.data());
      for (std::size_t i = 0; i < frame_count; ++i) {
        long acc = 0;
        for (std::size_t ch = 0; ch < num_channels; ++ch)
          acc += p[i * num_channels + ch];
        float v = static_cast<float>(acc) /
                  (32768.0f * static_cast<float>(num_channels));
        out_samples[i] = v;
      }
      return true;
    } else {
    ...
    if (bits_per_sample == 32) {
      ...
      for (std::size_t i = 0; i < frame_count; ++i) {
        ...
        for (std::size_t ch = 0; ch < num_channels; ++ch)
          acc += p[i * num_channels + ch];
        out_samples[i] =
            static_cast<float>(acc / static_cast<double>(num_channels));
      }
      return true;
    } else {
      return false;
...
static std::vector<float> nf_resample_linear(const std::vector<float> &in,
                                             int sr_in, int sr_out) {
  if (in.empty() || sr_in <= 0 || sr_out <= 0 || sr_in == sr_out)
    return in;
...
public:
  explicit MazeEnv(int n = 8, float wall_density = 0.20f, int max_steps = -1)
      : N(std::max(2, n)), wall_density_(wall_density),
        max_episode_steps_(max_steps < 0 ? 4 * n * n : max_steps) {
  ...
  void initializeFirstPersonRenderer() {
    ...
    fp_renderer_ =
        std::make_unique<NeuroForge::Core::FirstPersonMazeRenderer>(config);
  ...
  const NeuroForge::Core::FirstPersonMazeRenderer::AgentState &
  getAgentState() const {
    return agent_state_;
  }

  // actions: 0=up,1=down,2=left,3=right (grid mode) or
  // 0=forward,1=backward,2=turn_left,3=turn_right (first-person mode)
  float step(int action, bool &done) {
    ...
    if (fp_renderer_) {
      ...
      bool movement_success =
          fp_renderer_->updateAgentPosition(agent_state_, action, walls_, N);
      ...
      if (!movement_success && (action == 0 || action == 1)) {
        ...
        if (shaping_mode_ != ShapingMode::Off) {
          ...
          if (shaping_mode_ == ShapingMode::Euclid) {
            old_d = std::sqrt((agent_state_.x - (gx + 0.5f)) *
                                  (agent_state_.x - (gx + 0.5f)) +
                              (agent_state_.y - (gy + 0.5f)) *
                                  (agent_state_.y - (gy + 0.5f)));
            // For shaping, we need the previous distance, so we approximate
            new_d = old_d; // Simplified for now
          } else {         // Manhattan
            old_d = std::abs(agent_state_.x - (gx + 0.5f)) +
                    std::abs(agent_state_.y - (gy + 0.5f));
            new_d = old_d; // Simplified for now
          }
        }
      }
      ...
      float goal_distance = std::sqrt(
          (agent_state_.x - (gx + 0.5f)) * (agent_state_.x - (gx + 0.5f)) +
      ...
      if (walls_[static_cast<std::size_t>(ny * N + nx)]) {
        ...
        if (shaping_mode_ != ShapingMode::Off) {
          if (shaping_mode_ == ShapingMode::Euclid) {
            old_d = std::sqrt(static_cast<float>((ax - gx) * (ax - gx) +
                                                 (ay - gy) * (ay - gy)));
          } else { // Manhattan
        ...
        if (shaping_mode_ != ShapingMode::Off) {
          float new_d;
          if (shaping_mode_ == ShapingMode::Euclid) {
            new_d = std::sqrt(static_cast<float>((ax - gx) * (ax - gx) +
                                                 (ay - gy) * (ay - gy)));
          } else { // Manhattan
...
// First-person rendering support
  ...
  void ensurePathExists() {
    ...
    if (!can_reach_goal) {
      ...
      for (int y = 0; y < N; ++y) {
        walls_[static_cast<std::size_t>(y * N + (N - 1))] =
            false; // Right column
      }
    }
  }
...
// Returns true if handled; sets 'err' to non-zero on parse error and prints
// message
static bool handle_context_flag(const std::string &arg, double &context_gain,
                                int &context_update_ms, int &context_window,
  ...
  if (arg.rfind("--context-gain=", 0) == 0) {
    ...
    try {
      context_gain = std::stod(v);
    } catch (...) {
      std::cerr << "Error: invalid float for --context-gain" << std::endl;
      err = 2;
    }
    return true;
  }
  ...
  if (arg.rfind("--context-update-ms=", 0) == 0) {
    ...
    try {
      context_update_ms = std::stoi(v);
      if (context_update_ms <= 0) {
        std::cerr << "Error: --context-update-ms must be > 0" << std::endl;
        err = 2;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --context-update-ms"
                << std::endl;
      err = 2;
    }
    return true;
  ...
  if (arg.rfind("--context-update=", 0) == 0) {
    ...
    try {
      context_update_ms = std::stoi(v);
      if (context_update_ms <= 0) {
        std::cerr << "Error: --context-update must be > 0" << std::endl;
        err = 2;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --context-update" << std::endl;
      err = 2;
    }
    return true;
  }
  ...
  if (arg.rfind("--context-window=", 0) == 0) {
    ...
    try {
      context_window = std::stoi(v);
      if (context_window < 1) {
        std::cerr << "Error: --context-window must be >= 1" << std::endl;
        err = 2;
      }
    } catch (...) {
      std::cerr << "Error: invalid integer for --context-window" << std::endl;
      err = 2;
    }
    return true;
  }
  ...
  if (arg.rfind("--context-peer=", 0) == 0) {
    ...
    if (v.empty()) {
      std::cerr << "Error: --context-peer requires "
                   "name[,gain][,update_ms][,window][,label]"
                << std::endl;
      err = 2;
    } else {
      context_peer_args.push_back(v);
    }
    return true;
  }
  ...
  if (arg.rfind("--context-couple=", 0) == 0) {
    ...
    if (v.empty()) {
      std::cerr << "Error: --context-couple requires src:dst[,weight]"
                << std::endl;
      err = 2;
    } else {
      context_coupling_args.push_back(v);
    }
    return true;
  }
  return false;
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    int retina_rect_x = 0, retina_rect_y = 0, retina_rect_w = 1280,
        retina_rect_h = 720;
    ...
    int last_fovea_x = -1, last_fovea_y = -1, last_fovea_w = -1,
        last_fovea_h = -1;
    // Sandbox configuration (restricted interaction area and optional embedded
    // browser)
    ...
    float hazard_density = -1.0f; // if >0 in [0,1], overrides audio-derived
                                  // hazard; 0 = audio fallback
    float phase_c_hazard_weight =
        0.2f; // coherence down-modulation weight in [0,1]
    ...
    float epsilon =
        -1.0f; // <0 disables epsilon-greedy (kept for Q-learning baseline mode)
    float softmax_temp = 0.5f; // Default to biological stochastic policy (0.5 =
                               // moderate exploration)
    bool maze_view = false;    // OpenCV live maze view
    int maze_view_interval_ms = 300; // refresh interval for maze view
    int maze_max_episode_steps =
        -1; // if <0, defaults to 4*N*N once maze_size is known
    ...
    float hybrid_lambda = -1.0f; // <0 disables blending; in [0,1] blends neural
                                 // (lambda) with Q (1-lambda)
    // Maze shaping flags
    ...
    // Phase-4 reward shaping weights (novelty alpha, task gamma, uncertainty
    // eta)
    ...
    bool memdb_interval_cli_set =
        false; // tracks if CLI explicitly set interval
    ...
    std::deque<std::pair<NeuroForge::NeuronID, NeuroForge::TimePoint>>
        spike_events;
    std::mutex spikes_mutex;
    ...
    // Mirror mode: derive student embedding from sensory features instead of
    // action scores
    ...
    std::string current_teacher_id =
        "teacher_embed"; // active teacher embedding id for Phase A
    struct TripletItem {
      std::string stem;
    ...
    bool telemetry_extended =
        false; // include extended metadata in experience snapshots
    ...
    auto scan_triplets_dataset = [&](const std::string &root, int limit,
                                     bool shuffle) -> std::vector<TripletItem> {
      ...
      try {
        ...
        if (structured) {
          ...
          scan_dir(image_dir, image_by_stem,
                   {".jpg", ".jpeg", ".png", ".bmp", ".gif"});
        } else {
          for (auto it = fs::recursive_directory_iterator(
                   rootp, fs::directory_options::skip_permission_denied);
               it != fs::recursive_directory_iterator(); ++it) {
            if (!it->is_regular_file())
              continue;
            const auto &p = it->path();
        ...
        auto assemble = [&]() {
          ...
          for (const auto &kv : text_by_stem) {
            ...
            if (img_it != image_by_stem.end())
              it.image_path = img_it->second;
            if (!it.text.empty())
    ...
    std::unique_ptr<NeuroForge::Core::Phase10SelfExplanation>
        phase10_selfexplainer;
    ...
    double phase14_trust_degraded =
        0.40;                            // trust below this considered degraded
    ...
    double phase15_risk_threshold =
        0.60; // risk above this triggers review/deny
    ...
    std::vector<std::string>
        context_peer_args; // --context-peer=name[,gain][,update_ms][,window][,label]
    std::vector<std::string>
        context_coupling_args; // --context-couple=src:dst[,weight]
  ...
  bool ablate_set = false;
    ...
    float phase_c_variance_sensitivity =
        1.0f;                            // SurvivalBias variance sensitivity
    ...
    // Unified substrate mode (Phase C + WM + SurvivalBias + Language
    // integration)
    ...
    bool survival_bias_enable =
        true; // unified SurvivalBias effector on by default
    // Emergent-only mode (true emergence)
    bool emergent_only = false;
    bool emergent_only_set = false;

    bool rwci_enable = false;
    bool rwci_enable_set = false;
    bool rwci_drives_revision = false;
    bool rwci_drives_revision_set = false;
    ...
    // Phase N1: Active Concept Injection Harness Flag
    bool test_semantic_injection = false;

    // Phase N2: Competing Concept Dynamics Flag
    bool test_semantic_n2 = false;

    // Phase N3: Prediction-Guided Semantic Feedback Flag
    bool test_semantic_n3 = false;

    // Phase N4: Prediction-Constrained Learning Flag
    bool test_semantic_n4 = false;

    // Phase N5: Prediction-Guided Attention Allocation Flag
    bool test_semantic_n5 = false;

    // Phase N6: Self-Regulating Cognition Flag
    bool test_semantic_n6 = false;

    bool test_n7_minimal = false;
    bool test_n7_bounded = false;
    bool test_n7_modelbased = false;
    bool unified_bounded = false;

    int unified_bounded_steps = 5000;
    int unified_bounded_warmup_steps = 50;
    int unified_bounded_episode_steps = 128;
    std::uint32_t unified_bounded_seed = 7;
    std::uint32_t unified_bounded_collision_budget = 50;
    std::string unified_bounded_log_csv_path = "unified_bounded_log.csv";
    std::string unified_bounded_summary_csv_path =
        "unified_bounded_summary.csv";

    // N8 Human Gated runner config
    NeuroForge::Runtime::N8HumanGatedConfig n8_cfg;
    bool run_n8 = false;

    // N9 Automated Gated runner config
    NeuroForge::Runtime::N9AutomatedGatedConfig n9_cfg;
    bool run_n9 = false;

    // Unified Alive (Option A) config
    NeuroForge::Runtime::UnifiedAliveConfig alive_cfg;
    bool run_alive = false;

    // Phase N-Web Config
    NeuroForge::Runtime::WebSandboxConfig web_cfg;
    bool run_web = false;

    // Phase N-Genesis: Grand Unified Runner
    NeuroForge::Runtime::GrandUnifiedConfig genesis_cfg;
    bool run_genesis = false;

    bool unified_bounded_live = false;
    std::string unified_bounded_live_mode = "match";
    int unified_bounded_live_match_window = 500;
    double unified_bounded_live_min_match = 0.80;
    double unified_bounded_live_min_advantage = 17.0;
    double unified_bounded_live_min_pred_dist_advantage = 0.0;
    double unified_bounded_live_max_collision_prob = 0.05;
    double unified_bounded_live_min_dist_improve = 0.05;
    ...
    std::string log_json_events_csv; // comma separated list of event names or
                                     // Phase:Event
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (arg == "--help") {
        print_usage();
        return 0;
      } else if (arg == "--test-semantic-injection") {
        test_semantic_injection = true;
        continue;
      } else if (arg == "--test-semantic-n2") {
        test_semantic_n2 = true;
        continue;
      } else if (arg == "--test-semantic-n3") {
        test_semantic_n3 = true;
        continue;
      } else if (arg == "--test-semantic-n4") {
        test_semantic_n4 = true;
        continue;
      } else if (arg == "--test-semantic-n5") {
        test_semantic_n5 = true;
        continue;
      } else if (arg == "--test-semantic-n6") {
        test_semantic_n6 = true;
        continue;
      } else if (arg == "--test-n7-minimal") {
        test_n7_minimal = true;
        continue;
      } else if (arg == "--test-n7-bounded") {
        test_n7_bounded = true;
        continue;
      } else if (arg == "--test-n7-modelbased") {
        test_n7_modelbased = true;
        continue;
      } else if (arg == "--unified-bounded") {
        unified_bounded = true;
        continue;
      }
      ...
      if (starts_with(arg, "--log-json-sample=")) {
        ...
        try {
          ...
          std::cerr << "Error: invalid integer for --log-json-sample"
                    << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--log-json-events=")) {
        log_json_events_csv =
            arg.substr(std::string("--log-json-events=").size());
        continue;
      }
    }
    set_log_json_filters(log_json_sample_val, log_json_events_csv);

    // Pre-scan for Context flags to keep the main parse shallow and avoid deep
    // nesting
    for (int i_ctx = 1; i_ctx < argc; ++i_ctx) {
      std::string arg_ctx = argv[i_ctx];
      int ctx_err = 0;
      if (handle_context_flag(arg_ctx, context_gain, context_update_ms,
                              context_window, context_peer_args,
                              context_coupling_args, ctx_err)) {
        if (ctx_err) {
          return ctx_err;
        }
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      // Swallow JSON filter flags here (handled in pre-scan) to avoid growing
      // the else-if chain
      if (starts_with(arg, "--log-json-sample=") ||
          starts_with(arg, "--log-json-events=")) {
        continue;
      }

      // Swallow Test flags (handled in pre-scan)
      if (arg == "--test-semantic-injection" || arg == "--test-semantic-n2" ||
          arg == "--test-semantic-n3" || arg == "--test-semantic-n4" ||
          arg == "--test-semantic-n5" || arg == "--test-semantic-n6" ||
          arg == "--test-n7-minimal" || arg == "--test-n7-bounded" ||
          arg == "--test-n7-modelbased" || arg == "--unified-bounded") {
        continue;
      }
      // Swallow Context flags here (handled in pre-scan) to avoid growing the
      // else-if chain
      if (starts_with(arg, "--context-gain=") ||
          starts_with(arg, "--context-update-ms=") ||
          starts_with(arg, "--context-update=") ||
          starts_with(arg, "--context-window=") ||
          starts_with(arg, "--context-peer=") ||
          starts_with(arg, "--context-couple=")) {
        continue;
      }

      if (arg == "--unified-bounded-live") {
        unified_bounded = true;
        unified_bounded_live = true;
        continue;
      } else if (starts_with(arg, "--unified-bounded-live=")) {
        unified_bounded = true;
        auto v = arg.substr(std::string("--unified-bounded-live=").size());
        if (!parse_on_off_flag(v, unified_bounded_live)) {
          std::cerr
              << "Error: --unified-bounded-live must be on|off|true|false|1|0"
              << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-live-mode=")) {
        unified_bounded = true;
        unified_bounded_live_mode =
            arg.substr(std::string("--unified-bounded-live-mode=").size());
        if (!(unified_bounded_live_mode == "match" ||
              unified_bounded_live_mode == "advantage")) {
          std::cerr
              << "Error: --unified-bounded-live-mode must be match|advantage"
              << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-live-match-window=")) {
        unified_bounded = true;
        auto v = arg.substr(
            std::string("--unified-bounded-live-match-window=").size());
        try {
          unified_bounded_live_match_window = std::stoi(v);
          if (unified_bounded_live_match_window < 1) {
            std::cerr
                << "Error: --unified-bounded-live-match-window must be >= 1"
                << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for "
                       "--unified-bounded-live-match-window"
                    << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-live-min-match=")) {
        unified_bounded = true;
        auto v =
            arg.substr(std::string("--unified-bounded-live-min-match=").size());
        try {
          unified_bounded_live_min_match = std::stod(v);
          if (unified_bounded_live_min_match < 0.0 ||
              unified_bounded_live_min_match > 1.0) {
            std::cerr
                << "Error: --unified-bounded-live-min-match must be in [0,1]"
                << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr
              << "Error: invalid float for --unified-bounded-live-min-match"
              << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-live-min-advantage=")) {
        unified_bounded = true;
        auto v = arg.substr(
            std::string("--unified-bounded-live-min-advantage=").size());
        try {
          unified_bounded_live_min_advantage = std::stod(v);
          if (unified_bounded_live_min_advantage < 0.0) {
            std::cerr
                << "Error: --unified-bounded-live-min-advantage must be >= 0"
                << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr
              << "Error: invalid float for --unified-bounded-live-min-advantage"
              << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(
                     arg, "--unified-bounded-live-min-pred-dist-advantage=")) {
        unified_bounded = true;
        auto v = arg.substr(
            std::string("--unified-bounded-live-min-pred-dist-advantage=")
                .size());
        try {
          unified_bounded_live_min_pred_dist_advantage = std::stod(v);
          if (unified_bounded_live_min_pred_dist_advantage < 0.0) {
            std::cerr
                << "Error: --unified-bounded-live-min-pred-dist-advantage "
                   "must be >= 0"
                << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for "
                       "--unified-bounded-live-min-pred-dist-advantage"
                    << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg,
                             "--unified-bounded-live-max-collision-prob=")) {
        unified_bounded = true;
        auto v = arg.substr(
            std::string("--unified-bounded-live-max-collision-prob=").size());
        try {
          unified_bounded_live_max_collision_prob = std::stod(v);
          if (unified_bounded_live_max_collision_prob < 0.0 ||
              unified_bounded_live_max_collision_prob > 1.0) {
            std::cerr << "Error: --unified-bounded-live-max-collision-prob "
                         "must be in [0,1]"
                      << std::endl;
            return 2;
          }
        ...
        continue;
      } else if (starts_with(arg, "--unified-bounded-live-min-dist-improve=")) {
        unified_bounded = true;
        auto v = arg.substr(
            std::string("--unified-bounded-live-min-dist-improve=").size());
        try {
          unified_bounded_live_min_dist_improve = std::stod(v);
          if (unified_bounded_live_min_dist_improve < 0.0) {
            std::cerr
                << "Error: --unified-bounded-live-min-dist-improve must be >= 0"
                << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for "
                       "--unified-bounded-live-min-dist-improve"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--unified-bounded-steps=")) {
        unified_bounded = true;
        auto v = arg.substr(std::string("--unified-bounded-steps=").size());
        try {
          unified_bounded_steps = std::stoi(v);
          if (unified_bounded_steps < 1) {
            std::cerr << "Error: --unified-bounded-steps must be >= 1"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --unified-bounded-steps"
                    << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-warmup-steps=")) {
        unified_bounded = true;
        auto v =
            arg.substr(std::string("--unified-bounded-warmup-steps=").size());
        try {
          unified_bounded_warmup_steps = std::stoi(v);
          if (unified_bounded_warmup_steps < 0) {
            std::cerr << "Error: --unified-bounded-warmup-steps must be >= 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr
              << "Error: invalid integer for --unified-bounded-warmup-steps"
              << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-episode-steps=")) {
        unified_bounded = true;
        auto v =
            arg.substr(std::string("--unified-bounded-episode-steps=").size());
        try {
          unified_bounded_episode_steps = std::stoi(v);
          if (unified_bounded_episode_steps < 1) {
            std::cerr << "Error: --unified-bounded-episode-steps must be >= 1"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr
              << "Error: invalid integer for --unified-bounded-episode-steps"
              << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-seed=")) {
        unified_bounded = true;
        auto v = arg.substr(std::string("--unified-bounded-seed=").size());
        try {
          long long s = std::stoll(v);
          if (s < 0 || s > 0xFFFFFFFFLL) {
            std::cerr << "Error: --unified-bounded-seed out of range"
                      << std::endl;
            return 2;
          }
          unified_bounded_seed = static_cast<std::uint32_t>(s);
        } catch (...) {
          std::cerr << "Error: invalid integer for --unified-bounded-seed"
                    << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-collision-budget=")) {
        unified_bounded = true;
        auto v = arg.substr(
            std::string("--unified-bounded-collision-budget=").size());
        try {
          long long b = std::stoll(v);
          if (b < 0 || b > 0xFFFFFFFFLL) {
            std::cerr
                << "Error: --unified-bounded-collision-budget out of range"
                << std::endl;
            return 2;
          }
          unified_bounded_collision_budget = static_cast<std::uint32_t>(b);
        } catch (...) {
          std::cerr
              << "Error: invalid integer for --unified-bounded-collision-budget"
              << std::endl;
          return 2;
        }
        continue;
      } else if (starts_with(arg, "--unified-bounded-log=")) {
        unified_bounded = true;
        unified_bounded_log_csv_path =
            arg.substr(std::string("--unified-bounded-log=").size());
        continue;
      } else if (starts_with(arg, "--unified-bounded-summary=")) {
        unified_bounded = true;
        unified_bounded_summary_csv_path =
            arg.substr(std::string("--unified-bounded-summary=").size());
        continue;
      }

      if (arg == "--n8-human-gated") {
        run_n8 = true;
        continue;
      }

      if (arg == "--n9-auto-gated") {
        run_n9 = true;
        continue;
      }

      if (arg == "--alive-bounded") {
        run_alive = true;
        alive_cfg.infinite_mode = true; // Default to infinite
        continue;
      }
      ...
      if (starts_with(arg, "--genesis-url=")) {
        genesis_cfg.start_url =
            arg.substr(std::string("--genesis-url=").size());
        continue;
      }
      if (arg == "--genesis-vision") {
        genesis_cfg.enable_vision = true;
        continue;
      }
      if (arg == "--camera") {
        genesis_cfg.enable_camera = true;
        continue;
      }
      if (arg == "--no-webview") {
        genesis_cfg.enable_web = false;
        continue;
      }
      if (starts_with(arg, "--genesis-steps=")) {
        genesis_cfg.max_steps =
            std::stoi(arg.substr(std::string("--genesis-steps=").size()));
        genesis_cfg.infinite_mode = false;
        continue;
      }
      if (arg == "--enable-parallel") {
        genesis_cfg.enable_parallel = true;
        continue;
      }
      if (starts_with(arg, "--perception-hz=")) {
        genesis_cfg.perception_hz =
            std::stoi(arg.substr(std::string("--perception-hz=").size()));
        continue;
      }
      if (starts_with(arg, "--grid-hz=")) {
        genesis_cfg.grid_hz =
            std::stoi(arg.substr(std::string("--grid-hz=").size()));
        continue;
      }
      if (starts_with(arg, "--web-hz=")) {
        genesis_cfg.web_hz =
            std::stoi(arg.substr(std::string("--web-hz=").size()));
        continue;
      }
      if (starts_with(arg, "--cognition-hz=")) {
        genesis_cfg.cognition_hz =
            std::stoi(arg.substr(std::string("--cognition-hz=").size()));
        continue;
      }
      if (starts_with(arg, "--brain-state-dir=")) {
        genesis_cfg.brain_state_dir =
            arg.substr(std::string("--brain-state-dir=").size());
        continue;
      }
      if (arg == "--no-save") {
        genesis_cfg.auto_save = false;
        continue;
      }
      if (arg == "--no-load") {
        genesis_cfg.auto_load = false;
        continue;
      }
      if (arg == "--chat") {
        genesis_cfg.conversational_mode = true;
        continue;
      }
      ...
      if (arg == "--gpu") {
        ...
        if (!parse_on_off_flag(v, prefer_gpu)) {
          std::cerr << "Error: --gpu must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--phase8=")) {
        ...
        if (!parse_on_off_flag(v, phase8_enable)) {
          std::cerr << "Error: --phase8 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--phase-a-replay-interval=")) {
        ...
        try {
          phase_a_replay_interval_steps = std::stoi(v);
          if (phase_a_replay_interval_steps < 1) {
            std::cerr << "Error: --phase-a-replay-interval must be >= 1"
                      << std::endl;
            return 2;
          }
          phase_a_replay_interval_set = true;
        } catch (...) {
          std::cerr << "Error: invalid int for --phase-a-replay-interval"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--phase-a-replay-top-k=")) {
        ...
        try {
          phase_a_replay_top_k = std::stoi(v);
          if (phase_a_replay_top_k < 1) {
            std::cerr << "Error: --phase-a-replay-top-k must be >= 1"
                      << std::endl;
            return 2;
          }
          phase_a_replay_top_k_set = true;
        } catch (...) {
          std::cerr << "Error: invalid int for --phase-a-replay-top-k"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--phase-a-replay-boost=")) {
        ...
        try {
          phase_a_replay_boost = std::stod(v);
          if (phase_a_replay_boost < 0.0) {
            std::cerr << "Error: --phase-a-replay-boost must be >= 0"
                      << std::endl;
            return 2;
          }
          phase_a_replay_boost_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-a-replay-boost"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--phase-a-replay-lr-scale=")) {
        ...
        try {
          phase_a_replay_lr_scale = std::stod(v);
          if (phase_a_replay_lr_scale < 0.0) {
            std::cerr << "Error: --phase-a-replay-lr-scale must be >= 0"
                      << std::endl;
            return 2;
          }
          phase_a_replay_lr_scale_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-a-replay-lr-scale"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--phase-a-replay-include-hard-negatives=")) {
        auto v = arg.substr(
            std::string("--phase-a-replay-include-hard-negatives=").size());
        if (!parse_on_off_flag(v, phase_a_replay_include_hard)) {
          std::cerr << "Error: --phase-a-replay-include-hard-negatives must be "
      ...
      if (starts_with(arg, "--phase-a-replay-hard-k=")) {
        ...
        try {
          phase_a_replay_hard_k = std::stoi(v);
          if (phase_a_replay_hard_k < 1) {
            std::cerr << "Error: --phase-a-replay-hard-k must be >= 1"
                      << std::endl;
            return 2;
          }
          phase_a_replay_hard_k_set = true;
        } catch (...) {
          std::cerr << "Error: invalid int for --phase-a-replay-hard-k"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--phase-a-replay-repulsion-weight=")) {
        auto v = arg.substr(
            std::string("--phase-a-replay-repulsion-weight=").size());
        try {
          phase_a_replay_repulsion_weight = std::stod(v);
          if (phase_a_replay_repulsion_weight < 0.0) {
            std::cerr << "Error: --phase-a-replay-repulsion-weight must be >= 0"
                      << std::endl;
            return 2;
          }
          phase_a_replay_repulsion_weight_set = true;
        } catch (...) {
          std::cerr
              << "Error: invalid float for --phase-a-replay-repulsion-weight"
              << std::endl;
          return 2;
        }
        continue;
      ...
      if (handle_phase4_arg(arg, argc, argv, i, alpha_weight, alpha_set,
                            gamma_weight, gamma_set, eta_weight, eta_set,
                            lambda_param, lambda_set, etaElig_param,
                            etaElig_set, kappa_param, kappa_set,
                            phase4_unsafe)) {
        continue;
      }

      // Handle --add-region early to avoid increasing nesting depth of the main
      // else-if chain
        ...
        if (p1 == std::string::npos) {
          ...
          if (p2 == std::string::npos) {
            ...
            try {
              ...
              if (ll < 0) {
                std::cerr << "Error: --add-region COUNT must be non-negative"
                          << std::endl;
                return 2;
              }
              count = static_cast<std::size_t>(ll);
            } catch (...) {
              std::cerr << "Error: invalid COUNT for --add-region" << std::endl;
              return 2;
            }
          }
        }
        if (key.empty()) {
          std::cerr << "Error: --add-region requires KEY" << std::endl;
          return 2;
        }
        if (name.empty())
          name = key;
        ...
        if (g_memdb && g_memdb_run_id > 0) {
          std::int64_t ts_ms =
              std::chrono::duration_cast<std::chrono::milliseconds>(
                  ...
                  .count();
          std::int64_t event_id = 0;
          (void)g_memdb->insertRunEvent(
              g_memdb_run_id, ts_ms, g_last_step.load(), std::string("exit"),
              std::string("normal"), 0, nf_process_rss_mb(), 0.0, event_id);
        }
        return 0;
      }
      ...
      if (arg == "--memdb-color") {
        // Bare flag = auto (keep enabled; HypergraphBrain will colorize only on
        // TTY)
        memdb_color = true;
        continue;
        ...
        std::string vlow = v;
        std::transform(
            vlow.begin(), vlow.end(), vlow.begin(),
            [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        if (vlow == "auto") {
        ...
        if (!parse_on_off_flag(v, memdb_color)) {
          std::cerr << "Error: --memdb-color must be auto|on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      // Handle --mimicry-internal early to avoid extending the large else-if
      // chain depth
      if (arg == "--mimicry-internal") {
        ...
        if (!parse_on_off_flag(v, mimicry_internal)) {
          std::cerr << "Error: --mimicry-internal must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (arg == "--unified-substrate") {
        unified_substrate_enable = true;
        continue;
      } else if (starts_with(arg, "--unified-substrate=")) {
        auto v = arg.substr(std::string("--unified-substrate=").size());
        if (!parse_on_off_flag(v, unified_substrate_enable)) {
          std::cerr
              << "Error: --unified-substrate must be on|off|true|false|1|0"
              << std::endl;
          return 2;
        }
        continue;
      ...
      if (arg == "--adaptive") {
        ...
        if (!parse_on_off_flag(v, adaptive_enable)) {
          std::cerr << "Error: --adaptive must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (arg == "--survival-bias") {
        survival_bias_enable = true;
        continue;
      } else if (starts_with(arg, "--survival-bias=")) {
        auto v = arg.substr(std::string("--survival-bias=").size());
        if (!parse_on_off_flag(v, survival_bias_enable)) {
          std::cerr << "Error: --survival-bias must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          long long ll = std::stoll(v);
          if (ll <= 0) {
            std::cerr << "Error: --wm-neurons must be positive" << std::endl;
            return 2;
          }
          unified_wm_neurons = static_cast<std::size_t>(ll);
        } catch (...) {
          std::cerr << "Error: invalid integer for --wm-neurons" << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          long long ll = std::stoll(v);
          if (ll <= 0) {
            std::cerr << "Error: --phasec-neurons must be positive"
                      << std::endl;
            return 2;
          }
          unified_phasec_neurons = static_cast<std::size_t>(ll);
        } catch (...) {
          std::cerr << "Error: invalid integer for --phasec-neurons"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (arg == "--viewer") {
        ...
        if (!parse_on_off_flag(v, viewer_enabled)) {
          std::cerr << "Error: --viewer must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        std::string vlow = viewer_layout;
        std::transform(
            vlow.begin(), vlow.end(), vlow.begin(),
            [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        if (!(vlow == "shells" || vlow == "layers")) {
          std::cerr << "Error: --viewer-layout must be shells|layers"
                    << std::endl;
          return 2;
        }
        viewer_layout = vlow;
        ...
        try {
          viewer_refresh_ms = std::stoi(v);
          if (viewer_refresh_ms < 0)
            viewer_refresh_ms = 0;
        } catch (...) {
          std::cerr << "Error: invalid integer for --viewer-refresh-ms"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          viewer_threshold = std::stof(v);
        } catch (...) {
          std::cerr << "Error: invalid float for --viewer-threshold"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      // Sandbox flags: enable a dedicated window and optional embedded WebView2
      // browser
      if (arg == "--sandbox") {
        sandbox_enable = true;
        continue;
        ...
        if (!parse_on_off_flag(v, sandbox_enable)) {
          std::cerr << "Error: --sandbox must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        if (sandbox_url.empty())
          sandbox_url = "https://www.youtube.com";
        continue;
        ...
        char xchar;
        std::stringstream ss(v);
        if (!(ss >> w >> xchar) || xchar != 'x' || !(ss >> h) || w <= 0 ||
            h <= 0) {
          std::cerr
              << "Error: --sandbox-size must be WxH with positive integers"
              << std::endl;
          return 2;
        }
        sandbox_w = w;
        ...
        sandbox_actions_enable = false;
        continue;
      } else if (starts_with(arg, "--no-web-actions=")) {
        auto v = arg.substr(std::string("--no-web-actions=").size());
        bool no_web_actions = false;
        if (!parse_on_off_flag(v, no_web_actions)) {
          std::cerr << "Error: --no-web-actions must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (ll < 0) {
            std::cerr
                << "Error: --simulate-blocked-actions must be non-negative"
                << std::endl;
            return 2;
          }
          simulate_blocked_actions = static_cast<int>(ll);
        } catch (...) {
          std::cerr << "Error: invalid integer for --simulate-blocked-actions"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          long long ll = std::stoll(v);
          if (ll < 0) {
            std::cerr << "Error: --simulate-rewards must be non-negative"
                      << std::endl;
            return 2;
          }
          simulate_rewards = static_cast<int>(ll);
        } catch (...) {
          std::cerr << "Error: invalid integer for --simulate-rewards"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (starts_with(arg, "--dataset-triplets=")) {
        ...
        try {
          long long ll = std::stoll(v);
          if (ll < 0) {
            std::cerr << "Error: --dataset-limit must be non-negative"
                      << std::endl;
            return 2;
          }
          dataset_limit = static_cast<int>(ll);
        } catch (...) {
          std::cerr << "Error: invalid integer for --dataset-limit"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (arg == "--phase-c") {
        ...
        if (!parse_on_off_flag(v, phase_c)) {
          std::cerr << "Error: --phase-c must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        std::string vlow = phase_c_mode;
        std::transform(
            vlow.begin(), vlow.end(), vlow.begin(),
            [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        if (!(vlow == "binding" || vlow == "sequence")) {
          std::cerr << "Error: --phase-c-mode must be binding|sequence"
                    << std::endl;
          return 2;
        }
        phase_c_mode = vlow;
        ...
        if (phase_c_out.empty()) {
          std::cerr << "Error: --phase-c-out requires a directory path"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          unsigned long long s = std::stoull(v);
          if (s > std::numeric_limits<unsigned int>::max()) {
            std::cerr << "Error: --phase-c-seed out of range" << std::endl;
            return 2;
          }
          phase_c_seed = static_cast<unsigned int>(s);
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase-c-seed" << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          long long ll = std::stoll(v);
          if (ll <= 0) {
            std::cerr << "Error: --phase-c-wm-capacity must be positive"
                      << std::endl;
            return 2;
          }
          phase_c_wm_capacity = static_cast<std::size_t>(ll);
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase-c-wm-capacity"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d > 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --phase-c-wm-decay must be in (0,1]"
                      << std::endl;
            return 2;
          }
          phase_c_wm_decay = d;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-c-wm-decay"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          long long ll = std::stoll(v);
          if (ll < 0) {
            std::cerr << "Error: --phase-c-seq-window must be non-negative"
                      << std::endl;
            return 2;
          }
          phase_c_seq_window = static_cast<std::size_t>(ll);
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase-c-seq-window"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        if (!parse_on_off_flag(v, phase_c_survival_bias)) {
          std::cerr
              << "Error: --phase-c-survival-bias must be on|off|true|false|1|0"
              << std::endl;
          return 2;
        }
        continue;
        ...
        auto v =
            arg.substr(std::string("--phase-c-variance-sensitivity=").size());
        try {
          float d = std::stof(v);
          if (d <= 0.0f) {
            std::cerr
                << "Error: --phase-c-variance-sensitivity must be positive"
                << std::endl;
            return 2;
          }
          phase_c_variance_sensitivity = d;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-c-variance-sensitivity"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (d < 0.0f) {
            std::cerr << "Error: --phase-c-survival-scale must be >= 0"
                      << std::endl;
            return 2;
          }
          ...
          std::cerr << "Error: invalid float for --phase-c-survival-scale"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --phase-c-hazard-weight must be in [0,1]"
                      << std::endl;
            return 2;
          }
          ...
          std::cerr << "Error: invalid float for --phase-c-hazard-weight"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --binding-threshold must be in [0,1]"
                      << std::endl;
            return 2;
          }
          ...
          std::cerr << "Error: invalid float for --binding-threshold"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --sequence-threshold must be in [0,1]"
                      << std::endl;
            return 2;
          }
          ...
          std::cerr << "Error: invalid float for --sequence-threshold"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --binding-coherence-min must be in [0,1]"
                      << std::endl;
            return 2;
          }
          ...
          std::cerr << "Error: invalid float for --binding-coherence-min"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --sequence-coherence-min must be in [0,1]"
                      << std::endl;
            return 2;
          }
          ...
          std::cerr << "Error: invalid float for --sequence-coherence-min"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --prune-coherence-threshold must be in [0,1]"
                      << std::endl;
            return 2;
          }
          ...
          std::cerr << "Error: invalid float for --prune-coherence-threshold"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --phase-c-hazard-alpha must be in [0,1]"
                      << std::endl;
            return 2;
          }
          phase_c_hazard_alpha = d;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-c-hazard-alpha"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --phase-c-hazard-beta must be in [0,1]"
                      << std::endl;
            return 2;
          }
          phase_c_hazard_beta = d;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-c-hazard-beta"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          ...
          std::cerr << "Error: invalid integer for --phase-c-lag-align"
                    << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          float d = std::stof(v);
          if (!(d >= 0.0f && d <= 1.0f)) {
            std::cerr << "Error: --hazard-density must be in [0,1]"
                      << std::endl;
            return 2;
          }
          hazard_density = d;
        } catch (...) {
          std::cerr << "Error: invalid float for --hazard-density" << std::endl;
          return 2;
        }
        continue;
      }
      ...
      // Reward pipeline weights and shaped logging controls
        ...
        try {
          wt_teacher = std::stod(v);
        } catch (...) {
          std::cerr << "Error: invalid float for --wt-teacher" << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          wt_novelty = std::stod(v);
        } catch (...) {
          std::cerr << "Error: invalid float for --wt-novelty" << std::endl;
          return 2;
        }
        continue;
        ...
        try {
          wt_survival = std::stod(v);
        } catch (...) {
          std::cerr << "Error: invalid float for --wt-survival" << std::endl;
          return 2;
        }
        continue;
        ...
        if (!parse_on_off_flag(v, log_shaped_zero)) {
          std::cerr << "Error: --log-shaped-zero must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        continue;
      ...
      if (arg == "--emergent-only" || arg == "--true-emergence") {
        emergent_only = true;
        emergent_only_set = true;
        continue;
      } else if (starts_with(arg, "--emergent-only=") ||
                 starts_with(arg, "--true-emergence=")) {
        bool tmp = emergent_only;
        std::string key = starts_with(arg, "--emergent-only=")
                              ? "--emergent-only="
                              : "--true-emergence=";
        auto v = arg.substr(key.size());
        if (!parse_on_off_flag(v, tmp)) {
          std::cerr << "Error: --emergent-only/--true-emergence must be "
                       "on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
      ...
      if (arg == "--help" || arg == "-h" || arg == "/?") {
        ...
        if (!parse_on_off_flag(v, maze_demo)) {
          std::cerr << "Error: --maze-demo must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, maze_first_person)) {
          std::cerr
              << "Error: --maze-first-person must be on|off|true|false|1|0"
              << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (maze_size < 2) {
            std::cerr << "Error: --maze-size must be >= 2" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --maze-size" << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (!(maze_wall_density >= 0.0f && maze_wall_density <= 0.45f)) {
            std::cerr << "Error: --maze-wall-density must be in [0,0.45]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --maze-wall-density"
                    << std::endl;
          return 2;
        }
        ...
        if (v == "off" || v == "euclid" || v == "manhattan") {
          ...
          std::cerr << "Error: --maze-shaping must be off|euclid|manhattan"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (maze_shaping_k < 0.0f) {
            std::cerr << "Error: --maze-shaping-k must be >= 0" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --maze-shaping-k" << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (!(maze_shaping_gamma >= 0.0f && maze_shaping_gamma <= 1.0f)) {
            std::cerr << "Error: --maze-shaping-gamma must be in [0,1]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --maze-shaping-gamma"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (!(epsilon >= 0.0f && epsilon <= 1.0f)) {
            std::cerr << "Error: --epsilon must be in [0,1]" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --epsilon" << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (softmax_temp <= 0.0f) {
            std::cerr << "Error: --softmax-temp must be > 0" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --softmax-temp" << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (hybrid_lambda < 0.0f || hybrid_lambda > 1.0f) {
            std::cerr << "Error: --hybrid-lambda must be in [0,1]" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --hybrid-lambda" << std::endl;
          return 2;
        }
        ...
        std::transform(
            vlow.begin(), vlow.end(), vlow.begin(),
            [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        if (vlow == "none" || vlow == "greedy" || vlow == "bfs") {
          teacher_policy = vlow;
        } else {
          std::cerr
              << "Error: --teacher-policy must be one of: none, greedy, bfs"
              << std::endl;
          return 2;
        }
        ...
        try {
          teacher_mix = std::stof(v);
          if (teacher_mix < 0.0f || teacher_mix > 1.0f) {
            std::cerr << "Error: --teacher-mix must be in [0,1]" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --teacher-mix" << std::endl;
          return 2;
        }
      } else if (arg == "--mimicry") {
        mimicry_enable = true;
        ...
        if (!parse_on_off_flag(v, mimicry_enable)) {
          std::cerr << "Error: --mimicry must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          mimicry_weight_mu = std::stof(v);
          mimicry_weight_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --mimicry-weight" << std::endl;
          return 2;
        }
        ...
        if (teacher_embed_path.empty()) {
          std::cerr << "Error: --teacher-embed requires a file path"
                    << std::endl;
          return 2;
        }
        ...
        if (student_embed_path.empty()) {
          std::cerr << "Error: --student-embed requires a file path"
                    << std::endl;
          return 2;
        }
        ...
        std::transform(
            vlow.begin(), vlow.end(), vlow.begin(),
            [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        if (vlow == "off" || vlow == "vision" || vlow == "audio") {
          mirror_mode = vlow;
        } else {
          std::cerr << "Error: --mirror-mode must be one of: off, vision, audio"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase_a_student_lr = std::stod(v);
          if (phase_a_student_lr < 0.0) {
            std::cerr << "Error: --student-learning-rate must be >= 0"
                      << std::endl;
            return 2;
          }
          phase_a_student_lr_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --student-learning-rate"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, phase5_language_enable)) {
          std::cerr << "Error: --phase5-language must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
      } else if (arg == "--phase-a") {
        ...
        if (!parse_on_off_flag(v, phase_a_enable)) {
          std::cerr << "Error: --phase-a must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
      } else if (starts_with(arg, "--phase-a-similarity-threshold=")) {
        auto v =
            arg.substr(std::string("--phase-a-similarity-threshold=").size());
        try {
          phase_a_similarity_threshold = std::stof(v);
          phase_a_similarity_threshold_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-a-similarity-threshold"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase_a_novelty_threshold = std::stof(v);
          phase_a_novelty_threshold_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-a-novelty-threshold"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, phase_a_ema_enable)) {
          std::cerr << "Error: --phase-a-ema must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        phase_a_ema_enable_set = true;
        ...
        try {
          ...
          if (phase_a_ema_min <= 0.0 || phase_a_ema_min >= 1.0) {
            std::cerr << "Error: --phase-a-ema-min must be in (0,1)"
                      << std::endl;
            return 2;
          }
          phase_a_ema_min_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-a-ema-min"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase_a_ema_max <= 0.0 || phase_a_ema_max >= 1.0) {
            std::cerr << "Error: --phase-a-ema-max must be in (0,1)"
                      << std::endl;
            return 2;
          }
          phase_a_ema_max_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --phase-a-ema-max"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase_a_mimicry_repeats < 1) {
            std::cerr << "Error: --phase-a-mimicry-repeats must be >= 1"
                      << std::endl;
            return 2;
          }
          phase_a_mimicry_repeats_set = true;
        } catch (...) {
          std::cerr << "Error: invalid int for --phase-a-mimicry-repeats"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase_a_negative_k < 0) {
            std::cerr << "Error: --negative-sampling-k must be >= 0"
                      << std::endl;
            return 2;
          }
          phase_a_negative_k_set = true;
        } catch (...) {
          std::cerr << "Error: invalid int for --negative-sampling-k"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase_a_negative_weight < 0.0f) {
            std::cerr << "Error: --negative-weight must be >= 0" << std::endl;
            return 2;
          }
          phase_a_negative_weight_set = true;
        } catch (...) {
          std::cerr << "Error: invalid float for --negative-weight"
                    << std::endl;
          return 2;
        }
      } else if (arg == "--phase6") {
        ...
        if (!parse_on_off_flag(v, phase6_enable)) {
          std::cerr << "Error: --phase6 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        if (vv == "on" || vv == "off" || vv == "audit") {
          ...
          std::cerr << "Error: --phase6-active must be on|off|audit"
                    << std::endl;
          return 2;
        }
        ...
        try {
          double m = std::stod(v);
          if (m < 0.0 || m > 1.0) {
            std::cerr << "Error: --phase6-margin must be in [0,1]" << std::endl;
            return 2;
          }
          phase6_margin = m;
        } catch (...) {
          std::cerr << "Error: --phase6-margin expects a floating point value"
                    << std::endl;
          return 2;
        }
      } else if (arg == "--phase7") {
        ...
        if (!parse_on_off_flag(v, phase7_enable)) {
          std::cerr << "Error: --phase7 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, phase7_affect_enable)) {
          std::cerr << "Error: --phase7-affect must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, phase7_reflect_enable)) {
          std::cerr << "Error: --phase7-reflect must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
      } else if (arg == "--phase9") {
        ...
        if (!parse_on_off_flag(v, phase9_enable)) {
          std::cerr << "Error: --phase9 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, phase9_modulation_enable)) {
          std::cerr
              << "Error: --phase9-modulation must be on|off|true|false|1|0"
              << std::endl;
          return 2;
        }
      } else if (arg == "--phase10") {
        ...
        if (!parse_on_off_flag(v, phase10_enable)) {
          std::cerr << "Error: --phase10 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
      } else if (arg == "--phase11") {
        ...
        if (!parse_on_off_flag(v, phase11_enable)) {
          std::cerr << "Error: --phase11 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase11_revision_interval_ms <= 0) {
            std::cerr << "Error: --phase11-revision-interval must be > 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase11-revision-interval"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase11_min_gap_ms < 0) {
            std::cerr << "Error: --phase11-min-gap-ms must be >= 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase11-min-gap-ms"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase11_outcome_eval_window_ms < 0) {
            std::cerr << "Error: --phase11-outcome-window-ms must be >= 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase11-outcome-window-ms"
                    << std::endl;
          return 2;
        }
      } else if (arg == "--stagec") {
        ...
        if (!parse_on_off_flag(v, stagec_enable)) {
          std::cerr << "Error: --stagec must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
      } else if (arg == "--phase12") {
        ...
        if (!parse_on_off_flag(v, phase12_enable)) {
          std::cerr << "Error: --phase12 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase12_window < 2) {
            std::cerr << "Error: --phase12-window must be >= 2" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase12-window"
                    << std::endl;
          return 2;
        }
        // Phase 13 flags
        ...
        if (!parse_on_off_flag(v, phase13_enable)) {
          std::cerr << "Error: --phase13 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (phase13_window < 2) {
            std::cerr << "Error: --phase13-window must be >= 2" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase13-window"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase13_trust_tighten = std::stod(v);
          if (!(phase13_trust_tighten >= 0.0 && phase13_trust_tighten <= 1.0)) {
            std::cerr << "Error: --phase13-trust-tighten must be in [0,1]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --phase13-trust-tighten"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase13_trust_expand = std::stod(v);
          if (!(phase13_trust_expand >= 0.0 && phase13_trust_expand <= 1.0)) {
            std::cerr << "Error: --phase13-trust-expand must be in [0,1]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --phase13-trust-expand"
                    << std::endl;
          return 2;
        }
      } else if (starts_with(arg, "--phase13-consistency-tighten=")) {
        auto v =
            arg.substr(std::string("--phase13-consistency-tighten=").size());
        try {
          phase13_consistency_tighten = std::stod(v);
          if (!(phase13_consistency_tighten >= 0.0 &&
                phase13_consistency_tighten <= 1.0)) {
            std::cerr << "Error: --phase13-consistency-tighten must be in [0,1]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --phase13-consistency-tighten"
                    << std::endl;
          return 2;
        }
      } else if (starts_with(arg, "--phase13-consistency-expand=")) {
        auto v =
            arg.substr(std::string("--phase13-consistency-expand=").size());
        try {
          phase13_consistency_expand = std::stod(v);
          if (!(phase13_consistency_expand >= 0.0 &&
                phase13_consistency_expand <= 1.0)) {
            std::cerr << "Error: --phase13-consistency-expand must be in [0,1]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --phase13-consistency-expand"
                    << std::endl;
          return 2;
        }
      } else if (starts_with(arg, "--phase13-contraction-hysteresis-ms=")) {
        auto v = arg.substr(
            std::string("--phase13-contraction-hysteresis-ms=").size());
        try {
          phase13_contraction_hysteresis_ms = std::stoi(v);
          if (phase13_contraction_hysteresis_ms < 0) {
            std::cerr
                << "Error: --phase13-contraction-hysteresis-ms must be >= 0"
                << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for "
                       "--phase13-contraction-hysteresis-ms"
                    << std::endl;
          return 2;
        }
      } else if (starts_with(arg, "--phase13-expansion-hysteresis-ms=")) {
        auto v = arg.substr(
            std::string("--phase13-expansion-hysteresis-ms=").size());
        try {
          phase13_expansion_hysteresis_ms = std::stoi(v);
          if (phase13_expansion_hysteresis_ms < 0) {
            std::cerr << "Error: --phase13-expansion-hysteresis-ms must be >= 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr
              << "Error: invalid integer for --phase13-expansion-hysteresis-ms"
              << std::endl;
          return 2;
        }
      } else if (starts_with(arg, "--phase13-min-log-interval-ms=")) {
        auto v =
            arg.substr(std::string("--phase13-min-log-interval-ms=").size());
        try {
          phase13_min_log_interval_ms = std::stoi(v);
          if (phase13_min_log_interval_ms < 0) {
            std::cerr << "Error: --phase13-min-log-interval-ms must be >= 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr
              << "Error: invalid integer for --phase13-min-log-interval-ms"
              << std::endl;
          return 2;
        }
        // Phase 14 flags
        ...
        if (!parse_on_off_flag(v, phase14_enable)) {
          std::cerr << "Error: --phase14 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase14_window = std::stoi(v);
          if (phase14_window < 2) {
            std::cerr << "Error: --phase14-window must be >= 2" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase14-window"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase14_trust_degraded = std::stod(v);
          if (!(phase14_trust_degraded >= 0.0 &&
                phase14_trust_degraded <= 1.0)) {
            std::cerr << "Error: --phase14-trust-degraded must be in [0,1]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --phase14-trust-degraded"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase14_rmse_degraded = std::stod(v);
          if (phase14_rmse_degraded < 0.0) {
            std::cerr << "Error: --phase14-rmse-degraded must be >= 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --phase14-rmse-degraded"
                    << std::endl;
          return 2;
        }
        // Phase 15 flags
        ...
        if (!parse_on_off_flag(v, phase15_enable)) {
          std::cerr << "Error: --phase15 must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase15_window = std::stoi(v);
          if (phase15_window < 1) {
            std::cerr << "Error: --phase15-window must be >= 1" << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --phase15-window"
                    << std::endl;
          return 2;
        }
        ...
        try {
          phase15_risk_threshold = std::stod(v);
          if (!(phase15_risk_threshold >= 0.0 &&
                phase15_risk_threshold <= 1.0)) {
            std::cerr << "Error: --phase15-risk-threshold must be in [0,1]"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid float for --phase15-risk-threshold"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, telemetry_extended)) {
          std::cerr
              << "Error: --telemetry-extended must be on|off|true|false|1|0"
              << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, maze_view)) {
          std::cerr << "Error: --maze-view must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (maze_view_interval_ms < 0) {
            std::cerr << "Error: --maze-view-interval must be non-negative"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --maze-view-interval"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (maze_max_episode_steps <= 0) {
            std::cerr << "Error: --maze-max-episode-steps must be > 0"
                      << std::endl;
            return 2;
          }
        } catch (...) {
          std::cerr << "Error: invalid integer for --maze-max-episode-steps"
                    << std::endl;
          return 2;
        }
        ...
        if (episode_csv_path.empty()) {
          std::cerr << "Error: --episode-csv requires a file path" << std::endl;
          return 2;
        }
      } else if (arg == "--summary") {
        summary = true;
        ...
        if (!parse_on_off_flag(v, summary)) {
          std::cerr << "Error: --summary must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        if (!parse_on_off_flag(v, qlearning)) {
          std::cerr << "Error: --qlearning must be on|off|true|false|1|0"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          std::cerr << "Error: invalid float for --attention-boost"
                    << std::endl;
          return 2;
        }
        ...
        if (vlow == "none" || vlow == "off") {
          lconf.attention_mode =
              NeuroForge::Core::LearningSystem::AttentionMode::Off;
        } else if (vlow == "external" || vlow == "map" ||
                   vlow == "externalmap") {
          lconf.attention_mode =
              NeuroForge::Core::LearningSystem::AttentionMode::ExternalMap;
        } else if (vlow == "saliency") {
          ...
          std::cerr << "Error: --attention-mode must be one of: none, "
                       "external, saliency, topk"
                    << std::endl;
          return 2;
        }
        attention_mode_set = true;
        ...
        if (vlow == "off" || vlow == "none") {
          ...
          lconf.competence_mode =
              NeuroForge::Core::LearningSystem::CompetenceMode::ScalePGate;
                   ...
                   vlow == "scale-learning") {
          lconf.competence_mode = NeuroForge::Core::LearningSystem::
              CompetenceMode::ScaleLearningRates;
        } else {
          std::cerr << "Error: --competence-mode must be one of: off, "
                       "scale-pgate, scale-lr"
                    << std::endl;
          return 2;
        }
        competence_mode_set = true;
        ...
        try {
          ...
          std::cerr << "Error: invalid float for --homeostasis-eta"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          if (lconf.attention_anneal_ms < 0) {
            std::cerr << "Error: --attention-anneal-ms must be non-negative"
                      << std::endl;
            return 2;
          }
          attention_anneal_ms_set = true;
        ...
        try {
          ...
          std::cerr << "Error: invalid integer for --novelty-window"
                    << std::endl;
          return 2;
        }
        ...
        try {
          ...
          std::cerr << "Error: invalid float for --prune-threshold"
                    << std::endl;
          return 2;
        }
      } else {
        bool handled_any = false;
        if (handle_io_arg(arg, snapshot_csv_path, snapshot_live_path,
                          snapshot_interval_ms, spikes_live_path,
                          spikes_ttl_sec, save_brain_path, load_brain_path)) {
          handled_any = true;
          continue;
        }
        if (handle_telemetry_arg(
                arg, log_json, log_json_path, log_json_sample_val,
                log_json_events_csv, memory_db_path, memdb_debug,
                memdb_interval_ms, memdb_interval_cli_set, flag_list_runs,
                flag_list_episodes, list_episodes_run_id, flag_recent_rewards,
                recent_rewards_run_id, recent_rewards_limit,
                flag_recent_run_events, recent_run_events_run_id,
                recent_run_events_limit)) {
          handled_any = true;
          continue;
        }
        if (handle_demo_arg(
                arg, heatmap_view, heatmap_interval_ms, heatmap_size,
                heatmap_threshold, vision_demo, audio_demo, motor_cortex,
                social_perception, social_view, cross_modal, audio_mic,
                audio_system, audio_file_path, camera_index, camera_backend,
                vision_source, retina_rect_x, retina_rect_y, retina_rect_w,
                retina_rect_h, youtube_mode, foveation_enable, fovea_w, fovea_h,
                fovea_mode, fovea_alpha)) {
          handled_any = true;
          continue;
        }
        if (handle_dataset_arg(arg, dataset_triplets_root, dataset_mode,
                               dataset_limit, dataset_shuffle, reward_scale)) {
          handled_any = true;
          continue;
        }
        ...
        if (handle_rwci_parameters(arg, rwci_enable, rwci_enable_set,
                                   rwci_drives_revision,
                                   rwci_drives_revision_set)) {
          handled_any = true;
          continue;
        }
        if (handle_m6_m7_parameters(
                arg, hippocampal_snapshots, hippocampal_snapshots_set,
                memory_independent, memory_independent_set,
                consolidation_interval_m6, consolidation_interval_m6_set,
                autonomous_mode, autonomous_mode_set, substrate_mode,
                substrate_mode_set, curiosity_threshold,
                curiosity_threshold_set, uncertainty_threshold,
                uncertainty_threshold_set, prediction_error_threshold,
                prediction_error_threshold_set, max_concurrent_tasks,
                max_concurrent_tasks_set, task_generation_interval,
                task_generation_interval_set, eliminate_scaffolds,
                eliminate_scaffolds_set, autonomy_metrics, autonomy_metrics_set,
                autonomy_target, autonomy_target_set, motivation_decay,
                motivation_decay_set, exploration_bonus, exploration_bonus_set,
                novelty_memory_size, novelty_memory_size_set, enable_selfnode,
                enable_selfnode_set, enable_pfc, enable_pfc_set,
    ...
    // Configure MemoryDB periodic logging interval via environment variable or
    // assertion-mode hybrid default Priority: CLI > NF_MEMDB_INTERVAL_MS >
    // assertion-mode default (50ms) > code default (1000ms)
      ...
      if (env_interval && *env_interval) {
        try {
          ...
          if (parsed > 0) {
            ...
            std::cerr << "Info: Using NF_MEMDB_INTERVAL_MS="
                      << memdb_interval_ms << " for periodic telemetry interval"
                      << std::endl;
          } else {
            std::cerr
                << "Warning: NF_MEMDB_INTERVAL_MS must be > 0; keeping default "
                << memdb_interval_ms << std::endl;
          }
        } catch (...) {
          std::cerr << "Warning: invalid NF_MEMDB_INTERVAL_MS; keeping default "
                    << memdb_interval_ms << std::endl;
        }
      } else {
        ...
        if (env_assert && *env_assert && std::string(env_assert) != "0") {
          ...
          std::cerr
              << "Info: Assertion mode detected; using fast telemetry interval "
              << memdb_interval_ms << "ms" << std::endl;
        }
      }
    }
    ...
    if (rwci_drives_revision) {
      std::cerr << "Error: --rwci-drives-revision is forbidden under Stage C v1"
                << std::endl;
      return 2;
    }
    if (rwci_enable && autonomous_mode) {
      std::cerr << "Error: --rwci cannot be combined with --autonomous-mode "
                   "under Stage C v1"
                << std::endl;
      return 2;
    }
    ...
    if (homeostasis_eta_set &&
        (lconf.homeostasis_eta < 0.0f || lconf.homeostasis_eta > 1.0f)) {
      std::cerr << "Error: --homeostasis-eta must be in [0,1]" << std::endl;
      return 2;
    }
    if (competence_rho_set &&
        (lconf.competence_rho < 0.0f || lconf.competence_rho > 1.0f)) {
      std::cerr << "Error: --competence-rho must be in [0,1]" << std::endl;
      return 2;
    }
    if (attention_Amin_set &&
        (lconf.attention_Amin < 0.0f || lconf.attention_Amin > 1.0f)) {
    ...
    if (emergent_only) {
      ...
      std::cout << "[Emergence] Emergent-only mode active: Q-learning/Teacher "
                   "disabled; using pure motor-cortex argmax"
                << std::endl;
    }
    ...
    if (phase_c) {
      try {
        namespace fs = std::filesystem;
        // Default Phase C output under the executable directory to keep logs
        ...
        if (ec) {
          std::cerr << "Error: failed to create output directory '"
                    << out_dir.string() << "' : " << ec.message() << std::endl;
          return 2;
        }
        ...
        logger.setJsonSink([&](const std::string &line) {
          emit_json_line(log_json, log_json_path, line);
        });
        ...
        auto phasec_brain_shared =
            std::make_shared<NeuroForge::Core::HypergraphBrain>(phasec_conn);
        ...
        auto substrate_working_memory =
            std::make_shared<NeuroForge::Core::SubstrateWorkingMemory>(
        ...
        if (memory_db_path.empty()) {
          ...
          if (env_telemetry && *env_telemetry) {
            ...
            std::cerr << "Info: Using NF_TELEMETRY_DB for Phase C MemoryDB ('"
                      << memory_db_path << "')" << std::endl;
          }
        }
        if (!memory_db_path.empty()) {
          // Open MemoryDB and begin a run; propagate to Phase C brain for
          // reward logging
          phasec_memdb =
              std::make_shared<NeuroForge::Core::MemoryDB>(memory_db_path);
          phasec_memdb->setDebug(memdb_debug);
          if (!phasec_memdb->open()) {
            std::cerr << "Warning: failed to open MemoryDB at '"
                      << memory_db_path
                      << "' for Phase C path; continuing without persistence"
                      << std::endl;
            phasec_memdb.reset();
          } else {
            ...
            meta << "{\"phase\":\"C\",\"mode\":\"" << phase_c_mode << "\""
                 ...
                 << ",\"wm_decay\":" << std::setprecision(4)
                 << wm_config.decay_rate
                 << ",\"hazard_weight\":" << std::setprecision(4)
                 << (phase_c_hazard_weight_set ? phase_c_hazard_weight : 0.0f)
                 << ",\"survival_reward_scale\":" << std::setprecision(4)
                 << phase_c_survival_scale << ",\"emit_survival_rewards\":"
                 << (phase_c_survival_bias ? "true" : "false") << "}";
            if (!phasec_memdb->beginRun(meta.str(), phasec_memdb_run_id)) {
              std::cerr << "Warning: failed to begin MemoryDB run for Phase C "
                           "path; continuing without persistence"
                        << std::endl;
              phasec_memdb.reset();
            } else {
              std::cerr << "Info: Phase C MemoryDB enabled at '"
                        << memory_db_path << "' (run=" << phasec_memdb_run_id
                        << ")" << std::endl;
              phasec_brain_shared->setMemoryDB(phasec_memdb,
                                               phasec_memdb_run_id);
              // Emit lifecycle start marker for Phase C run
              {
                std::int64_t ts_ms =
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        ...
                        .count();
                std::int64_t event_id = 0;
                (void)phasec_memdb->insertRunEvent(
                    phasec_memdb_run_id, ts_ms, 0ULL, std::string("start"),
                    std::string("phasec_start"), 0, nf_process_rss_mb(), 0.0,
                    event_id);
              }
            }
          }
        ...
        public:
          SubstratePhaseCAdapter(
              std::shared_ptr<NeuroForge::Core::HypergraphBrain> brain,
              std::shared_ptr<NeuroForge::Core::SubstrateWorkingMemory>
                  working_memory,
              NeuroForge::PhaseCCSVLogger &logger,
              const NeuroForge::Core::SubstratePhaseC::Config &cfg)
              : brain_(brain), working_memory_(working_memory),
                logger_(logger) {

            // Initialize substrate Phase C
            NeuroForge::Core::SubstratePhaseC::Config config = cfg;
            substrate_phase_c_ =
                std::make_unique<NeuroForge::Core::SubstratePhaseC>(
                    brain, working_memory, config);
          ...
          void stepBinding(int step) {
            ...
            // Generate binding parameters from substrate state or external
            // input
          ...
          void setWorkingMemoryParams(std::size_t capacity, float decay) {
            if (working_memory_) {
              // Update configuration - SubstrateWorkingMemory doesn't have
              // direct setters The configuration is set during construction For
              // runtime updates, we would need to add these methods to
          ...
          void setEmitSurvivalRewards(bool enable) {
            // Enable or disable SurvivalBias-based shaped reward emission per
            // step
            if (substrate_phase_c_) {
          ...
          // Inject SurvivalBias to modulate assembly coherence and emit
          // telemetry
          void setSurvivalBias(
              std::shared_ptr<NeuroForge::Biases::SurvivalBias> bias) {
            if (substrate_phase_c_) {
        ...
        if (phase_c_binding_threshold_set)
          phasec_cfg.binding_threshold = phase_c_binding_threshold;
        if (phase_c_sequence_threshold_set)
          phasec_cfg.sequence_threshold = phase_c_sequence_threshold;
        if (phase_c_binding_coherence_min_set)
          phasec_cfg.binding_coherence_min = phase_c_binding_coherence_min;
        if (phase_c_sequence_coherence_min_set)
          phasec_cfg.sequence_coherence_min = phase_c_sequence_coherence_min;
        if (phase_c_prune_coherence_threshold_set)
          phasec_cfg.prune_coherence_threshold =
              phase_c_prune_coherence_threshold;
        SubstratePhaseCAdapter gw(phasec_brain_shared, substrate_working_memory,
                                  logger, phasec_cfg);

        // Apply runtime WM/sequence parameters
        gw.setWorkingMemoryParams(phase_c_wm_capacity, phase_c_wm_decay);
        if (phase_c_seq_window > 0)
          gw.setSequenceWindow(phase_c_seq_window);
        ...
        if (phase_c_survival_bias) {
          ...
          phasec_sb =
              std::make_shared<NeuroForge::Biases::SurvivalBias>(sb_cfg);
        ...
        {
          auto r_in = phasec_brain_shared->createRegion(
              "PhaseC_Input", NeuroForge::Core::Region::Type::Cortical,
              NeuroForge::Core::Region::ActivationPattern::Asynchronous);
          auto r_out = phasec_brain_shared->createRegion(
              "PhaseC_Output", NeuroForge::Core::Region::Type::Subcortical,
              NeuroForge::Core::Region::ActivationPattern::Competitive);
          if (r_in)
            r_in->createNeurons(32);
          if (r_out)
            r_out->createNeurons(32);
          // Light recurrent structure and forward projection
          if (r_in) {
            (void)phasec_brain_shared->connectRegions(
                r_in->getId(), r_in->getId(), 0.10f, {0.05f, 0.20f});
          }
          if (r_in && r_out) {
            (void)phasec_brain_shared->connectRegions(
                r_in->getId(), r_out->getId(), 0.50f, {0.10f, 0.90f});
          }
        }
        bool phasec_learning_initialized = false;
        if (enable_learning || hebbian_rate_set || stdp_rate_set ||
            stdp_mult_set || attention_boost_set || homeostasis_set ||
            consolidation_interval_set || consolidation_strength_set ||
            alpha_set || gamma_set || eta_set || lambda_set || etaElig_set ||
            kappa_set || attention_mode_set || p_gate_set ||
            homeostasis_eta_set || attention_Amin_set || attention_Amax_set ||
            attention_anneal_ms_set || chaos_steps_set ||
            consolidate_steps_set || novelty_window_set ||
            prune_threshold_set || auto_elig_set || competence_mode_set ||
            competence_rho_set || !snapshot_csv_path.empty()) {
          // Initialize brain (even with no regions; learning updates will be
          // safe no-ops if empty)
          ...
          if (phasec_brain_shared->initializeLearning(lconf)) {
            ...
            if (ls_init) {
              ...
              if (alpha_set || gamma_set || eta_set || lambda_set ||
                  etaElig_set || kappa_set) {
                ls_init->configurePhase4(lambda_param, etaElig_param,
                                         kappa_param, alpha_weight,
                                         gamma_weight, eta_weight);
              }
              phasec_learning_initialized = true;
            }
          } else {
            phasec_learning_initialized =
                (phasec_brain_shared->getLearningSystem() != nullptr);
          }
        }
        ...
        if (phasec_learning_initialized) {
          ...
          if (ls0) {
            last_consolidation_events =
                ls0->getStatistics().consolidation_events;
          }
        }
        bool warned_live_no_ls = false;
        for (int s = 0; s < steps; ++s) {
          // Inject external hazard into SurvivalBias (priority: CLI constant
          // >0, else audio RMS)
          if (phasec_sb) {
          ...
          float dt =
              (step_ms > 0 ? static_cast<float>(step_ms) / 1000.0f : 0.01f);
          ...
          // Periodic MemoryDB logging for Phase C path
          {
            ...
            auto elapsed_ms =
                std::chrono::duration_cast<std::chrono::milliseconds>(
                    now - phasec_last_memdb_log)
                    .count();
            if (elapsed_ms >= memdb_interval_ms) {
              int steps_since =
                  static_cast<int>(elapsed_ms / (step_ms > 0 ? step_ms : 10));
              if (steps_since <= 0) {
                steps_since = 1;
              }
              float hz = (elapsed_ms > 0)
                             ? (1000.0f * static_cast<float>(steps_since) /
              ...
              if (phasec_memdb && phasec_memdb_run_id != 0) {
                ...
                if (statsOpt2.has_value()) {
                  auto ts_ms = static_cast<uint64_t>(
                      std::chrono::duration_cast<std::chrono::milliseconds>(
                          std::chrono::system_clock::now().time_since_epoch())
                          .count());
                  phasec_memdb->insertLearningStats(
                      ts_ms, s, hz, statsOpt2.value(),
          ...
          if (phasec_learning_initialized) {
            ...
            if (statsOpt.has_value()) {
              ...
              if (sstats.consolidation_events > last_consolidation_events) {
                uint64_t delta =
                    sstats.consolidation_events - last_consolidation_events;
                std::ostringstream js;
                js << "{\"version\":1,\"phase\":\"C\",\"event\":"
                      "\"consolidation\",\"time\":\""
                   << json_escape(iso8601_utc_now()) << "\",";
                js << "\"payload\":{\"count\":" << delta
                   ...
                   << ",\"potentiated_synapses\":"
                   << sstats.potentiated_synapses
          ...
          if (!snapshot_live_path.empty()) {
            if (!phasec_learning_initialized) {
              if (!warned_live_no_ls) {
                std::cerr << "Info: live export requested (--snapshot-live) "
                             "but LearningSystem is not initialized. Enable "
                             "learning to export live data."
                          << std::endl;
              ...
              if (std::chrono::duration_cast<std::chrono::milliseconds>(
                      now - last_snapshot)
                      .count() >= snapshot_interval_ms) {
                auto *ls = phasec_brain_shared->getLearningSystem();
                if (ls) {
                  auto snapshots = ls->getSynapseSnapshot();
                  std::ofstream ofs(snapshot_live_path,
                                    std::ios::out | std::ios::trunc);
                  if (!ofs) {
                    std::cerr << "Error: failed to open '" << snapshot_live_path
                              << "' for live snapshot writing" << std::endl;
                  } else {
                    ofs << "pre_neuron,post_neuron,weight\n";
                    for (const auto &srec : snapshots) {
                      ofs << srec.pre_neuron << "," << srec.post_neuron << ","
                          << srec.weight << "\n";
                    }
                    ofs.flush();
                  }
        ...
        if (!snapshot_csv_path.empty()) {
          ...
          if (!ls) {
            std::cerr
                << "Warning: --snapshot-csv was provided, but LearningSystem "
                   "is not initialized. Enable learning to export snapshots."
                << std::endl;
          } else {
            auto snapshots = ls->getSynapseSnapshot();
            std::ofstream ofs(snapshot_csv_path,
                              std::ios::out | std::ios::trunc);
            if (!ofs) {
              std::cerr << "Error: failed to open '" << snapshot_csv_path
              ...
              ofs.flush();
              std::cout << "Wrote synapse snapshot: " << snapshots.size()
                        << " edges to '" << snapshot_csv_path << "'\n";
            }
          }
        }

        // Always print learning statistics section for CI visibility (Phase C
        // path)
        {
        ...
        std::cout << "Phase C completed. Logs written to: " << out_dir.string()
                  << std::endl;
        return 0;
    ...
    if (consolidate_steps_set && lconf.consolidate_steps < 0) {
      std::cerr << "Error: --consolidate-steps must be non-negative"
                << std::endl;
      return 2;
    }
    ...
    if (prune_threshold_set &&
        (lconf.prune_threshold < 0.0f || lconf.prune_threshold > 1.0f)) {
      std::cerr << "Error: --prune-threshold must be in [0,1]" << std::endl;
      return 2;
    }

    // Phase-4 parameter validation (unless explicitly bypassed)
    if (!phase4_unsafe && (alpha_set || gamma_set || eta_set || lambda_set ||
                           etaElig_set || kappa_set)) {
    ...
    if (flag_list_runs || flag_list_episodes || flag_recent_rewards ||
        flag_recent_run_events) {
      if (memory_db_path.empty()) {
        std::cerr
            << "Error: --memory-db=PATH is required when using listing flags"
            << std::endl;
        return 2;
      }
      ...
      if (!mdb.open()) {
        std::cerr << "Error: failed to open MemoryDB at '" << memory_db_path
                  << "'" << std::endl;
        return 1;
      }
      if (flag_list_runs) {
      ...
      if (flag_list_episodes) {
        ...
        try {
          ...
          std::cerr << "Error: RUN_ID for --list-episodes must be an integer"
                    << std::endl;
          return 2;
        }
        auto eps = mdb.getEpisodes(static_cast<int64_t>(run_id_ll));
        std::cout << "Episodes(run=" << list_episodes_run_id
                  << ") count=" << eps.size() << "\n";
        for (const auto &e : eps) {
          std::cout << e.id << "," << e.name << "," << e.start_ms << ","
                    << (e.end_ms == 0 ? -1 : e.end_ms) << "\n";
        }
      }
      if (flag_recent_rewards) {
        ...
        try {
          ...
          std::cerr << "Error: RUN_ID for --recent-rewards must be an integer"
                    << std::endl;
          return 2;
        }
        auto rewards = mdb.getRecentRewards(static_cast<int64_t>(run_id_ll),
                                            recent_rewards_limit);
        std::cout << "RecentRewards(run=" << recent_rewards_run_id
                  << ",limit=" << recent_rewards_limit
                  << ") count=" << rewards.size() << "\n";
        for (const auto &r : rewards) {
          std::cout << r.id << "," << r.ts_ms << "," << r.step << ","
                    << r.reward << "," << r.source << "," << r.context_json
                    << "\n";
        }
      }
      if (flag_recent_run_events) {
        ...
        try {
          ...
          std::cerr
              << "Error: RUN_ID for --recent-run-events must be an integer"
              << std::endl;
          return 2;
        }
        auto events = mdb.getRecentRunEvents(static_cast<int64_t>(run_id_ll),
                                             recent_run_events_limit);
        std::cout << "RunEvents(run=" << recent_run_events_run_id
                  << ",limit=" << recent_run_events_limit
                  << ") count=" << events.size() << "\n";
        for (const auto &e : events) {
          std::cout << e.id << "," << e.ts_ms << "," << e.step << "," << e.type
                    << "," << e.message << "," << e.exit_code << "," << e.rss_mb
                    << "," << e.gpu_mem_mb << "\n";
        }
      }
      return 0;
    ...
    if (!load_brain_path.empty()) {
      if (!brain.loadCheckpoint(load_brain_path)) {
        std::cerr << "Error: failed to load brain checkpoint from '"
                  << load_brain_path << "'" << std::endl;
        return 1;
      }
      loaded_from_checkpoint = true;
      if (vision_demo || audio_demo || motor_cortex || maze_demo ||
          cross_modal) {
        std::cerr << "Info: --load-brain provided; demo topology flags will be "
                     "ignored."
                  << std::endl;
      }
    }
    ...
    if (!memory_db_path.empty()) {
      std::cerr << "Info: --memory-db provided ('" << memory_db_path
                << "'). If SQLite3 is available, telemetry will be logged."
                << std::endl;
    }
    ...
    if (!loaded_from_checkpoint) {
      if (vision_demo) {
        any_demo = true;
        visual_region = std::make_shared<NeuroForge::Regions::VisualCortex>(
            "VisualCortex",
            static_cast<std::size_t>(vcfg.grid_size * vcfg.grid_size));
        brain.addRegion(visual_region);
        // Ensure neuron count matches feature vector length
        visual_region->createNeurons(
            static_cast<std::size_t>(vcfg.grid_size * vcfg.grid_size));
        visual_region->initializeLayers();
        brain.connectRegions(visual_region->getId(), visual_region->getId(),
                             0.05f, {0.1f, 0.9f});
      }
      if (audio_demo) {
        any_demo = true;
        auditory_region = std::make_shared<NeuroForge::Regions::AuditoryCortex>(
            "AuditoryCortex", static_cast<std::size_t>(acfg.feature_bins));
        brain.addRegion(auditory_region);
        auditory_region->createNeurons(
            static_cast<std::size_t>(acfg.feature_bins));
        auditory_region->initializeTonotopicMap();
        brain.connectRegions(auditory_region->getId(), auditory_region->getId(),
                             0.05f, {0.1f, 0.9f});
      }
      if (motor_cortex) {
        any_demo = true;
        motor_region = std::make_shared<NeuroForge::Regions::MotorCortex>(
            "MotorCortex", static_cast<std::size_t>(75000));
        brain.addRegion(motor_region);
        motor_region->createNeurons(static_cast<std::size_t>(75000));
        motor_region->initializeSomatotopicMap();
        brain.connectRegions(motor_region->getId(), motor_region->getId(),
                             0.05f, {0.1f, 0.9f});
      }
      if (maze_demo) {
        any_demo = true;
        // Create Maze observation and action regions
        maze_obs_region = brain.createRegion(
            "MazeObservation", NeuroForge::Core::Region::Type::Custom,
            NeuroForge::Core::Region::ActivationPattern::Asynchronous);
        maze_obs_region->createNeurons(
            static_cast<std::size_t>(maze_size * maze_size));
        maze_action_region = brain.createRegion(
            "MazeAction", NeuroForge::Core::Region::Type::Custom,
            NeuroForge::Core::Region::ActivationPattern::Competitive);
        maze_action_region->createNeurons(static_cast<std::size_t>(4));
        // Connect observations -> actions and add mild recurrent dynamics in
        // action region
        brain.connectRegions(maze_obs_region->getId(),
                             maze_action_region->getId(), 0.20f,
                             {0.05f, 0.15f});
        brain.connectRegions(maze_action_region->getId(),
                             maze_action_region->getId(), 0.05f,
                             {0.02f, 0.08f});
      }
      if (social_perception) {
        any_demo = true;
        // Create Social region for enhanced social perception
        social_region = brain.createRegion(
            "SocialPerception", NeuroForge::Core::Region::Type::Cortical,
            NeuroForge::Core::Region::ActivationPattern::Asynchronous);
        social_region->createNeurons(
            static_cast<std::size_t>(32 * 32)); // 32x32 Social grid
        ...
        social_bias =
            std::make_unique<NeuroForge::Biases::SocialPerceptionBias>(
                social_config);
        if (!social_bias->initialize()) {
          std::cerr << "Warning: SocialPerceptionBias failed to initialize "
                       "(OpenCV cascade files may be missing)"
                    << std::endl;
        } else {
          std::cout << "SocialPerceptionBias initialized with enhanced "
                       "biological features"
                    << std::endl;
        }
...
#else
        std::cout << "SocialPerceptionBias disabled (OpenCV not available)"
                  << std::endl;
#endif

        // Connect social region to other regions for cross-modal integration
        if (visual_region) {
          brain.connectRegions(social_region->getId(), visual_region->getId(),
                               0.03f, {0.05f, 0.15f});
          brain.connectRegions(visual_region->getId(), social_region->getId(),
                               0.03f, {0.05f, 0.15f});
        }
        if (auditory_region) {
          brain.connectRegions(social_region->getId(), auditory_region->getId(),
                               0.03f, {0.05f, 0.15f});
          brain.connectRegions(auditory_region->getId(), social_region->getId(),
                               0.03f, {0.05f, 0.15f});
        }

        std::cout << "Social perception region created with " << (32 * 32)
                  << " neurons and cross-modal connectivity" << std::endl;
      }
      ...
      if (audio_demo || social_perception) {
        ...
        voice_bias =
            std::make_unique<NeuroForge::Biases::VoiceBias>(voice_config);
        std::cout << "VoiceBias initialized with human voice prioritization"
                  << std::endl;
      }
      ...
      if (vision_demo || social_perception) {
        ...
        motion_bias =
            std::make_unique<NeuroForge::Biases::MotionBias>(motion_config);
        std::cout << "MotionBias initialized with biological motion detection"
                  << std::endl;
      }
      if (cross_modal && visual_region && auditory_region) {

        // Establish modest bidirectional cross-modal links between visual and
        // auditory regions
        brain.connectRegions(visual_region->getId(), auditory_region->getId(),
                             0.02f, {0.05f, 0.2f});
        brain.connectRegions(auditory_region->getId(), visual_region->getId(),
                             0.02f, {0.05f, 0.2f});
      }
      if (!any_demo) {
      ...
      for (const auto &r : add_region_specs) {
        auto region = NeuroForge::Core::RegionRegistry::instance().create(
            r.key, r.name, r.count);
        if (!region) {
          std::cerr << "Error: --add-region unknown key '" << r.key << "'"
                    << std::endl;
          std::cerr << "Known keys: ";
          auto keys = NeuroForge::Core::RegionRegistry::instance().listKeys();
          for (size_t i = 0; i < keys.size(); ++i) {
            std::cerr << keys[i] << (i + 1 < keys.size() ? ", " : "");
          }
          std::cerr << std::endl;
          return 2;
        }
    ...
    if (enable_learning || hebbian_rate_set || stdp_rate_set || stdp_mult_set ||
        attention_boost_set || homeostasis_set || consolidation_interval_set ||
        consolidation_strength_set || alpha_set || gamma_set || eta_set ||
        lambda_set || etaElig_set || kappa_set || mimicry_enable ||
        mimicry_weight_set || !teacher_embed_path.empty() ||
        !student_embed_path.empty() || attention_mode_set || p_gate_set ||
        homeostasis_eta_set || attention_Amin_set || attention_Amax_set ||
        attention_anneal_ms_set || chaos_steps_set || consolidate_steps_set ||
        novelty_window_set || prune_threshold_set || auto_elig_set ||
        competence_mode_set || competence_rho_set) {
      // Align update interval with step size unless explicitly overridden
      if (!consolidation_interval_set) {
        lconf.update_interval =
            std::chrono::milliseconds(step_ms > 0 ? step_ms : 0);
      ...
      // hebbian_rate and stdp_rate as 0.0f, so without this `--enable-learning`
      // on its own initialises the learning system with zero rates: it runs,
      // reports "Total Updates: 0" and changes no weight. Measured 2026-08-22:
      // `--steps=200 --enable-learning` gave 0 updates, while adding
      // `--hebbian-rate=0.1 --stdp-rate=0.1` gave 63,006. The documented flag
      ...
      if (auto_elig_set) {
        ...
        if (ls) {
          ...
          std::cout << "[Learning] auto-eligibility accumulation "
                    << (auto_elig_enabled ? "ENABLED" : "DISABLED")
                    << std::endl;
        } else {
          std::cerr << "Warning: --auto-eligibility provided but "
                       "LearningSystem not available."
                    << std::endl;
        }
      }
      // Phase-5 logging of key parameters if set
      if (attention_mode_set || p_gate_set || homeostasis_eta_set ||
          attention_Amin_set || attention_Amax_set || attention_anneal_ms_set ||
          chaos_steps_set || consolidate_steps_set || novelty_window_set ||
          prune_threshold_set || competence_mode_set || competence_rho_set) {
        auto cfg = brain.getLearningSystem()
                       ? brain.getLearningSystem()->getConfig()
                       : lconf;
        std::cout << "Phase-5: attention_mode="
                  << static_cast<int>(cfg.attention_mode)
                  << " p_gate=" << cfg.p_gate
                  ...
                  << " competence_mode="
                  << static_cast<int>(cfg.competence_mode)
                  << " competence_rho=" << cfg.competence_rho << std::endl;
      }
      // Apply Phase-4 reward shaping configuration if any Phase-4 parameters
      // were provided via CLI
      if (alpha_set || gamma_set || eta_set || lambda_set || etaElig_set ||
          kappa_set) {
        auto *ls = brain.getLearningSystem();
        if (ls) {
          ls->configurePhase4(lambda_param, etaElig_param, kappa_param,
                              alpha_weight, gamma_weight, eta_weight);
          std::cout << "Configured Phase-4: lambda=" << lambda_param
                    << " etaElig=" << etaElig_param << " kappa=" << kappa_param
                    << " | alpha=" << alpha_weight << " gamma=" << gamma_weight
                    << " eta=" << eta_weight
                    << (phase4_unsafe ? " (unsafe)" : "") << std::endl;
        } else {
          std::cerr << "Warning: LearningSystem not available to configure "
                       "Phase-4 weights."
                    << std::endl;
        }
      }
      // Phase-5 Mimicry wiring (enable, weight, and embedding vectors)
      {
        ...
        if (ls) {
          ...
          auto load_float_file = [](const std::string &path,
          ...
          if (!teacher_embed_path.empty()) {
            ...
            if (!t_ok) {
              std::cerr << "Warning: failed to load teacher embedding from '"
                        << teacher_embed_path << "'" << std::endl;
            } else {
              ls->setTeacherVector(tvec);
              std::cout << "Mimicry: loaded teacher embedding of length "
                        << tvec.size() << " from '" << teacher_embed_path << "'"
                        << std::endl;
              // Also pass teacher embedding to LanguageSystem / Phase A if
              // enabled
              ...
              if (phase_a_enable && phase_a_system) {
                ...
                phase_a_system->addTeacherEmbedding(
                    tvec, NeuroForge::Core::PhaseAMimicry::TeacherType::Custom,
                    NeuroForge::Core::PhaseAMimicry::Modality::Multimodal,
                    "teacher_embed", teacher_embed_path, 1.0f);
              }
            }
          }
          if (!student_embed_path.empty()) {
            ...
            if (!s_ok) {
              std::cerr << "Warning: failed to load student embedding from '"
                        << student_embed_path << "'" << std::endl;
            } else {
              ls->setStudentEmbedding(svec);
              std::cout
                  << "Mimicry: loaded initial student embedding of length "
                  << svec.size() << " from '" << student_embed_path << "'"
                  << std::endl;
            }
          }
          if (t_ok && s_ok && tvec.size() != svec.size()) {
            std::cerr
                << "Warning: teacher (" << tvec.size() << ") and student ("
                << svec.size()
                << ") embedding lengths differ; mimicry similarity will be 0."
                << std::endl;
          }
        } else if (mimicry_enable || mimicry_weight_set ||
                   !teacher_embed_path.empty() || !student_embed_path.empty()) {
          std::cerr << "Warning: Mimicry flags provided but LearningSystem not "
                       "available."
                    << std::endl;
        }
      }
    ...
    if (hippocampal_snapshots_set || memory_independent_set ||
        consolidation_interval_m6_set) {
      std::cout << "[M6] Memory Internalization parameters detected:"
                << std::endl;
      ...
      if (hippocampal_snapshots_set) {
        std::cout << "  Hippocampal snapshots: "
                  << (hippocampal_snapshots ? "ENABLED" : "DISABLED")
                  << std::endl;
        brain.setHippocampalEnabled(hippocampal_snapshots);
      }
      if (memory_independent_set) {
        std::cout << "  Memory-independent learning: "
                  << (memory_independent ? "ENABLED" : "DISABLED") << std::endl;
        ...
        if (memory_independent) {
          ...
          config.consolidation_threshold =
              0.6f; // Lower threshold for more frequent consolidation
          brain.configureHippocampalSnapshotting(config);
        }
      }
      if (consolidation_interval_m6_set) {
        std::cout << "  M6 consolidation interval: "
                  << consolidation_interval_m6 << " ms" << std::endl;
        NeuroForge::Core::HypergraphBrain::HippocampalConfig config;
        config.enabled = true;
        config.snapshot_interval_ms =
            static_cast<std::uint64_t>(consolidation_interval_m6);
        brain.configureHippocampalSnapshotting(config);
      }
    }

    // M7 Autonomous Operation Integration
    if (autonomous_mode_set || substrate_mode_set || curiosity_threshold_set ||
        uncertainty_threshold_set || prediction_error_threshold_set ||
        max_concurrent_tasks_set || task_generation_interval_set ||
        eliminate_scaffolds_set || autonomy_metrics_set ||
        autonomy_target_set || motivation_decay_set || exploration_bonus_set ||
        novelty_memory_size_set || enable_selfnode_set || enable_pfc_set ||
        enable_motor_cortex_set) {

      std::cout << "[M7] Autonomous Operation parameters detected:"
                << std::endl;

      // Enable learning system for M7 functionality (intrinsic motivation
      // requires it)
      if (!brain.initializeLearning(lconf)) {
        // If already initialized, it's fine
      }
      ...
      if (autonomous_mode_set) {
        std::cout << "  Autonomous mode: "
                  << (autonomous_mode ? "ENABLED" : "DISABLED") << std::endl;
        ...
        if (autonomous_mode) {
          ...
          if (!brain.initializeAutonomousScheduler()) {
            std::cerr << "Warning: Failed to initialize autonomous scheduler"
                      << std::endl;
          }
        }
      }
      if (substrate_mode_set) {
        ...
        if (substrate_mode == "off")
          mode = NeuroForge::Core::HypergraphBrain::SubstrateMode::Off;
        else if (substrate_mode == "mirror")
          mode = NeuroForge::Core::HypergraphBrain::SubstrateMode::Mirror;
        else if (substrate_mode == "train")
          mode = NeuroForge::Core::HypergraphBrain::SubstrateMode::Train;
        else if (substrate_mode == "native")
          mode = NeuroForge::Core::HypergraphBrain::SubstrateMode::Native;
        else
          mode = NeuroForge::Core::HypergraphBrain::SubstrateMode::Off;
        brain.setSubstrateMode(mode);
      }
      if (curiosity_threshold_set) {
        std::cout << "  Curiosity threshold: " << curiosity_threshold
                  << std::endl;
        brain.setCuriosityThreshold(curiosity_threshold);
      }
      if (uncertainty_threshold_set) {
        std::cout << "  Uncertainty threshold: " << uncertainty_threshold
                  << std::endl;
        brain.setUncertaintyThreshold(uncertainty_threshold);
      }
      if (prediction_error_threshold_set) {
        std::cout << "  Prediction error threshold: "
                  << prediction_error_threshold << std::endl;
        brain.setPredictionErrorThreshold(prediction_error_threshold);
      }
      if (max_concurrent_tasks_set) {
        std::cout << "  Max concurrent tasks: " << max_concurrent_tasks
                  << std::endl;
        brain.setMaxConcurrentTasks(max_concurrent_tasks);
      }
      if (task_generation_interval_set) {
        std::cout << "  Task generation interval: " << task_generation_interval
                  << " ms" << std::endl;
        brain.setTaskGenerationInterval(task_generation_interval);
      }
      if (eliminate_scaffolds_set) {
        std::cout << "  Eliminate scaffolds: "
                  << (eliminate_scaffolds ? "ENABLED" : "DISABLED")
                  << std::endl;
        brain.setEliminateScaffolds(eliminate_scaffolds);
      }
      if (autonomy_metrics_set) {
        std::cout << "  Autonomy metrics: "
                  << (autonomy_metrics ? "ENABLED" : "DISABLED") << std::endl;
      ...
      if (novelty_memory_size_set) {
        std::cout << "  Novelty memory size: " << novelty_memory_size
                  << std::endl;
        brain.setNoveltyMemorySize(novelty_memory_size);
      }
      if (enable_selfnode_set) {
        std::cout << "  SelfNode integration: "
                  << (enable_selfnode ? "ENABLED" : "DISABLED") << std::endl;
        brain.setSelfNodeIntegrationEnabled(enable_selfnode);
      }
      if (enable_pfc_set) {
        std::cout << "  PrefrontalCortex integration: "
                  << (enable_pfc ? "ENABLED" : "DISABLED") << std::endl;
        brain.setPrefrontalCortexIntegrationEnabled(enable_pfc);
      }
      if (enable_motor_cortex_set) {
        std::cout << "  MotorCortex integration: "
                  << (enable_motor_cortex ? "ENABLED" : "DISABLED")
                  << std::endl;
    ...
    bool m6_m7_detected =
        (hippocampal_snapshots_set || memory_independent_set ||
         consolidation_interval_m6_set || autonomous_mode_set ||
         substrate_mode_set || curiosity_threshold_set ||
         uncertainty_threshold_set || prediction_error_threshold_set ||
         max_concurrent_tasks_set || task_generation_interval_set ||
         eliminate_scaffolds_set || autonomy_metrics_set ||
         autonomy_target_set || motivation_decay_set || exploration_bonus_set ||
    ...
    if (m6_m7_detected && !enable_learning && !hebbian_rate_set &&
        !stdp_rate_set) {
      ...
      std::cout << "[M6/M7] Auto-enabling basic learning (hebbian_rate=0.001) "
                   "for M6/M7 functionality"
                << std::endl;
    }
    ...
    if (!spikes_live_path.empty()) {
      brain.setSpikeObserver(
          [&](NeuroForge::NeuronID nid, NeuroForge::TimePoint t) {
            std::lock_guard<std::mutex> lg(spikes_mutex);
            spike_events.emplace_back(nid, t);
          });
    }

    // If viewer is enabled, ensure live snapshot path is set and launch viewer
    // process Helper for file existence without <filesystem>
    ...
    if (viewer_enabled) {
      try {
        // If user didn't provide live snapshot path, default to a file in
        // current dir
        if (snapshot_live_path.empty()) {
        ...
        // Seed file with header to allow the viewer to start immediately
        {
          ...
          if (ofs) {
            ...
            std::cerr
                << "Warning: could not create '" << snapshot_live_path
                << "'. Viewer may not see updates until the path is writable."
                << std::endl;
          }
        }
        // If spikes overlay is enabled, seed its CSV header too
        if (!spikes_live_path.empty()) {
          std::ofstream spofs(spikes_live_path,
                              std::ios::out | std::ios::trunc);
          if (spofs) {
        ...
        try {
          snapshot_path_for_viewer =
              std::filesystem::absolute(snapshot_live_path).string();
          if (!spikes_live_path.empty()) {
            spikes_path_for_viewer =
                std::filesystem::absolute(spikes_live_path).string();
          }
        } catch (...) {
        ...
        if (!viewer_exe_path.empty() && file_exists(viewer_exe_path)) {
          ...
          std::string cmd;
#ifdef _WIN32
          cmd = "cmd /c start \"NF Viewer\" \"" + viewer_exe_path +
                "\" --snapshot-file=\"" + snapshot_path_for_viewer +
                "\" --weight-threshold=" + std::to_string(viewer_threshold) +
                " --layout=" + viewer_layout +
                " --refresh-ms=" + std::to_string(viewer_refresh_ms);
          if (!spikes_live_path.empty()) {
            cmd += " --spikes-file=\"" + spikes_path_for_viewer + "\"";
          }
#else
          cmd = "\"" + viewer_exe_path + "\" --snapshot-file=\"" +
                snapshot_path_for_viewer +
                "\" --weight-threshold=" + std::to_string(viewer_threshold) +
                " --layout=" + viewer_layout +
                " --refresh-ms=" + std::to_string(viewer_refresh_ms);
          ...
          std::cout << "Launched 3D viewer: " << viewer_exe_path
                    ...
                    << "\n  layout='" << viewer_layout
                    << "' refresh=" << viewer_refresh_ms
                    << " ms threshold=" << viewer_threshold;
          if (!spikes_live_path.empty())
            std::cout << " spikes=\"" << spikes_path_for_viewer << "\"";
          std::cout << std::endl;
        } else {
          std::cerr
              << "Info: 3D viewer executable not found. Expected at "
                 "--viewer-exe path or in build directories. You can run it "
                 "manually with: neuroforge_viewer.exe --snapshot-file=\""
              << snapshot_live_path << "\" --layout=" << viewer_layout
              << " --refresh-ms=" << viewer_refresh_ms
              << " --weight-threshold=" << viewer_threshold;
          if (!spikes_live_path.empty())
            std::cerr << " --spikes-file=\"" << spikes_live_path << "\"";
          std::cerr << std::endl;
        }
      } catch (const std::exception &ex) {
        std::cerr << "Warning: failed to set up 3D viewer: " << ex.what()
                  << std::endl;
      }
    ...
    const float delta_time_seconds =
        (step_ms > 0) ? (static_cast<float>(step_ms) / 1000.0f) : 0.0f;
    ...
    NeuroForge::Audio::SystemAudioCapture syscap(
        {static_cast<std::uint32_t>(acfg.sample_rate), 2});
    bool sys_ok = false;

    // Create sandbox window if enabled and restrict capture to its client area
    NeuroForge::Sandbox::WebSandbox sandbox_window;
    if (sandbox_enable) {
      if (sandbox_window.create(sandbox_w, sandbox_h,
                                std::string("NeuroForge Sandbox"))) {
        ...
        std::cerr << "Warning: failed to create sandbox window; continuing "
                     "without sandbox"
                  << std::endl;
      }
    }

    NeuroForge::IO::ScreenCapturer screen(
        {retina_rect_x, retina_rect_y, retina_rect_w, retina_rect_h});

    bool mic_ok = false;
    if (audio_demo && audio_mic) {
      std::cerr << "Info: Initializing audio capture at " << acfg.sample_rate
                << " Hz" << std::endl;
      if (mic.initialize()) {
        ...
        std::cerr << "Info: Microphone capture not available; falling back to "
                     "synthetic audio"
                  << std::endl;
      }
    }
    if (audio_demo && audio_system) {
      ...
      sys_ok = syscap.start();
      if (!sys_ok) {
        std::cerr << "Warning: System audio capture failed; falling back to "
    ...
    if (audio_demo && !audio_mic && !audio_file_path.empty()) {
      if (!std::filesystem::exists(audio_file_path)) {
        std::cerr << "Error: audio file not found: '" << audio_file_path << "'"
                  << std::endl;
      } else {
        ...
        if (!ok) {
          std::cerr << "Error: failed to load WAV file: '" << audio_file_path
                    << "'" << std::endl;
        } else {
          if (wav_sr != acfg.sample_rate) {
            std::cerr << "Info: resampling audio from " << wav_sr << " Hz to "
                      << acfg.sample_rate << " Hz" << std::endl;
            audio_file_samples =
                nf_resample_linear(wav_f, wav_sr, acfg.sample_rate);
          } else {
            audio_file_samples = std::move(wav_f);
          }
          std::cerr << "Info: loaded audio file samples="
                    << audio_file_samples.size() << std::endl;
        }
      }
    }
    ...
    // Apply shaping configuration
    {
      ...
      if (maze_shaping == "euclid")
        mode = MazeEnv::ShapingMode::Euclid;
      else if (maze_shaping == "manhattan")
        mode = MazeEnv::ShapingMode::Manhattan;
    ...
    if (qlearning) {
      qtable.assign(
          static_cast<std::size_t>(maze_env.size() * maze_env.size() * 4),
          0.0f);
    }
#ifdef NF_HAVE_OPENCV
    cv::VideoCapture cap;
    ...
    if (vision_demo && vision_source == "camera") {
      std::cerr << "Info: Opening camera index=" << camera_index
                << " backend=" << camera_backend << std::endl;
      ...
      if (cap.isOpened()) {
        ...
        std::cerr
            << "Info: Camera not available; falling back to synthetic vision"
            << std::endl;
      }
    }
#endif
    ...
    std::chrono::steady_clock::time_point last_heatmap =
    ...
    if (maze_view) {
      ...
      // Verify the window actually exists; getWindowProperty returns -1 when
      // the window does not exist
      maze_window_created =
          (cv::getWindowProperty("Maze", cv::WND_PROP_VISIBLE) != -1);
      if (!maze_window_created) {
        std::cerr << "Warning: Failed to create OpenCV window 'Maze'. "
                     "Disabling maze view."
                  << std::endl;
        maze_view = false;
      }
    ...
    if (memory_db_path.empty()) {
      ...
      if (env_telemetry && *env_telemetry) {
        memory_db_path = env_telemetry;
        std::cerr << "Info: Using NF_TELEMETRY_DB for MemoryDB path ('"
                  << memory_db_path << "')" << std::endl;
      }
    }
    if (!memory_db_path.empty()) {
      ...
      if (!memdb->open()) {
        std::cerr << "Warning: failed to open memory DB at '" << memory_db_path
                  << "' (SQLite3 may be unavailable)." << std::endl;
        memdb.reset();
      } else {
        std::string meta =
            std::string("{\"argv_size\":") + std::to_string(argc) + "}";
        if (!memdb->beginRun(meta, memdb_run_id)) {
          std::cerr
              << "Warning: failed to begin run in memory DB; logging disabled."
              << std::endl;
          memdb.reset();
        } else {
          std::cerr << "Info: Memory DB logging enabled at '" << memory_db_path
                    << "' (run=" << memdb_run_id << ")" << std::endl;
          ...
          // Emit lifecycle start marker for run_events
          {
            std::int64_t ts_ms =
                std::chrono::duration_cast<std::chrono::milliseconds>(
                    ...
                    .count();
            std::int64_t event_id = 0;
            (void)memdb->insertRunEvent(memdb_run_id, ts_ms, 0ULL,
                                        std::string("start"),
                                        std::string("main_start"), 0,
                                        nf_process_rss_mb(), 0.0, event_id);
          }
          // Initialize RSS thresholds from environment
          if (const char *p = std::getenv("NF_RSS_WARN_MB")) {
            try {
              rss_warn_threshold_mb = std::stod(p);
            } catch (...) {
            }
          }
          if (const char *p = std::getenv("NF_RSS_FAIL_MB")) {
            try {
              rss_fail_threshold_mb = std::stod(p);
            } catch (...) {
            }
          }
          if (const char *p = std::getenv("NF_RSS_WARN_INTERVAL_MS")) {
            try {
              rss_warn_interval_ms = std::max(1, std::stoi(p));
            } catch (...) {
            }
          }
          ...
          if (env_assert && *env_assert && std::string(env_assert) != "0") {
            try {
              brain.logReward(0.0, "engine_init",
                              "{\"assert\":\"NF_ASSERT_ENGINE_DB\"}");
              std::cerr
                  << "Info: Seeded reward_log entry for telemetry assertion."
                  << std::endl;
              // Also seed a minimal learning_stats entry to satisfy smoke
              // assertions
              using namespace std::chrono;
              std::int64_t ts_ms = duration_cast<milliseconds>(
                                       system_clock::now().time_since_epoch())
                                       .count();
              NeuroForge::Core::LearningSystem::Statistics
                  st{}; // minimal default stats
              (void)memdb->insertLearningStats(ts_ms, 0ULL, 0.0f, st,
                                               memdb_run_id);
              std::cerr << "Info: Seeded learning_stats entry for telemetry "
                           "assertion."
                        << std::endl;
            } catch (...) {
          ...
          NeuroForge::Core::NF_InitContext(context_gain, context_update_ms,
                                           context_window);
          std::cout << "[Context] Initialized (gain=" << context_gain
                    << ", update_ms=" << context_update_ms
                    << ", window=" << context_window << ")" << std::endl;
          ...
          for (const auto &spec : context_peer_args) {
            ...
            try {
              ...
              if (parts.size() >= 2 && !parts[1].empty())
                gain = std::stod(parts[1]);
              if (parts.size() >= 3 && !parts[2].empty())
                update_ms = std::stoi(parts[2]);
              if (parts.size() >= 4 && !parts[3].empty())
                window = std::stoi(parts[3]);
              if (parts.size() >= 5)
                label = parts[4];
              if (name.empty()) {
                std::cerr
                    << "Warning: --context-peer ignored due to empty name ('"
                    << spec << "')" << std::endl;
                continue;
              }
              NeuroForge::Core::NF_RegisterContextPeer(name, gain, update_ms,
                                                       window);
              if (!label.empty())
                context_peer_labels[name] =
                    label; // store label for sampling/logging
              std::cout << "[Context] Peer registered: name='" << name
                        << "' gain=" << gain << " update_ms=" << update_ms
                        << " window=" << window;
              if (!label.empty())
                std::cout << " label='" << label << "'";
              std::cout << std::endl;
            } catch (...) {
              std::cerr << "Warning: failed to parse --context-peer='" << spec
                        << "'" << std::endl;
            }
          }
          ...
          for (const auto &spec : context_coupling_args) {
            ...
            try {
              ...
              if (colon == std::string::npos) {
                std::cerr
                    << "Warning: --context-couple requires src:dst[,weight] ('"
                    << spec << "')" << std::endl;
                continue;
              }
              ...
              if (comma == std::string::npos) {
                ...
                std::string wstr = rest.substr(comma + 1);
                if (!wstr.empty())
                  w = std::stod(wstr);
              }
              if (src.empty() || dst.empty()) {
                std::cerr << "Warning: --context-couple ignored due to empty "
                             "src/dst ('"
                          << spec << "')" << std::endl;
                continue;
              }
              NeuroForge::Core::NF_SetContextCoupling(src, dst, w);
              std::cout << "[Context] Coupling set: '" << src << "' -> '" << dst
                        << "' (w=" << w << ")" << std::endl;
            } catch (...) {
              std::cerr << "Warning: failed to parse --context-couple='" << spec
                        << "'" << std::endl;
            }
          }
        }
    ...
    if (unified_substrate_enable) {
      std::cerr << "Info: Running unified substrate mode (WM + Phase C + "
                   "SurvivalBias + Language)"
                << std::endl;
      // Collect per-step substrate stats for optional JSON export
      // CohRow holds per-step coherence metrics and the associated MemoryDB run
      // identifier
      struct CohRow {
        long long ts_ms;
        int step;
      ...
      // Minimal Adaptive Reflection module: modulates SurvivalBias config based
      // on coherence and growth velocity
      struct AdaptiveReflection {
        ...
        int last_bindings = 0;
        void apply(int step, double avg_coh, int assemblies, int bindings,
                   std::shared_ptr<NeuroForge::Biases::SurvivalBias> &sb,
                   NeuroForge::Core::LearningSystem *ls) {
          if (last_apply_step >= 0 && (step - last_apply_step) < apply_interval)
            return;
          ...
          if (avg_coh < low_thresh) {
            if (sb) {
              ...
              cfg.hazard_coherence_weight =
                  std::max(0.0f, cfg.hazard_coherence_weight * 0.85f);
              ...
              std::cerr << "[AdaptiveReflection] coh=" << avg_coh
                        << " ↓ → reduce risk weighting (hazard_weight="
                        << cfg.hazard_coherence_weight << ") step=" << step
                        << std::endl;
            }
            ...
            if (ls) {
              ...
              std::cerr << "[AdaptiveReflection] learning_rate↑=" << new_lr
                        << std::endl;
            }
          }
          // Coherence high & little growth → increase exploration by increasing
          // variance sensitivity slightly
          else if (avg_coh > high_thresh && (d_asm + d_bnd) <= 0) {
            if (sb) {
              auto cfg = sb->getConfig();
              cfg.variance_sensitivity =
                  std::min(2.0f, cfg.variance_sensitivity * 1.05f);
              sb->updateConfig(cfg);
              std::cerr << "[AdaptiveReflection] coh=" << avg_coh
                        << " ↑ & no growth → increase variance sensitivity ("
                        << cfg.variance_sensitivity << ") step=" << step
                        << std::endl;
            }
          ...
          // Mid regime → gently relax towards defaults to avoid drift
            ...
            if (sb) {
              auto cfg = sb->getConfig();
              cfg.hazard_coherence_weight =
                  std::min(1.0f, cfg.hazard_coherence_weight * 1.02f);
              cfg.variance_sensitivity =
                  std::max(1.0f, cfg.variance_sensitivity * 0.98f);
              sb->updateConfig(cfg);
            }
          }
        }
      ...
      wm_cfg.neurons_per_region =
          (unified_wm_neurons > 0 ? unified_wm_neurons : 64);
      // Wrap existing brain in a non-owning shared_ptr for APIs that expect
      // shared_ptr
      auto brain_shared = std::shared_ptr<NeuroForge::Core::HypergraphBrain>(
          &brain, [](NeuroForge::Core::HypergraphBrain *) {});
      auto wm = std::make_shared<NeuroForge::Core::SubstrateWorkingMemory>(
          brain_shared, wm_cfg);
      if (!wm->initialize()) {
        std::cerr << "ERROR: SubstrateWorkingMemory initialize failed"
                  << std::endl;
        return 3;
      }
      ...
      pc_cfg.neurons_per_region =
          (unified_phasec_neurons > 0 ? unified_phasec_neurons : 64);
      if (phase_c_binding_threshold_set)
        pc_cfg.binding_threshold = phase_c_binding_threshold;
      if (phase_c_sequence_threshold_set)
        pc_cfg.sequence_threshold = phase_c_sequence_threshold;
      if (phase_c_binding_coherence_min_set)
        pc_cfg.binding_coherence_min = phase_c_binding_coherence_min;
      if (phase_c_sequence_coherence_min_set)
        pc_cfg.sequence_coherence_min = phase_c_sequence_coherence_min;
      if (phase_c_prune_coherence_threshold_set)
        pc_cfg.prune_coherence_threshold = phase_c_prune_coherence_threshold;
      auto phaseC = std::make_unique<NeuroForge::Core::SubstratePhaseC>(
          brain_shared, wm, pc_cfg);
      ...
      auto language_system_local =
          std::make_shared<NeuroForge::Core::LanguageSystem>(ls_cfg);
      NeuroForge::Core::SubstrateLanguageIntegration::Config lang_cfg{};
      auto lang =
          std::make_shared<NeuroForge::Core::SubstrateLanguageIntegration>(
              language_system_local, brain_shared, lang_cfg);
      if (!lang->initialize()) {
        std::cerr << "ERROR: SubstrateLanguageIntegration initialize failed"
                  << std::endl;
        return 5;
      }
      ...
      for (int s = 0; s < unified_steps; ++s) {
        ...
        // Update global last step for accurate lifecycle logging in unified
        // substrate mode
              ...
              if (st_opt.has_value()) {
                auto ts_ms = static_cast<std::uint64_t>(
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::system_clock::now().time_since_epoch())
                        .count());
                memdb->insertLearningStats(ts_ms, static_cast<std::uint64_t>(s),
                                           hz, st_opt.value(), memdb_run_id);
                std::int64_t event_id = 0;
                (void)memdb->insertRunEvent(
                    memdb_run_id, static_cast<std::int64_t>(ts_ms),
                    static_cast<std::uint64_t>(s), std::string("heartbeat"),
                    std::string(), 0, nf_process_rss_mb(), 0.0, event_id);
                // Optional memory pressure notifications (RSS)
                double rss_mb = nf_process_rss_mb();
                if (rss_warn_threshold_mb > 0.0 &&
                    rss_mb >= rss_warn_threshold_mb) {
                  auto now_warn = std::chrono::steady_clock::now();
                  if (last_rss_warn.time_since_epoch().count() == 0 ||
                      std::chrono::duration_cast<std::chrono::milliseconds>(
                          now_warn - last_rss_warn)
                              .count() >= rss_warn_interval_ms) {
                    std::int64_t warn_id = 0;
                    (void)memdb->insertRunEvent(
                        memdb_run_id, static_cast<std::int64_t>(ts_ms),
                        static_cast<std::uint64_t>(s), std::string("warning"),
                        std::string("rss_threshold_exceeded"), 0, rss_mb, 0.0,
                        warn_id);
                    last_rss_warn = now_warn;
                  }
                }
                if (rss_fail_threshold_mb > 0.0 &&
                    rss_mb >= rss_fail_threshold_mb) {
                  std::int64_t err_id = 0;
                  (void)memdb->insertRunEvent(
                      memdb_run_id, static_cast<std::int64_t>(ts_ms),
                      static_cast<std::uint64_t>(s), std::string("error"),
                      std::string("rss_fail_threshold_exceeded"), 0, rss_mb,
                      0.0, err_id);
                }
              }
            }
        ...
        // Per-step capture for export
        {
          ...
          long long ts_ms = static_cast<long long>(
              std::chrono::duration_cast<std::chrono::milliseconds>(
                  std::chrono::system_clock::now().time_since_epoch())
                  .count());
          int prevAsm_capture = (coh_rows.size() >= 1)
                                    ? coh_rows.back().assemblies
                                    : static_cast<int>(stats.assemblies_formed);
          int prevBnd_capture = (coh_rows.size() >= 1)
                                    ? coh_rows.back().bindings
                                    : static_cast<int>(stats.bindings_created);
          double growth = static_cast<double>(
              (static_cast<int>(stats.assemblies_formed) - prevAsm_capture) +
              (static_cast<int>(stats.bindings_created) - prevBnd_capture));
          // Capture current metrics into row with 64-bit run_id to avoid
          // narrowing
          CohRow row{ts_ms,
                     s,
                     static_cast<double>(stats.average_coherence),
                     static_cast<int>(stats.assemblies_formed),
                     static_cast<int>(stats.bindings_created),
                     growth,
                     memdb_run_id};
          ...
          if (adaptive_enable) {
            ...
            int prevAsm_reflect = (coh_rows.size() >= 2)
                                      ? coh_rows[coh_rows.size() - 2].assemblies
                                      : row.assemblies;
            int prevBnd_reflect = (coh_rows.size() >= 2)
                                      ? coh_rows[coh_rows.size() - 2].bindings
                                      : row.bindings;
            ...
            if ((s - adapt.last_apply_step) >= adapt.apply_interval) {
              if (row.avg_coh < adapt.low_thresh)
                adaptive_low_events++;
              else if (row.avg_coh > adapt.high_thresh && (dAsm + dBnd) <= 0)
                adaptive_high_events++;
            }
            // Apply reflection (may adjust SurvivalBias config and LR)
            adapt.apply(s, row.avg_coh, row.assemblies, row.bindings,
                        survival_bias, learning_system);
          }
        }
        ...
        if (((s + 1) % 250) == 0) {
          ...
          std::vector<std::size_t> asm_sizes;
          asm_sizes.reserve(assemblies.size());
          ...
          std::cout << "[Unified Metrics] step=" << (s + 1)
                    ...
                    << " | language_coherence="
                    << l_stats.substrate_language_coherence
                    << " binding_strength_avg="
                    << l_stats.average_binding_strength
                    << " tokens=" << l_stats.total_neural_tokens
                    << " patterns=" << l_stats.active_neural_patterns
                    << " energy=" << ls.avg_energy
                    << " metabolic_hazard=" << ls.metabolic_hazard << std::endl;
        }
        ...
        if (((s + 1) % 500) == 0) {
          ...
          if (!assemblies.empty()) {
            ...
            for (const auto &a : assemblies) {
              if (a.coherence_score > best->coherence_score)
                best = &a;
            }
            if (best->coherence_score > 0.84f) {
              double trust = 0.0;
              if (phase9_metacog)
                trust = phase9_metacog->getSelfTrust();
              std::cout << "[MIND:" << 0 << " C=" << std::fixed
                        << std::setprecision(4) << best->coherence_score
                        << " N=" << best->neurons.size() << " T=" << (s + 1)
                        << " Trust=" << std::fixed << std::setprecision(4)
                        << trust << "]" << std::endl;
            }
          }
        }
      ...
      std::cout << "Language: substrate_language_coherence="
                << stL.substrate_language_coherence
      ...
      try {
        ...
        for (std::size_t i = 0; i < coh_rows.size(); ++i) {
          ...
          ofs << "    { \"ts_ms\": " << r.ts_ms << ", \"step\": " << r.step
              << ", \"avg_coherence\": " << std::fixed << std::setprecision(6)
              << r.avg_coh << ", \"assemblies\": " << r.assemblies
              << ", \"bindings\": " << r.bindings
        ...
        ofs << "  ]\n}";
        ofs.close();
        std::cerr << "[Export] substrate_states written to " << out.string()
                  << std::endl;
      } catch (...) {
        std::cerr
            << "[Export] Warning: failed to write substrate_states.json (web)"
            << std::endl;
      }
      return 0;
    }
    ...
    if (phase6_enable && memdb && memdb_run_id > 0) {
      try {
        phase6_reasoner = std::make_unique<NeuroForge::Core::Phase6Reasoner>(
            memdb.get(), memdb_run_id);
        if (self_model) {
        ...
        if (phase6_active_mode == "audit") {
          ...
          std::cout << "[Phase 6] Reasoner enabled (active, margin="
                    << phase6_margin << ")" << std::endl;
        } else {
          std::cout << "[Phase 6] Reasoner enabled (shadow logging)"
                    << std::endl;
        }
      } catch (...) {
        std::cerr << "Warning: Failed to initialize Phase 6 Reasoner"
                  << std::endl;
      }
    }
    ...
    if ((init_affect || init_reflect) && memdb && memdb_run_id > 0) {
      try {
        if (init_affect) {
          phase7_affect =
              std::make_unique<NeuroForge::Core::Phase7AffectiveState>(
                  memdb.get(), memdb_run_id);
          std::cout << "[Phase 7] Affective State initialized" << std::endl;
        }
        if (init_reflect) {
          phase7_reflect = std::make_unique<NeuroForge::Core::Phase7Reflection>(
              memdb.get(), memdb_run_id);
        ...
        if (phase6_reasoner && (phase7_affect || phase7_reflect)) {
          phase6_reasoner->setPhase7Components(phase7_affect.get(),
                                               phase7_reflect.get());
          std::cout << "[Phase 7] Bridged to Phase 6 Reasoner" << std::endl;
        }
      } catch (...) {
        std::cerr << "Warning: Failed to initialize Phase 7 components"
                  << std::endl;
      }
    }
    ...
    if (phase8_enable && memdb && memdb_run_id > 0) {
      try {
        phase8_goals = std::make_unique<NeuroForge::Core::Phase8GoalSystem>(
            memdb, memdb_run_id);
        ...
        if (phase9_enable) {
          phase9_metacog =
              std::make_unique<NeuroForge::Core::Phase9Metacognition>(
                  memdb.get(), memdb_run_id);
          std::cout << "[Phase 9] Metacognition initialized" << std::endl;
          std::cout << "[Phase 9] Metacognition active (modulation="
                    << (phase9_modulation_enable ? "on" : "off") << ")"
                    << std::endl;
          ...
          if (phase8_goals && phase9_metacog) {
            ...
            std::cout << "[Phase 9] Bridged to Phase 8 Goal System"
                      << std::endl;
          }
          if (phase6_reasoner && phase9_metacog && phase9_modulation_enable) {
            phase6_reasoner->setPhase9Metacognition(phase9_metacog.get());
            std::cout << "[Phase 9] Modulation bridged to Phase 6 Reasoner"
                      << std::endl;
          }
          // Initialize Phase 10 Self-Explanation and inject into Phase 9
          if (phase10_enable && phase9_metacog) {
            phase10_selfexplainer =
                std::make_unique<NeuroForge::Core::Phase10SelfExplanation>(
                    memdb.get(), memdb_run_id);
            phase9_metacog->setPhase10SelfExplanation(
                phase10_selfexplainer.get());
            std::cout << "[Phase 10] Self-Explanation initialized and injected "
                         "into Phase 9"
                      << std::endl;
          }
          ...
          if (phase13_enable && phase9_metacog) {
            ...
            p13cfg.contraction_hysteresis_ms =
                phase13_contraction_hysteresis_ms;
            ...
            phase13_autonomy =
                std::make_unique<NeuroForge::Core::Phase13AutonomyEnvelope>(
                    memdb.get(), memdb_run_id, p13cfg);
            phase9_metacog->setPhase13AutonomyEnvelope(phase13_autonomy.get());
            std::cout << "[Phase 13] Autonomy Envelope initialized and "
                         "injected into Phase 9 (window="
                      << phase13_window << ")" << std::endl;
          }
          ...
          if (phase14_enable && phase9_metacog) {
            ...
            phase14_metareason =
                std::make_unique<NeuroForge::Core::Phase14MetaReasoner>(
                    memdb.get(), memdb_run_id, p14cfg);
            phase9_metacog->setPhase14MetaReasoner(phase14_metareason.get());
            std::cout << "[Phase 14] Meta-Reasoner initialized and injected "
                         "into Phase 9 (window="
                      << phase14_window << ")" << std::endl;
          }
          ...
          if (phase15_enable && phase9_metacog) {
            ...
            phase15_ethics =
                std::make_unique<NeuroForge::Core::Phase15EthicsRegulator>(
                    memdb.get(), memdb_run_id, p15cfg);
            phase9_metacog->setPhase15EthicsRegulator(phase15_ethics.get());
            std::cout << "[Phase 15] Ethics Regulator initialized and injected "
                         "into Phase 9 (window="
                      << phase15_window
                      << ", risk_threshold=" << phase15_risk_threshold << ")"
                      << std::endl;
          }
        }
      } catch (...) {
        std::cerr << "Warning: Failed to initialize Phase 8/9 systems"
                  << std::endl;
      }
    }
    ...
    if (phase5_language_enable || phase_a_enable) {
      ...
      language_system =
          std::make_shared<NeuroForge::Core::LanguageSystem>(lang_config);
      if (!language_system->initialize()) {
        std::cerr << "Warning: LanguageSystem failed to initialize"
                  << std::endl;
      } else {
    ...
    if (phase_a_enable) {
      NeuroForge::Core::PhaseAMimicry::Config phase_a_config =
          NeuroForge::Core::PhaseAMimicryFactory::createDefaultConfig();
      phase_a_config.negative_sampling_k = 5;
      phase_a_config.negative_weight = 0.2f;
      if (phase_a_negative_k_set) {
        phase_a_config.negative_sampling_k = phase_a_negative_k;
      }
      if (phase_a_negative_weight_set) {
        phase_a_config.negative_weight = phase_a_negative_weight;
      }
      // Apply CLI overrides for Phase A thresholds when provided
      if (phase_a_similarity_threshold_set) {
        phase_a_config.similarity_threshold = phase_a_similarity_threshold;
      }
      if (phase_a_novelty_threshold_set) {
        phase_a_config.novelty_threshold = phase_a_novelty_threshold;
      }
      if (phase_a_student_lr_set) {
        phase_a_config.student_learning_rate =
            static_cast<float>(phase_a_student_lr);
      }
      // Apply EMA stabilizer CLI overrides when provided
      if (phase_a_ema_enable_set) {
        phase_a_config.enable_ema_stabilizer = phase_a_ema_enable;
      }
      if (phase_a_ema_min_set) {
        phase_a_config.ema_alpha_min = static_cast<float>(phase_a_ema_min);
      }
      if (phase_a_ema_max_set) {
        phase_a_config.ema_alpha_max = static_cast<float>(phase_a_ema_max);
      }
      ...
      if (phase_a_replay_interval_set) {
        phase_a_config.replay_interval_steps =
            static_cast<std::size_t>(phase_a_replay_interval_steps);
      }
      if (phase_a_replay_top_k_set) {
        phase_a_config.replay_top_k =
            static_cast<std::size_t>(phase_a_replay_top_k);
      }
      if (phase_a_replay_boost_set) {
        phase_a_config.replay_boost_factor =
            static_cast<float>(phase_a_replay_boost);
      }
      if (phase_a_replay_lr_scale_set) {
        phase_a_config.replay_lr_scale =
            static_cast<float>(phase_a_replay_lr_scale);
      }
      if (phase_a_replay_include_hard_set) {
        phase_a_config.replay_include_hard_negatives =
            phase_a_replay_include_hard;
      }
      if (phase_a_replay_hard_k_set) {
        phase_a_config.replay_hard_k =
            static_cast<std::size_t>(phase_a_replay_hard_k);
      }
      if (phase_a_replay_repulsion_weight_set) {
        phase_a_config.replay_repulsion_weight =
            static_cast<float>(phase_a_replay_repulsion_weight);
      }
      ...
      // Compute mirror-mode implied dimension regardless, for conflict
      ...
      if (desired_dim <= 0) {
        if (mirror_mode == "vision") {
          ...
          derived_source = std::string("vision grid (") +
                           std::to_string(vcfg.grid_size) + "^2)";
        } else if (mirror_mode == "audio") {
          desired_dim = mirror_implied_dim;
          derived_source = "audio feature bins";
        }
      }
      // Conflict detection: if both teacher and mirror-mode are provided and
      // disagree, warn that teacher wins
      if (teacher_dim_available && mirror_implied_dim > 0 &&
          teacher_len != mirror_implied_dim) {
        std::cerr << "[Phase A][Warning] Teacher embedding length ("
                  << teacher_len
                  << ") differs from mirror-mode implied dimension ("
                  << mirror_implied_dim << "). Using teacher dimension.\n";
        // JSON event for conflict
        {
          std::ostringstream js;
          js << "{\"version\":1,\"phase\":\"A\",\"event\":\"conflict\","
                "\"time\":\""
             << json_escape(iso8601_utc_now()) << "\",";
        ...
        // Back-compat event for tests: phase_a_embed_conflict
        {
          ...
          js2 << "{\"version\":1,\"phase\":\"A\",\"time\":\""
              << json_escape(iso8601_utc_now()) << "\",";
          js2 << "\"t\":\"phase_a_embed_conflict\",\"teacher_len\":"
              << teacher_len << ",\"mirror_mode\":\""
              << json_escape(mirror_mode) << "\""
      ...
      const int final_dim =
          (desired_dim > 0) ? desired_dim : phase_a_config.embedding_dimension;
      std::cout << "[Phase A] Embedding dimension set to " << final_dim
                << " (derived from " << derived_source << ")\n";
      // JSON event for decision
      {
        std::ostringstream js;

        js << "{\"version\":1,\"phase\":\"A\",\"event\":\"decision\",\"time\":"
              "\""
           << json_escape(iso8601_utc_now()) << "\",";
        js << "\"payload\":{";
        js << "\"teacher_len\":" << teacher_len << ",";
      ...
      phase_a_system = NeuroForge::Core::PhaseAMimicryFactory::create(
          language_system, memdb, phase_a_config);
      if (!phase_a_system || !phase_a_system->initialize()) {
        std::cerr << "Warning: Phase A Mimicry failed to initialize"
                  << std::endl;
      } else {
        ...
        NeuroForge::Core::PhaseAMimicry::SubstrateMode p_mode =
            NeuroForge::Core::PhaseAMimicry::SubstrateMode::Off;
        if (substrate_mode == "mirror")
          p_mode = NeuroForge::Core::PhaseAMimicry::SubstrateMode::Mirror;
        else if (substrate_mode == "train")
          p_mode = NeuroForge::Core::PhaseAMimicry::SubstrateMode::Train;
        else if (substrate_mode == "native")
          p_mode = NeuroForge::Core::PhaseAMimicry::SubstrateMode::Native;
        phase_a_system->setSubstrateMode(p_mode);
        phase_a_system->setRewardScale(static_cast<float>(reward_scale));
        // Ensure teacher embedding is registered now that Phase A is
        // initialized
        if (!teacher_embed_path.empty()) {
          std::vector<float> tvec2;
          std::ifstream ifs(teacher_embed_path);
          if (!ifs) {
            std::cerr << "Warning: failed to load teacher embedding for Phase "
                         "A/LanguageSystem from '"
                      << teacher_embed_path << "'" << std::endl;
          } else {
            tvec2.clear();
            std::string line;
            ...
            if (!tvec2.empty()) {
              ...
              if (phase_a_system) {
                phase_a_system->addTeacherEmbedding(
                    tvec2, NeuroForge::Core::PhaseAMimicry::TeacherType::Custom,
                    NeuroForge::Core::PhaseAMimicry::Modality::Multimodal,
                    current_teacher_id, teacher_embed_path, 1.0f);
              }
            } else {
              std::cerr << "Warning: empty teacher embedding from '"
                        << teacher_embed_path << "'" << std::endl;
            }
          }
        }
    ...
    if (dataset_mode == "triplets" && !dataset_triplets_root.empty()) {
      auto items = scan_triplets_dataset(dataset_triplets_root, dataset_limit,
                                         dataset_shuffle);
      if (!items.empty()) {
        ...
        if (memdb && memdb_run_id > 0) {
          try {
            ...
            input << "\"teacher_id\":\""
                  << json_escape(std::string("triplet_") + it.stem) << "\"";
            input << "}";
            std::int64_t exp_id = -1;
            auto gs0 = brain.getGlobalStatistics();
            std::uint64_t step_pc0 =
                static_cast<std::uint64_t>(gs0.processing_cycles);
            bool ok_exp = memdb->insertExperience(
                ts_ms, step_pc0, "triplet_ingestion", input.str(),
                std::string(), true, memdb_run_id, exp_id);
            std::cout << "Triplet ingestion (initial): run=" << memdb_run_id
                      << " step=" << step_pc0 << " exp_id=" << exp_id
        ...
        if (!current_audio_path.empty()) {
          ...
          if (nf_load_wav_any_mono(current_audio_path, loaded, sr)) {
            if (sr != acfg.sample_rate && !loaded.empty()) {
              audio_file_samples =
                  nf_resample_linear(loaded, sr, acfg.sample_rate);
            } else {
        ...
        if (phase_a_system) {
          ...
          if (!current_image_path.empty())
            emb_v = phase_a_system->processCLIPVision(current_image_path);
          if (!current_audio_path.empty())
            emb_a = phase_a_system->processWhisperAudio(current_audio_path);
          if (!current_caption.empty())
            emb_t = phase_a_system->processBERTText(current_caption);
          std::size_t dim = 0;
          ...
          if (dim > 0) {
            ...
            int count = 0;
            if (!emb_v.empty()) {
              for (std::size_t i = 0; i < dim && i < emb_v.size(); ++i)
                teacher_mm[i] += emb_v[i];
              ++count;
            }
            if (!emb_a.empty()) {
              for (std::size_t i = 0; i < dim && i < emb_a.size(); ++i)
                teacher_mm[i] += emb_a[i];
              ++count;
            }
            if (!emb_t.empty()) {
              for (std::size_t i = 0; i < dim && i < emb_t.size(); ++i)
                teacher_mm[i] += emb_t[i];
              ++count;
            }
            if (count > 1) {
              for (std::size_t i = 0; i < dim; ++i)
                teacher_mm[i] = teacher_mm[i] / static_cast<float>(count);
            }
            std::string cid = std::string("triplet_") + it.stem;
            phase_a_system->addTeacherEmbedding(
                teacher_mm,
                NeuroForge::Core::PhaseAMimicry::TeacherType::CLIP_Vision,
                NeuroForge::Core::PhaseAMimicry::Modality::Multimodal, cid,
                current_caption, 1.0f);
            current_teacher_id = cid;
          }
        }
        std::cout << "Triplet dataset loaded: " << triplet_items.size()
                  << " items from '" << dataset_triplets_root << "'"
                  << std::endl;
      } else {
        std::cerr << "Warning: No valid triplets found under '"
                  << dataset_triplets_root << "'" << std::endl;
      }
    }
    ...
    {
      std::ostringstream js;
      js << "{\"version\":1,\"phase\":\"B\",\"event\":\"episode_start\","
            "\"time\":\""
         << json_escape(iso8601_utc_now()) << "\",";
      js << "\"episode_index\":" << episode_index
    ...
    // Initialize WorldModelCortex and SemanticProjection
    NeuroForge::Perception::WorldModelCortexConfig world_model_config;
    if (test_semantic_injection || test_semantic_n2 || test_semantic_n3 ||
        test_semantic_n4 || test_semantic_n5 || test_semantic_n6 ||
        test_n7_minimal || test_n7_bounded || test_n7_modelbased ||
        unified_bounded) {
      world_model_config.min_cycle_interval_ms = 0;
    }
    if (test_semantic_n3 || test_semantic_n4 || test_semantic_n5 ||
        test_semantic_n6 || test_n7_minimal || test_n7_bounded ||
        test_n7_modelbased || unified_bounded) {
      world_model_config.predictor_config.enable_learning = false;
    }
    if (test_semantic_n6 || test_n7_bounded || test_n7_modelbased ||
        unified_bounded) {
      world_model_config.enable_cognitive_regulation = true;
    }
    if (unified_bounded) {
      world_model_config.predictor_config.action_dim = 5;
    }
    // Configure world model defaults if needed
    auto world_model_cortex =
        std::make_shared<NeuroForge::Perception::WorldModelCortex>(
            world_model_config);

    NeuroForge::Perception::SemanticProjectionConfig sem_proj_config;
    auto semantic_projector =
        std::make_shared<NeuroForge::Perception::SemanticProjection>(
            sem_proj_config);
    std::cout << "[Init] WorldModelCortex and SemanticProjection initialized."
              << std::endl;

    if (test_semantic_injection) {
      NeuroForge::Runtime::SemanticInjectionConfig cfg;
      cfg.steps = 1000;
      cfg.log_csv_path = "semantic_injection_log.csv";
      return NeuroForge::Runtime::runSemanticInjection(
          *world_model_cortex, *semantic_projector, cfg);
    }
    if (test_semantic_n2) {
      NeuroForge::Runtime::SemanticN2Config cfg;
      cfg.steps = 1000;
      cfg.log_csv_path = "semantic_n2_log.csv";
      return NeuroForge::Runtime::runSemanticN2(*world_model_cortex,
                                                *semantic_projector, cfg);
    }
    if (test_semantic_n3) {
      NeuroForge::Runtime::SemanticN3Config cfg;
      cfg.steps = 1000;
      cfg.log_csv_path = "semantic_n3_log.csv";
      return NeuroForge::Runtime::runSemanticN3(*world_model_cortex,
                                                *semantic_projector, cfg);
    }

    if (test_semantic_n4) {
      NeuroForge::Runtime::SemanticN4Config cfg;
      cfg.steps = 1000;
      cfg.log_csv_path = "semantic_n4_log.csv";
      return NeuroForge::Runtime::runSemanticN4(*world_model_cortex,
                                                *semantic_projector, cfg);
    }
    if (test_semantic_n5) {
      NeuroForge::Runtime::SemanticN5Config cfg;
      cfg.steps = 1000;
      cfg.log_csv_path = "semantic_n5_log.csv";
      return NeuroForge::Runtime::runSemanticN5(*world_model_cortex,
                                                *semantic_projector, cfg);
    }
    if (test_semantic_n6) {
      NeuroForge::Runtime::SemanticN6Config cfg;
      cfg.steps = 1000;
      cfg.log_csv_path = "semantic_n6_log.csv";
      return NeuroForge::Runtime::runSemanticN6(*world_model_cortex,
                                                *semantic_projector, cfg);
    }

    if (test_n7_minimal) {
      NeuroForge::Runtime::N7MinimalConfig cfg;
      cfg.max_steps = 1000;
      cfg.seed = 7;
      cfg.collision_budget = 250;
      cfg.log_csv_path = "n7_minimal_gridworld_log.csv";
      return NeuroForge::Runtime::runN7Minimal(*world_model_cortex, cfg);
    }

    if (test_n7_bounded) {
      NeuroForge::Runtime::N7BoundedConfig cfg;
      cfg.max_steps = 1000;
      cfg.max_episode_steps = 64;
      cfg.seed = 7;
      cfg.collision_budget = 50;
      cfg.log_csv_path = "n7_bounded_gridworld_log.csv";
      return NeuroForge::Runtime::runN7Bounded(*world_model_cortex, cfg);
    }

    if (test_n7_modelbased) {
      NeuroForge::Runtime::N7ModelBasedConfig cfg;
      cfg.max_steps = 1000;
      cfg.warmup_steps = 50;
      cfg.max_episode_steps = 64;
      cfg.seed = 7;
      cfg.collision_budget = 50;
      cfg.action_dim = 5;
      cfg.log_csv_path = "n7_modelbased_gridworld_log.csv";
      return NeuroForge::Runtime::runN7ModelBased(*world_model_cortex, cfg);
    }

    if (unified_bounded) {
      NeuroForge::Runtime::UnifiedBoundedConfig cfg;
      cfg.max_steps = unified_bounded_steps;
      cfg.warmup_steps = unified_bounded_warmup_steps;
      cfg.max_episode_steps = unified_bounded_episode_steps;
      cfg.collision_budget = unified_bounded_collision_budget;
      cfg.seed = unified_bounded_seed;
      cfg.action_dim = 5;
      cfg.log_csv_path = unified_bounded_log_csv_path;
      cfg.summary_csv_path = unified_bounded_summary_csv_path;
      cfg.stage_d3_live = unified_bounded_live;
      cfg.stage_d3_live_mode = unified_bounded_live_mode;
      cfg.stage_d3_match_window = unified_bounded_live_match_window;
      cfg.stage_d3_min_match_rate = unified_bounded_live_min_match;
      cfg.stage_d3_min_advantage = unified_bounded_live_min_advantage;
      cfg.stage_d3_min_pred_dist_advantage =
          unified_bounded_live_min_pred_dist_advantage;
      cfg.stage_d3_max_pred_collision = unified_bounded_live_max_collision_prob;
      cfg.stage_d3_min_dist_improve = unified_bounded_live_min_dist_improve;

      NeuroForge::Runtime::UnifiedBoundedSinks sinks;
      if (log_json) {
        sinks.emit_json_line = [&](const std::string &line) {
          emit_json_line(log_json, log_json_path, line);
        };
      }
      if (memdb && memdb_run_id > 0) {
        sinks.insert_experience = [&](std::int64_t ts_ms, std::uint64_t step,
                                      const std::string &event,
                                      const std::string &payload_json) {
          std::int64_t exp_id = -1;
          (void)memdb->insertExperience(ts_ms, step, event, payload_json,
                                        std::string(), false, memdb_run_id,
                                        exp_id);
        };
      }

      return NeuroForge::Runtime::runUnifiedBounded(
          *world_model_cortex, memdb.get(), memdb_run_id, log_json, sinks, cfg);
    }

    if (run_n8) {
      NeuroForge::Runtime::N8HumanGatedSinks sinks;
      if (log_json) {
        sinks.emit_json_line = [&](const std::string &line) {
          emit_json_line(log_json, log_json_path, line);
        };
      }
      return NeuroForge::Runtime::runN8HumanGated(*world_model_cortex,
                                                  memdb.get(), memdb_run_id,
                                                  log_json, sinks, n8_cfg);
    }

    if (run_n9) {
      NeuroForge::Runtime::N9AutomatedGatedSinks sinks;
      if (log_json) {
        sinks.emit_json_line = [&](const std::string &line) {
          emit_json_line(log_json, log_json_path, line);
        };
      }
      return NeuroForge::Runtime::runN9AutomatedGated(*world_model_cortex,
                                                      memdb.get(), memdb_run_id,
                                                      log_json, sinks, n9_cfg);
    }

    if (run_alive) {
      NeuroForge::Runtime::UnifiedAliveSinks sinks;
      if (log_json) {
        sinks.emit_json_line = [&](const std::string &line) {
          emit_json_line(log_json, log_json_path, line);
        };
      }
      return NeuroForge::Runtime::runUnifiedAlive(
          *world_model_cortex, memdb.get(), memdb_run_id, log_json, sinks,
          alive_cfg, g_abort);
    }

    if (run_web) {
      NeuroForge::Runtime::WebSandboxSinks sinks;
      if (log_json) {
        sinks.emit_json_line = [&](const std::string &line) {
          emit_json_line(log_json, log_json_path, line);
        };
      }
      return NeuroForge::Runtime::runWebSandbox(*world_model_cortex,
                                                memdb.get(), memdb_run_id,
                                                log_json, sinks, web_cfg);
    }

    if (run_genesis) {
      NeuroForge::Runtime::GrandUnifiedSinks sinks;
      if (log_json) {
        sinks.emit_json_line = [&](const std::string &line) {
          emit_json_line(log_json, log_json_path, line);
        };
      }
      return NeuroForge::Runtime::runGrandUnified(
          *world_model_cortex, memdb.get(), memdb_run_id, log_json, sinks,
          genesis_cfg, g_abort);
    }
    ...
    // This prevents main() from exiting immediately when autonomous thread is
    // running
    if (brain.isAutonomousModeEnabled()) {
      ...
      while (autonomous_running.load()) {
        ...
        if (dataset_active && !triplet_items.empty()) {
          std::size_t idx =
              static_cast<std::size_t>(mimicry_counter % triplet_items.size());
          if (idx != dataset_index) {
            ...
            if (memdb && memdb_run_id > 0) {
              try {
                ...
                std::uint64_t step_pc1 =
                    static_cast<std::uint64_t>(gs1.processing_cycles);
                bool ok_exp = memdb->insertExperience(
                    ts_ms, step_pc1, "triplet_ingestion", input.str(),
                    std::string(), true, memdb_run_id, exp_id);
                std::cout << "Triplet ingestion (step): run=" << memdb_run_id
                          << " step=" << step_pc1 << " exp_id=" << exp_id
                          << " ok=" << (ok_exp ? 1 : 0) << std::endl;
                if (current_episode_id > 0 && exp_id > 0) {
                  (void)memdb->linkExperienceToEpisode(exp_id,
                                                       current_episode_id);
                }
              } catch (...) {
              }
            ...
            if (phase_a_system) {
              ...
              if (!current_caption.empty())
                emb_t = phase_a_system->processBERTText(current_caption);
              std::size_t dim =
                  std::max({emb_v.size(), emb_a.size(), emb_t.size()});
              if (dim > 0) {
                std::vector<float> teacher_mm(dim, 0.0f);
                int count = 0;
                if (!emb_v.empty()) {
                  for (std::size_t j = 0; j < dim && j < emb_v.size(); ++j)
                    teacher_mm[j] += emb_v[j];
                  ++count;
                }
                if (!emb_a.empty()) {
                  for (std::size_t j = 0; j < dim && j < emb_a.size(); ++j)
                    teacher_mm[j] += emb_a[j];
                  ++count;
                }
                if (!emb_t.empty()) {
                  for (std::size_t j = 0; j < dim && j < emb_t.size(); ++j)
                    teacher_mm[j] += emb_t[j];
                  ++count;
                }
                if (count > 1) {
                  for (std::size_t j = 0; j < dim; ++j)
                    teacher_mm[j] = teacher_mm[j] / static_cast<float>(count);
                }
                std::string cid = std::string("triplet_") + it.stem;
                phase_a_system->addTeacherEmbedding(
                    teacher_mm,
        ...
        if (phase_a_enable && phase_a_system) {
          ++mimicry_counter;
          if (auto *t =
                  phase_a_system->getTeacherEmbedding(current_teacher_id)) {
            std::vector<float> empty_student_vec;
            auto attempt = phase_a_system->attemptMimicry(
                empty_student_vec, current_teacher_id,
                std::string("autonomous_step"));
            if (!mimicry_internal) {
            ...
            phase_a_last_stu_len =
                static_cast<int>(attempt.student_embedding.size());
            phase_a_last_tea_len =
                static_cast<int>(attempt.teacher_embedding.size());
            {
              double d = 0.0, ns = 0.0, nt = 0.0;
              std::size_t n = std::min(attempt.student_embedding.size(),
                                       attempt.teacher_embedding.size());
            ...
            if (auto *ls_mim = brain.getLearningSystem()) {
              ls_mim->setMimicryAttemptScores(
                  phase_a_last_similarity, phase_a_last_novelty,
                  phase_a_last_reward, phase_a_last_success);
            }
            if (memdb && g_memdb_run_id > 0) {
              ...
              js << "\"last_success\":"
                 << (phase_a_last_success ? "true" : "false") << ",";
              ...
              std::uint64_t step_pcX =
                  static_cast<std::uint64_t>(gsX.processing_cycles);
              (void)memdb->insertExperience(ts_ms, step_pcX, "snapshot:phase_a",
                                            js.str(), std::string(), false,
                                            g_memdb_run_id, exp_id);
            }
          }
        }
      }
      ...
      if ((test_semantic_n3 || test_semantic_n4 || test_semantic_n5 ||
           test_semantic_n6 || test_n7_minimal) &&
          steps < 1000) {
        steps = 1000;
      }
      ...
      // ActionFilter (Safety Gate): baseline thresholds and cooldowns for
      // sandbox actions
      int base_action_click_threshold =
          8; // base stability threshold before clicking
      int action_click_threshold =
          base_action_click_threshold;     // dynamically adjusted by Phase 13
                                           // decisions
      int action_scroll_cooldown_ms = 250; // minimal cooldown between scrolls
      auto last_action_tp =
          std::chrono::steady_clock::now(); // last sandbox action timepoint
      ...
      for (int i = 0; i < steps; ++i) {
        if (i == 0) {
          std::cerr << "[AutonomyDiag]"
                    ...
                    << " phase9=" << (phase9_metacog != nullptr) << std::endl;
        }
        g_last_step.store(static_cast<std::uint64_t>(i));

        // Integrate Semantic Projection into World Model Cortex
        if (g_semantic_memory && semantic_projector && world_model_cortex) {
          using namespace std::chrono;
          std::uint64_t now_ms = duration_cast<milliseconds>(
                                     steady_clock::now().time_since_epoch())
                                     .count();

          static bool semantic_surprise_cb_set = false;
          if (!semantic_surprise_cb_set) {
            world_model_cortex->setSurpriseCallback(
                [&](float surprise_level, float prediction_error) {
                  (void)surprise_level;
                  (void)prediction_error;
                });
            semantic_surprise_cb_set = true;
          }

          std::vector<NeuroForge::Perception::ActiveConcept> active_concepts;
          auto recent_nodes = g_semantic_memory->getRecentlyAccessedConcepts(
              now_ms > 10000 ? now_ms - 10000 : 0);
          active_concepts.reserve(recent_nodes.size());
          for (const auto &node : recent_nodes) {
            NeuroForge::Perception::ActiveConcept ac;
            ac.concept_id = node.label;
            ac.embedding = node.feature_vector;
            std::uint64_t age = now_ms - node.last_access_timestamp_ms;
            if (age > 10000) {
              age = 10000;
            }
            float recency_weight = 1.0f - (static_cast<float>(age) / 10000.0f);
            ac.activation =
                recency_weight * node.certainty * node.consolidation_strength;
            ac.grounding_confidence = node.consolidation_strength;
            ac.predictive_power = node.certainty;
            ac.last_grounded_ms = node.last_access_timestamp_ms;
            active_concepts.push_back(ac);
          }

          std::vector<float> semantic_input =
              semantic_projector->projectFromConcepts(active_concepts, now_ms);
          (void)world_model_cortex->processCycle({}, {}, {}, {}, {}, {}, {},
                                                 semantic_input);
        }
        ...
        if (phase15_ethics) {
          try {
            p15_decision =
                phase15_ethics->runForLatest(std::string("sandbox_action"));
          } catch (...) {
          }
        }
        ...
        if (phase13_autonomy) {
          try {
            p13_decision = phase13_autonomy->maybeAdjustEnvelope(
        ...
        if (memdb && memdb_run_id > 0 && self_model && phase9_metacog) {
          ...
          latest_autonomy_envelope = NeuroForge::Core::ComputeAutonomyEnvelope(
              a_inputs, ts_ms, g_last_step.load(),
              std::string("sandbox_action"));
          (void)NeuroForge::Core::LogAutonomyEnvelope(
              memdb.get(), memdb_run_id, latest_autonomy_envelope, a_inputs,
              std::string("sandbox_action"));
          if (phase6_reasoner) {
        ...
        if (!p13_decision.empty()) {
          if (p13_decision == std::string("tighten")) {
            ...
            action_click_threshold =
                std::max(3, base_action_click_threshold - 2);
          } else {
        ...
        if (g_abort.load())
          break;
        if (simulate_blocked_actions > 0) {
          blocked_action_count += simulate_blocked_actions;
          blocked_by_simulate_flag += simulate_blocked_actions;
        }
        if (dataset_active && !triplet_items.empty()) {
        ...
        if (vision_demo && visual_region) {
          ...
          if (vision_source == "camera") {
            ...
            if (cam_ok) {
              ...
              cap >> frame;
              if (frame.empty()) {
                std::cerr << "Warning: Captured empty frame; falling back to "
                             "synthetic frame"
                          << std::endl;
            ...
            if (foveation_enable) {
#ifdef _WIN32
              int bx = retina_rect_x, by = retina_rect_y, bw = retina_rect_w,
                  bh = retina_rect_h;
              ...
              if (fovea_mode == std::string("cursor")) {
                POINT pt{};
                if (GetCursorPos(&pt)) {
                  cx = std::clamp(static_cast<int>(pt.x), bx,
                                  bx + std::max(0, bw - 1));
                  cy = std::clamp(static_cast<int>(pt.y), by,
                                  by + std::max(0, bh - 1));
                }
              } else if (fovea_mode == std::string("attention")) {
                if (last_best_idx != static_cast<std::size_t>(-1)) {
                  int br = static_cast<int>(last_best_idx /
                                            static_cast<std::size_t>(G));
                  int bc = static_cast<int>(last_best_idx %
                                            static_cast<std::size_t>(G));
                  double fxn =
                      (static_cast<double>(bc) + 0.5) / static_cast<double>(G);
                  double fyn =
                      (static_cast<double>(br) + 0.5) / static_cast<double>(G);
              ...
              if (fovea_center_x < 0.0 || fovea_center_y < 0.0) {
                ...
                fovea_center_x = fovea_alpha * static_cast<double>(cx) +
                                 (1.0 - fovea_alpha) * fovea_center_x;
                fovea_center_y = fovea_alpha * static_cast<double>(cy) +
                                 (1.0 - fovea_alpha) * fovea_center_y;
              }
              int fx =
                  static_cast<int>(std::round(fovea_center_x)) - fovea_w / 2;
              int fy =
                  static_cast<int>(std::round(fovea_center_y)) - fovea_h / 2;
              fx = std::clamp(fx, bx, bx + std::max(0, bw - fovea_w));
              fy = std::clamp(fy, by, by + std::max(0, bh - fovea_h));
              screen.setRect({fx, fy, fovea_w, fovea_h});
              last_fovea_x = fx;
              last_fovea_y = fy;
              last_fovea_w = fovea_w;
              last_fovea_h = fovea_h;
#endif
            }
              ...
              if (fp_pixels.size() != static_cast<size_t>(G * G)) {
                ...
                for (int r = 0; r < G; ++r) {
                  for (int c = 0; c < G; ++c) {
                    ...
                    size_t src_idx =
                        static_cast<size_t>(src_y * config.width + src_x);
            ...
            if (!current_image_path.empty()) {
              cv::Mat img =
                  cv::imread(current_image_path, cv::IMREAD_GRAYSCALE);
              if (!img.empty()) {
                cv::Mat resized;
          ...
          if (motion_bias) {
            ...
            motion_bias->applyMotionBias(
                enhanced_features, motion_data, G, G,
                std::chrono::duration_cast<std::chrono::milliseconds>(
...

          {
            ...
            for (std::size_t k = 0; k < features.size() && k < mask.size();
                 ++k) {
              features[k] *= mask[k];
            }
          }
          ...
          if (phase_a_enable && phase_a_system && !current_teacher_id.empty()) {
            phase_a_system->addTeacherEmbedding(
                ...
                current_teacher_id, std::string("vision_features"), 1.0f);
          }
          ...
          if (motor_cortex && vision_source == "screen") {
            ...
            if (!features.empty()) {
              ...
              int tx =
                  retina_rect_x +
                  static_cast<int>(fx * static_cast<double>(retina_rect_w));
              int ty =
                  retina_rect_y +
                  static_cast<int>(fy * static_cast<double>(retina_rect_h));
              tx = std::clamp(tx, retina_rect_x,
                              retina_rect_x + retina_rect_w - 1);
              ty = std::clamp(ty, retina_rect_y,
                              retina_rect_y + retina_rect_h - 1);

              if (sandbox_enable) {
                auto sb = sandbox_window.bounds();
                int cx =
                    std::clamp(static_cast<int>(fx * static_cast<double>(sb.w)),
                               0, std::max(0, sb.w - 1));
                int cy =
                    std::clamp(static_cast<int>(fy * static_cast<double>(sb.h)),
                               0, std::max(0, sb.h - 1));

                if (!sandbox_seed_done) {
                  auto dec_type = NeuroForge::Core::ActionFilter_check(
                      NeuroForge::Core::ActionKind::TypeText,
                      sandbox_actions_enable, p15_decision, p13_decision,
                      simulate_blocked_actions);
                  auto dec_key = NeuroForge::Core::ActionFilter_check(
                      NeuroForge::Core::ActionKind::KeyPress,
                      sandbox_actions_enable, p15_decision, p13_decision,
                      simulate_blocked_actions);
                  if (dec_type.allow && dec_key.allow) {
                    sandbox_window.focus();
                    sandbox_window.typeText(std::string("news"));
                    sandbox_window.sendKey(
                        static_cast<unsigned int>(VK_RETURN));
                    ...
                    if (g_memdb && g_memdb_run_id > 0) {
                      ...
                      std::int64_t ts_ms =
                          duration_cast<milliseconds>(
                              ...
                              .count();
                      std::int64_t aid = 0;
                      (void)g_memdb->insertAction(
                          ts_ms, static_cast<std::uint64_t>(i),
                          std::string("type_text"),
                          std::string("{\"text\":\"news\",\"reason\":\"ok\"}"),
                          true, g_memdb_run_id, aid);
                      (void)g_memdb->insertAction(
                          ts_ms, static_cast<std::uint64_t>(i),
                          std::string("key_press"),
                          std::string("{\"vk\":13,\"reason\":\"ok\"}"), true,
                          g_memdb_run_id, aid);
                    }
                    brain.deliverReward(0.2, std::string("action"),
                                        std::string("{\"kind\":\"search\"}"));
                  } else {
                    blocked_action_count += 2;
                    auto reason_type = dec_type.allow ? std::string("unknown")
                                                      : dec_type.reason;
                    auto reason_key =
                        dec_key.allow ? std::string("unknown") : dec_key.reason;
                    if (!dec_type.allow) {
                      if (reason_type == std::string("phase15_deny"))
                        ++blocked_by_phase15;
                      else if (reason_type == std::string("no_web_actions"))
                        ++blocked_by_no_web_actions;
                      else if (reason_type == std::string("phase13_freeze"))
                        ++blocked_by_phase13;
                    }
                    if (!dec_key.allow) {
                      if (reason_key == std::string("phase15_deny"))
                        ++blocked_by_phase15;
                      else if (reason_key == std::string("no_web_actions"))
                        ++blocked_by_no_web_actions;
                      else if (reason_key == std::string("phase13_freeze"))
                        ++blocked_by_phase13;
                    }
                    if (g_memdb && g_memdb_run_id > 0) {
                      using namespace std::chrono;
                      ...
                      std::int64_t aid = 0;
                      {
                        std::ostringstream js;
                        js.setf(std::ios::fixed);
                        js << "{\"text\":\"news\",\"blocked\":true,\"reason\":"
                              "\""
                           << reason_type << "\"}";
                        (void)g_memdb->insertAction(
                            ts_ms, static_cast<std::uint64_t>(i),
                            std::string("type_text"), js.str(), false,
                            g_memdb_run_id, aid);
                      }
                      {
                        std::ostringstream js;
                        js.setf(std::ios::fixed);
                        js << "{\"vk\":13,\"blocked\":true,\"reason\":\""
                           << reason_key << "\"}";
                        (void)g_memdb->insertAction(
                            ts_ms, static_cast<std::uint64_t>(i),
                            std::string("key_press"), js.str(), false,
                            g_memdb_run_id, aid);
                      }
                    }
                  }
                ...
                if (br < (G / 3)) {
                  ...
                  bool cooled =
                      std::chrono::duration_cast<std::chrono::milliseconds>(
                          now_tp - last_action_tp)
                          .count() >= action_scroll_cooldown_ms;
                  auto dec = NeuroForge::Core::ActionFilter_check(
                      NeuroForge::Core::ActionKind::ScrollUp,
                      sandbox_actions_enable, p15_decision, p13_decision,
                      simulate_blocked_actions);
                  if (dec.allow && cooled) {
                    ...
                    if (g_memdb && g_memdb_run_id > 0) {
                      ...
                      (void)g_memdb->insertAction(
                          ts_ms, static_cast<std::uint64_t>(i),
                          std::string("scroll"),
                          std::string("{\"dir\":\"up\",\"reason\":\"ok\"}"),
                          true, g_memdb_run_id, aid);
                    }
                  } else {
                    blocked_action_count++;
                    std::string reason =
                        dec.allow ? std::string("cooldown") : dec.reason;
                    if (!dec.allow) {
                      if (reason == std::string("phase15_deny"))
                        ++blocked_by_phase15;
                      else if (reason == std::string("no_web_actions"))
                        ++blocked_by_no_web_actions;
                      else if (reason == std::string("phase13_freeze"))
                        ++blocked_by_phase13;
                    } else {
                      if (p13_decision == std::string("tighten"))
                        ++blocked_by_phase13;
                    ...
                    if (g_memdb && g_memdb_run_id > 0) {
                      ...
                      std::ostringstream js;
                      js.setf(std::ios::fixed);
                      js << "{\"dir\":\"up\",\"blocked\":true,\"reason\":\""
                         << reason << "\"}";
                      (void)g_memdb->insertAction(
                          ts_ms, static_cast<std::uint64_t>(i),
                          std::string("scroll"), js.str(), false,
                          g_memdb_run_id, aid);
                    }
                  }
                } else if (br > (2 * G / 3)) {
                  ...
                  bool cooled =
                      std::chrono::duration_cast<std::chrono::milliseconds>(
                          ...
                          .count() >= action_scroll_cooldown_ms;
                  auto dec = NeuroForge::Core::ActionFilter_check(
                      NeuroForge::Core::ActionKind::ScrollDown,
                      sandbox_actions_enable, p15_decision, p13_decision,
                      simulate_blocked_actions);
                  if (dec.allow && cooled) {
                    sandbox_window.scroll(-wheel);
                    ...
                    if (g_memdb && g_memdb_run_id > 0) {
                      ...
                      std::ostringstream js;
                      js.setf(std::ios::fixed);
                      js << "{\"dir\":\"down\",\"blocked\":true,\"reason\":\""
                         << reason << "\"}";
                      (void)g_memdb->insertAction(
                          ts_ms, static_cast<std::uint64_t>(i),
                          std::string("scroll"), js.str(), false,
                ...
                {
                  auto dec = NeuroForge::Core::ActionFilter_check(
                      NeuroForge::Core::ActionKind::Click,
                      sandbox_actions_enable, p15_decision, p13_decision,
                      simulate_blocked_actions);
                  if (dec.allow && best_stable >= action_click_threshold) {
                    ...
                    if (g_memdb && g_memdb_run_id > 0) {
                      ...
                      (void)g_memdb->insertAction(
                          ts_ms, static_cast<std::uint64_t>(i),
                          std::string("click"), js.str(),
                          ok_click ? true : false, g_memdb_run_id, aid);
                    }
                    best_stable = 0;
                  } else if (best_stable >= action_click_threshold) {
                    blocked_action_count++;
                    std::string reason =
                        dec.allow ? (p13_decision == std::string("tighten")
                                         ? std::string("phase13_tighten")
                                         : std::string("stability_threshold"))
                                  : dec.reason;
                    if (!dec.allow) {
                      if (reason == std::string("phase15_deny"))
                        ++blocked_by_phase15;
                      ...
                      if (reason == std::string("phase13_tighten"))
                        ++blocked_by_phase13;
                    }
                    if (g_memdb && g_memdb_run_id > 0) {
                      using namespace std::chrono;
                      ...
                      (void)g_memdb->insertAction(
                          ts_ms, static_cast<std::uint64_t>(i),
                          std::string("click"), js.str(), false, g_memdb_run_id,
                          aid);
                    }
                    best_stable = 0;
                  }
                ...
                if (g_memdb && g_memdb_run_id > 0) {
                  ...
                  (void)g_memdb->insertAction(
                      ts_ms, static_cast<std::uint64_t>(i),
                      std::string("cursor_move"), js.str(), ok ? true : false,
                      g_memdb_run_id, aid);
                }
              }
            }
        ...
        if (audio_demo && auditory_region) {
          ...
          const std::size_t N = static_cast<std::size_t>(
              std::max(256, acfg.sample_rate / 32)); // ~31ms at 16kHz
          ...
          if (audio_system && sys_ok) {
            ...
            for (std::size_t k = 0; k < N; ++k) {
              samples[k] = audio_file_samples[(audio_file_pos + k) %
                                              audio_file_samples.size()];
            }
          ...
          if (voice_bias) {
            std::vector<float> enhanced_features = features;
            voice_bias->applyVoiceBias(enhanced_features, samples,
                                       acfg.sample_rate, acfg.feature_bins);
            features = enhanced_features;
          }
          ...
          if (sandbox_enable) {
            ...
            if (last_action_click) {
              if (audio_rms - last_audio_rms > thresh) {
                brain.deliverReward(
                    1.0, std::string("action"),
                    std::string("{\"kind\":\"play_video\",\"step\":") +
                        std::to_string(last_click_step) + std::string("}"));
              } else if (last_audio_rms - audio_rms > thresh) {
                brain.deliverReward(
                    0.5, std::string("action"),
                    std::string("{\"kind\":\"pause_video\",\"step\":") +
                        std::to_string(last_click_step) + std::string("}"));
              }
...
// Social Perception Processing
        ...
        if (social_perception && social_bias && social_region) {
          ...
          if (cam_ok) {
            ...
            if (!frame.empty()) {
              ...
              NeuroForge::Biases::SocialPerceptionBias::AudioBuffer
                  audio_buffer;
              ...
              auto social_events =
                  social_bias->processSocialFrame(frame, audio_buffer);
              ...
              const std::size_t social_len =
                  std::min(social_neurons.size(), social_features.size());
              ...
              if (social_view && !social_events.empty()) {
                ...
                for (const auto &event : social_events) {
                  ...
                  if (!event.face_mask.empty() && !event.face_contour.empty()) {
                    ...
                    std::vector<std::vector<cv::Point>> contours = {
                        event.face_contour};
                    cv::drawContours(display_frame, contours, -1, face_color,
                                     2);

                    // Overlay face mask with transparency
                    if (event.face_mask.size() ==
                        cv::Size(event.face_box.width, event.face_box.height)) {
                      cv::Mat mask_overlay;
                      cv::cvtColor(event.face_mask, mask_overlay,
                                   cv::COLOR_GRAY2BGR);
                      cv::Mat face_roi = display_frame(event.face_box);
                      cv::addWeighted(face_roi, 0.7, mask_overlay, 0.3, 0,
                                      face_roi);
                    }
                  }
                  ...
                  if (event.gaze_confidence > 0.3f &&
                    ...
                    cv::Point face_center(
                        event.face_box.x + event.face_box.width / 2,
                    ...
                    cv::Point arrow_end(
                        face_center.x + static_cast<int>(event.gaze_vector.x *
                                                         arrow_length),
                        face_center.y + static_cast<int>(event.gaze_vector.y *
                                                         arrow_length));

                    // Ensure arrow end is within frame
                    arrow_end.x =
                        (std::max)(0, (std::min)(arrow_end.x,
                                                 display_frame.cols - 1));
                    arrow_end.y =
                        (std::max)(0, (std::min)(arrow_end.y,
                                                 display_frame.rows - 1));

                    // Draw vectorized gaze arrow
                    cv::arrowedLine(display_frame, face_center, arrow_end,
                                    cv::Scalar(255, 0, 0), 3, 8, 0, 0.3);

                    // Draw pupil positions
                    for (int p = 0; p < 2; ++p) {
                      if (event.pupil_positions[p].x > 0 &&
                          event.pupil_positions[p].y > 0) {
                        ...
                        if (pupil_global.x >= 0 && pupil_global.y >= 0 &&
                            pupil_global.x < display_frame.cols &&
                            pupil_global.y < display_frame.rows) {
                          cv::circle(display_frame, pupil_global, 3,
                  ...
                  if (!event.mouth_mask.empty() &&
                      !event.mouth_region.empty()) {
                    ...
                    if (event.mouth_mask.size() ==
                        cv::Size(event.mouth_region.width,
                                 event.mouth_region.height)) {
                      cv::Mat mask_overlay;
                      cv::cvtColor(event.mouth_mask, mask_overlay,
                                   cv::COLOR_GRAY2BGR);
                      cv::Mat mouth_roi = display_frame(event.mouth_region);
                      cv::addWeighted(mouth_roi, 0.6, mask_overlay, 0.4, 0,
                                      mouth_roi);
                    }

                    // Draw mouth region outline
                    cv::rectangle(display_frame, event.mouth_region,
                                  mouth_color, 1);
                  }
                  ...
                  for (int e = 0; e < 2; ++e) {
                    if (!event.eye_contours[e].empty()) {
                      std::vector<std::vector<cv::Point>> eye_contours = {
                          event.eye_contours[e]};
                      cv::drawContours(display_frame, eye_contours, -1,
                  ...
                  if (event.tracking_id >= 0) {
                    ...
                    info_text << "ID:" << event.tracking_id
                              << " G:" << std::fixed << std::setprecision(2)
                              << event.gaze_confidence
                ...
                cv::putText(display_frame,
                            "NeuroForge Social Perception - Enhanced "
                            "Biological Realism",
                ...
                cv::imshow("Social Perception - Biological Realism",
                           display_frame);
              ...
              if (i % 100 == 0 &&
                  !social_events.empty()) { // Log every 100 steps
                std::cout << "Social events detected: " << social_events.size()
                          << " (faces with masks, gaze vectors, lip-sync)"
                          << std::endl;
              }
            }
          }
        ...
        if (maze_demo && maze_obs_region && maze_action_region) {
          ...
          for (std::size_t k = 0; k < obs_len; ++k) {
            if (obs_neurons[k])
              obs_neurons[k]->setActivation(obs[k]);
          }
        }
        ...
        if (maze_demo && maze_obs_region && maze_action_region) {
          ...
          if (qlearning) {
            ...
            if (q_state >= 0) {
              ...
              for (int a = 0; a < copyA; ++a) {
                qvals[a] =
                    qtable[static_cast<std::size_t>(q_state * Q_ACTIONS + a)];
              }
            }
          }
          ...
          if (qlearning && hybrid_lambda >= 0.0f) {
            for (int a = 0; a < A; ++a) {
              scores[a] =
                  hybrid_lambda * prefs[a] + (1.0f - hybrid_lambda) * qvals[a];
            }
          } else if (qlearning) {
          ...
          if (teacher_policy != "none" && A > 0) {
            ...
            auto valid_next = [&](int a, int &nx, int &ny) {
              nx = ax;
              ny = ay;
              switch (a) {
              case 0:
                ...
                break;
              case 1:
                ny = std::min(Nm - 1, ay + 1);
                break;
              case 2:
                nx = std::max(0, ax - 1);
                break;
              case 3:
                nx = std::min(Nm - 1, ax + 1);
                break;
              default:
                break;
              }
              if (maze_env.isWall(nx, ny)) {
                nx = ax;
                ny = ay;
              }
            };
            if (teacher_policy == "greedy") {
              auto manh = [&](int x, int y) {
                return std::abs(x - gx) + std::abs(y - gy);
              };
              int best_a = 0;
              int best_d = 1000000000;
              for (int a = 0; a < std::min(A, 4); ++a) {
                int nx, ny;
                valid_next(a, nx, ny);
                int d = manh(nx, ny);
                if (d < best_d) {
              ...
              while (!q.empty() && !found) {
                ...
                q.pop();
                for (int a = 0; a < std::min(A, 4); ++a) {
                  int nx = std::clamp(cx + dx[a], 0, W - 1);
                  int ny = std::clamp(cy + dy[a], 0, H - 1);
                  if (maze_env.isWall(nx, ny)) {
                    nx = cx;
                    ny = cy;
                  }
          ...
          if (teacher_mix > 0.0f && A > 0) {
            for (int a = 0; a < A; ++a) {
              float t =
                  (teacher_action >= 0 && a == teacher_action) ? 1.0f : 0.0f;
          ...
          // Emit Phase B decision context
          {
            auto vec_to_json = [](const std::vector<float> &v) -> std::string {
              ...
              for (size_t idx = 0; idx < v.size(); ++idx) {
                if (idx)
                  oss << ",";
                oss << std::fixed << std::setprecision(4) << v[idx];
              }
              oss << "]";
              return oss.str();
            };
            std::ostringstream js;
            js << "{\"version\":1,\"phase\":\"B\",\"event\":\"decision\","
                  "\"time\":\""
               << json_escape(iso8601_utc_now()) << "\",";
            js << "\"episode_index\":" << episode_index << ",\"step\":" << i
               << ",\"payload\":{\"prefs\":" << vec_to_json(prefs)
               << ",\"qvals\":" << vec_to_json(qvals)
               << ",\"scores\":" << vec_to_json(scores) << "}}";
            emit_json_line(log_json, log_json_path, js.str());
          }
          ...
          // Exploration: prefer softmax/wta for neural control; epsilon-greedy
          // only when Q-learning baseline is active
          if (A > 0 && qlearning && epsilon >= 0.0f && epsilon <= 1.0f) {
            ...
            double maxp = -1e30;
            for (float s : scores)
              maxp = std::max(maxp, static_cast<double>(s));
            const double T = static_cast<double>(softmax_temp);
            for (int a = 0; a < A; ++a) {
              double e = std::exp((static_cast<double>(scores[a]) - maxp) / T);
              probs[a] = e;
              sum += e;
            }
            if (sum > 0.0) {
          ...
          if (phase6_enable && phase6_reasoner && A > 0 &&
              (phase6_active_mode == "on" || phase6_active_mode == "audit")) {
            try {
              ...
              double policy_score =
                  (base_action_before_phase6 >= 0 &&
                   base_action_before_phase6 < A)
                      ? static_cast<double>(scores[base_action_before_phase6])
                      : -1e9;
              bool within = (p6_score >= policy_score - phase6_margin);
              bool applied = false;
              if (phase6_active_mode == "on" && within && p6_choice >= 0 &&
                  p6_choice < A) {
                action = p6_choice;
                applied = true;
              }
              ...
              {
                std::ostringstream js;
                js << "{\"version\":1,\"phase\":\"6\",\"event\":\"gate\","
                      "\"time\":\""
                   << json_escape(iso8601_utc_now()) << "\",";
                js << "\"payload\":{\"mode\":\""
                   << json_escape(phase6_active_mode)
                   << "\",\"policy_action\":" << base_action_before_phase6
          ...
          if (phase6_enable && phase6_reasoner && A > 0) {
            try {
              ...
              std::int64_t ts_ms = duration_cast<milliseconds>(
                                       system_clock::now().time_since_epoch())
                                       .count();
              auto option_ids = phase6_reasoner->registerOptions(
                  phase6_opts, static_cast<std::uint64_t>(i), ts_ms,
                  static_cast<std::size_t>(action));
              if (!option_ids.empty() && action >= 0 && action < A) {
                phase6_selected_option_id =
                    option_ids[static_cast<std::size_t>(action)];
                phase6_selected_key =
                    std::string("action_") + std::to_string(action);
              }
            } catch (...) {
          ...
          {
            std::ostringstream js;
            js << "{\"version\":1,\"phase\":\"B\",\"event\":\"action\","
                  "\"time\":\""
               << json_escape(iso8601_utc_now()) << "\",";
            js << "\"episode_index\":" << episode_index << ",\"step\":" << i
          ...
          // Live-update mimicry attempt using mirror-mode sensory features or
          // action scores
          if (mimicry_enable) {
            ...
            if (ls_mim) {
              ...
              // Per-step Phase A mimicry attempt and reward application
              // (requires teacher embedding)
              if (phase_a_enable && phase_a_system) {
                if (auto *t = phase_a_system->getTeacherEmbedding(
                        current_teacher_id)) {
                  std::vector<float> empty_student_vec;
                  auto attempt = phase_a_system->attemptMimicry(
                      empty_student_vec, current_teacher_id,
                      dataset_active ? "triplet_step" : "maze_step");
                  if (!mimicry_internal) {
                    phase_a_system->applyMimicryReward(attempt);
                  }
                  phase_a_last_similarity = attempt.similarity_score;
                      ...
                      static_cast<int>(attempt.teacher_embedding.size());
                  {
                    ...
                    for (std::size_t ii = 0; ii < n; ++ii) {
                      double sv =
                          static_cast<double>(attempt.student_embedding[ii]);
                      double tv =
                          static_cast<double>(attempt.teacher_embedding[ii]);
                      d += sv * tv;
                      ns += sv * sv;
                      nt += tv * tv;
                    }
                  ...
                  if (self_node) {
                    ...
                    self_node->updateSelfRepresentation(
                        NeuroForge::Regions::SelfNode::SelfAspect::Cognitive,
                        embed);
                    // Emotional aspect from reward/success
                    std::vector<float> emo = {
                        phase_a_last_reward, phase_a_last_similarity,
                        phase_a_last_novelty,
                        phase_a_last_success ? 1.0f : 0.0f};
                    self_node->updateSelfRepresentation(
                        NeuroForge::Regions::SelfNode::SelfAspect::Emotional,
                        emo);
                    // Narrative aspect: integrate a richer experience pattern
                    // (step, task reward, mimicry reward, similarity, novelty)
                    std::vector<float> xp = {
                        static_cast<float>(i),
                        static_cast<float>(maze_last_reward),
                        static_cast<float>(phase_a_last_reward),
                        static_cast<float>(phase_a_last_similarity),
                        static_cast<float>(phase_a_last_novelty)};
                    self_node->integrateExperience(xp);
                    // Update identity with current teacher/context hint
                    // (string)
                    try {
                      self_node->updateIdentity(std::string("teacher:") +
                                                current_teacher_id);
                    } catch (...) {
                    }
                  }
              ...
              if (self_node && phase5_language_enable && language_system) {
                ...
                std::vector<float> narr = {
                    static_cast<float>(lang_stats.narration_entries),
                    static_cast<float>(lang_stats.total_tokens_generated)};
                self_node->updateSelfRepresentation(
                    NeuroForge::Regions::SelfNode::SelfAspect::Narrative, narr);
                // Also fold language-driven self signals into Emotional aspect
                // (proxy: narration count and active vocab)
                std::vector<float> emo_lang = {
                    static_cast<float>(lang_stats.narration_entries),
                    static_cast<float>(lang_stats.active_vocabulary_size)};
                self_node->updateSelfRepresentation(
                    NeuroForge::Regions::SelfNode::SelfAspect::Emotional,
                    emo_lang);
              }
          ...
          if (qlearning) {
            ...
            case 0:
              ...
              break;
            case 1:
              ny = std::min(N - 1, ay0 + 1);
              break;
            case 2:
              nx = std::max(0, ax0 - 1);
              break;
            case 3:
              nx = std::min(N - 1, ax0 + 1);
              break;
            default:
              break;
            }
          ...
          if (phase6_enable && phase6_reasoner &&
              phase6_selected_option_id >= 0) {
            try {
              using namespace std::chrono;
              std::int64_t ts_ms2 = duration_cast<milliseconds>(
                                        system_clock::now().time_since_epoch())
                                        .count();
              phase6_reasoner->applyOptionResult(
                  phase6_selected_option_id, phase6_selected_key,
                  static_cast<double>(r), ts_ms2, true);
            } catch (...) {
              // swallow any Phase 6 errors
            }
          }
          ...
          if (qlearning && q_state >= 0) {
            ...
            if (!maze_done && q_next_state >= 0) {
              ...
              for (int a2 = 1; a2 < 4; ++a2) {
                max_next = std::max(
                    max_next,
                    qtable[static_cast<std::size_t>(q_next_state * 4 + a2)]);
              }
            }
          ...
          {
            std::ostringstream js;
            js << "{\"version\":1,\"phase\":\"B\",\"event\":\"reward\","
                  "\"time\":\""
               << json_escape(iso8601_utc_now()) << "\",";
            js << "\"episode_index\":" << episode_index << ",\"step\":" << i
               << ",\"payload\":{\"reward\":" << r
               << ",\"done\":" << (maze_done ? "true" : "false") << "}}";
            emit_json_line(log_json, log_json_path, js.str());
          }
          ...
          if (maze_done) {
            ...
            {
              std::ostringstream js;
              js << "{\"version\":1,\"phase\":\"B\",\"event\":\"episode_end\","
                    "\"time\":\""
                 << json_escape(iso8601_utc_now()) << "\",";
              js << "\"episode_index\":" << episode_index
                 << ",\"payload\":{\"episode_id\":" << current_episode_id
            ...
            if (episode_csv) {
              episode_csv << episode_index << "," << episode_steps << ","
                          << episode_return << "," << ms << ","
                          << (success ? 1 : 0) << "\n";
              episode_csv.flush();
            }
            ...
            double avg_reward =
                (episode_steps > 0)
                    ? (episode_return / static_cast<double>(episode_steps))
                    : 0.0;
            double contradiction_rate = 0.0;
            if (memdb && memdb_run_id > 0) {
              contradiction_rate = memdb->getEpisodeContradictionRate(
                  memdb_run_id, ended_episode_id);
            }

            // Update MemoryDB episode stats and rotate episodes if MemoryDB is
            // enabled
            if (memdb && current_episode_id > 0) {
              (void)memdb->upsertEpisodeStats(current_episode_id, episode_steps,
            ...
            if (phase7_enable || phase7_reflect_enable) {
              try {
                if (phase6_reasoner) {
                  phase6_reasoner->onEpisodeEnd(
                      static_cast<std::int64_t>(episode_index),
                      contradiction_rate, avg_reward);
                } else if (phase7_reflect && phase7_affect) {
                  auto st = phase7_affect->getState();
                  phase7_reflect->maybeReflect(
                      static_cast<std::int64_t>(episode_index),
                      contradiction_rate, avg_reward, st.valence, st.arousal);
                }
              } catch (...) {
            ...
            try {
              if (phase8_goals) {
                double coherence =
                    std::clamp(1.0 - contradiction_rate, 0.0, 1.0);
                double motivation = std::clamp(0.5 + avg_reward, 0.0, 1.0);
                if (success)
                  motivation = std::clamp(motivation + 0.1, 0.0, 1.0);
                std::ostringstream notes;
                notes << "maze episode_end episode_index=" << episode_index
                      << " success=" << (success ? 1 : 0)
                      << " avg_reward=" << avg_reward
                      << " contradiction_rate=" << contradiction_rate;
                (void)phase8_goals->updateMotivationState(motivation, coherence,
                                                          notes.str());
              }
            } catch (...) {
            }
            ...
            {
              std::ostringstream js;
              js << "{\"version\":1,\"phase\":\"B\",\"event\":\"episode_"
                    "start\",\"time\":\""
                 << json_escape(iso8601_utc_now()) << "\",";
              js << "\"episode_index\":" << episode_index
                 << ",\"payload\":{\"episode_id\":" << current_episode_id
          ...
          static std::chrono::steady_clock::time_point last_maze_draw =
              std::chrono::steady_clock::now();
          if (maze_view) {
            auto now = std::chrono::steady_clock::now();
            if (std::chrono::duration_cast<std::chrono::milliseconds>(
                    now - last_maze_draw)
                    .count() >= maze_view_interval_ms) {
              last_maze_draw = now;
              int G = maze_env.size();
              const int scale = 24;
              cv::Mat img(G * scale, G * scale, CV_8UC3,
                          cv::Scalar(30, 30, 30));
              for (int y = 0; y < G; ++y) {
                for (int x = 0; x < G; ++x) {
              ...
              // Draw goal and agent
              cv::rectangle(img,
                            cv::Rect(maze_env.goalX() * scale,
                                     maze_env.goalY() * scale, scale, scale),
                            cv::Scalar(0, 180, 0), cv::FILLED);
              cv::circle(img,
                         cv::Point(maze_env.agentX() * scale + scale / 2,
                                   maze_env.agentY() * scale + scale / 2),
                         scale / 3, cv::Scalar(40, 160, 255), cv::FILLED);
              // Collision feedback border
              if (maze_env.lastCollision()) {
                cv::rectangle(img, cv::Rect(0, 0, img.cols - 1, img.rows - 1),
                              cv::Scalar(0, 0, 255), 2);
              }
              {
                std::ostringstream oss;
                oss << "r=" << std::fixed << std::setprecision(2) << r;
                cv::putText(img, oss.str(), cv::Point(6, 16),
              ...
              }
              {
                ...
                std::string mode;
                if (qlearning && epsilon >= 0.0f && epsilon <= 1.0f)
                  mode = "epsilon";
                else if (softmax_temp > 0.0f)
                  mode = "softmax";
                else
                  mode = "wta";
                std::ostringstream oss2;
                oss2 << "lambda=" << std::fixed << std::setprecision(2)
                     << (hybrid_lambda >= 0.0f ? hybrid_lambda
                                               : (qlearning ? 0.0f : 1.0f))
                     << "  mode=" << mode;
                cv::putText(img, oss2.str(), cv::Point(6, 34),
                            cv::FONT_HERSHEY_SIMPLEX, 0.45,
                            cv::Scalar(200, 255, 200), 1, cv::LINE_AA);
              }
              {
                // Action scores are computed in the selection block; recompute
                // a light-weight view here for overlay
                const auto &act_neurons_dbg = maze_action_region->getNeurons();
                const int A_dbg = static_cast<int>(act_neurons_dbg.size());
                std::vector<float> prefs_dbg(A_dbg, 0.0f),
                    qvals_dbg(A_dbg, 0.0f), scores_dbg(A_dbg, 0.0f);
                for (int a = 0; a < A_dbg; ++a) {
                  prefs_dbg[a] = act_neurons_dbg[a]
                                     ? act_neurons_dbg[a]->getActivation()
                                     : 0.0f;
                }
                if (qlearning) {
                  ...
                  if (q_state_dbg >= 0) {
                    ...
                    for (int a = 0; a < copyA; ++a) {
                      qvals_dbg[a] = qtable[static_cast<std::size_t>(
                          q_state_dbg * Q_ACTIONS + a)];
                    }
                  }
                }
                if (qlearning && hybrid_lambda >= 0.0f) {
                  for (int a = 0; a < A_dbg; ++a)
                    scores_dbg[a] = hybrid_lambda * prefs_dbg[a] +
                                    (1.0f - hybrid_lambda) * qvals_dbg[a];
                } else if (qlearning) {
                ...
                for (int a = 0; a < A_dbg && a < 4; ++a) {
                  if (a)
                    oss3 << ", ";
                  oss3 << anames[a] << ":" << std::fixed << std::setprecision(2)
                       << scores_dbg[a];
                }
                cv::putText(img, oss3.str(), cv::Point(6, 52),
                            cv::FONT_HERSHEY_SIMPLEX, 0.45,
        ...
        if (heatmap_view) {
          ...
          if (std::chrono::duration_cast<std::chrono::milliseconds>(
                  now - last_heatmap)
                  .count() >= heatmap_interval_ms) {
            last_heatmap = now;
            auto *ls = brain.getLearningSystem();
            if (!ls) {
              if (!heatmap_warned_no_ls) {
                std::cerr
                    << "Info: Heatmap enabled but LearningSystem is not "
                       "initialized. Enable learning to visualize synapses."
                    << std::endl;
              ...
              if (!snapshots.empty()) {
                for (const auto &s : snapshots) {
                  ...
                  if (w < heatmap_threshold)
                    continue;
                  int r = static_cast<int>(s.pre_neuron %
                                           static_cast<uint64_t>(heatmap_size));
                  int c = static_cast<int>(s.post_neuron %
                                           static_cast<uint64_t>(heatmap_size));
                  hm.at<float>(r, c) += w;
                }
          ...
          if (std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            if (!ls) {
              if (!live_warned_no_ls) {
                std::cerr
                    << "Info: live export requested "
                       "(--snapshot-live/--spikes-live) but LearningSystem is "
                       "not initialized. Enable learning to export live data."
                    << std::endl;
              ...
              if (!snapshot_live_path.empty()) {
                ...
                if (!ofs) {
                  ...
                  ofs << "pre_neuron,post_neuron,weight\n";
                  for (const auto &s : snapshots) {
                    ofs << s.pre_neuron << "," << s.post_neuron << ","
                        << s.weight << "\n";
                  }
                  ofs.flush();
                }
              ...
              if (!spikes_live_path.empty()) {
                // Build window of recent spikes and write unique IDs with
                // timestamps (ms since now-ttl)
                std::vector<std::pair<NeuroForge::NeuronID, long long>> recent;
                {
                  ...
                  const auto cutoff =
                      now - std::chrono::milliseconds(
                                static_cast<int>(spikes_ttl_sec * 1000.0));
                  while (!spike_events.empty() &&
                         spike_events.front().second < cutoff)
          ...
          if (std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            std::size_t spike_count = 0;
            {
              ...
              while (!spike_events.empty() &&
                ...
                spike_events.pop_front();
              for (const auto &ev : spike_events) {
                if (ev.second >= window_start && ev.second <= now)
                  ++spike_count;
              }
            }
            const auto win_ms =
            ...
            if (vision_demo && visual_region) {
              ...
              for (const auto &n : neurons) {
                if (n)
                  region_acts.push_back(n->getActivation());
              }
            }
            if (audio_demo && auditory_region) {
            ...
            // Use activations as the observation proxy for shaping (keeps
            // integration minimal)
            std::vector<float> obs = region_acts;

            // Prepare telemetry via LearningSystem, then compute
            // pipeline-shaped reward
            ...
            if (auto *ls_reward = brain.getLearningSystem()) {
              (void)ls_reward->computeShapedReward(
                  obs, region_acts, static_cast<float>(task_reward));
            ...
            double survival_r =
                static_cast<double>(phase_c_survival_scale) *
                static_cast<double>(substrate_similarity - substrate_novelty);
            if (survival_r > 1.0)
              survival_r = 1.0;
            else if (survival_r < -1.0)
              survival_r = -1.0;
            double wt_T = wt_teacher, wt_S = wt_survival, wt_N = wt_novelty;
            double shaped_d =
                wt_T * teacher_r + wt_N * novelty_r + wt_S * survival_r;
            if (shaped_d > 1.0)
              shaped_d = 1.0;
            else if (shaped_d < -1.0)
              shaped_d = -1.0;
            ...
            // Log shaped reward (with context including task spikes and vector
            // sizes)
            std::string ctx =
                std::string("{\"spikes\":") + std::to_string(spike_count) +
                ...
                std::to_string(substrate_similarity) +
                ...
                ",\"blocked_by_phase15\":" +
                std::to_string(blocked_by_phase15) +
                ",\"blocked_by_phase13\":" +
                std::to_string(blocked_by_phase13) +
                ",\"blocked_by_no_web_actions\":" +
                std::to_string(blocked_by_no_web_actions) +
                ",\"blocked_by_simulate_flag\":" +
                std::to_string(blocked_by_simulate_flag);
            ...
            try {
              ...
              ctx +=
                  ",\"context_cfg\":{\"gain\":" + std::to_string(cfg_ctx.gain) +
            ...
            if (phase_a_enable && phase_a_system) {
              ...
              ctx += ",\"phase_a_mimicry_attempts\":" +
                     std::to_string(phase_a_stats.total_mimicry_attempts);
              ctx += ",\"phase_a_teacher_embeddings\":" +
                     std::to_string(phase_a_stats.teacher_embeddings_stored);
              ctx +=
                  ",\"phase_a_alignments\":" +
                  std::to_string(phase_a_stats.multimodal_alignments_created);
              // Last mimicry attempt details for trajectory plotting (gated
              // behind --telemetry-extended)
              if (telemetry_extended) {
                try {
                  // We don't have direct accessor; use LearningSystem for sim
                  // and Phase A stats for counts
                  ctx += ",\"phase_a_current_teacher_id\":\"" +
                         current_teacher_id + "\"";
                  ctx += ",\"phase_a_last_similarity\":" +
                         std::to_string(phase_a_last_similarity);
                  ctx += ",\"phase_a_last_novelty\":" +
                         std::to_string(phase_a_last_novelty);
                  ctx += ",\"phase_a_last_reward\":" +
                         std::to_string(phase_a_last_reward);
                  ctx += ",\"phase_a_last_success\":" +
                         std::string(phase_a_last_success ? "true" : "false");
                } catch (...) { /* ignore */
                }
              }
              // Nested phase_a block mirroring experiences schema (always
              // include when Phase A is enabled)
              try {
                ctx += ",\"phase_a\":{";
                ...
                ctx += ",\"last_similarity\":" +
                       std::to_string(phase_a_last_similarity);
                ctx +=
                    ",\"last_novelty\":" + std::to_string(phase_a_last_novelty);
                ctx +=
                    ",\"last_reward\":" + std::to_string(phase_a_last_reward);
                ctx += ",\"last_success\":" +
                       std::string(phase_a_last_success ? "true" : "false");
                ctx += "}";
              } catch (...) { /* ignore */
            ...
            if (phase5_language_enable && language_system) {
              auto lang_stats = language_system->getStatistics();
              ctx += ",\"language_stage\":" +
                     std::to_string(static_cast<int>(lang_stats.current_stage));
              ctx += ",\"language_tokens_generated\":" +
                     std::to_string(lang_stats.total_tokens_generated);
              ctx += ",\"language_narrations\":" +
                     std::to_string(lang_stats.narration_entries);
              ctx += ",\"language_vocab_active\":" +
                     std::to_string(lang_stats.active_vocabulary_size);
            ...
            if (self_node) {
              try {
                auto cog = self_node->getSelfRepresentation(
                    NeuroForge::Regions::SelfNode::SelfAspect::Cognitive);
                auto emo = self_node->getSelfRepresentation(
                    NeuroForge::Regions::SelfNode::SelfAspect::Emotional);
                ctx += ",\"self_awareness\":" +
                       std::to_string(self_node->getSelfAwarenessLevel());
                ctx += ",\"self_identity\":\"" +
                       self_node->getCurrentIdentity() + "\"";
                ctx += ",\"self_cognitive_conf\":" +
                       std::to_string(cog.confidence_level);
                ctx += ",\"self_emotional_conf\":" +
                       std::to_string(emo.confidence_level);
            ...
            ctx += "}";
            bool all_zero = (shaped_reward == 0.0f && teacher_r == 0.0 &&
                             novelty_r == 0.0 && survival_r == 0.0);
            if (!all_zero || log_shaped_zero) {
              brain.logReward(static_cast<double>(shaped_reward), "shaped",
                              ctx);
              try {
                ...
                ctxs << "{"
                     ...
                     << "\"components\":{\"teacher\":" << std::setprecision(4)
                     << teacher_r << ",\"survival\":" << std::setprecision(4)
                     << survival_r << ",\"novelty\":" << std::setprecision(4)
                     << novelty_r << "},"
              ...
              try {
                ...
                ctxm << "{"
                     << "\"source\":\"merged\","
                     << "\"teacher_id\":\"" << current_teacher_id << "\","
                     << "\"weights\":{\"teacher\":" << std::setprecision(4)
                     << wt_T << ",\"survival\":" << std::setprecision(4) << wt_S
            ...
            try {
              ...
              for (const auto &p : peers) {
                ...
                if (it != context_peer_labels.end())
                  sample_label = it->second;
                else
                  sample_label = "runtime";
                double s =
                    NeuroForge::Core::NF_SampleContextPeer(p, sample_label);
                ...
                try {
                  // kappa_param is parsed earlier; replicate local copy if
                  // needed This scope relies on captured kappa_param from main
                ...
                std::string mode_val =
                    "coop"; // default mode until CLI adds couple-mode
                std::int64_t out_id = 0;
                (void)memdb->insertContextPeerLog(
                    memdb_run_id, ts_ms_peer, p, s, cfgp.gain, cfgp.update_ms,
                    cfgp.window, sample_label, mode_val, lambda_eff, kappa_eff,
                    out_id);
              }
              ...
              st = statsOpt2.value();
            {
              ...
              std::uint64_t step_pc =
                  static_cast<std::uint64_t>(gs.processing_cycles);
              (void)memdb->insertLearningStats(ts_ms, step_pc, hz, st,
                                               memdb_run_id);
              std::int64_t event_id = 0;
              (void)memdb->insertRunEvent(
                  memdb_run_id, ts_ms, step_pc, std::string("heartbeat"),
                  std::string(), 0, nf_process_rss_mb(), 0.0, event_id);
              // Optional memory pressure notifications (RSS)
              double rss_mb = nf_process_rss_mb();
              if (rss_warn_threshold_mb > 0.0 &&
                ...
                if (last_rss_warn.time_since_epoch().count() == 0 ||
                  ...
                  std::int64_t warn_id = 0;
                  (void)memdb->insertRunEvent(
                      memdb_run_id, ts_ms, step_pc, std::string("warning"),
                      std::string("rss_threshold_exceeded"), 0, rss_mb, 0.0,
                      warn_id);
                  last_rss_warn = now_warn;
                }
              ...
              if (rss_fail_threshold_mb > 0.0 &&
                ...
                std::int64_t err_id = 0;
                (void)memdb->insertRunEvent(
                    memdb_run_id, ts_ms, step_pc, std::string("error"),
                    std::string("rss_fail_threshold_exceeded"), 0, rss_mb, 0.0,
                    err_id);
              }
            }
            ...
            try {
              ...
              std::string tag = std::string("snapshot:") +
                                (phase_a_enable ? "phase_a" : "core");
              if (dataset_active && mimicry_enable && phase_a_enable &&
                  phase_a_system && !current_teacher_id.empty()) {
                auto *ls_mim2 = brain.getLearningSystem();
                std::vector<float> empty_student_vec;
                int mimicry_repeats = phase_a_mimicry_repeats;
                for (int rep = 0; rep < mimicry_repeats; ++rep) {
                  auto attempt2 = phase_a_system->attemptMimicry(
                      empty_student_vec, current_teacher_id,
                      std::string("triplet_step"));
                  if (!mimicry_internal) {
                    phase_a_system->applyMimicryReward(attempt2);
                  }
                  ...
                  phase_a_last_stu_len =
                      static_cast<int>(attempt2.student_embedding.size());
                  phase_a_last_tea_len =
                      static_cast<int>(attempt2.teacher_embedding.size());
                  double d2 = 0.0, ns2 = 0.0, nt2 = 0.0;
                  std::size_t n2 = std::min(attempt2.student_embedding.size(),
                                            attempt2.teacher_embedding.size());
                  for (std::size_t ii = 0; ii < n2; ++ii) {
                    double sv2 =
                        static_cast<double>(attempt2.student_embedding[ii]);
                    double tv2 =
                        static_cast<double>(attempt2.teacher_embedding[ii]);
                    d2 += sv2 * tv2;
                    ns2 += sv2 * sv2;
                    nt2 += tv2 * tv2;
                  }
                  ...
                  if (mimicry_internal && ls_mim2) {
                    ls_mim2->setMimicryAttemptScores(
                        phase_a_last_similarity, phase_a_last_novelty,
                        phase_a_last_reward, phase_a_last_success);
                  }
                  if (self_node) {
                    self_node->updateSelfRepresentation(
                        NeuroForge::Regions::SelfNode::SelfAspect::Cognitive,
                        attempt2.student_embedding);
                    std::vector<float> emo2 = {
                        phase_a_last_reward, phase_a_last_similarity,
                        phase_a_last_novelty,
                        phase_a_last_success ? 1.0f : 0.0f};
                    self_node->updateSelfRepresentation(
              ...
              // Input: observation proxy; optionally include extended metadata;
              // Output: action activations
              ...
              if (telemetry_extended) {
                ...
                try {
                  auto ctx_samples =
                      NeuroForge::Core::NF_GetRecentContextSamples();
                  auto cfg_ctx = NeuroForge::Core::NF_GetContextConfig();
                  std::ostringstream oss_ctx;
                  oss_ctx.setf(std::ios::fixed);
                  oss_ctx << std::setprecision(6) << "[";
                ...
                if (phase_a_enable && phase_a_system) {
                  meta << ",\"phase_a\":{"
                       ...
                       << ","
                          "\"stu_norm\":"
                       << phase_a_last_stu_norm
                       << ","
                          "\"tea_norm\":"
                       << phase_a_last_tea_norm
                       << ","
                          "\"dot\":"
                       << phase_a_last_dot << "}";
                }
                if (phase5_language_enable && language_system) {
                  ...
                  meta << ",\"language\":{"
                          "\"stage\":"
                       << static_cast<int>(lang_stats.current_stage)
                       << ","
                          "\"tokens_generated\":"
                       << lang_stats.total_tokens_generated
                       << ","
                          "\"narrations\":"
                       << lang_stats.narration_entries
                       << ","
                          "\"vocab_active\":"
                       << lang_stats.active_vocabulary_size
                       << ","
                          "\"metrics\":{\"stage\":"
                       << static_cast<int>(lang_stats.current_stage)
                       << ",\"tokens_generated\":"
                ...
                // Vision telemetry
                {
                  int vx = retina_rect_x, vy = retina_rect_y,
                      vw = retina_rect_w, vh = retina_rect_h;
                  if (foveation_enable && last_fovea_w > 0 &&
                      last_fovea_h > 0) {
                    vx = last_fovea_x;
                    vy = last_fovea_y;
                    vw = last_fovea_w;
                ...
                if (self_node) {
                  try {
                    ...
                    meta << ",\"self\":{"
                            "\"awareness\":"
                         << self_node->getSelfAwarenessLevel()
                         << ","
                            "\"identity\":\""
                         << self_node->getCurrentIdentity()
                         << "\","
                            "\"cognitive_conf\":"
                         << cog.confidence_level
                         << ","
                            "\"emotional_conf\":"
                         << emo.confidence_level
                         << ","
                            "\"state\":{\"awareness\":"
                         << self_node->getSelfAwarenessLevel()
                         << ",\"identity\":\""
              ...
              std::uint64_t step_pc =
                  static_cast<std::uint64_t>(gs.processing_cycles);
              (void)memdb->insertExperience(ts_ms, step_pc, tag, input_json,
                                            output_json, significant,
                                            memdb_run_id, exp_id);
              if (current_episode_id > 0 && exp_id > 0) {
                (void)memdb->linkExperienceToEpisode(exp_id,
                                                     current_episode_id);
              ...
              if (self_node) {
                try {
                  ...
                  double avg_conf =
                      0.5 * (cog.confidence_level + emo.confidence_level);
    ...
    if (phase11_revision && phase11_outcome_eval_window_ms > 0) {
      // Drain self-revision outcomes so the invariant `outcomes == revisions -
      // 1` holds at shutdown. Phase 11 outcome evaluation requires the revision
      // to be at least `phase11_outcome_eval_window_ms` old (pre/post window),
      // so we poll until either all pending outcomes are written or we time
      // out.
      const auto drain_start = std::chrono::steady_clock::now();
      const auto max_drain_ms =
          std::max<std::int64_t>(2000, 3LL * phase11_outcome_eval_window_ms);
      const auto drain_deadline =
          drain_start + std::chrono::milliseconds(max_drain_ms);

      while (std::chrono::steady_clock::now() < drain_deadline) {
        bool progressed = false;
        for (int i = 0; i < 1000; ++i) {
          if (!phase11_revision->evaluatePendingOutcomes())
            break;
          progressed = true;
        }
        ...
        if (memdb && memdb_run_id > 0) {
          has_pending =
              memdb
                  ->getLatestUnevaluatedSelfRevisionId(
                      memdb_run_id, std::numeric_limits<std::int64_t>::max())
                  .has_value();
        }
        if (!has_pending)
          break;
    ...
    if (!snapshot_csv_path.empty()) {
      auto *ls = brain.getLearningSystem();
      if (!ls) {
        std::cerr << "Warning: --snapshot-csv was provided, but LearningSystem "
                     "is not initialized. Enable learning to export snapshots."
                  << std::endl;
      } else {
        auto snapshots = ls->getSynapseSnapshot();
    ...
    if (!save_brain_path.empty()) {
      if (brain.saveCheckpoint(save_brain_path)) {
        ...
        std::cerr << "Error: failed to save brain checkpoint to '"
                  << save_brain_path << "'\n";
      }
    }

    if (phase_a_export_set && phase_a_system) {
      std::string teachers_json =
          phase_a_system->exportTeacherEmbeddingsToJson();
      std::string attempts_json = phase_a_system->exportMimicryHistoryToJson();
      std::string path1 =
          phase_a_export_dir + "/phase_a_teacher_embeddings.json";
    ...
    if (summary && finished_episodes > 0) {
      const double avg_steps = static_cast<double>(sum_episode_steps) /
                               static_cast<double>(finished_episodes);
      const double avg_return =
          sum_episode_return / static_cast<double>(finished_episodes);
      const double avg_time_ms = static_cast<double>(sum_episode_time_ms) /
                                 static_cast<double>(finished_episodes);
      const double success_rate = 100.0 *
                                  static_cast<double>(successful_episodes) /
                                  static_cast<double>(finished_episodes);
    ...
    if (g_memdb && g_memdb_run_id > 0) {
      ...
      std::int64_t event_id = 0;
      (void)g_memdb->insertRunEvent(g_memdb_run_id, ts_ms, g_last_step.load(),
                                    std::string("exit"), std::string("normal"),
                                    0, nf_process_rss_mb(), 0.0, event_id);
    }
    return 0;
  } catch (const std::exception &ex) {
    ...
    if (g_memdb && g_memdb_run_id > 0) {
      ...
      std::int64_t event_id = 0;
      (void)g_memdb->insertRunEvent(
          g_memdb_run_id, ts_ms, g_last_step.load(), std::string("fatal"),
          std::string(ex.what()), 1, nf_process_rss_mb(), 0.0, event_id);
    }
    return 1;
  } catch (...) {
    ...
    if (g_memdb && g_memdb_run_id > 0) {
      ...
      (void)g_memdb->insertRunEvent(
          g_memdb_run_id, ts_ms, g_last_step.load(), std::string("fatal"),
          std::string("unknown"), 1, nf_process_rss_mb(), 0.0, event_id);
    }
    return 1;
  }
}
...
static void NfTerminateHandler() {
  try {
    if (g_memdb && g_memdb_run_id > 0) {
      ...
      (void)g_memdb->insertRunEvent(
          g_memdb_run_id, ts_ms, g_last_step.load(), std::string("fatal"),
          std::string("std::terminate"), 1, nf_process_rss_mb(), 0.0, event_id);
    }
  } catch (...) {
  }
  std::abort();
...
static void NfNewHandler() {
  try {
    if (g_memdb && g_memdb_run_id > 0) {
      ...
      (void)g_memdb->insertRunEvent(g_memdb_run_id, ts_ms, g_last_step.load(),
                                    std::string("fatal"),
                                    std::string("new_handler_out_of_memory"), 1,
                                    nf_process_rss_mb(), 0.0, event_id);
    }
  } catch (...) {
  }
