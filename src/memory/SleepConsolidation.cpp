...
void SleepConsolidation::registerEpisodicMemory(
    EpisodicMemoryManager *episodic_memory) {
  std::lock_guard<std::shared_mutex> lock(consolidation_mutex_);
  episodic_memory_ = episodic_memory;
}

void SleepConsolidation::registerSemanticMemory(
...
void SleepConsolidation::registerProceduralMemory(
    ProceduralMemory *procedural_memory) {
  std::lock_guard<std::shared_mutex> lock(consolidation_mutex_);
  procedural_memory_ = procedural_memory;
}

void SleepConsolidation::registerLearningSystem(
    NeuroForge::Core::LearningSystem *learning_system) {
  std::lock_guard<std::shared_mutex> lock(consolidation_mutex_);
  learning_system_ = learning_system;
}

void SleepConsolidation::registerBrain(
    NeuroForge::Core::HypergraphBrain *brain) {
  std::lock_guard<std::shared_mutex> lock(consolidation_mutex_);
  brain_ = brain;
}

bool SleepConsolidation::triggerConsolidation(bool force_consolidation,
  ...
  bool systems_registered =
      episodic_memory_ != nullptr && semantic_memory_ != nullptr &&
      working_memory_ != nullptr && procedural_memory_ != nullptr &&
      learning_system_ != nullptr && brain_ != nullptr;
  ...
  logConsolidationActivity("Consolidation Completed",
                           "Operations: " +
                               std::to_string(operations_performed));
...
size_t
SleepConsolidation::replayEpisodes(const std::vector<EnhancedEpisode> &episodes,
                                   float replay_speed) {
...
SleepConsolidation::selectEpisodesForReplay(size_t max_episodes) const {
  if (!episodic_memory_) {
    return {};
  }
...
bool SleepConsolidation::replaySingleEpisode(const EnhancedEpisode &episode,
                                             float replay_speed) {
  ...
  try {
    // Simulate episode replay by feeding the episode data back through the
    // system This is a simplified implementation
    ...
    float replay_learning_rate =
        0.1f / replay_speed; // Slower learning during replay
...
size_t
SleepConsolidation::performCompetitiveScaling(float competition_strength) {
  if (!learning_system_) {
    return 0;
  }
  ...
  try {
    // Apply competitive scaling to enhance strong connections and weaken weak
    // ones This is a simplified implementation
...
size_t
SleepConsolidation::applySynapticScalingToRegion(const std::string &region_name,
  ...
  try {
    ...
    logConsolidationActivity(
        "Region Scaling", "Region: " + region_name +
                              ", Factor: " + std::to_string(scaling_factor) +
                              ", Synapses: " + std::to_string(scaled_synapses));
...
size_t SleepConsolidation::transferWorkingToProcedural(size_t max_transfers) {
  ...
  try {
    ...
    // For now, we'll just simulate the process
    transfers_performed =
        std::min(max_transfers, size_t(3)); // Simulated transfers
...
size_t
SleepConsolidation::performCrossModalIntegration(float integration_threshold) {
...
size_t SleepConsolidation::processSlowWaveSleep(std::uint64_t duration_ms) {
  ...
  auto episodes_to_replay =
      selectEpisodesForReplay(config_.max_replay_episodes / 2);
  operations_performed += replayEpisodes(
      episodes_to_replay, config_.replay_speed_multiplier * 0.5f);
...
size_t SleepConsolidation::processREMSleep(std::uint64_t duration_ms) {
  ...
  auto episodes_to_replay =
      selectEpisodesForReplay(config_.max_replay_episodes / 4);
  operations_performed += replayEpisodes(
      episodes_to_replay, config_.replay_speed_multiplier * 2.0f);
...
Statistics SleepConsolidation::getStatistics() const {
  std::shared_lock<std::shared_mutex> lock(consolidation_mutex_);
  ...
  stats.total_synaptic_scaling_operations =
      total_synaptic_scaling_operations_.load();
  ...
  if (stats.total_consolidation_sessions > 0) {
    ...
    stats.average_consolidation_duration_ms =
        static_cast<float>(config_.min_consolidation_duration_ms +
                           config_.max_consolidation_duration_ms) /
        2.0f;

    // Calculate efficiency based on operations performed
    float total_operations =
        static_cast<float>(stats.total_episodes_replayed +
                           stats.total_synaptic_scaling_operations +
                           stats.total_memory_integrations);

    stats.consolidation_efficiency =
        total_operations / stats.total_consolidation_sessions;
  }
...
bool SleepConsolidation::areAllMemorySystemsRegistered() const {
  // Use atomic reads to avoid deadlock - these pointers are only set during
  // registration
...
size_t
SleepConsolidation::performConsolidationSession(std::uint64_t duration_ms) {
  size_t total_operations = 0;
  ...
  std::uint64_t rem_duration =
      static_cast<std::uint64_t>(duration_ms * config_.rem_duration_ratio);
...
float SleepConsolidation::calculateReplayPriority(
  ...
  float recency_factor =
      std::exp(-static_cast<float>(age) / 3600000.0f); // 1 hour decay
...
std::vector<EnhancedEpisode> SleepConsolidation::selectRandomEpisodes(
    const std::vector<EnhancedEpisode> &episodes, size_t max_episodes) const {
...
SleepConsolidation::ScalingFactors SleepConsolidation::calculateScalingFactors(
  ...
  factors.inhibitory_scaling =
      factors.global_scaling * 1.1f; // Slightly stronger for inhibitory
...
bool SleepConsolidation::validateConfig(
    const ConsolidationConfig &config) const {
  // Basic validation
  return config.replay_speed_multiplier > 0.0f &&
         config.synaptic_scaling_factor >= 0.0f &&
         config.synaptic_scaling_factor <= 1.0f &&
         config.integration_threshold >= 0.0f &&
         config.integration_threshold <= 1.0f &&
         config.slow_wave_duration_ratio >= 0.0f &&
         config.slow_wave_duration_ratio <= 1.0f &&
         config.rem_duration_ratio >= 0.0f &&
         config.rem_duration_ratio <= 1.0f &&
         (config.slow_wave_duration_ratio + config.rem_duration_ratio) <=
             1.0f &&
         config.min_consolidation_duration_ms <=
             config.max_consolidation_duration_ms;
}
...
void SleepConsolidation::logConsolidationActivity(
    const std::string &activity_type, const std::string &details) const {

// Simple logging - in a full implementation, this would use a proper logging
// system
...
std::size_t SleepConsolidation::replayWeightPatterns(std::size_t max_patterns) {
  if (!episodic_memory_ || !procedural_memory_)
    return 0;

  const auto &patterns = episodic_memory_->getPatterns();
  std::size_t replayed = 0;

  for (std::size_t i = 0; i < std::min(patterns.size(), max_patterns); ++i) {
    const auto &pat = patterns[i];
    if (pat.consolidated && !pat.sensory_state.empty()) {
      // Replay through procedural weight matrix
      procedural_memory_->reinforce(
          pat.sensory_state,
          pat.context_state.empty() ? pat.sensory_state : pat.context_state,
          pat.salience * config_.replay_reinforcement_lr);
      ++replayed;
    }
  }
  return replayed;
...
void SleepConsolidation::scaleAllWeights(float factor) {
  if (factor < 0)
    factor = config_.synaptic_scaling_factor;

  // Scale procedural weights
  if (procedural_memory_) {
    procedural_memory_->applyWeightDecay(1.0f - factor);
  }
}

void SleepConsolidation::decayAllWeights(float rate) {
  if (rate < 0)
    rate = config_.weight_decay_during_sleep;

  if (procedural_memory_) {
    procedural_memory_->applyWeightDecay(rate);
  }
  if (working_memory_) {
    working_memory_->decay(rate);
  }
}
