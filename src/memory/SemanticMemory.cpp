...
// This file implements the deprecated ConceptNode/concept_graph_ legacy API.
...
ConceptNode::ConceptNode(const std::string &concept_label,
                         ...
                         ConceptType concept_type, const std::string &desc)
    : label(concept_label), feature_vector(features), description(desc),
      type(concept_type),
  ...
  if (!features.empty()) {
    ...
    consolidation_strength =
        std::min(1.0f, std::sqrt(magnitude) / features.size());
  }
}
...
float ConceptNode::calculateSimilarity(const ConceptNode &other) const {
  ...
  size_t min_size =
      std::min(feature_vector.size(), other.feature_vector.size());
  ...
  float abstraction_diff =
      std::abs(abstraction_level - other.abstraction_level);
  similarity +=
      (1.0f - abstraction_diff) * 0.05f; // 5% bonus for similar abstraction
...
void ConceptNode::updateWithEvidence(const std::vector<float> &new_features,
  ...
  consolidation_strength =
      std::min(1.0f, consolidation_strength + evidence_weight * 0.1f);
...
void ConceptNode::addRelationship(int concept_id, float strength,
                                  bool bidirectional) {
  // Add to related concepts if not already present
  if (std::find(related_concepts.begin(), related_concepts.end(), concept_id) ==
      related_concepts.end()) {
...
SemanticMemory::SemanticMemory(const Config &config)
    : config_(config), concept_dim_(config.concept_dim) {
  // Initialize concept weight matrix (Hopfield-like auto-associative)
  concept_weights_.resize(concept_dim_ * concept_dim_, 0.0f);
...
int SemanticMemory::activateConcept(const std::vector<float> &features,
                                    float strength) {
  // Pad/truncate to concept_dim
  std::vector<float> pattern(concept_dim_, 0.0f);
  for (size_t i = 0; i < std::min(features.size(), concept_dim_); ++i)
    pattern[i] = features[i];

  // Check if similar attractor already exists
  int closest =
      findClosestAttractor(pattern, config_.attractor_merge_threshold);
  if (closest >= 0) {
    auto &att = attractors_[static_cast<size_t>(closest)];
    att.strength += strength * 0.1f;
    att.activation_count++;
    att.last_activation_ms = getCurrentTimestamp();
    // Blend pattern
    float blend = 1.0f / static_cast<float>(att.activation_count + 1);
    for (size_t i = 0; i < concept_dim_; ++i) {
      att.pattern[i] = (1.0f - blend) * att.pattern[i] + blend * pattern[i];
    }
    return closest;
  }

  // Create new attractor
  ConceptAttractor att;
  att.pattern = pattern;
  att.strength = strength;
  att.activation_count = 1;
  att.creation_timestamp_ms = getCurrentTimestamp();
  att.last_activation_ms = att.creation_timestamp_ms;
  attractors_.push_back(std::move(att));

  // Update Hopfield-like weight matrix: W += pattern âŠ— pattern
  for (size_t i = 0; i < concept_dim_; ++i) {
    for (size_t j = 0; j < concept_dim_; ++j) {
      if (i != j) { // No self-connections
        concept_weights_[i * concept_dim_ + j] +=
            config_.attractor_lr * pattern[i] * pattern[j];
      }
    }
  }

  return static_cast<int>(attractors_.size()) - 1;
}

std::vector<float> SemanticMemory::settle(const std::vector<float> &cue,
                                          int iterations) const {
  std::vector<float> state(concept_dim_, 0.0f);
  for (size_t i = 0; i < std::min(cue.size(), concept_dim_); ++i)
    state[i] = cue[i];

  // Iterative settling: state = sign(W Â· state)
  for (int iter = 0; iter < iterations; ++iter) {
    std::vector<float> new_state(concept_dim_, 0.0f);
    for (size_t i = 0; i < concept_dim_; ++i) {
      float sum = 0.0f;
      for (size_t j = 0; j < concept_dim_; ++j) {
        sum += concept_weights_[i * concept_dim_ + j] * state[j];
      }
...
void SemanticMemory::loadAttractors(
    const std::vector<ConceptAttractor> &attractors) {
  attractors_ = attractors;
}

void SemanticMemory::loadConceptWeights(const std::vector<float> &weights,
                                        std::size_t dim) {
  concept_weights_ = weights;
  concept_dim_ = dim;
}

int SemanticMemory::findClosestAttractor(const std::vector<float> &pattern,
                                         float threshold) const {
  int best_idx = -1;
  float best_sim = threshold;
  for (size_t i = 0; i < attractors_.size(); ++i) {
    float sim = calculateCosineSimilarity(pattern, attractors_[i].pattern);
    if (sim > best_sim) {
      best_sim = sim;
...
int SemanticMemory::createConcept(const std::string &label,
  ...
  std::lock_guard<std::shared_mutex> lock(concepts_mutex_);
  ...
  // â”€â”€â”€ ALSO encode as attractor â”€â”€â”€
  activateConcept(features, 1.0f);
...
std::vector<int>
SemanticMemory::extractConceptsFromEpisode(const EnhancedEpisode &episode,
  ...
  combined_features.insert(combined_features.end(),
                           episode.sensory_state.begin(),
                           episode.sensory_state.end());
  combined_features.insert(combined_features.end(),
                           episode.action_state.begin(),
                           episode.action_state.end());

  // Subsample substrate state to avoid overwhelming feature vector
  if (!episode.substrate_state.empty()) {
    size_t subsample_size =
        std::min(episode.substrate_state.size(), size_t(50));
  ...
  auto similar_concepts =
      findSimilarConcepts(combined_features, 5, extraction_threshold);

  if (similar_concepts.empty() ||
      similar_concepts[0].second < extraction_threshold) {
    // Create new concept
    std::string concept_label = generateConceptLabel(
        combined_features, ConceptNode::ConceptType::Abstract);
    ...
    if (concept_id != -1) {
      ...
      if (concept_it != concept_graph_.end()) {
        ...
        concept_it->second.consolidation_strength +=
            episode.emotional_weight * 0.1f;
      }
    }
  } else {
    ...
    for (const auto &[candidate, similarity] : similar_concepts) {
      ...
      if (label_it != label_to_id_.end()) {
        auto concept_it = concept_graph_.find(label_it->second);
        if (concept_it != concept_graph_.end()) {
          concept_it->second.updateWithEvidence(combined_features,
                                                similarity * 0.1f);
...
SemanticMemory::retrieveConcept(int concept_id) const {
  std::shared_lock<std::shared_mutex> lock(concepts_mutex_);
...
std::unique_ptr<ConceptNode>
SemanticMemory::retrieveConceptByLabel(const std::string &label) const {
  std::shared_lock<std::shared_mutex> lock(concepts_mutex_);
...
SemanticMemory::findSimilarConcepts(const std::vector<float> &query_features,
                                    size_t max_results,
  ...
  for (const auto &[concept_id, node] : concept_graph_) {
    float similarity =
        calculateCosineSimilarity(query_features, node.feature_vector);
...
SemanticMemory::findConceptsByType(ConceptNode::ConceptType type,
...
std::vector<ConceptNode>
SemanticMemory::getRecentlyAccessedConcepts(std::uint64_t min_timestamp,
  ...
  for (const auto &[id, node] : concept_graph_) {
    if (node.last_access_timestamp_ms >= min_timestamp) {
      results.push_back(node);
    }
  }
  ...
  std::sort(results.begin(), results.end(),
            [](const ConceptNode &a, const ConceptNode &b) {
              return a.last_access_timestamp_ms > b.last_access_timestamp_ms;
            });
...
bool SemanticMemory::linkConcepts(int concept_id_1, int concept_id_2,
  ...
  if (concept_1_it == concept_graph_.end() ||
      concept_2_it == concept_graph_.end()) {
    return false;
  }
...
bool SemanticMemory::createHierarchicalRelationship(int child_id, int parent_id,
                                                    float strength) {
  std::lock_guard<std::shared_mutex> lock(concepts_mutex_);
...
std::vector<std::pair<ConceptNode, float>>
SemanticMemory::getRelatedConcepts(int concept_id, size_t max_results,
...
ConceptHierarchy SemanticMemory::getConceptHierarchy(int concept_id,
...
size_t SemanticMemory::consolidateFromEpisodicMemory(
    const EpisodicMemoryManager &episodic_manager, size_t max_episodes) {

  // Get recent episodes from episodic memory
  auto stats = episodic_manager.getStatistics();
  size_t episodes_to_process =
      std::min(max_episodes, static_cast<size_t>(stats.recent_episodes_count));
...
size_t SemanticMemory::mergeSimilarConcepts(float merge_threshold) {
  ...
  for (size_t i = 0; i < concept_ids.size(); ++i) {
    for (size_t j = i + 1; j < concept_ids.size(); ++j) {
      ...
      float similarity =
          concept_1_it->second.calculateSimilarity(concept_2_it->second);
      ...
      if (similarity >= merge_threshold) {
        ...
        if (total_weight > 0.0f) {
          for (size_t k = 0; k < std::min(concept_1.feature_vector.size(),
                                          concept_2.feature_vector.size());
               ++k) {
            concept_1.feature_vector[k] =
                (weight_1 * concept_1.feature_vector[k] +
                 weight_2 * concept_2.feature_vector[k]) /
                total_weight;
          }
        }

        // Merge other properties
        concept_1.consolidation_strength = std::max(
            concept_1.consolidation_strength, concept_2.consolidation_strength);
        concept_1.access_count += concept_2.access_count;
        concept_1.episodic_support =
            std::max(concept_1.episodic_support, concept_2.episodic_support);

        // Merge relationships
        for (int related_id : concept_2.related_concepts) {
          if (std::find(concept_1.related_concepts.begin(),
                        concept_1.related_concepts.end(),
                        related_id) == concept_1.related_concepts.end()) {
...
size_t
SemanticMemory::formHierarchicalRelationships(float hierarchy_threshold) {
...
std::vector<ConceptNode> SemanticMemory::queryKnowledgeGraph(
    int query_concept, const std::vector<std::string> &relationship_types,
    size_t max_depth) const {
...
std::vector<int>
SemanticMemory::findConceptualPath(int start_concept_id, int end_concept_id,
...
std::vector<std::pair<ConceptNode, size_t>>
SemanticMemory::getConceptNeighborhood(int concept_id, size_t radius) const {
...
SemanticStatistics SemanticMemory::getStatistics() const {
  ...
  if (!concept_graph_.empty()) {
    ...
    stats.average_concept_age_ms =
        static_cast<float>(total_age) / concept_graph_.size();
    stats.average_consolidation_strength =
        total_consolidation / concept_graph_.size();
    stats.average_relationships_per_concept =
        static_cast<float>(stats.total_relationships) / concept_graph_.size();
  }
  ...
  // Attractor stats
  stats.total_attractors = attractors_.size();
  float wnorm = 0;
  for (auto w : concept_weights_)
    wnorm += w * w;
  stats.concept_weight_norm = std::sqrt(wnorm);
...
void SemanticMemory::clearAllConcepts() {
  ...
  attractors_.clear();
  std::fill(concept_weights_.begin(), concept_weights_.end(), 0.0f);
...
float SemanticMemory::calculateCosineSimilarity(
    const std::vector<float> &vec_a, const std::vector<float> &vec_b) const {
...
std::vector<float> SemanticMemory::extractFeaturesFromEpisode(
    const EnhancedEpisode &episode) const {
  std::vector<float> features;

  // Combine different episode components
  features.insert(features.end(), episode.sensory_state.begin(),
                  episode.sensory_state.end());
  features.insert(features.end(), episode.action_state.begin(),
                  episode.action_state.end());
...
std::string
SemanticMemory::generateConceptLabel(const std::vector<float> &features,
...
void SemanticMemory::updateIndices(const ConceptNode &concept_node,
                                   int concept_id) {
  // Update type index
...
void SemanticMemory::removeFromIndices(const ConceptNode &concept_node,
                                       int concept_id) {
  // Remove from type index
  auto &type_vec = type_index_[concept_node.type];
  type_vec.erase(std::remove(type_vec.begin(), type_vec.end(), concept_id),
                 type_vec.end());
  ...
  if (!concept_node.label.empty()) {
    ...
    keyword_vec.erase(
        std::remove(keyword_vec.begin(), keyword_vec.end(), concept_id),
        keyword_vec.end());
