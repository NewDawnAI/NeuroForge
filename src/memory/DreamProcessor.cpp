...
void DreamProcessor::registerEpisodicMemory(
    EpisodicMemoryManager *episodic_memory) {
  std::lock_guard<std::shared_mutex> lock(dream_mutex_);
...
void DreamProcessor::registerSleepConsolidation(
...
void DreamProcessor::registerLearningSystem(
    NeuroForge::Core::LearningSystem *learning_system) {
  std::lock_guard<std::shared_mutex> lock(dream_mutex_);
  learning_system_ = learning_system;
}

void DreamProcessor::registerCompositionMetrics(
    NeuroForge::Core::CompositionMetrics *composition_metrics) {
  std::lock_guard<std::shared_mutex> lock(dream_mutex_);
  composition_metrics_ = composition_metrics;
}

// Dream Generation

DreamProcessor::DreamNarrative
DreamProcessor::generateDream(std::uint64_t rem_duration_ms,
  ...
  if (episodic_memory_) {
    recent_experiences = episodic_memory_->getRecentEpisodes(
        24 * 60 * 60 * 1000, 20); // Last 24 hours
  }

  // Select dream type based on context
  DreamType dream_type =
      selectDreamType(emotional_state, stress_level, recent_experiences);
  current_dream_type_ = dream_type;

  // Calculate dream duration
  std::uint64_t dream_duration =
      calculateDreamDuration(dream_type, rem_duration_ms);

  // Generate the dream
  DreamNarrative dream =
      generateSpecificDream(dream_type, dream_duration, emotional_state);
...
DreamProcessor::DreamNarrative
DreamProcessor::generateSpecificDream(DreamType dream_type,
  ...
  std::vector<EnhancedEpisode> source_memories =
      selectSourceMemories(dream_type, config_.max_memory_sources);
  ...
  case DreamType::Episodic:
    dream_content = generateEpisodicContent(source_memories,
                                            config_.memory_distortion_factor);
    break;
    ...
    if (semantic_memory_) {
      ...
      for (const auto &episode : source_memories) {
        ...
        if (!episode.context_tag.empty()) {
          ...
          // Legacy call — will be migrated to settle() in future cleanup
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
          auto concept_ptr =
              semantic_memory_->retrieveConceptByLabel(episode.context_tag);
#pragma GCC diagnostic pop
          if (concept_ptr) {
            // Find the concept ID from label_to_id mapping
            // Since we can't access private members, we'll use a different
            // approach Let's use findSimilarConcepts with the concept's feature
            // vector
            auto similar_concepts = semantic_memory_->findSimilarConcepts(
                concept_ptr->feature_vector, 5);
  ...
  case DreamType::Creative:
    dream_content =
        generateCreativeContent(std::vector<std::vector<float>>(), 0.8f);
    break;
  ...
  case DreamType::Nightmare: {
    ...
    for (auto &val : dream_content) {
      if (val < 0)
        val *= 1.5f; // Amplify negative values
    }
    break;
  }
  ...
  case DreamType::Lucid: {
    ...
    dream_content =
        generateCreativeContent(std::vector<std::vector<float>>(), 0.9f);
    ...
    dream_content.insert(dream_content.end(), metacognitive_elements.begin(),
                         metacognitive_elements.end());
    break;
  }
  }
  ...
  if (dream_content.size() >= 300) {
    dream.emotional_content = std::vector<float>(dream_content.begin() + 100,
                                                 dream_content.begin() + 200);
    dream.symbolic_content = std::vector<float>(dream_content.begin() + 200,
                                                dream_content.begin() + 300);
  }

  // Store memory sources
  for (size_t i = 0; i < source_memories.size(); ++i) {
    dream.memory_sources.push_back(
        "Episode_" + std::to_string(i) + "_" +
        std::to_string(source_memories[i].timestamp_ms));
  }
  ...
  dream.coherence_score =
      calculateCoherenceScore(dream_content, dream.narrative_text);
  dream.creativity_score =
      calculateCreativityScore(dream_content, source_memories);

  // Calculate emotional intensity
  if (!dream.emotional_content.empty()) {
    float sum = std::accumulate(
        dream.emotional_content.begin(), dream.emotional_content.end(), 0.0f,
...
std::size_t
DreamProcessor::processREMDreams(std::uint64_t rem_duration_ms,
  ...
  while (remaining_time > config_.min_dream_duration_ms) {
    ...
    if (dream.dream_duration_ms > 0) {
      ...
      std::uint64_t inter_dream_time =
          std::min(remaining_time, static_cast<std::uint64_t>(2000));
  ...
  // ━━━ Phase 5: Emergent Concept Composition ━━━
  // After dreaming, scan recent episodic memories for co-occurring
  // pattern pairs, evaluate compositional stability via
  // CompositionMetrics, and auto-create new Composite concepts.
  if (composition_metrics_ && semantic_memory_ && episodic_memory_) {
    std::vector<EnhancedEpisode> recent =
        episodic_memory_->getRecentEpisodes(24 * 60 * 60 * 1000, 20);

    // Pair-wise scan for compositional emergence
    for (std::size_t i = 0; i + 1 < recent.size(); ++i) {
      const auto &ep_a = recent[i];
      const auto &ep_b = recent[i + 1];
      if (ep_a.sensory_state.empty() || ep_b.sensory_state.empty())
        continue;

      // Build a "whole" by element-wise averaging the two sensory states
      std::size_t dim =
          std::min(ep_a.sensory_state.size(), ep_b.sensory_state.size());
      std::vector<float> whole(dim, 0.0f);
      for (std::size_t d = 0; d < dim; ++d) {
        whole[d] = 0.5f * (ep_a.sensory_state[d] + ep_b.sensory_state[d]);
      }

      auto score = composition_metrics_->evaluate(whole, ep_a.sensory_state,
                                                  ep_b.sensory_state);
      composition_metrics_->recordMerger(static_cast<uint64_t>(i),
                                         static_cast<uint64_t>(i + 1), score);

      if (score.is_compositional) {
        // Generate label from indices (placeholder — could use LanguageSystem)
        std::string label = "composed_" + std::to_string(i) + "_" +
                            std::to_string(i + 1) + "_t" +
                            std::to_string(total_dreams_generated_.load());
        // Legacy call — will be migrated to activateConcept() in future cleanup
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        semantic_memory_->createConcept(
            label, whole, ConceptNode::ConceptType::Composite,
            "Emergent composition discovered during sleep");
#pragma GCC diagnostic pop
        std::cout << "[DreamProcessor] Phase 5: Created composite concept '"
                  << label
                  << "' (cond_complexity=" << score.conditional_complexity
                  << ")\n";
      }
    }
  }
...
std::vector<float> DreamProcessor::generateEpisodicContent(
  ...
  for (const auto &episode : source_episodes) {
    ...
    std::vector<float> distorted_content =
        applyMemoryDistortion(episode.sensory_state, distortion_factor);
    content.insert(content.end(), distorted_content.begin(),
                   distorted_content.end());

    if (content.size() > 1000)
      break; // Limit content size
  }
...
std::vector<float> DreamProcessor::generateCreativeContent(
  ...
  if (concept_vectors.empty()) {
    ...
    std::uniform_real_distribution<float> dist(-creativity_level,
                                               creativity_level);
    for (int i = 0; i < 800; ++i) {
      content.push_back(dist(dream_generator_));
    }
    ...
    std::uniform_int_distribution<std::size_t> concept_dist(
        0, concept_vectors.size() - 1);
    ...
    for (int i = 0; i < 800; ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        const auto &concept_vector =
            concept_vectors[concept_dist(dream_generator_)];
        if (!concept_vector.empty()) {
          std::uniform_int_distribution<std::size_t> idx_dist(
              0, concept_vector.size() - 1);
          value += concept_vector[idx_dist(dream_generator_)] *
                   blend_dist(dream_generator_);
        }
      }

      // Add creative noise
      value +=
          blend_dist(dream_generator_) * (blend_dist(dream_generator_) - 0.5f);
...
std::vector<float> DreamProcessor::generateProblemSolvingContent(
  ...
  if (!problem_context.empty()) {
    content.insert(content.end(), problem_context.begin(),
                   problem_context.end());
  }
...
std::vector<float> DreamProcessor::generateEmotionalContent(
  ...
  for (const auto &memory : emotional_memories) {
    ...
    if (memory.sensory_state.size() > 100) {
      emotional_component = std::vector<float>(
          memory.sensory_state.begin(), memory.sensory_state.begin() + 100);
    } else {
    ...
    for (float &val : emotional_component) {
      val *=
          (1.0f + memory.emotional_weight * config_.stress_processing_weight);
    }

    content.insert(content.end(), emotional_component.begin(),
                   emotional_component.end());
...
std::string
DreamProcessor::constructNarrative(const std::vector<float> &dream_content,
                                   DreamType dream_type,
  ...
  case DreamType::Episodic:
    template_base =
        "I found myself reliving a memory, but everything was different. ";
    break;
  case DreamType::Creative:
  ...
  case DreamType::ProblemSolving:
    template_base =
        "The solution appeared to me in a dream, clear as daylight. ";
    break;
  case DreamType::Emotional:
    template_base =
        "Emotions flowed through the dream like a river of feeling. ";
    break;
  case DreamType::Nightmare:
    template_base =
        "The dream began with unease, growing into something darker. ";
    break;
  case DreamType::Lucid:
    template_base =
        "I realized I was dreaming, and with that awareness came power. ";
    break;
  default:
    template_base =
        "The dream unfolded like a story written in light and shadow. ";
    break;
  }
  ...
  std::uniform_int_distribution<std::size_t> content_dist(
      0, dream_content.size() - 1);
  ...
  default:
    narrative
        << "The dream dissolved like morning mist, leaving only impressions.";
    break;
  }
...
std::vector<float>
DreamProcessor::addSymbolicElements(const std::vector<float> &base_content,
  ...
  std::uniform_real_distribution<float> symbol_dist(-symbolic_intensity,
                                                    symbolic_intensity);
  std::uniform_int_distribution<std::size_t> position_dist(
      0, enhanced_content.size() - 1);

  // Apply symbolic transformations at random positions
  int num_transformations =
      static_cast<int>(enhanced_content.size() * symbolic_intensity * 0.1f);

  for (int i = 0; i < num_transformations; ++i) {
    std::size_t pos = position_dist(dream_generator_);

    // Apply symbolic transformation (mathematical operations representing
    // symbolic processing)
...
std::vector<float> DreamProcessor::blendCrossModalContent(
    ...
    const std::vector<float> &tactile_content, float blend_factor) {
  std::vector<float> blended_content;

  std::size_t max_size = std::max(
      {visual_content.size(), auditory_content.size(), tactile_content.size()});
...
DreamProcessor::DreamAnalysis
DreamProcessor::analyzeDream(const DreamNarrative &dream) {
  ...
  if (!dream.sensory_content.empty()) {
    ...
    float content_mean = std::accumulate(dream.sensory_content.begin(),
                                         dream.sensory_content.end(), 0.0f) /
                         dream.sensory_content.size();
  ...
  if (dream.dream_type == DreamType::Emotional ||
      dream.dream_type == DreamType::Nightmare) {
    analysis.emotional_processing_value *= 1.5f;
  }

  // Calculate memory consolidation benefit
  analysis.memory_consolidation_benefit =
      (dream.coherence_score + analysis.novelty_score) * 0.5f;

  // Generate insights based on dream characteristics
  if (dream.creativity_score > 0.7f) {
    analysis.insights.push_back(
        "High creative potential - novel connections formed");
  }

  if (dream.emotional_intensity > 0.8f) {
    analysis.insights.push_back(
        "Intense emotional processing - potential stress regulation");
  }

  if (dream.coherence_score > 0.8f) {
    analysis.insights.push_back(
        "Highly coherent narrative - strong memory integration");
  }

  if (dream.dream_type == DreamType::Lucid) {
    analysis.insights.push_back(
        "Lucid awareness detected - metacognitive processing active");
  }

  // Determine if further processing is needed
  analysis.requires_further_processing =
      (analysis.novelty_score > 0.8f) ||
      (analysis.problem_solving_potential > 0.7f) ||
      (dream.dream_type == DreamType::Nightmare &&
       dream.emotional_intensity > 0.9f);

  return analysis;
}

void DreamProcessor::storeDream(const DreamNarrative &dream,
...
std::vector<DreamProcessor::DreamNarrative>
DreamProcessor::getDreamsByType(DreamType dream_type,
                                std::size_t max_dreams) const {
  std::shared_lock<std::shared_mutex> lock(dream_mutex_);
  ...
  std::size_t start_idx =
      dreams.size() > max_dreams ? dreams.size() - max_dreams : 0;

  return std::vector<DreamNarrative>(dreams.begin() + start_idx, dreams.end());
}

std::vector<DreamProcessor::DreamNarrative>
DreamProcessor::getRecentDreams(std::uint64_t hours_back,
                                std::size_t max_dreams) const {
  std::shared_lock<std::shared_mutex> lock(dream_mutex_);

  std::uint64_t cutoff_time =
      getCurrentTimestamp() - (hours_back * 60 * 60 * 1000);
  std::vector<DreamNarrative> recent_dreams;

  for (auto it = dream_history_.rbegin();
       it != dream_history_.rend() && recent_dreams.size() < max_dreams; ++it) {
...
DreamProcessor::Statistics DreamProcessor::getStatistics() const {
  ...
  if (stats.total_dreams_generated > 0) {
    stats.average_dream_duration_ms =
        static_cast<float>(stats.total_dream_time_ms) /
        stats.total_dreams_generated;
  }
  ...
  if (!dream_history_.empty()) {
    ...
    std::size_t recent_count =
        std::min(dream_history_.size(), static_cast<std::size_t>(100));
    for (std::size_t i = dream_history_.size() - recent_count;
         i < dream_history_.size(); ++i) {
...
bool DreamProcessor::areAllSystemsRegistered() const {
  return episodic_memory_ != nullptr && semantic_memory_ != nullptr &&
         working_memory_ != nullptr && sleep_consolidation_ != nullptr &&
         brain_ != nullptr && learning_system_ != nullptr;
}

bool DreamProcessor::isOperational() const {
  return areAllSystemsRegistered() && !symbolic_dictionary_.empty() &&
         !narrative_templates_.empty();
}

bool DreamProcessor::isDreaming() const { return dreaming_active_.load(); }

// Private Methods

DreamProcessor::DreamType DreamProcessor::selectDreamType(
    const std::vector<float> &emotional_state, float stress_level,
  ...
  if (rand_val < nightmare_prob) {
    ...
    return DreamType::Creative;
                            ...
                            problem_solving_prob) {
    return DreamType::ProblemSolving;
  } else if (high_emotion) {
    return DreamType::Emotional;
  } else {
    // Default to episodic or semantic
    return prob_dist(dream_generator_) < 0.6f ? DreamType::Episodic
                                              : DreamType::Semantic;
  }
}

std::uint64_t
DreamProcessor::calculateDreamDuration(DreamType dream_type,
                                       std::uint64_t available_time) {
  ...
  case DreamType::ProblemSolving:
    base_duration =
        static_cast<std::uint64_t>(config_.min_dream_duration_ms * 1.5f);
    break;
  case DreamType::Lucid:
    base_duration =
        static_cast<std::uint64_t>(config_.min_dream_duration_ms * 2.0f);
    break;
  case DreamType::Nightmare:
    base_duration =
        static_cast<std::uint64_t>(config_.min_dream_duration_ms * 0.8f);
    break;
  default:
    break;
  }
  ...
  std::uint64_t max_duration =
      std::min(available_time, config_.max_dream_duration_ms);
  return std::min(base_duration, max_duration);
}

std::vector<EnhancedEpisode>
DreamProcessor::selectSourceMemories(DreamType dream_type,
  ...
  // Get recent episodes
  std::vector<EnhancedEpisode> recent_episodes =
      episodic_memory_->getRecentEpisodes(7 * 24 * 60 * 60 * 1000,
                                          max_sources * 2); // Last week
  ...
  case DreamType::ProblemSolving: {
    ...
    std::shuffle(recent_episodes.begin(), recent_episodes.end(),
                 dream_generator_);
    break;
  }
  ...
  default: {
    ...
    std::uniform_int_distribution<std::size_t> shuffle_dist(
        0, std::min(recent_episodes.size(), max_sources * 2) - 1);
    std::shuffle(recent_episodes.begin(),
                 recent_episodes.begin() + shuffle_dist(dream_generator_) + 1,
                 dream_generator_);
    break;
  }
  }
  ...
  selected_memories.assign(recent_episodes.begin(),
                           recent_episodes.begin() + count);

  return selected_memories;
}

std::vector<float> DreamProcessor::applyMemoryDistortion(
    const std::vector<float> &original_content, float distortion_factor) {
  std::vector<float> distorted_content = original_content;

  std::uniform_real_distribution<float> distortion_dist(-distortion_factor,
                                                        distortion_factor);
...
void DreamProcessor::initializeSymbolicDictionary() {
  symbolic_dictionary_ = {
      ...
      "mirror",         "door",    "key",    "bridge",  "mountain", "ocean",
      "forest",         "light",   "shadow", "mask",    "spiral",   "circle",
      "tower",          "cave",    "garden", "storm",   "rainbow",  "star",
      "moon",           "sun",     "wind",   "river",   "desert",   "ice",
      "crystal",        "flower",  "tree",   "bird",    "snake",    "butterfly",
      "wolf",           "lion",    "eagle",  "whale",   "dragon"};
}
...
float DreamProcessor::calculateCoherenceScore(
  ...
  float mean =
      std::accumulate(dream_content.begin(), dream_content.end(), 0.0f) /
      dream_content.size();
  float variance = 0.0f;
...
//  WEIGHT-SPACE DREAM GENERATION
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

std::vector<float>
DreamProcessor::generateDreamPattern(const std::vector<float> &seed_pattern,
                                     float distortion, float creativity) {

  // Start with seed or random if empty
  std::vector<float> pattern;
  if (seed_pattern.empty()) {
    std::uniform_real_distribution<float> dist(-1.0f, 1.0f);
    pattern.resize(64);
    for (auto &v : pattern)
      v = dist(dream_generator_);
  } else {
    pattern = seed_pattern;
  }

  // Settle through semantic attractor network if available
  if (semantic_memory_ && distortion < 0.8f) {
    pattern = semantic_memory_->settle(pattern, 3);
  }

  // Add distortion noise
  std::normal_distribution<float> noise(0.0f, distortion);
  for (auto &v : pattern) {
    v = std::tanh(v + noise(dream_generator_));
  }

  // Blend with creative random component
  if (creativity > 0.0f) {
    std::uniform_real_distribution<float> creative_dist(-creativity,
                                                        creativity);
    for (auto &v : pattern) {
      v = std::tanh(v + creative_dist(dream_generator_));
    }
  }
