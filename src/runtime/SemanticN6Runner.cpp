...
int runSemanticN6(NeuroForge::Perception::WorldModelCortex &world_model_cortex,
  ...
  std::ofstream n6_log(cfg.log_csv_path);
  if (n6_log.is_open() && n6_log.tellp() == 0) {
    n6_log << "step,timestamp,phase,latent_norm,latent_entropy,"
              "prediction_error,prediction_error_norm,"
              "semantic_weight,visual_weight,attention_alpha,"
              "attention_lambda,lr_multiplier,learning_allowed,"
              "learning_rate,parameter_delta_norm,semantic_active\n";
  }
  ...
  for (int i = 0; i < cfg.steps; ++i) {
    ...
    float lr_mult = world_model_cortex.getRegulationState().learning_rate_multiplier;
    if (learning_window && has_pred && !semantic_active &&
        pred_err_norm > epsilon_min && pred_err_norm < epsilon_max) {
      learning_allowed = true;
      learning_rate = base_lr * lr_mult * std::clamp(pred_err_norm, 0.1f, 1.0f);
    ...
    if (n6_log.is_open()) {
      n6_log << i << "," << now_ms << "," << phase << "," << latent_norm(observed)
             << "," << latent_entropy(observed) << "," << pred_err << ","
             << pred_err_norm << "," << att.semantic_weight << "," << att.visual_weight
             << "," << reg.attention_alpha << "," << reg.attention_lambda << ","
             << reg.learning_rate_multiplier << "," << (learning_allowed ? 1 : 0) << ","
             << learning_rate << "," << delta_norm << "," << (semantic_active ? 1 : 0)
             << "\n";
      if (i % 10 == 0) {
        n6_log.flush();
      }
