...
## 2025-05-29 - [Async-Signal-Safety in Multi-threaded fork/exec]
**Vulnerability:** When replacing `std::system` with `fork/exec` in a C++ application (`src/main.cpp`), initial implementation allocated `std::string` and `std::vector` inside the child process (between `fork` and `exec`). In multi-threaded programs, `fork()` only replicates the calling thread. If another thread held the memory allocator's mutex during the fork, the child inherits a locked mutex with no owner, causing deadlocks upon memory allocation.
**Learning:** `fork()` in multi-threaded environments is extremely restrictive. Only Async-Signal-Safe functions (like `execvp`, `write`, `_exit`) are safe to call in the child process. `malloc`/`new` are NOT safe.
**Prevention:** Perform all memory allocations and complex logic (like constructing argument vectors) in the **parent** process before calling `fork()`. The child process should only access pre-existing data and call system calls.
